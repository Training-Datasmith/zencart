...
if (($action == 'send_email_to_user') && isset($_POST['customers_email_address']) && !isset($_POST['back'])) {
    ...
    $attachment_fname = basename((string) $_POST['attachment_file']);
    ...
    foreach ($mail as $item) {
        ...
        $rc = zen_mail($item['customers_firstname'] . ' ' . $item['customers_lastname'], $item['customers_email_address'], $subject, $message, STORE_NAME, $from, $html_msg, 'direct_email', ['file' => $attachment_file, 'name' => basename((string) $attachment_file), 'mime_type' => $attachment_filetype]);
        if ($rc === '') {
            $recip_count++;
        }
    }
    ...
    zen_redirect(zen_href_link(FILENAME_MAIL, 'mail_sent_to=' . urlencode((string) $mail_sent_to) . '&recip_count=' . $recip_count . (isset($_GET['origin']) ? '&origin=' . zen_output_string_protected($_GET['origin']) : '') . (isset($_GET['cID']) ? '&cID=' . (int)$_GET['cID'] : '') . (isset($_GET['customer']) ? '&customer=' . zen_output_string_protected($_GET['customer']) : '')));
}
...
if (EMAIL_ATTACHMENTS_ENABLED && $action == 'preview') {
    ...
    if (isset($_FILES['upload_file']) && !empty($_FILES['upload_file']) && ($_POST['upload_file'] != 'none')) {
        if ($attachments_obj = new upload('upload_file')) {
            $attachments_obj->set_extensions(['jpg', 'jpeg', 'gif', 'png', 'zip', 'gzip', 'pdf', 'mp3', 'wma', 'wmv', 'wav', 'epub', 'ogg', 'webm', 'm4v', 'm4a']);
...
?>
      ...
      if (($action == 'preview') && isset($_POST['customers_email_address'])) {
        ...
        <div class="row">
          ...
          <div class="col-sm-9"><?php echo htmlspecialchars(stripslashes((string) $_POST['from']), ENT_COMPAT, CHARSET, true); ?></div>
          <div class="col-sm-12"><?php echo zen_draw_separator('pixel_trans.gif', '100%', '1'); ?></div>
          <div class="col-sm-3 text-right"><b><?php echo TEXT_SUBJECT; ?></b></div>
          <div class="col-sm-9"><?php echo htmlspecialchars(stripslashes((string) $_POST['subject']), ENT_COMPAT, CHARSET, true); ?></div>
          ...
          <div class="col-sm-9">
              ...
              <?php
              $html_preview = zen_output_string($_POST['message_html'] ?? '');
          ...
          $message_preview = (false !== stripos((string) $message_preview, '<br') ? $message_preview : nl2br((string) $message_preview));
          ...
          <?php echo zen_draw_form('mail', FILENAME_MAIL, 'action=send_email_to_user' . (isset($_GET['cID']) ? '&cID=' . (int)$_GET['cID'] : '') . (isset($_GET['customer']) ? '&customer=' . zen_output_string_protected(urldecode(str_replace('+', '%2B', urlencode((string) $_GET['customer'])))) : '') . (isset($_GET['origin']) ? '&origin=' . zen_output_string_protected($_GET['origin']) : '')); ?>
          <?php
          /* Re-Post all POST'ed variables */
          foreach ($_POST as $key => $value) {
              if (!is_array($_POST[$key])) {
                  echo zen_draw_hidden_field($key, stripslashes((string) $value));
              }
          }
        ...
        <?php
          ...
          ?>
        <div class="row">
            <?php echo zen_draw_form('mail', FILENAME_MAIL, 'action=preview' . (isset($_GET['cID']) ? '&cID=' . (int)$_GET['cID'] : '') . (isset($_GET['customer']) ? '&customer=' . zen_output_string_protected(urldecode(str_replace('+', '%2B', urlencode((string) $_GET['customer'])))) : '') . (isset($_GET['origin']) ? '&origin=' . zen_output_string_protected($_GET['origin']) : ''), 'post'); ?>
            <?php
              $customers = get_audiences_list('email', '', (isset($_GET['customer']) ? zen_output_string_protected(urldecode(str_replace('+', '%2B', urlencode((string) $_GET['customer'])))) : ''));
          ?>
          <div class="form-group">
              <?php echo zen_draw_label(TEXT_CUSTOMER, 'customers_email_address', 'class="col-sm-3 control-label"'); ?>
            <div class="col-sm-9"><?php echo zen_draw_pull_down_menu('customers_email_address', $customers, (isset($_GET['customer']) ? zen_output_string_protected(urldecode(str_replace('+', '%2B', urlencode((string) $_GET['customer'])))) : ''), 'class="form-control"');  //, 'multiple'?></div>
          </div>
          <div class="form-group">
          ...
          <div class="form-group">
            ...
            <div class="col-sm-9">
              ...
              if (EMAIL_USE_HTML == 'true') {
                  echo zen_draw_textarea_field('message_html', 'soft', '', '25', htmlspecialchars(stripslashes($_POST['message_html'] ?? ''), ENT_COMPAT, CHARSET, true), 'id="message_html" class="editorHook form-control"');
              } else {
          ...
          <div class="form-group">
            ...
            <div class="col-sm-9"><?php echo zen_draw_textarea_field('message', 'soft', '', '15', htmlspecialchars($_POST['message'] ?? '', ENT_COMPAT, CHARSET, true), 'class="noEditor form-control"'); ?></div>
          </div>
          ...
          <?php if (defined('EMAIL_ATTACHMENTS_ENABLED') && EMAIL_ATTACHMENTS_ENABLED === true && defined('DIR_WS_ADMIN_ATTACHMENTS') && is_dir(DIR_WS_ADMIN_ATTACHMENTS) && is_writable(DIR_WS_ADMIN_ATTACHMENTS)) { ?>
            <?php if (defined('EMAIL_ATTACHMENT_UPLOADS_ENABLED') && EMAIL_ATTACHMENT_UPLOADS_ENABLED === true) { ?>
              ...
              <div class="form-group">
                ...
                <div class="col-sm-9"><?php echo zen_draw_file_field('upload_file') . '<br>' . stripslashes((string) $_POST['upload_file']) . zen_draw_hidden_field('prev_upload_file', stripslashes((string) $_POST['upload_file'])); ?></div>
