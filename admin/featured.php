...
if (!empty($action)) {
    ...
    switch ($action) {
        ...
        case 'insert':
            if (empty($_POST['products_id'])) {
                ...
                $db->Execute('INSERT INTO ' . TABLE_FEATURED . ' (products_id, featured_date_added, expires_date, status, featured_date_available)
                      VALUES (' . $products_id . ", now(), '" . zen_db_input(!empty($expires_date) ? $expires_date : '0001-01-01') . "', 1, '" . zen_db_input(!empty($featured_date_available) ? $featured_date_available : '0001-01-01') . "')");
        ...
        case 'update':
            ...
            zen_redirect(zen_href_link(FILENAME_FEATURED, ($currentPage != 0 ? 'page=' . $currentPage . '&' : '') . (isset($_GET['search']) ? 'search=' . $_GET['search'] . '&' : '') . 'fID=' . $featured_id));
            break;
        case 'deleteconfirm':
  ...
  <body>
    ...
    <div class="container-fluid">
      ...
      if (($action === 'new') || ($action === 'edit')) {
          ...
          ?>
        <div class="row">
          <?php echo zen_draw_form('new_featured', FILENAME_FEATURED, zen_get_all_get_params(['action', 'info', 'fID']) . 'action=' . $form_action . (!empty($_GET['go_back']) ? '&go_back=' . $_GET['go_back'] : ''), 'post'); ?>
          <?php
      ...
      <?php } else { ?>
        ...
        <div class="row">
          ...
          foreach ($featureds as $featured) {
            ...
            <div class="row">
              <div class="col-sm-6"><?php echo $featured_split->display_count($featured_query_numrows); ?></div>
              <div class="col-sm-6 text-right"><?php echo $featured_split->display_links($featured_query_numrows, MAX_DISPLAY_SEARCH_RESULTS_FEATURED_ADMIN, MAX_DISPLAY_PAGE_LINKS, $_GET['page']); ?></div>
            </div>
          </div>
          ...
          switch ($action) {
              ...
              case 'pre_add':
                  ...
                  $contents = ['form' => zen_draw_form('featured', FILENAME_FEATURED, 'action=pre_add_confirmation' . ($currentPage != 0 ? '&page=' . $currentPage : '') . (isset($_GET['search']) ? '&search=' . $_GET['search'] : ''), 'post')];
