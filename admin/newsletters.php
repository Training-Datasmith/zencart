    ...
    <div class="container-fluid">
      ...
      <div class="row">
        <div class="col-sm-offset-6 col-sm-6">
            ...
            <?php echo zen_draw_form('set_editor_form', FILENAME_NEWSLETTERS, '', 'get'); ?>
      ...
      if ($action == 'new') {
          ...
          foreach (zen_get_files_in_directory(DIR_WS_MODULES . 'newsletters') as $file) {
              $modules_array[] = [
                  'id' => basename((string) $file, '.php'),
                  'text' => basename((string) $file, '.php'),
              ];
          }
          ?>
        <?php
          echo zen_draw_form('newsletter', FILENAME_NEWSLETTERS, (isset($_GET['page']) ? 'page=' . $_GET['page'] . '&' : '') . 'action=' . $form_action, 'post');
        ...
        <?php
          ...
          $languageLoader->loadExtraLanguageFiles(DIR_WS_LANGUAGES, $_SESSION['language'], $nInfo->module . substr((string) $PHP_SELF, strrpos((string) $PHP_SELF, '.')), '/modules/newsletters/');
          include(DIR_WS_MODULES . 'newsletters/' . $nInfo->module . substr((string) $PHP_SELF, strrpos((string) $PHP_SELF, '.')));
        ...
        <div class="row">
          ...
          foreach ($newsletters as $newsletter) {
              if ((!isset($_GET['nID']) || (isset($_GET['nID']) && ($_GET['nID'] == $newsletter['newsletters_id']))) && !isset($nInfo) && (!str_starts_with((string) $action, 'new'))) {
        ...
        <div class="row">
          <table class="table">
            <tr>
              <td><?php echo $newsletters_split->display_count($newsletters_query_numrows); ?></td>
