...
foreach ($whos_online as $item) {
    ...
    ?>
                <td class="dataTableContentWhois <?php
    echo $item['icon_class'];
    ?>"><?php
    echo $item['icon_image'] . '&nbsp;' . gmdate('H:i:s', $item['time_online']);
    ?></td>
    ...
    ?>" rel="noreferrer noopener" target="_blank">
                        <?php
    echo '<i class="fa-solid fa-magnifying-glass"></i> <u>' . $item['ip_address'] . '</u>';
    ?>
                    </a>
                    <?php
    echo $additional_ipaddress_links;
    ?>
                </td>
                <td>&nbsp;</td>
                <td class="dataTableContentWhois text-center align-top"><?php
    echo date('H:i:s', $item['time_entry']);
    ?></td>
                <td class="dataTableContentWhois text-center align-top"><?php
    echo date('H:i:s', $item['time_last_click']);
    ?></td>
    ...
    ?>
                  <td class="dataTableContentWhois align-top" colspan=3>&nbsp;&nbsp;<?php
    echo TIME_PASSED_LAST_CLICKED . '<br>&nbsp;&nbsp;&nbsp;&nbsp;' . $item['time_since_last_click'];
    ?> ago</td>
    ...
    if (preg_match('/^(.*)' . zen_session_name() . '=[a-f,0-9]+[&]*(.*)/i', (string) $item['last_page_url'], $array)) {
...
if (!empty($selectedSession)) {
    ...
    $cart = $whos_online[$selectedSession]['cart'] ?? null;
...
?>
    ...
    <script>
        <?php
        $inspectLink = str_replace('&amp;', '&', zen_href_link(FILENAME_WHOS_ONLINE, zen_get_all_get_params(['inspect', 'action']) . 'inspect=[*]'));
?>
        jQuery(function () {
