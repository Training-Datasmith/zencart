...
foreach ($gv_lists as $gv_list) {
    ...
    if (isset($gInfo) && (is_object($gInfo)) && ($gv_list['unique_id'] == $gInfo->unique_id)) {
        ?>
                  <tr class="dataTableRowSelected" onclick="document.location.href = '<?php echo zen_href_link('gv_queue.php', zen_get_all_get_params(['gid', 'action']) . 'gid=' . $gInfo->unique_id . '&action=edit'); ?>'">
                  <?php } else { ?>
                  <tr class="dataTableRow" onclick="document.location.href = '<?php echo zen_href_link('gv_queue.php', zen_get_all_get_params(['gid', 'action']) . 'gid=' . $gv_list['unique_id']); ?>'">
                  <?php } ?>
    ...
    ?>
          ...
          <table class="table">
            <tr>
              <td><?php echo $gv_split->display_count($gv_query_numrows); ?></td>
...
switch ($_GET['action']) {
    case 'release':
        $heading[] = ['text' => '<h4>[' . $gInfo->unique_id . '] ' . zen_datetime_short($gInfo->date_created) . ' ' . $currencies->format($gInfo->amount) . '</h4>'];
        $contents = ['form' => zen_draw_form('gv_release', FILENAME_GV_QUEUE, 'action=confirmrelease&page=' . $_GET['page'])];
        $contents[] = ['align' => 'text-center', 'text' => zen_draw_hidden_field('gid', $gInfo->unique_id) . '<button type="submit" class="btn btn-primary">' . IMAGE_CONFIRM . '</button>&nbsp;<a href="' . zen_href_link('gv_queue.php', 'action=cancel&gid=' . $gInfo->unique_id . '&page=' . $_GET['page'], 'NONSSL') . '" class="btn btn-default" role="button">' . IMAGE_CANCEL . '</a>'];
    ...
    default:
        ...
        if (empty($gv_list)) {
            $contents[] = ['align' => 'text-center', 'text' => TEXT_GV_NONE];
        } else {
            $contents[] = ['align' => 'text-center', 'text' => '<a href="' . zen_href_link('gv_queue.php', 'action=release&gid=' . $gInfo->unique_id . '&page=' . $_GET['page'], 'NONSSL') . '" class="btn btn-primary" role="button">' . IMAGE_RELEASE . '</a>'];

            // quick link to order
            $contents[] = ['align' => 'text-center', 'text' => zen_image(DIR_WS_IMAGES . 'pixel_black.gif', '', '90%', '3')];
            $contents[] = ['align' => 'text-center', 'text' => TEXT_EDIT_ORDER . $gInfo->order_id];
            $contents[] = ['align' => 'text-center', 'text' => '<a href="' . zen_href_link(FILENAME_ORDERS, 'oID=' . $gInfo->order_id . '&action=edit', 'NONSSL') . '" class="btn btn-info" role="button">' . IMAGE_ORDER . '</a>'];
        }
        break;
}
$box = new box();
