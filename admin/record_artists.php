...
if (!empty($action)) {
    switch ($action) {
        ...
        case 'save':
            ...
            $sql_data_array = ['artists_name' => $artists_name];
            ...
            if ($_POST['artists_image_manual'] != '') {
                ...
                $artists_image->set_extensions(['jpg', 'jpeg', 'gif', 'png', 'webp', 'flv', 'webm', 'ogg']);
            ...
            for ($i = 0, $n = sizeof($languages); $i < $n; $i++) {
                ...
                $sql_data_array = ['artists_url' => zen_db_prepare_input($artists_url_array[$language_id])];
...
foreach ($artists as $artist) {
    if ((!isset($_GET['mID']) || (isset($_GET['mID']) && ($_GET['mID'] == $artist['artists_id']))) && !isset($aInfo) && (!str_starts_with((string) $action, 'new'))) {
    ...
    if (isset($aInfo) && is_object($aInfo) && ($artist['artists_id'] == $aInfo->artists_id)) {
        ?>
          ...
          if (isset($aInfo) && is_object($aInfo) && ($artist['artists_id'] == $aInfo->artists_id)) {
              ...
              echo '<a href="' . zen_href_link(FILENAME_RECORD_ARTISTS, zen_get_all_get_params(['mID']) . 'mID=' . $artist['artists_id']) . '" data-toggle="tooltip" title="' . IMAGE_ICON_INFO . '" role="button">' . zen_icon('circle-info', '', '2x', true, false) . '</a>';
          }
    ?>
                  </td>
                </tr>
...
switch ($action) {
    case 'new':
        $heading[] = ['text' => '<h4>' . TEXT_HEADING_NEW_RECORD_ARTIST . '</h4>'];
        $contents = ['form' => zen_draw_form('artists', FILENAME_RECORD_ARTISTS, 'action=insert', 'post')];
        $contents[] = ['text' => TEXT_NEW_INTRO];
        $contents[] = ['text' => zen_draw_label(TEXT_RECORD_ARTIST_NAME, 'artists_name', 'class="control-label"') . zen_draw_input_field('artists_name', '', zen_set_field_length(TABLE_RECORD_ARTISTS, 'artists_name') . ' class="form-control"')];
        $contents[] = ['text' => zen_draw_label(TEXT_RECORD_ARTIST_IMAGE, 'artists_image', 'class="control-label"') . zen_draw_file_field('artists_image', '')];

        $dir_info = zen_build_subdirectories_array(DIR_FS_CATALOG_IMAGES);
        $default_directory = 'artists/';

        $contents[] = ['text' => zen_draw_label(TEXT_ARTISTS_IMAGE_DIR, 'img_dir', 'class="control-label"') . zen_draw_pull_down_menu('img_dir', $dir_info, $default_directory, 'class="form-control"')];
        $contents[] = ['text' => zen_draw_label(TEXT_IMAGE_MANUAL, 'artists_image_manual', 'class="control-label"') . zen_draw_input_field('artists_image_manual', '', 'class="form-control"')];
        ...
        $contents[] = ['text' => zen_draw_label(TEXT_RECORD_ARTIST_URL, 'artists_url', 'class="control-label"') . $manufacturer_inputs_string];
        $contents[] = ['align' => 'center', 'text' => '<button type="submit" class="btn btn-primary">' . IMAGE_SAVE . '</button> <a href="' . zen_href_link(FILENAME_RECORD_ARTISTS, 'page=' . $_GET['page'] . (isset($_GET['mID']) ? '&mID=' . $_GET['mID'] : '')) . '" class="btn btn-default" role="button">' . IMAGE_CANCEL . '</a>'];
        break;
    case 'edit':
        $heading[] = ['text' => '<h4>' . TEXT_HEADING_EDIT_RECORD_ARTIST . '</h4>'];
        $contents = ['form' => zen_draw_form('artists', FILENAME_RECORD_ARTISTS, 'page=' . $_GET['page'] . $rArtists_parameter . '&action=save', 'post')];
        $contents[] = ['text' => TEXT_INFO_EDIT_INTRO];
        $contents[] = ['text' => zen_draw_label(TEXT_RECORD_ARTIST_NAME, 'artists_name', 'class="control-label"') . zen_draw_input_field('artists_name', htmlspecialchars((string) $aInfo->artists_name, ENT_COMPAT, CHARSET, true), zen_set_field_length(TABLE_RECORD_ARTISTS, 'artists_name') . ' class="form-control"')];
        $contents[] = ['text' => zen_draw_label(TEXT_RECORD_ARTIST_IMAGE, 'artists_image', 'class="control-label"') . zen_draw_file_field('artists_image', '') . '<br>' . $aInfo->artists_image];

        $dir_info = zen_build_subdirectories_array(DIR_FS_CATALOG_IMAGES);
        $default_directory = substr((string) $aInfo->artists_image, 0, strpos((string) $aInfo->artists_image, '/') + 1);

        $contents[] = ['text' => zen_draw_label(TEXT_ARTISTS_IMAGE_DIR, 'img_dir', 'class="control-label"') . zen_draw_pull_down_menu('img_dir', $dir_info, $default_directory, 'class="form-control"')];
        $contents[] = ['text' => zen_draw_label(TEXT_IMAGE_MANUAL, 'artists_image_manual', 'class="control-label"') . zen_draw_input_field('artists_image_manual', '', 'class="form-control"')];
        $contents[] = ['text' => zen_info_image($aInfo->artists_image, $aInfo->artists_name)];
        $manufacturer_inputs_string = '';
        $languages = zen_get_languages();
        for ($i = 0, $n = sizeof($languages); $i < $n; $i++) {
        ...
        $contents[] = ['text' => zen_draw_label(TEXT_RECORD_ARTIST_URL, 'artists_url', 'class="control-label"') . $manufacturer_inputs_string];
        $contents[] = ['align' => 'center', 'text' => '<button type="submit" class="btn btn-primary">' . IMAGE_SAVE . '</button> <a href="' . zen_href_link(FILENAME_RECORD_ARTISTS, 'page=' . $_GET['page'] . $rArtists_parameter) . '" class="btn btn-default" role="button">' . IMAGE_CANCEL . '</a>'];
        break;
    case 'delete':
        $heading[] = ['text' => '<h4>' . TEXT_HEADING_DELETE_RECORD_ARTIST . '</h4>'];

        $contents = ['form' => zen_draw_form('artists', FILENAME_RECORD_ARTISTS, 'page=' . $_GET['page'] . '&action=deleteconfirm') . zen_draw_hidden_field('mID', $aInfo->artists_id)];
        $contents[] = ['text' => TEXT_DELETE_INTRO];
        $contents[] = ['text' => '<br><b>' . $aInfo->artists_name . '</b>'];
        ...
        $contents[] = ['align' => 'center', 'text' => '<button type="submit" class="btn btn-danger">' . IMAGE_DELETE . '</button> <a href="' . zen_href_link(FILENAME_RECORD_ARTISTS, 'page=' . $_GET['page'] . $rArtists_parameter) . '" class="btn btn-default" role="button">' . IMAGE_CANCEL . '</a>'];
        break;
    default:
        if (isset($aInfo) && is_object($aInfo)) {
            $heading[] = ['text' => '<h4>' . $aInfo->artists_name . '</h4>'];

            $contents[] = ['align' => 'center', 'text' => '<a href="' . zen_href_link(FILENAME_RECORD_ARTISTS, 'page=' . $_GET['page'] . $rArtists_parameter . '&action=edit') . '" class="btn btn-primary" role="button">' . IMAGE_EDIT . '</a> <a href="' . zen_href_link(FILENAME_RECORD_ARTISTS, 'page=' . $_GET['page'] . $rArtists_parameter . '&action=delete') . '" class="btn btn-warning" role="button">' . IMAGE_DELETE . '</a>'];
            ...
            $contents[] = ['text' => '<br>' . zen_info_image($aInfo->artists_image, $aInfo->artists_name)];
...
?>
      ...
      <table class="table">
        <tr>
          <td><?php echo $artists_split->display_count($artists_query_numrows); ?></td>
