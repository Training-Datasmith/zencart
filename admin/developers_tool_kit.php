...
if (!empty($_POST) && !isset($_POST['context_lines'])) {
    $_POST['context_lines'] = abs($default_context_lines);
}

if (!empty($_GET['configuration_key_lookup'])) {
    $_POST['configuration_key'] = strtoupper((string) $_GET['configuration_key_lookup']);
    $_POST['zv_files'] = 1;
...
function zen_display_files($include_root = false, $filetypesincluded = 1): bool
{
    ...
    for ($i = 0, $n = count($check_directory); $i < $n; $i++) {
        ...
        $file_extensions = match ($filetypesincluded) {
            1 => ['.php'],
            2 => ['.php', '.css'],
            3 => ['.css'],
            4 => ['.html', '.txt'],
    ...
    if (strtoupper((string) $configuration_key_lookup) == $configuration_key_lookup) {
        ...
        $sql = $db->BindVars($sql, ':zcconfigkey:', strtoupper((string) $configuration_key_lookup), 'string');
    ...
    for ($i = 0, $n = count($directory_array); $i < $n; $i++) {
        ...
        if (file_exists($file)) {
            ...
            foreach ($lines as $line_num => $line) {
                ...
                $search_pattern = preg_quote((string) $configuration_key_lookup, '#');
 ...
 * @since ZC v1.5.5
 */
function cleanup_dtk_output_text($input = '', ?string $highlight = '', $case_sensitive = false): null|false|int|float|string|array
{
    if ($input == '') {
        return $input;
    }
    ...
    // sanitize output
    $input = htmlspecialchars((string) $input, ENT_QUOTES, CHARSET);
...
$search = $_POST['search'] ?? '';
$flags = (isset($_GET['v']) ? '&v=' : '') . (isset($_GET['s']) ? '&s=' . preg_replace('/[^a-z]/', '', (string) $_GET['s']) : '');
        ...
        switch ($zv_files_group) {
            ...
            case (2): // all catalog
                ...
                for ($i = 0, $n = count($check_dir); $i < $n; $i++) {
                    ...
                    if (strstr($zv_add_dir, (string) DIR_WS_ADMIN) == '') {
          ...
          <tr>
            ...
            <td class="main text-center">
                ...
                if ($show_products_type_layout == false and ($check_configure->fields['configuration_id'] != 0 and $check_configure_group->fields['visible'] != 0)) {
                    ...
                    $page = '';
                    if (strstr((string) $check_configure->fields['configuration_key'], 'MODULE_SHIPPING')) {
                        $page .= 'shipping';
                    }
                    if (strstr((string) $check_configure->fields['configuration_key'], 'MODULE_PAYMENT')) {
                        $page .= 'payment';
                    }
                    if (strstr((string) $check_configure->fields['configuration_key'], 'MODULE_ORDER_TOTAL')) {
                        $page .= 'ordertotal';
                    }
...
// disabled and here for an example
      if (false) {
        ...
        <?php echo zen_draw_form('locate_configure', FILENAME_DEVELOPERS_TOOL_KIT, 'action=locate_configuration', 'post'); ?>
      ...
      ?>
        ...
        <?php echo zen_draw_form('search_keys', FILENAME_DEVELOPERS_TOOL_KIT, 'action=search_config_keys' . $flags, 'post'); ?>
        <div class="form-group">
            ...
            <table class="table">
              ...
              <tbody>
                  ...
                  <tr class="dataTableRow">
                    ...
                    <td class="<?php echo $tdClass; ?>"><?php echo htmlspecialchars((string) $keySearchResult['configuration_value']); /* implode("<br>\n", preg_split("/[\s,.]+/", $configuration->fields['configuration_value'])) */ ?></td>
      ...
      ?>
        ...
        <?php echo zen_draw_form('locate_function', FILENAME_DEVELOPERS_TOOL_KIT, 'action=locate_function', 'post'); ?>
        <div class="form-group">
      ...
      ?>
        ...
        <?php echo zen_draw_form('locate_class', FILENAME_DEVELOPERS_TOOL_KIT, 'action=locate_class', 'post'); ?>
        <div class="form-group">
      ...
      ?>
        ...
        <?php echo zen_draw_form('locate_template', FILENAME_DEVELOPERS_TOOL_KIT, 'action=locate_template', 'post'); ?>
        <div class="form-group">
      ...
      ?>
        ...
        <?php echo zen_draw_form('locate_all_files', FILENAME_DEVELOPERS_TOOL_KIT, 'action=locate_all_files', 'post'); ?>
        <div class="form-group">
      ...
      ?>
        ...
        <div class="form-group">
          ...
          <div class="col-sm-9 col-md-6">
            <div class="checkbox">
              <label for="include_zc_plugins"><?php echo zen_draw_checkbox_field('include_plugins', true, $include_plugins, ''); ?></label>
            </div>
          </div>
        </div>
        <div class="form-group">
      ...
      ?>
        ...
        <div class="form-group">
          ...
          <div class="col-sm-9 col-md-6"><?php echo zen_draw_input_field('context_lines', strval($default_context_lines), 'id="context_lines" class="form-control" size="1"'); ?></div>
        </div>
        <div class="form-group">
