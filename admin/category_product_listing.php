...
?>
    ...
    <div class="container-fluid">
        <h1>
            <a href="<?php 
<?= zen_catalog_href_link('index', zen_get_path($current_category_id)) ?>
?>" rel="noopener" target="_blank" title="<?php 
<?= BOX_HEADING_CATALOG ?>
?>"><?php 
...
<?= HEADING_TITLE ?>
?>&nbsp;-&nbsp;<?php 
<?= zen_output_generated_category_path($current_category_id) ?>
...
if ($action === '') { ?>
            <div class="row">
                ...
                <div class="col-md-4">
                    <?= zen_draw_form('set_editor_form', FILENAME_CATEGORY_PRODUCT_LISTING, '', 'get') ?>
                    ...
                    echo zen_draw_form('set_categories_products_sort_order_form', FILENAME_CATEGORY_PRODUCT_LISTING, '', 'get');
                    ?>
                    <div class="form-group">
                ...
                <?php require DIR_WS_MODULES . 'search_box.php'; ?>
                    <?= zen_draw_form('goto', FILENAME_CATEGORY_PRODUCT_LISTING, '', 'get') ?>
                    <?= zen_hide_session_id() ?>
                    <div class="form-group">
...
?>
        <div class="row"><?php 
<?= zen_draw_separator('pixel_black.gif') ?>
?></div>
        <div class="row">
            <div<?php 
<?= (empty($action)) ? '' : ' class="col-xs-12 col-sm-12 col-md-9 col-lg-9 configurationColumnLeft"' ?>
?>>
                <?php 
$order_by = match ($_SESSION['categories_sort_order']) {
    0 => " ORDER BY c.sort_order, cd.categories_name",
    1 => " ORDER BY cd.categories_name",
    2 => " ORDER BY cd.categories_id",
    3 => " ORDER BY cd.categories_id DESC",
    4 => " ORDER BY c.categories_status, cd.categories_name",
    5 => " ORDER BY c.categories_status, cd.categories_name DESC",
    default => " ",
};
$categories_count = 0;
...
?></th>
                            <th><?php 
<?= TABLE_HEADING_CATEGORIES_PRODUCTS ?>
?></th>
                            <th class="hidden-sm hidden-xs imageView <?php 
<?= $additionalClass ?>
?>"><?php 
<?= TABLE_HEADING_IMAGE ?>
?></th>
...
// Get parent_id for subcategories if search
    ...
    $cPath = $category['parent_id'];
                        ...
                        if ((!isset($_GET['cID']) && !isset($_GET['pID']) || (isset($_GET['cID']) && ($_GET['cID'] == $category['categories_id']))) && !isset($cInfo) && (!str_starts_with((string) $action, 'new'))) {
...
$order_by = match ($_SESSION['products_sort_order']) {
    0 => " ORDER BY p.products_sort_order, pd.products_name",
    1 => " ORDER BY pd.products_name",
    2 => " ORDER BY p.products_model",
    3 => " ORDER BY p.products_quantity, pd.products_name",
    4 => " ORDER BY p.products_quantity DESC, pd.products_name",
    5 => " ORDER BY p.products_price_sorter, pd.products_name",
    6 => " ORDER BY p.products_price_sorter DESC, pd.products_name",
    7 => " ORDER BY p.products_model DESC",
    8 => " ORDER BY p.products_status",
    9 => " ORDER BY p.products_status DESC",
    10 => " ORDER BY p.products_id",
    11 => " ORDER BY p.products_weight",
    default => " ",
};
$products_count = 0;
// -----
...
// Get categories_id for product if search
    ...
    if ((!isset($_GET['pID']) && !isset($_GET['cID']) || (isset($_GET['pID']) && ($_GET['pID'] === (int)$product['products_id']))) && !isset($pInfo) && !isset($cInfo) && (!str_starts_with((string) $action, 'new'))) {
...
switch ($action) {
    case 'setflag_categories':
        ...
        $contents = ['form' => zen_draw_form('categories', FILENAME_CATEGORY_PRODUCT_LISTING, zen_get_all_get_params(['action']) . 'action=update_category_status', 'post') . zen_draw_hidden_field('categories_id', $cInfo->categories_id) . zen_draw_hidden_field('categories_status', $cInfo->categories_status)];
    ...
    case 'move_category':
        ...
        $contents = ['form' => zen_draw_form('move_category', FILENAME_CATEGORY_PRODUCT_LISTING, 'action=move_category_confirm&cPath=' . $cPath, 'post') . zen_draw_hidden_field('categories_id', $cInfo->categories_id)];
    ...
    case 'attribute_features_copy_to_category':
        ...
        $heading[] = ['text' => '<h4>' . TEXT_INFO_HEADING_ATTRIBUTE_FEATURES . $pInfo->products_id . '</h4>'];
        $contents = ['form' => zen_draw_form('products', FILENAME_CATEGORY_PRODUCT_LISTING, 'action=update_attributes_copy_to_category&cPath=' . $cPath . (isset($_GET['page']) ? '&page=' . $_GET['page'] : ''), 'post') . zen_draw_hidden_field('products_id', $pInfo->products_id) . zen_draw_hidden_field('categories_update_id', $_GET['categories_update_id']) . zen_draw_hidden_field('copy_attributes', $_GET['copy_attributes'] ?? '')];
        $contents[] = ['text' => zen_draw_label(TEXT_COPY_ATTRIBUTES_CONDITIONS, 'copy_attributes', 'class="control-label"') . '<div class="radio"><label>' . zen_draw_radio_field('copy_attributes', 'copy_attributes_delete', true) . TEXT_COPY_ATTRIBUTES_DELETE . '</label></div><div class="radio"><label>' . zen_draw_radio_field('copy_attributes', 'copy_attributes_update') . TEXT_COPY_ATTRIBUTES_UPDATE . '</label></div><div class="radio"><label>' . zen_draw_radio_field('copy_attributes', 'copy_attributes_ignore') . TEXT_COPY_ATTRIBUTES_IGNORE . '</label></div>'];
...
?>
    ...
    <div class="col-md-9 text-right">
        ...
        <?php if ($zc_skip_products === false) { ?>
                        <?= zen_draw_form('newproduct', FILENAME_PRODUCT, 'action=new_product', 'post') ?>
...
// Split Page
    if ($products_query_numrows > 0) {
        echo $products_split->display_count($products_query_numrows) . '<br>' . $products_split->display_links($products_query_numrows, $max_results, MAX_DISPLAY_PAGE_LINKS, $_GET['page']);
    }
    ?>
            </div>
        <?php }
...
?>
            jQuery(function () {
                    const categorySelectLink = '<?php 
<?= $categorySelectLink ?>
?>';
                    const productEditLink = '<?php 
<?= $productEditLink ?>
?>';
