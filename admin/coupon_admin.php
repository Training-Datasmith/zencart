...
if ($_GET['action'] == 'send_email_to_user' && !empty($_POST['customers_email_address'])) {
    ...
    zen_redirect(zen_href_link(FILENAME_COUPON_ADMIN, 'mail_sent_to=' . urlencode((string) $mail_sent_to) . '&recip_count=' . $recip_count));
}
...
switch ($_GET['action']) {
    ...
    case 'confirmcopy':
        $coupon_copy_to = trim((string) $_POST['coupon_copy_to']);
    ...
    case 'update':
        $update_errors = 0;
        $_POST['coupon_code'] = trim((string) $_POST['coupon_code']);
        for ($i = 0, $n = count($languages); $i < $n; $i++) {
            $language_id = $languages[$i]['id'];
            $_POST['coupon_name'][$language_id] = trim((string) $_POST['coupon_name'][$language_id]);
            if (!$_POST['coupon_name'][$language_id]) {
                $update_errors = 1;
                $messageStack->add(ERROR_NO_COUPON_NAME . $languages[$i]['name'], 'error');
            }
            $_POST['coupon_desc'][$language_id] = trim((string) $_POST['coupon_desc'][$language_id]);
        }
        $_POST['coupon_amount'] = trim((string) $_POST['coupon_amount']);
    ...
    case 'update_confirm':
        ...
        if (str_ends_with((string) $_POST['coupon_amount'], '%') && $_POST['coupon_free_ship']) {
            $coupon_type = 'E'; // percentage off and free shipping
        }
        $_POST['coupon_amount'] = preg_replace('/[^0-9.]/', '', (string) $_POST['coupon_amount']);
        $sql_data_array = [
        ...
        if ($_GET['oldaction'] == 'voucheredit') {
            ...
            // referrers
            $trimmed_referrers = array_map(trim(...), explode(',', $_POST['coupon_referrer'] ?? []));
    ...
    switch ($_GET['action']) {
        case 'voucherreport':
            ...
            foreach ($cc_list as $item) {
              ...
              <table class="table">
                <tr>
                  <td><?= $cc_split->display_count($cc_query_numrows) ?></td>
                  <td class="text-right"><?= $cc_split->display_links($cc_query_numrows, MAX_DISPLAY_SEARCH_RESULTS_DISCOUNT_COUPONS_REPORTS, MAX_DISPLAY_PAGE_LINKS, $_GET['reports_page']) ?></td>
                </tr>
                <tr>
        ...
        case 'preview_email':
            ...
            <table class="table">
              ...
              <tr>
                ...
                <td><?= htmlspecialchars(stripslashes((string) $_POST['from']), ENT_COMPAT, CHARSET, true) ?></td>
              </tr>
              <tr>
                <td class="text-right"><b><?= TEXT_SUBJECT ?></b></td>
                <td><?= htmlspecialchars(stripslashes((string) $_POST['subject']), ENT_COMPAT, CHARSET, true) ?></td>
              </tr>
              <?php if (EMAIL_USE_HTML == 'true') { ?>
                <tr>
                  <td class="text-right"><hr><b><?= TEXT_RICH_TEXT_MESSAGE ?></b></td>
                  <td><?= stripslashes((string) $_POST['message_html']) ?></td>
                </tr>
              <?php } ?>
              <tr>
                <td class="text-right"><b><?= TEXT_MESSAGE ?></b></td>
                <td class="tt"><?= nl2br(htmlspecialchars(stripslashes((string) $_POST['message']), ENT_COMPAT, CHARSET, true)) ?></td>
              </tr>
              <tr>
                <td>
                  <?php
        ...
        case 'email':
            $coupon_result = $db->Execute('SELECT c.coupon_code, cd.coupon_name
                                           ...
                                           WHERE c.coupon_id = ' . (int)$_GET['cid']);
            echo zen_draw_form('mail', FILENAME_COUPON_ADMIN, 'action=preview_email&cid=' . (int)$_GET['cid'], 'post');
            ?>
            ...
            <?php if (EMAIL_USE_HTML == 'true') { ?>
              <div class="form-group">
                ...
                <div class="col-sm-9 col-md-6"><?= zen_draw_textarea_field('message_html', 'soft', '', '25', htmlspecialchars(empty($_POST['message_html']) ? TEXT_COUPON_ANNOUNCE : stripslashes((string) $_POST['message_html']), ENT_COMPAT, CHARSET, true), 'id="message_html" class="editorHook form-control" id="message_html"') ?></div>
              </div>
            <?php } ?>
            <div class="form-group">
              <?= zen_draw_label(TEXT_MESSAGE, 'message', 'class="control-label col-sm-3"') ?>
              <div class="col-sm-9 col-md-6"><?= zen_draw_textarea_field('message', 'soft', '60', '15', htmlspecialchars(strip_tags((!isset($_POST['message_html']) || $_POST['message_html'] == '') ? TEXT_COUPON_ANNOUNCE : stripslashes((string) $_POST['message_html'])), ENT_COMPAT, CHARSET, true), 'class="noEditor form-control" id="message"') ?></div>
            </div>
            <div class="form-group">
        ...
        case 'update_preview':
            ...
            ?>
              ...
              <tr>
                ...
                <td><?php
                  ...
                  $trimmed_referrers = array_map(trim(...), $_POST['coupon_referrer'] ?? []);
            $referrers = array_filter($trimmed_referrers, fn (string $referrer): bool => !empty(trim($referrer)));
            ...
            echo $referrers ?: 'none'; ?></td>
              ...
              for ($i = 0, $n = count($languages); $i < $n; $i++) {
                  $language_id = $languages[$i]['id'];
                  echo zen_draw_hidden_field('coupon_name[' . $languages[$i]['id'] . ']', stripslashes((string) $_POST['coupon_name'][$language_id]));
                  echo zen_draw_hidden_field('coupon_desc[' . $languages[$i]['id'] . ']', stripslashes((string) $_POST['coupon_desc'][$language_id]));
              }
            ...
            echo zen_draw_hidden_field('coupon_code', stripslashes((string) $c_code));
        ...
        case 'new':
            ...
            echo zen_draw_form('coupon', FILENAME_COUPON_ADMIN, 'action=update&oldaction=' . $_GET['action'] . '&cid=' . $_GET['cid'] . (isset($_GET['status']) ? '&status=' . $_GET['status'] : '') . (isset($_GET['page']) ? '&page=' . $_GET['page'] : ''), 'post');
            ?>
            <div class="form-group">
          <?= zen_draw_label(COUPON_NAME, 'coupon_name[' . $languages[0]['id'] . ']', 'class="control-label col-sm-3"') ?>
              <div class="col-sm-9 col-md-6">
                ...
                for ($i = 0, $n = count($languages); $i < $n; $i++) {
                  ...
                  <div class="input-group">
                    ...
                    <?= zen_draw_input_field('coupon_name[' . $languages[$i]['id'] . ']', (!empty($coupon_name[$language_id]) ? htmlspecialchars(stripslashes((string) $coupon_name[$language_id]), ENT_COMPAT, CHARSET, true) : ''), zen_set_field_length(TABLE_COUPONS_DESCRIPTION, 'coupon_name') . ' id="coupon_name[' . $languages[$i]['id'] . ']" class="form-control"') ?>
                    <?php if ($i == 0) { ?>
                      <span class="input-group-addon">
              ...
              <div class="col-sm-9 col-md-6">
                ...
                for ($i = 0, $n = count($languages); $i < $n; $i++) {
                  ...
                  <div class="input-group">
                    ...
                    <?= zen_draw_textarea_field('coupon_desc[' . $languages[$i]['id'] . ']', 'hard', '24', '8', (!empty($coupon_desc[$language_id]) ? htmlspecialchars(stripslashes((string) $coupon_desc[$language_id]), ENT_COMPAT, CHARSET, true) : ''), 'id="coupon_desc[' . $languages[$i]['id'] . ']" class="editorHook form-control"') ?>
                    <?php if ($i == 0) { ?>
                      <span class="input-group-addon">
            ...
            <div class="form-group">
              ...
              <div class="col-sm-9 col-md-6">
                <label><?= zen_draw_checkbox_field('coupon_free_ship', '', (!empty($coupon_free_ship)), '') ?></label>
            ...
            <div class="form-group">
              ...
              <div class="col-sm-9 col-md-6">
                <div class="input-group"><?= zen_draw_input_field('coupon_code', (!empty($coupon_code) ? htmlspecialchars((string) $coupon_code, ENT_COMPAT, CHARSET, true) : ''), 'class="form-control" id="coupon_code"') ?>
                  <span class="input-group-addon">
        ...
        default:
            ...
            echo zen_draw_form('statusForm', FILENAME_COUPON_ADMIN, '', 'get');
            ?>
              <div class="form-group">
            ...
            <div class="col-sm-4">
              <?= zen_draw_form('set_editor_form', FILENAME_COUPON_ADMIN, '', 'get') ?>
              <div class="form-group">
            ...
            foreach ($cc_list as $item) {
                ...
                ?>
                      ...
                      <td class="dataTableContent<?= (strtotime((string) $item['coupon_start_date']) > time() ? ' coupon-future' : '') ?> text-center"><?= zen_date_short($item['coupon_start_date']) ?></td>
                      <td class="dataTableContent<?= (strtotime((string) $item['coupon_expire_date']) < time() ? ' coupon-expired' : '') ?> text-center"><?= zen_date_short($item['coupon_expire_date']) ?></td>
              ...
              <table class="table">
                <tr>
                  <td><?= $cc_split->display_count($cc_query_numrows) ?></td>
                  <td class="text-right"><?= $cc_split->display_links($cc_query_numrows, $maxDisplaySearchResults, MAX_DISPLAY_PAGE_LINKS, $_GET['page']) ?></td>
                </tr>
                <tr>
            ...
            switch ($_GET['action']) {
                ...
                case 'vouchercopy':
                    $heading[] = ['text' => '<h4>[' . $cInfo->coupon_id . ']  ' . $cInfo->coupon_code . ($cInfo->coupon_id == '' ? ' - (' . (!empty($_GET['cid']) ? $_GET['cid'] : 0) . ')' : '') . '</h4>'];
                    $contents = ['form' => zen_draw_form('new_coupon', FILENAME_COUPON_ADMIN, 'action=confirmcopy' . '&cid=' . $_GET['cid'] . (isset($_GET['status']) ? '&status=' . $_GET['status'] : '') . (isset($_GET['page']) ? '&page=' . $_GET['page'] : ''), 'post')];
                ...
                case 'voucherduplicate':
                    $heading[] = ['text' => '<h4>[' . $cInfo->coupon_id . ']  ' . $cInfo->coupon_code . ($cInfo->coupon_id == '' ? ' - (' . (!empty($_GET['cid']) ? $_GET['cid'] : 0) . ')' : '') . '</h4>'];
                    $contents = ['form' => zen_draw_form('duplicate_coupon', FILENAME_COUPON_ADMIN, 'action=confirmcopyduplicate' . '&cid=' . $_GET['cid'] . (isset($_GET['status']) ? '&status=' . $_GET['status'] : '') . (isset($_GET['page']) ? '&page=' . $_GET['page'] : ''), 'post')];
                ...
                case 'voucherduplicatedelete':
                    $chk_duplicate_delete = $db->Execute('SELECT *
                                                      ...
                                                      WHERE coupon_id = ' . (int)$_GET['cid']);
                    $heading[] = ['text' => '<h4>[' . $cInfo->coupon_id . ']  ' . $cInfo->coupon_code . ($cInfo->coupon_id == '' ? ' - (' . (!empty($_GET['cid']) ? $_GET['cid'] : 0) . ')' : '') . '</h4>'];
                    $contents = ['form' => zen_draw_form('duplicate_coupon_delete', FILENAME_COUPON_ADMIN, 'action=confirmdeleteduplicate' . '&cid=' . $_GET['cid'] . (isset($_GET['status']) ? '&status=' . $_GET['status'] : '') . (isset($_GET['page']) ? '&page=' . $_GET['page'] : ''), 'post')];
