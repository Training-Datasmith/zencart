...
if (isset($hist_details->fields['project_version_patch']) && zen_not_null($hist_details->fields['project_version_patch'])) {
    $sinfo .= '&nbsp;&nbsp;Patch: ' . $hist_details->fields['project_version_patch'];
}
if (isset($hist_details->fields['project_version_date_applied']) && zen_not_null($hist_details->fields['project_version_date_applied'])) {
    $sinfo .= ' &nbsp;&nbsp;[' . $hist_details->fields['project_version_date_applied'] . '] ';
}
if (isset($hist_details->fields['project_version_comment']) && !empty($hist_details->fields['project_version_comment'])) {
...
while (!$hist_details->EOF) {
    ...
    if (!empty($hist_details->fields['project_version_comment'])) {
        $sinfo .= ' &nbsp;&nbsp;(' . $hist_details->fields['project_version_comment'] . ')';
    }
    $sinfo .=  '<br>';
...
if (!str_contains($disabled_functions, 'phpinfo')) {
    ?>
<div class="phpinfo">
<?php
...
foreach ($show_variables as $variable) {
    ?>
        <tr class="db-row">
            ...
            <td class="db-info db-value"><?= htmlspecialchars((string) $variable['Value']) ?></td>
        </tr>
        <?php
}
?>
