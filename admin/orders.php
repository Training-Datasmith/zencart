...
if (!isset($_GET['statusFilterSelect'])) {
...
// setting isn't set!
//
if (!defined('NOTIFY_CUSTOMER_DEFAULT')) {
    define('NOTIFY_CUSTOMER_DEFAULT', '1');
}
...
if (!empty($action) && $order_exists === true) {
    ...
    switch ($action) {
        ...
        case 'update_order':
            ...
            if ($order_updated === true) {
                ...
                if ($customer_notified === 1) {
                    $messageStack->add_session(sprintf(SUCCESS_EMAIL_SENT, ($admin_language !== $_SESSION['languages_code'] ? '(' . strtoupper((string) $_SESSION['languages_code']) . ') ' : '')), 'success'); // show an email sent confirmation message, with a language indicator if the order/email language was different to the admin user language
                }
        ...
        case 'mask_cc':
            ...
            $new_num = substr((string) $old_num, 0, 4) . str_repeat('*', (strlen((string) $old_num) - 8)) . substr((string) $old_num, -4);
...
if ($action === '') { ?>
                ...
                <div class="row noprint">
                    <div class="form-inline">
                        <div class="form-group col-xs-6 col-sm-3 col-md-3 col-lg-3">
                                <?php
                    echo zen_draw_form('search', FILENAME_ORDERS, '', 'get');
    ...
    if (!empty($_GET['search']) || !empty($_GET['cID'])) {
        ?>
                        ...
                        <div class="form-group col-xs-6 col-sm-3 col-md-3 col-lg-3">
                                <?php
                                echo zen_draw_form('search_orders_products', FILENAME_ORDERS, '', 'get');
    ...
    if (!empty($_GET['search_orders_products']) || !empty($_GET['cID'])) {
        ?>
                        ...
                        <div class="form-group col-xs-6 col-sm-3 col-md-3 col-lg-3">
                                <?php
                                echo zen_draw_form('orders', FILENAME_ORDERS, '', 'get');
    ...
    ?>
                        ...
                        <div class="form-group col-xs-6 col-sm-3 col-md-3 col-lg-3">
                                <?php
    echo zen_draw_form('statusFilterForm', FILENAME_ORDERS, '', 'get');
...
<?php
                ...
                <div class="row">
                    ...
                    <div class="col-sm-6 col-lg-4 noprint">
                        <div class="input-group">
                            ...
                            <?php
                echo zen_draw_form('input_oid', FILENAME_ORDERS, '', 'get');
    ...
    if (!empty($order->info['ip_address'])) {
        $ips = explode(' - ', (string) $order->info['ip_address']);
        $lookup_ip = $ips[0];
    ...
    if (!empty($order->info['cc_type']) || !empty($order->info['cc_owner']) || !empty($order->info['cc_number'])) {
        ...
        if (!empty($order->info['cc_cvv'])) { ?>
                                <tr>
                                    ...
                                    <td class="main"><?= $order->info['cc_cvv'] . (!empty($order->info['cc_cvv']) && !strstr((string) $order->info['cc_cvv'], (string) TEXT_DELETE_CVV_REPLACEMENT) ? '&nbsp;&nbsp;<a href="' . zen_href_link(FILENAME_ORDERS, '&action=delete_cvv&oID=' . $oID, 'NONSSL') . '" class="noprint">' . TEXT_DELETE_CVV_FROM_DATABASE . '</a>' : '') ?></td>
                                </tr>
                            <?php } ?>
...
<?php
    ...
    ?>
                ...
                <div class="row noprint">
                    <div class="formArea">
                            <?php
                                echo zen_draw_form('statusUpdateForm', FILENAME_ORDERS, zen_get_all_get_params(['action', 'language']) . 'action=update_order&language=' . $order->info['language_code'], 'post');
    ...
    if (count(zen_get_languages()) > 1) {
        echo '<br>' . zen_get_language_icon($order->info['language_code']) . ' <strong>' . sprintf(TEXT_EMAIL_LANGUAGE, ucfirst((string) zen_get_language_name($order->info['language_code']))) . '</strong>';
...
<?php
    ...
    ?>
                        ...
                        <div class="form-group">
                            ...
                            <div class="col-sm-9">
                                <?= zen_draw_checkbox_field('notify_comments', '', true, '') ?>
                            </div>
                        </div>
                        <div class="form-group">
...
<?php
    ...
    if (!empty($_GET['search_orders_products'])) {
        ...
        if (str_starts_with(strtoupper((string) $_GET['search_orders_products']), 'ID:')) {
            $keywords = trim(substr((string) $_GET['search_orders_products'], 3));
    ...
    while (!$orders->EOF) {
        ...
        $show_difference = '';
        if (!empty($orders->fields['delivery_name']) && (strtoupper((string) $orders->fields['delivery_name']) !== strtoupper((string) $orders->fields['billing_name']))) {
            $show_difference = zen_icon('status-red', TEXT_BILLING_SHIPPING_MISMATCH) . '&nbsp;';
        }
        if (!empty($orders->fields['delivery_street_address']) && (strtoupper((string) $orders->fields['delivery_street_address']) !== strtoupper((string) $orders->fields['billing_street_address']))) {
            $show_difference = zen_icon('status-red', TEXT_BILLING_SHIPPING_MISMATCH) . '&nbsp;';
        }
        if (strtoupper((string) $orders->fields['delivery_postcode']) !== strtoupper((string) $orders->fields['billing_postcode']) && trim((string) $orders->fields['delivery_postcode']) !== '') {
            $show_difference = zen_icon('status-red', TEXT_BILLING_SHIPPING_MISMATCH) . '&nbsp;';
        }
        ...
        if ($quick_view_popover_enabled) {
            ...
            $product_details = nl2br((string) $product_details);
        }
        ?>
...
<?php } ?>
        ...
        $status_color = $status_colors[$current_status_id] ?? '';
...
<?php
    ...
    ?>
                        ...
                        <table class="table">
                            <tr>
                                    <td><?= $orders_split->display_count($orders_query_numrows) ?></td>
                                    <td class="text-right"><?= $orders_split->display_links($orders_query_numrows, MAX_DISPLAY_SEARCH_RESULTS_ORDERS, MAX_DISPLAY_PAGE_LINKS, $_GET['page']) ?></td>
                            </tr>
                            <?php
    ...
    switch ($action) {
        case 'delete':
            ...
            $contents = ['form' => zen_draw_form('orders', FILENAME_ORDERS, zen_get_all_get_params(['oID', 'action']) . '&action=deleteconfirm', 'post') . zen_draw_hidden_field('oID', $oInfo->orders_id)];
        ...
        default:
            if (isset($oInfo) && is_object($oInfo)) {
                ...
                $contents[] = ['text' =>
                        zen_draw_form('statusUpdateForm', FILENAME_ORDERS, zen_get_all_get_params(['action','language']) . 'action=update_order' . (!isset($_GET['oID']) ? '&oID=' . $oInfo->orders_id : '') . '&language=' . $oInfo->language_code, 'post') . // form action uses the order language to change the session language on the update. On initial page load (from another page), $_GET['oID'] is not set, hence clause in form action
                        ...
                        zen_draw_checkbox_field('notify', '1', $notify_email, '') . "<br>\n" .
