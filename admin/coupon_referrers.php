...
switch ($action) {
    case 'new':
        ...
        $contents = ['form' => zen_draw_form('referrer_form', FILENAME_COUPON_REFERRERS, 'action=insert', 'post')];
    ...
    case 'edit':
        ...
        $contents = [
            'form' => zen_draw_form('referrer_form', FILENAME_COUPON_REFERRERS, $href_page_param . '&rID=' . $rInfo->referrer_id . '&action=save', 'post'),
        ];
        $contents[] = ['text' => TEXT_INFO_EDIT_INTRO];
        $contents[] = [
            'text' => '<br>' . zen_draw_label(TEXT_REFERRER_DOMAIN, 'referrer_domain', 'class="control-label"') .
                zen_draw_input_field('referrer_domain', htmlspecialchars((string) $rInfo->referrer_domain, ENT_COMPAT, CHARSET, true), zen_set_field_length(TABLE_COUPON_REFERRERS, 'referrer_domain') . ' class="form-control"', true),
        ];
        $contents[] = [
            'text' => '<br>' . zen_draw_label(TEXT_SELECT_A_COUPON, 'coupon_id', 'class="control-label"') .
...
?>
    ...
    <div class="row">
        <table class="table">
            <tr>
                <td><?php echo $referrers_split->display_count($referrers_query_numrows); ?></td>
