...
// first page (blank/A).
//
$currentPage = (!empty($_GET['page']) && ctype_alpha((string) $_GET['page'][0]) ? replace_accents($_GET['page'][0]) : '');
...
if (isset($_GET['cID'])) {
    ...
    //
    if (!ctype_digit((string) $_GET['cID'])) {
...
foreach ($countries as $country) {
    if ((!isset($_GET['cID']) || (isset($_GET['cID']) && ($_GET['cID'] == $country['countries_id']))) && !isset($cInfo) && (!str_starts_with((string) $action, 'new'))) {
    ...
    if (isset($cInfo) && is_object($cInfo) && ($country['countries_id'] == $cInfo->countries_id)) {
        ?>
                  ...
                  <?php } else { ?>
                  <tr class="dataTableRow country-listing-row" onclick="document.location.href = '<?php echo zen_href_link(FILENAME_COUNTRIES, zen_get_all_get_params(['cID', 'action']) . 'cID=' . $country['countries_id']); ?>'" data-cid="<?php echo $country['countries_id']; ?>" role="option" aria-selected="false">
                  <?php } ?>
...
switch ($action) {
    case 'new':
        $heading[] = ['text' => '<h4>' . TEXT_INFO_HEADING_NEW_COUNTRY . '</h4>'];
        $contents = ['form' => zen_draw_form('countries', FILENAME_COUNTRIES, $page_parameter . 'action=insert', 'post')];
        $contents[] = ['text' => TEXT_INFO_INSERT_INTRO];
        $contents[] = ['text' => '<br>' . zen_draw_label(TEXT_INFO_COUNTRY_NAME, 'countries_name', 'class="control-label"') . zen_draw_input_field('countries_name', '', 'class="form-control"')];
        $contents[] = ['text' => '<br>' . zen_draw_label(TEXT_INFO_COUNTRY_CODE_2, 'countries_iso_code_2', 'class="control-label"') . zen_draw_input_field('countries_iso_code_2', '', 'class="form-control"')];
        $contents[] = ['text' => '<br>' . zen_draw_label(TEXT_INFO_COUNTRY_CODE_3, 'countries_iso_code_3', 'class="control-label"') . zen_draw_input_field('countries_iso_code_3', '', 'class="form-control"')];
        $contents[] = ['text' => '<br>' . zen_draw_label(TEXT_INFO_ADDRESS_FORMAT, 'address_format_id', 'class="control-label"') . zen_draw_pull_down_menu('address_format_id', zen_get_address_formats(), '', 'class="form-control"')];
        $contents[] = ['text' => '<br>' . zen_draw_label(TEXT_INFO_COUNTRY_STATUS, 'status', 'class="control-label"') . zen_draw_checkbox_field('status', '', true, 'class="form-control"')];
        $contents[] = ['align' => 'text-center', 'text' => '<br><button type="submit" class="btn btn-primary">' . IMAGE_INSERT . '</button> <a href="' . zen_href_link(FILENAME_COUNTRIES, rtrim($page_parameter, '&')) . '" class="btn btn-default" role="button">' . IMAGE_CANCEL . '</a>'];
        break;
    case 'edit':
        $heading[] = ['text' => '<h4>' . TEXT_INFO_HEADING_EDIT_COUNTRY . '</h4>'];
        $contents = ['form' => zen_draw_form('countries', FILENAME_COUNTRIES, $page_parameter . 'cID=' . $cInfo->countries_id . '&action=save', 'post')];
        $contents[] = ['text' => TEXT_INFO_EDIT_INTRO];
        $contents[] = ['text' => '<br>' . zen_draw_label(TEXT_INFO_COUNTRY_NAME, 'countries_name', 'class="control-label"') . zen_draw_input_field('countries_name', htmlspecialchars((string) $cInfo->countries_name, ENT_COMPAT, CHARSET, true), 'class="form-control"')];
        $contents[] = ['text' => '<br>' . zen_draw_label(TEXT_INFO_COUNTRY_CODE_2, 'countries_iso_code_2', 'class="control-label"') . zen_draw_input_field('countries_iso_code_2', $cInfo->countries_iso_code_2, 'class="form-control"')];
        $contents[] = ['text' => '<br>' . zen_draw_label(TEXT_INFO_COUNTRY_CODE_3, 'countries_iso_code_3', 'class="control-label"') . zen_draw_input_field('countries_iso_code_3', $cInfo->countries_iso_code_3, 'class="form-control"')];
        $contents[] = ['text' => '<br>' . zen_draw_label(TEXT_INFO_ADDRESS_FORMAT, 'address_format_id', 'class="control-label"') . zen_draw_pull_down_menu('address_format_id', zen_get_address_formats(), $cInfo->address_format_id, 'class="form-control"')];
        $contents[] = ['text' => '<br>' . zen_draw_label(TEXT_INFO_COUNTRY_STATUS, 'status', 'class="control-label"') . zen_draw_checkbox_field('status', '', $cInfo->status)];
        $contents[] = ['align' => 'text-center', 'text' => '<br><button type="submit" class="btn btn-primary">' . IMAGE_UPDATE . '</button> <a href="' . zen_href_link(FILENAME_COUNTRIES, $page_parameter . 'cID=' . $cInfo->countries_id) . '" class="btn btn-default" role="button">' . IMAGE_CANCEL . '</a>'];
        break;
    case 'delete':
        $heading[] = ['text' => '<h4>' . TEXT_INFO_HEADING_DELETE_COUNTRY . '</h4>'];
        $contents = ['form' => zen_draw_form('countries', FILENAME_COUNTRIES, $page_parameter . 'action=deleteconfirm') . zen_draw_hidden_field('cID', $cInfo->countries_id)];
        $contents[] = ['text' => TEXT_INFO_DELETE_INTRO];
        $contents[] = ['text' => '<br><b>' . zen_output_string_protected($cInfo->countries_name) . '</b>'];
        $contents[] = ['align' => 'text-center', 'text' => '<br><button type="submit" class="btn btn-danger">' . IMAGE_UPDATE . '</button> <a href="' . zen_href_link(FILENAME_COUNTRIES, $page_parameter . 'cID=' . $cInfo->countries_id) . '" class="btn btn-default" role="button">' . IMAGE_CANCEL . '</a>'];
        break;
    default:
        if (is_object($cInfo)) {
            $heading[] = ['text' => '<h4>' . zen_output_string_protected($cInfo->countries_name) . '</h4>'];
            $contents[] = ['align' => 'text-center', 'text' => '<a href="' . zen_href_link(FILENAME_COUNTRIES, $page_parameter . 'cID=' . $cInfo->countries_id . '&action=edit') . '" class="btn btn-primary" role="button">' . IMAGE_EDIT . '</a> <a href="' . zen_href_link(FILENAME_COUNTRIES, $page_parameter . 'cID=' . $cInfo->countries_id . '&action=delete') . '" class="btn btn-warning" role="button">' . IMAGE_DELETE . '</a>'];
            $contents[] = ['text' => '<br>' . TEXT_INFO_COUNTRY_NAME . '<br>' . zen_output_string_protected($cInfo->countries_name)];
            ...
            $contents[] = ['text' => '<br>' . TEXT_INFO_COUNTRY_STATUS . ' ' . ($cInfo->status == 0 ? TEXT_NO : TEXT_YES)];
        }
        break;
}
...
?>
      ...
      <div class="row">
        <table class="table">
          <tr>
            <td><?php echo $countries_split->display_count($countries_query_numrows); ?></td>
