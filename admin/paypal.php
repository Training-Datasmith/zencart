...
$order_by = match ($paypal_ipn_sort_order) {
    0 => ' ORDER BY p.paypal_ipn_id DESC',
    1 => ' ORDER BY p.paypal_ipn_id',
    2 => ' ORDER BY p.order_id DESC, p.paypal_ipn_id',
    3 => ' ORDER BY p.order_id, p.paypal_ipn_id',
    4 => ' ORDER BY p.mc_gross DESC',
    5 => ' ORDER BY p.mc_gross',
    default => ' ORDER BY p.paypal_ipn_id DESC',
};
...
<?php
  if (zen_not_null($selected_status)) {
      ...
      $ipn_query_raw = match ($selected_status) {
          default => 'SELECT p.order_id, p.paypal_ipn_id, p.txn_type, p.payment_type, p.payment_status, p.pending_reason, p.payer_status, p.mc_currency, p.date_added, p.mc_gross, p.first_name, p.last_name, p.payer_business_name, p.parent_txn_id, p.txn_id FROM ' . TABLE_PAYPAL . ' AS p, ' .TABLE_ORDERS . ' AS o  WHERE o.orders_id = p.order_id ' . $ipn_search . $order_by,
      };
  } else {
...
foreach ($ipn_trans as $ipn_tran) {
    ...
    ?>
                ...
                <td class="dataTableContent text-right">
                    <?php if (isset($ipnInfo) && is_object($ipnInfo) && ($ipn_tran['paypal_ipn_id'] == $ipnInfo->paypal_ipn_id)) {
                        echo zen_icon('caret-right', '', '2x', true);
                    } else {
                        echo '<a href="' . zen_href_link(FILENAME_PAYPAL, 'page=' . $_GET['page'] . '&ipnID=' . $ipn_tran['paypal_ipn_id']) . (zen_not_null($selected_status) ? '&payment_status=' . $selected_status : '') . (zen_not_null($paypal_ipn_sort_order) ? '&paypal_ipn_sort_order=' . $paypal_ipn_sort_order : '') . '" data-toggle="tooltip" title="' . IMAGE_ICON_INFO . '" role="button">' . zen_icon('circle-info', '', '2x', true, false) . '</a>';
                    } ?></td>
              <?php echo '</tr>';
}
?>
              <tr>
                    <td colspan="3" class="smallText"><?php echo $ipn_split->display_count($ipn_query_numrows); ?></td>
                    <td colspan="3" class="smallText text-right"><?php echo $ipn_split->display_links($ipn_query_numrows, MAX_DISPLAY_SEARCH_RESULTS_PAYPAL_IPN, MAX_DISPLAY_PAGE_LINKS, isset($_GET['page']) ? (int)$_GET['page'] : 1); ?></td>
              </tr>
            </table>
           </div>
<?php
