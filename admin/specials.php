...
if (!empty($action)) {
    ...
    switch ($action) {
        ...
        case 'insert':
            if (empty($_POST['products_id'])) {
                ...
                $db->Execute('INSERT INTO ' . TABLE_SPECIALS . ' (products_id, specials_new_products_price, specials_date_added, expires_date, status, specials_date_available)
                      VALUES (' . $products_id . ', ' . (float)$specials_price . ", now(), '" . zen_db_input(!empty($expires_date) ? $expires_date : '0001-01-01') . "', 1, '" . zen_db_input(!empty($specials_date_available) ? $specials_date_available : '0001-01-01') . "')");
        ...
        case 'update':
            ...
            zen_redirect(zen_href_link(FILENAME_SPECIALS, ($currentPage != 0 ? 'page=' . $currentPage . '&' : '') . (isset($_GET['search']) ? 'search=' . $_GET['search'] . '&' : '') . 'sID=' . $specials_id));
            break;
        case 'deleteconfirm':
        ...
        case 'pre_add_confirmation':
            ...
            if (empty($_POST['pre_add_products_id'])) {
                ...
                } elseif ((!defined('MODULE_ORDER_TOTAL_GV_SPECIAL') || MODULE_ORDER_TOTAL_GV_SPECIAL === 'false') && (str_starts_with($check_product->fields['products_model'] ?? '', 'GIFT'))) { // check for PID as a gift voucher
                    $skip_special = true;
...
if (($action === 'new') || ($action === 'edit')) {
    ...
    if (($action === 'edit') && isset($_GET['sID'])) { //update existing Special
        ...
        if (!defined('MODULE_ORDER_TOTAL_GV_SPECIAL') || MODULE_ORDER_TOTAL_GV_SPECIAL === 'false') {
            ...
            foreach ($gift_vouchers as $gift_voucher) {
                if (str_starts_with((string) $gift_voucher['products_model'], 'GIFT')) {
    ...
    ?>
        <div class="row">
          <?php echo zen_draw_form('new_special', FILENAME_SPECIALS, zen_get_all_get_params(['action', 'info']) . 'action=' . $form_action . (!empty($_GET['go_back']) ? '&go_back=' . $_GET['go_back'] : ''), 'post'); ?>
          <?php
      ...
      <?php } else { ?>
        ...
        <div class="row">
          ...
          foreach ($specials as $special) {
            ...
            <div class="row">
              <div class="col-sm-6"><?php echo $specials_split->display_count($specials_query_numrows); ?></div>
              <div class="col-sm-6 text-right"><?php echo $specials_split->display_links($specials_query_numrows, MAX_DISPLAY_SEARCH_RESULTS, MAX_DISPLAY_PAGE_LINKS, $_GET['page']); ?></div>
            </div>
            </div>
          ...
          switch ($action) {
              ...
              case 'pre_add':
                  ...
                  $contents = ['form' => zen_draw_form('specials', FILENAME_SPECIALS, 'action=pre_add_confirmation' . ($currentPage != 0 ? '&page=' . $currentPage : '') . (isset($_GET['search']) ? '&search=' . $_GET['search'] : ''), 'post')];
