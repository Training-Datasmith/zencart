      ...
      if (isset($_GET['action']) && ($_GET['action'] == 'layout' || $_GET['action'] == 'layout_edit')) {
        ...
        <div class="row">
          ...
          foreach ($configuration as $item) {
              ...
              if ((!isset($_GET['cID']) || (isset($_GET['cID']) && ($_GET['cID'] == $item['configuration_id']))) && !isset($cInfo) && (!str_starts_with((string) $action, 'new'))) {
              ...
              ?>
                ...
                <td class="dataTableContent"><?php echo htmlspecialchars((string) $cfgValue, ENT_COMPAT, CHARSET, true); ?></td>
          ...
          switch ($action) {
              case 'layout_edit':
                  ...
                  if ($cInfo->set_function) {
                      eval('$value_field = ' . $cInfo->set_function . '"' . htmlspecialchars((string) $cInfo->configuration_value, ENT_COMPAT, CHARSET, true) . '");');
                  } else {
                      $value_field = zen_draw_input_field('configuration_value', htmlspecialchars((string) $cInfo->configuration_value, ENT_COMPAT, CHARSET, true), 'size="60"');
                  }
        ...
        <div class="row">
          ...
          foreach ($product_types as $product_type) {
              if ((!isset($_GET['ptID']) || (isset($_GET['ptID']) && ($_GET['ptID'] == $product_type['type_id']))) && !isset($ptInfo) && (!str_starts_with((string) $action, 'new'))) {
          ...
          switch ($action) {
              case 'new':
                  ...
                  $contents = ['form' => zen_draw_form('new_product_type', FILENAME_PRODUCT_TYPES, 'action=insert', 'post')];
                  $contents[] = ['text' => TEXT_NEW_INTRO];
                  break;
              case 'edit':
                  $heading[] = ['text' => '<h4>' . TEXT_HEADING_EDIT_PRODUCT_TYPE . ' :: ' . $ptInfo->type_name . '</h4>'];

                  $contents = ['form' => zen_draw_form('product_types', FILENAME_PRODUCT_TYPES, 'page=' . $_GET['page'] . '&ptID=' . $ptInfo->type_id . '&action=save', 'post')];
                  $contents[] = ['text' => TEXT_INFO_EDIT_INTRO];
                  ...
                  $default_directory = substr((string) $ptInfo->default_image, 0, strpos((string) $ptInfo->default_image, '/') + 1);
        ...
        <div class="row">
          <div class="col-cm-6"><?php echo $product_types_split->display_count($product_types_query_numrows); ?></div>
