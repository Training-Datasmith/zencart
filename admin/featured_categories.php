            ...
            if (empty($_POST['categories_id'])) {
                ...
                $db->Execute(
                    'INSERT INTO ' . TABLE_FEATURED_CATEGORIES . ' (categories_id, featured_date_added, expires_date, status, featured_date_available)
                        VALUES (' . $categories_id . ", now(), '" . zen_db_input(!empty($expires_date) ? $expires_date : '0001-01-01') . "', 1, '" . zen_db_input(!empty($featured_date_available) ? $featured_date_available : '0001-01-01') . "')"
                );
    ...
    <!-- header_eof //-->
        <div class="container-fluid">
            ...
            if ($action === 'new' || $action === 'edit') {
                ...
                ?>
                <div class="row">
                    <?php echo zen_draw_form('new_featured', FILENAME_FEATURED_CATEGORIES, zen_get_all_get_params(['action', 'info', 'fID']) . 'action=' . $form_action . (!empty($_GET['go_back']) ? '&go_back=' . $_GET['go_back'] : ''), 'post'); ?>
                    <?php
            ...
            <div class="row">
                ...
                switch ($action) {
                    ...
                    case 'pre_add':
                        ...
                        $contents = ['form' => zen_draw_form('featured', FILENAME_FEATURED_CATEGORIES, 'action=pre_add_confirmation' . $page_search_parameters, 'post')];
