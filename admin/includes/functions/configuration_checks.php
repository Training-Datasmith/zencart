...
function zen_validate_configuration_entry($variable, $check_string, $config_name = '')
{
    ...
    $data = json_decode((string) $check_string, true);
    ...
    if (!defined($data['error'])) {
        $error_msg = match (true) {
            str_starts_with((string) $data['error'], 'TEXT_MIN_ADMIN') => TEXT_MIN_GENERAL_ADMIN,
            str_starts_with((string) $data['error'], 'TEXT_MAX_ADMIN') => TEXT_MAX_GENERAL_ADMIN,
            default => TEXT_DATA_OUT_OF_RANGE,
        };
    } elseif ($config_name !== '') {
