 ...
 * @since ZC v1.5.7
 */
function zen_datepicker_format_full(): string
{
 ...
 * @since ZC v1.5.7
 */
function zen_datepicker_format_forsql(): string|array
{
    $date = DATE_FORMAT_DATE_PICKER;
    $date = str_replace('mm', '%m', $date);
    $date = str_replace('dd', '%d', $date);
...
function zen_prepare_date(string $raw_date, string $past_date = ''): string
{
    ...
    if (!empty($past_date)) {
        return (date('Y-m-d') < $date) ? $date : $past_date;
    }
    return $date;
}
