...
//
if (basename((string) $PHP_SELF) === FILENAME_AJAX . '.php') {
...
if (basename((string) $PHP_SELF) !== FILENAME_ALERT_PAGE . '.php') {
    ...
    if (str_contains(strtolower((string) $PHP_SELF), FILENAME_PASSWORD_FORGOTTEN . '.php')
        && substr_count(strtolower((string) $PHP_SELF), '.php') > 1) {
        zen_redirect(zen_href_link(FILENAME_LOGIN, '', 'SSL'));
    }

    if (!(basename((string) $PHP_SELF) === FILENAME_LOGIN . '.php')) {
        $page = basename((string) $PHP_SELF, '.php');

        // must be logged in
        if (!isset($_SESSION['admin_id'])) {
            if (!(basename((string) $PHP_SELF) == FILENAME_PASSWORD_FORGOTTEN . '.php')) {
                zen_redirect(zen_href_link(FILENAME_LOGIN, 'camefrom=' . basename((string) $PHP_SELF) . '&' . zen_get_all_get_params(), 'SSL'));
            }
        }

        // Do MFA validation
        if (basename((string) $PHP_SELF) !== FILENAME_MFA . '.php' && basename((string) $PHP_SELF) !== FILENAME_LOGOFF . '.php'
    ...
    // handle malicious URLs
    if ((basename((string) $PHP_SELF) === FILENAME_LOGIN . '.php')
        && (substr_count(dirname((string) $PHP_SELF), '//') > 0 || substr_count(dirname((string) $PHP_SELF), '.php') > 0)) {
        zen_redirect(zen_href_link(FILENAME_LOGIN, '', 'SSL'));
    }
}
