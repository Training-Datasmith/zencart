 ...
 */
if (!defined('WARN_SQL_CACHE_DIRECTORY_NOT_WRITEABLE')) {
...
if (strtolower(SQL_CACHE_METHOD) == 'file') {
...
if (basename((string) $PHP_SELF) == FILENAME_DEFAULT . '.php') {
    ...
    if ($chk_admin_log->fields['counter'] > 0) {
        ...
        if (!empty($chk_admin_log->fields['access_date'])) {
            $show_admin_activity_log_link = true;
            $_SESSION['reset_admin_activity_log'] = true;
            $messageStack->add(WARNING_ADMIN_ACTIVITY_LOG_DATE . date(DATE_FORMAT, strtotime((string) $chk_admin_log->fields['access_date'])), 'caution');
        }
    }
}
