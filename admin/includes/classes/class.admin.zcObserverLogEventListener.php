...
class zcObserverLogEventListener extends base
{
    ...
    public const INFO = 200; // user logs in, other basic logged activity
    public const NOTICE = 250; // uncommon
    public const WARNING = 300; //Exceptional occurrences that are not errors
    public const ERROR = 400; // Runtime errors
    public const CRITICAL = 500; // Application error
    public const ALERT = 550; // site down
    public const EMERGENCY = 600; // urgent
    /**
     ...
     * @since ZC v1.5.4
     */
    public function updateNotifyAdminActivityLogEvent(&$class, $eventID, $message_to_log = '', $requested_severity = ''): void
    {
     ...
     * @since ZC v1.5.4
     */
    public static function prepareLogdata($message_to_log = '', $requested_severity = ''): array
    {
        global $PHP_SELF;
        $flagged = 0;
        ...
        if ($specific_message == '') {
            $specific_message = 'Accessed page [' . basename((string) $PHP_SELF) . ']';
        ...
        $log_data = [
            ...
            'page_accessed'   => basename((string) $PHP_SELF) . (!isset($_SESSION['admin_id']) || (int)$_SESSION['admin_id'] == 0 ? ' ' . ($_POST['admin_name'] ?? $_POST['admin_email'] ?? '') : ''),
            ...
            'severity'        => strtolower((string) $levels[$severity]),  // converts int to corresponding string
        ];
    ...
    public static function filterArrayElements(array $data): array
    {
        foreach ($data as $key => $nul) {
            if (in_array($key, ['x','y','secur'.'ityTo'.'ken','admi'.'n_p'.'ass','pass'.'word','confirm', 'newpwd-'.$_SESSION['securityToken'],'oldpwd-'.$_SESSION['securityToken'],'confpwd-'.$_SESSION['securityToken']])) {
                unset($data[$key]);
            }
        }
        return $data;
     ...
     * @since ZC v1.5.4
     */
    public static function parseForMaliciousContent($string): string|false
    {
        $matches = '';
        if (preg_match_all('~(file://|<iframe|<frame|<embed|<script|<meta)~i', (string) $string, $matches)) {
