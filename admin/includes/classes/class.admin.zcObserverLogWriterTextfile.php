...
class zcObserverLogWriterTextfile extends base
{
     ...
     * @since ZC v1.5.4
     */
    public function setLogFilename($filepath = ''): void
    {
        if ($filepath == '') {
     ...
     * @since ZC v1.5.4
     */
    public function updateNotifyAdminFireLogWriters(&$class, $eventID, array $log_data): void
    {
        $this->initLogFile();
        /**
    ...
    public function initLogFile(): void
    {
        ...
        if ($init_required) {
            ...
            error_log('notice [' . date('M-d-Y H:i:s') . '] ' . substr((string) $_SERVER['REMOTE_ADDR'], 0, 45) . ' ' . $data . "\n", 3, $this->destinationLogFilename);
        }
    }
