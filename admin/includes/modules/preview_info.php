...
<div class="container-fluid">
    ...
    if (!isset($_GET['read']) || ($_GET['read'] !== 'only')) {
        echo zen_draw_form($form_action, FILENAME_PRODUCT, 'cPath=' . $cPath . (isset($_GET['pID']) ? '&pID=' . $_GET['pID'] : '') . '&action=' . $form_action . (isset($_GET['page']) ? '&page=' . $_GET['page'] : '') . (isset($_GET['search']) ? '&search=' . zen_preserve_search_quotes($_GET['search']) : ''), 'post');
    }
...
if (isset($_GET['read']) && ($_GET['read'] == 'only')) {
    ...
    for ($i = 0, $n = count($languages); $i < $n; $i++) {
        echo zen_draw_hidden_field('products_name[' . $languages[$i]['id'] . ']', htmlspecialchars(stripslashes((string) $products_name[$languages[$i]['id']]), ENT_COMPAT, CHARSET, true));
        echo zen_draw_hidden_field('products_description[' . $languages[$i]['id'] . ']', htmlspecialchars(stripslashes((string) $products_description[$languages[$i]['id']]), ENT_COMPAT, CHARSET, true));
        echo zen_draw_hidden_field('products_url[' . $languages[$i]['id'] . ']', htmlspecialchars(stripslashes((string) $products_url[$languages[$i]['id']]), ENT_COMPAT, CHARSET, true));
    }
    echo zen_draw_hidden_field('products_image', stripslashes((string) $products_image_name));
