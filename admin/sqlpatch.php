 ...
 * @since ZC v1.2.0d
 */
function executeSql($lines, $database, string $table_prefix = ''): array
{
    ...
    foreach ($lines as $line) {
        ...
        if (str_starts_with($line, '#NEXT_X_ROWS_AS_ONE_COMMAND:')) {
        ...
        if (!str_starts_with($line, '#') && !str_starts_with($line, '-') && $line != '') {
            ...
            switch (true) {
                case (str_starts_with($line_upper, 'DROP TABLE IF EXISTS ')):
                    ...
                    break;
                case (str_starts_with($line_upper, 'DROP TABLE ') && $param[2] != 'IF'):
                    ...
                    break;
                case (str_starts_with($line_upper, 'CREATE TABLE ')):
                    ...
                    break;
                case (str_starts_with($line_upper, 'TRUNCATE TABLE ')):
                    ...
                    break;
                case (str_starts_with($line_upper, 'REPLACE INTO ')):
                    ...
                    break;
                case (str_starts_with($line_upper, 'INSERT INTO ')):
                    //check to see if table prefix is going to match
                    if (!$tbl_exists = zen_table_exists($param[2])) {
                        $result = sprintf(REASON_TABLE_NOT_FOUND, $param[2]) . ' CHECK PREFIXES!';
                    }
                    ...
                    break;
                case (str_starts_with($line_upper, 'INSERT IGNORE INTO ')):
                    //check to see if table prefix is going to match
                    ...
                    break;
                case (str_starts_with($line_upper, 'ALTER TABLE ')):
                    ...
                    break;
                case (str_starts_with($line_upper, "AND TABLE_NAME = '")):
                ...
                case (str_starts_with(ltrim($line_upper), 'FOREIGN KEY')):
                    ...
                    break;
                case (str_starts_with($line_upper, 'RENAME TABLE ')):
                    ...
                    $ignore_line = true;
                    break;
                case (str_starts_with($line_upper, 'UPDATE ')):
                    //check to see if table prefix is going to match
                    ...
                    break;
                case (str_starts_with($line_upper, 'UPDATE IGNORE ')):
                    //check to see if table prefix is going to match
                    if (!$tbl_exists = zen_table_exists($param[2])) {
                    ...
                    break;
                case (str_starts_with($line_upper, 'DELETE FROM ')):
                    $line = 'DELETE FROM ' . $table_prefix . ltrim(substr($line, 12));
                    break;
                case (str_starts_with($line_upper, 'DROP INDEX ')):
                    ...
                    break;
                case (str_starts_with($line_upper, 'CREATE INDEX ') || (strtoupper($param[0]) == 'CREATE' && strtoupper($param[2]) == 'INDEX')):
                    ...
                    if ($result = zen_create_index_command($param)) {
                        ...
                        if (strtoupper($param[1]) == 'INDEX') {
                            $line = trim('CREATE INDEX ' . $param[2] . ' ON ' . $table_prefix . implode(' ', [$param[4], $param[5], $param[6], $param[7], $param[8], $param[9], $param[10], $param[11], $param[12], $param[13]])) . ';'; // add the ';' back since it was removed from $param at start
                        } else {
                            $line = trim('CREATE ' . $param[1] . ' INDEX ' . $param[3] . ' ON ' . $table_prefix . implode(' ', [$param[5], $param[6], $param[7], $param[8], $param[9], $param[10], $param[11], $param[12], $param[13]])); // add the ';' back since it was removed from $param at start
                        }
                    }
                    break;
                case (str_starts_with($line_upper, 'SELECT ') && substr_count($line_upper, 'FROM ') > 0):
                    $line = str_ireplace('FROM ', 'FROM ' . $table_prefix, $line);
                    break;
                case (str_starts_with($line_upper, 'INNER JOIN ')):
                    $line = 'INNER JOIN ' . $table_prefix . ltrim(substr($line, 11));
                    break;
                case (str_starts_with($line_upper, 'CROSS JOIN ')):
                    $line = 'CROSS JOIN ' . $table_prefix . ltrim(substr($line, 11));
                    break;
                case (str_starts_with($line_upper, 'LEFT JOIN ')):
                ...
                case ($line_upper === 'FROM INFORMATION_SCHEMA.COLUMNS'):
                    ...
                    break;
                case (str_starts_with($line_upper, 'FROM ')):
    ...
    return ['queries' => $results, 'string' => $string, 'output' => $return_output, 'ignored' => ($ignored_count), 'errors' => $errors];
}
// end function
 ...
 * @since ZC v1.2.5
 */
function zen_table_exists(string $tablename, $pre_install = false): bool
{
    global $db;
 ...
 * @since ZC v1.2.5
 */
function zen_check_database_privs($priv = '', $table = '', $show_privs = false): bool
{
...
function zen_create_index_command($param)
{
    ...
    $index = (strtoupper((string) $param[1]) == 'INDEX') ? $param[2] : $param[3];
    if (in_array('USING', $param)) {
        return 'USING parameter found. Cannot validate syntax. Please run manually in phpMyAdmin.';
    }
    $table = (strtoupper((string) $param[2]) == 'INDEX' && strtoupper((string) $param[4]) == 'ON') ? $param[5] : $param[4];
    ...
    foreach ($results as $result) {
        ...
        if (strtoupper((string) $result['Key_name']) == strtoupper((string) $index)) {
...
function zen_check_alter_command($param)
{
    ...
    switch (strtoupper((string) $param[3])) {
        case ('ADD'):
            if (strtoupper((string) $param[4]) == 'INDEX') {
            ...
            } elseif (strtoupper((string) $param[4]) == 'PRIMARY') {
            ...
            } elseif (!in_array(strtoupper((string) $param[4]), ['CONSTRAINT', 'UNIQUE', 'PRIMARY', 'FULLTEXT', 'FOREIGN', 'SPATIAL'])) {
            ...
            } elseif (strtoupper((string) $param[5]) == 'AFTER') {
            ...
            } elseif (strtoupper((string) $param[6]) == 'AFTER') {
                // check that the requested "after" field actually exists
        ...
        case ('DROP'):
            ...
            if (strtoupper((string) $param[4]) == 'PRIMARY') {
            ...
            if (!in_array(strtoupper((string) $param[4]), ['CONSTRAINT', 'UNIQUE', 'PRIMARY', 'FULLTEXT', 'FOREIGN', 'SPATIAL'])) {
...
function zen_purge_exceptions_table()
{
    global $db;
    zen_create_exceptions_table();
    return $db->Execute('TRUNCATE TABLE ' . DB_PREFIX . TABLE_UPGRADE_EXCEPTIONS);
}
...
function zen_create_exceptions_table()
{
    ...
    if (!zen_table_exists(TABLE_UPGRADE_EXCEPTIONS)) {
        return $db->Execute('CREATE TABLE ' . DB_PREFIX . TABLE_UPGRADE_EXCEPTIONS . " (
    ...
    switch ($action) {
        case 'execute':
            if (isset($_POST['query_string']) && $_POST['query_string'] != '') {
                ...
                $query_string = explode($linebreak, ((string) $query_string));
    ...
    <body>
      ...
      <div class="container-fluid">
        ...
        if ($action == 'execute' && !empty($_POST['query_string'])) {
          ...
          <div class="row">
            <div class="col-sm-12 text-danger"><?php echo nl2br(zen_output_string_protected(preg_replace('~<br[ /]*>~', "\n", (string) $query_results['string']))); ?></div>
          </div>
          <?php
        }
?>
        <div class="row">&nbsp;</div>
        <?php echo zen_draw_form('getquery', FILENAME_SQLPATCH, 'action=execute' . (($debug == true) ? '&debug=ON' : '') . (($skip_stripslashes == true) ? '&keepslashes=1' : ''), 'post'); ?>
        <div class="form-group">
            <?php echo zen_draw_label(TEXT_ENTER_QUERY_STRING, 'query_string', 'class="control-label col-sm-3"'); ?>
          <div class="col-sm-9 col-md-6"><?php echo zen_draw_textarea_field('query_string', 'soft', '80', '10', '', 'id="query_string" class="form-control noEditor"'); ?></div>
        </div>
        <div class="form-group">
        ...
        <div class="row">&nbsp;</div>
        <?php echo zen_draw_form('getqueryfile', FILENAME_SQLPATCH, 'action=uploadquery' . (($debug == true) ? '&debug=ON' : '') . (($skip_stripslashes == true) ? '&keepslashes=1' : ''), 'post'); ?>
        <?php
...
?>
  ...
  <?php echo zen_draw_label(TEXT_QUERY_FILENAME, 'sql_file', 'class="control-label col-sm-3"'); ?>
          <div class="col-sm-9 col-md-6"><?php echo zen_draw_file_field('sql_file', ''); ?></div>
        </div>
        <div class="form-group">
