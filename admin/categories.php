...
?>
    ...
    if ($action === 'new_category' || $action === 'edit_category') {
        ...
        <?php
        echo zen_draw_form('categories', FILENAME_CATEGORIES, 'action=' . $formAction . '&cPath=' . $cPath . ((isset($_GET['search']) && !empty($_GET['search'])) ? '&search=' . $_GET['search'] : ''), 'post');
...
<?php
        ...
        <div class="form-group">
          ...
          <div class="col-sm-9 col-md-6">
              <?php echo zen_draw_file_field('categories_image', ''); ?>
          </div>
        </div>
        <?php
        ...
        if (empty($cInfo->categories_image)) {
            ...
            $default_directory = substr((string) $cInfo->categories_image, 0, strpos((string) $cInfo->categories_image, '/') + 1);
        }
        ?>
        <div class="form-group">
        ...
        <?php echo zen_draw_form('categories', FILENAME_CATEGORIES, 'action=update_category_meta_tags&cPath=' . $cPath, 'post'); ?>
        ...
        <div class="form-group">
          ...
          <div class="col-sm-9 col-md-6">
              ...
              <div class="input-group">
                ...
                <?php echo zen_draw_input_field('metatags_title[' . $languages[$i]['id'] . ']', htmlspecialchars((string) zen_get_category_metatag_fields($cInfo->categories_id, $languages[$i]['id'], 'metatags_title'), ENT_COMPAT, CHARSET, true), zen_set_field_length(TABLE_METATAGS_CATEGORIES_DESCRIPTION, 'metatags_title') . ' class="form-control" id="metatags_title[' . $languages[$i]['id'] . ']"');
                  ?>
              </div>
              <br>
              <?php
        ...
        <div class="form-group">
          ...
          for ($i = 0, $n = count($languages); $i < $n; $i++) {
              ...
              <div class="input-group">
                ...
                <?php echo zen_draw_textarea_field('metatags_keywords[' . $languages[$i]['id'] . ']', 'soft', '100', '3', htmlspecialchars((string) zen_get_category_metatag_fields($cInfo->categories_id, $languages[$i]['id'], 'metatags_keywords'), ENT_COMPAT, CHARSET, true), 'class="form-control noEditor" id="metatags_keywords[' . $languages[$i]['id'] . ']"');
              ?>
              </div>
              <br>
              <?php
        ...
        <div class="form-group">
          ...
          for ($i = 0, $n = count($languages); $i < $n; $i++) {
              ...
              <div class="input-group">
                <span class="input-group-addon align-top"><?php echo zen_image(DIR_WS_CATALOG_LANGUAGES . $languages[$i]['directory'] . '/images/' . $languages[$i]['image'], $languages[$i]['name']); ?></span>
                <?php echo zen_draw_textarea_field('metatags_description[' . $languages[$i]['id'] . ']', 'soft', '100', '7', htmlspecialchars((string) zen_get_category_metatag_fields($cInfo->categories_id, $languages[$i]['id'], 'metatags_description'), ENT_COMPAT, CHARSET, true), 'class="form-control noEditor" id="metatags_description[' . $languages[$i]['id'] . ']"');
              ?>
              </div>
              <br>
              <?php
