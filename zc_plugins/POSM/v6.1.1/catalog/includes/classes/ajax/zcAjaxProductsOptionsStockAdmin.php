...
class zcAjaxProductsOptionsStockAdmin extends base
{
    ...
    //
    public function isModelDuplicate(): array
    {
        global $db;

        $model_is_ok = false;
        if (isset($_POST['model_num']) && isset($_POST['pos_id']) && strlen((string) $_POST['model_num']) <= zen_field_length(TABLE_PRODUCTS, 'products_model')) {
            $check_sql =
