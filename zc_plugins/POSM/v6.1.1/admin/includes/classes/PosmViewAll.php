...
class PosmViewAll extends base
{
    ...
    protected function gatherOptions($level = 0, string $option_name = '', array $option_values = [])
    {
    ...
    protected function sortOptions()
    {
        if ($this->sort_order === 'model-asc') {
            uasort($this->product_options, function (array $a, array $b): int {
                if (!empty($a['fields']['pos_model']) || !empty($b['fields']['pos_model'])) {
                    $result = strcasecmp((string) $a['fields']['pos_model'], (string) $b['fields']['pos_model']);
                } else {
            ...
            uasort($this->product_options, function (array $a, array $b): int {
                if (!empty($a['fields']['pos_model']) || !empty($b['fields']['pos_model'])) {
                    $result = strcasecmp((string) $b['fields']['pos_model'], (string) $a['fields']['pos_model']);
                } else {
                    $result = ($a['sort'] < $b['sort']) ? -1 : 1;
                }
                return ($result < 0) ? -1 : (($result === 0) ? 0 : 1);
