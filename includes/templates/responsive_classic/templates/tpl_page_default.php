...
echo $var_page_details->fields['pages_title'];
?></h1>
  <?php 
if (EZPAGES_SHOW_PREV_NEXT_BUTTONS == '2' && $counter > 1) {
    ...
    if ($_SESSION['layoutType'] == 'mobile') {
      ...
      <div id="navEZPageNextPrev">
        <a href="<?php 
        echo $prev_link;
        ?>"><i class="fa-solid fa-circle-chevron-left" title="<?php 
        echo BUTTON_PREVIOUS_ALT;
        ?>"></i></a>
        <?php 
        echo zen_back_link();
        ?><i class="fa-solid fa-list" title="<?php 
        echo BUTTON_VIEW_ALL_ALT;
        ?>"></i></a>
        <a href="<?php 
        echo $next_link;
        ?>"><i class="fa-solid fa-circle-chevron-right" title="<?php 
        echo BUTTON_NEXT_ALT;
        ?>"></i></a>
      </div>
    <?php 
    } else {
      ...
      <div id="navEZPageNextPrev">
        ...
        ?></a>
        <?php 
        echo zen_back_link() . $home_button;
        ?></a>
        <a href="<?php 
        echo $next_link;
        ?>"><?php 
        echo $next_item_button;
        ?></a>
      </div>
    <?php 
    }
  ...
  <?php 
    ...
    ?>
    <div id="navEZPageNextPrev"><?php 
    echo zen_back_link() . $home_button . '</a>';
    ?></div>
  <?php 
}
?>
...
if (count($toc_links) > 1 && EZPAGES_SHOW_TABLE_CONTENTS == '1') {
    ...
    <div id="navEZPagesTOCWrapper">
      <h2 id="ezPagesTOCHeading"><?php 
    echo TEXT_EZ_PAGES_TABLE_CONTEXT;
    ?></h2>
    ...
    foreach ($toc_links as $link) {
        ...
        if ($link['pages_id'] == $_GET['id']) {
            ?>
              <li><?php 
            echo CURRENT_PAGE_INDICATOR;
            ?><a href="<?php 
            echo zen_ez_pages_link($link['pages_id']);
            ?>"><?php 
            echo $link['pages_title'];
            ?></a></li>
            <?php 
        } else {
            ?>
              <li><?php 
            echo NOT_CURRENT_PAGE_INDICATOR;
            ?><a href="<?php 
            echo zen_ez_pages_link($link['pages_id']);
            ?>"><?php 
            echo $link['pages_title'];
...
?>
    <div><?php 
echo $var_page_details->fields['pages_html_text'];
?></div>
</div>
