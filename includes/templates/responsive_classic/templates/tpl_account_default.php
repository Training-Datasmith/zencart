...
<h1 id="accountDefaultHeading"><?php 
echo HEADING_TITLE;
?></h1>
<?php 
...
if (!empty($orders_array)) {
    ?>
<p class="forward"><?php 
    echo '<a class="show-all" href="' . zen_href_link(FILENAME_ACCOUNT_HISTORY, '', 'SSL') . '">' . OVERVIEW_SHOW_ALL_ORDERS . '</a>';
    ?></p>
<br class="clearBoth">
<h2 id="previous-orders"><?php 
    echo OVERVIEW_PREVIOUS_ORDERS;
    ?></h2>
<table id="prevOrders">
    ...
    ?></th>
    <th scope="col"><?php 
    echo TABLE_HEADING_ORDER_NUMBER;
    ?></th>
    <th scope="col"><?php 
    echo TABLE_HEADING_SHIPPED_TO;
    ?></th>
    <th scope="col"><?php 
    ...
    ?></th>
    <th scope="col" class="alignCenter"><?php 
    echo TABLE_HEADING_VIEW;
    ?></th>
  </tr>
<?php 
    foreach ($orders_array as $orders) {
        ?>
  <tr>
    <td class="accountOrderDate"><?php 
        if ($display_as_mobile) {
            echo '<b class="hide">' . TABLE_HEADING_DATE . '&#58;&nbsp;&nbsp;</b>';
        }
        echo zen_date_short($orders['date_purchased']);
        ?></td>
    <td class="accountOrderId"><?php 
        if ($display_as_mobile) {
            echo '<b class="hide">' . TABLE_HEADING_ORDER_NUMBER . '&#58;&nbsp;&nbsp;</b>';
        }
        echo TEXT_NUMBER_SYMBOL . $orders['orders_id'];
        ?></td>
    <td class="accountOrderAddress"><?php 
        if ($display_as_mobile) {
            echo '<b class="hide">' . TABLE_HEADING_SHIPPED_TO . '&#58;&nbsp;&nbsp;</b>';
        }
        ?><address><?php 
        echo zen_output_string_protected($orders['order_name']) . '<br>' . $orders['order_country'];
        ?></address></td>
    <td class="accountOrderStatus"><?php 
        if ($display_as_mobile) {
            echo '<b class="hide">' . TABLE_HEADING_STATUS . '&#58;&nbsp;&nbsp;</b>';
        }
        echo $orders['orders_status_name'];
        ?></td>
    <td class="accountOrderTotal alignRight"><?php 
        if ($display_as_mobile) {
            echo '<b class="hide">' . TABLE_HEADING_TOTAL . '&#58;&nbsp;&nbsp;</b>';
        }
        echo $orders['order_total'];
        ?></td>
    <td class="accountOrderViewButton alignCenter"><?php 
...
<li><?php 
echo ' <a href="' . zen_href_link(FILENAME_ACCOUNT_EDIT, '', 'SSL') . '">' . MY_ACCOUNT_INFORMATION . '</a>';
?></li>
<li><?php 
echo ' <a href="' . zen_href_link(FILENAME_ADDRESS_BOOK, '', 'SSL') . '">' . MY_ACCOUNT_ADDRESS_BOOK . '</a>';
?></li>
<li><?php 
echo ' <a href="' . zen_href_link(FILENAME_ACCOUNT_PASSWORD, '', 'SSL') . '">' . MY_ACCOUNT_PASSWORD . '</a>';
?></li>
</ul>
...
<?php 
    if ((int) ACCOUNT_NEWSLETTER_STATUS > 0) {
        ?>
<li><?php 
        echo ' <a href="' . zen_href_link(FILENAME_ACCOUNT_NEWSLETTERS, '', 'SSL') . '">' . EMAIL_NOTIFICATIONS_NEWSLETTERS . '</a>';
        ?></li>
<?php 
    }
    ...
    if (CUSTOMERS_PRODUCTS_NOTIFICATION_STATUS == '1') {
        ?>
<li><?php 
        echo ' <a href="' . zen_href_link(FILENAME_ACCOUNT_NOTIFICATIONS, '', 'SSL') . '">' . EMAIL_NOTIFICATIONS_PRODUCTS . '</a>';
        ?></li>
