...
<div class="prod-list-wrap group">
  <div id="productsListingListingTopLinks" class="navSplitPagesLinks back"><?php 
    echo TEXT_RESULT_PAGE . $listing_split->display_links($max_display_page_links, zen_get_all_get_params(['page', 'info', 'x', 'y', 'main_page']), $paginate_as_ul);
    ?></div>
  <div id="productsListingTopNumber" class="navSplitPagesResult back<?php 
    echo $listing_split->number_of_pages == 1 ? ' navSplitEmpty3rdColumn' : '';
    ?>"><?php 
...
<?php 
    ...
    ?>
    <div class="forward button-top"><?php 
...
if ($listing_split->number_of_rows && (PREV_NEXT_BAR_LOCATION == '2' || PREV_NEXT_BAR_LOCATION == '3')) {
    ?>
<div class="prod-list-wrap group">
  <div id="productsListingListingBottomLinks"  class="navSplitPagesLinks back"><?php 
    echo TEXT_RESULT_PAGE . $listing_split->display_links($max_display_page_links, zen_get_all_get_params(['page', 'info', 'x', 'y', 'main_page']), $paginate_as_ul);
    ?></div>
  <div id="productsListingBottomNumber" class="navSplitPagesResult back<?php 
    echo $listing_split->number_of_pages == 1 ? ' navSplitEmpty3rdColumn' : '';
    ?>"><?php 
    echo $listing_split->display_count(TEXT_DISPLAY_NUMBER_OF_PRODUCTS);
    ?></div>
...
<?php 
if ($how_many > 0 && PRODUCT_LISTING_MULTIPLE_ADD_TO_CART != 0 && $show_submit && $listing_split->number_of_rows > 0) {
    ?>
</form>
<?php 
}