...
<h1 id="checkoutConfirmDefaultHeading"><?php 
echo HEADING_TITLE;
?></h1>

<?php 
if ($message_stack->size('redemptions') > 0) {
    echo $message_stack->output('redemptions');
}
if ($message_stack->size('checkout_confirmation') > 0) {
    echo $message_stack->output('checkout_confirmation');
}
if ($message_stack->size('checkout') > 0) {
    echo $message_stack->output('checkout');
}
?>

<div id="checkoutBillto" class="back">
<h2 id="checkoutConfirmDefaultBillingAddress"><?php 
echo HEADING_BILLING_ADDRESS;
?></h2>
<?php 
if (!$flag_disable_payment_address_change) {
    ?>
<div class="buttonRow forward"><?php 
    echo '<a href="' . zen_href_link(FILENAME_CHECKOUT_PAYMENT, '', 'SSL') . '">' . zen_image_button(BUTTON_IMAGE_EDIT_SMALL, BUTTON_EDIT_SMALL_ALT) . '</a>';
    ?></div>
<?php 
}
?>
...
<h3 id="checkoutConfirmDefaultPayment"><?php 
echo HEADING_PAYMENT_METHOD;
?></h3>
<h4 id="checkoutConfirmDefaultPaymentTitle"><?php 
echo $GLOBALS[$class]->title;
?></h4>
...
<h2 id="checkoutConfirmDefaultShippingAddress"><?php 
    echo HEADING_DELIVERY_ADDRESS;
    ?></h2>
<div class="buttonRow forward"><?php 
    echo '<a href="' . $edit_shipping_button_link . '">' . zen_image_button(BUTTON_IMAGE_EDIT_SMALL, BUTTON_EDIT_SMALL_ALT) . '</a>';
    ?></div>
...
<?php 
    if ($order->info['shipping_method']) {
        ?>
<h3 id="checkoutConfirmDefaultShipment"><?php 
        echo HEADING_SHIPPING_METHOD;
        ?></h3>
<h4 id="checkoutConfirmDefaultShipmentTitle"><?php 
...
<h2 id="checkoutConfirmDefaultHeadingComments"><?php 
...
?></div>
<div><?php 
echo empty($order->info['comments']) ? NO_COMMENTS_TEXT : nl2br(zen_output_string_protected($order->info['comments'])) . zen_draw_hidden_field('comments', $order->info['comments']);
?></div>

</div>

<h2 id="checkoutConfirmDefaultHeadingCart"><?php 
echo HEADING_PRODUCTS;
?></h2>

<div class="buttonRow forward"><?php 
echo '<a href="' . zen_href_link(FILENAME_SHOPPING_CART, '', 'SSL') . '">' . zen_image_button(BUTTON_IMAGE_EDIT_SMALL, BUTTON_EDIT_SMALL_ALT) . '</a>';
?></div>
<br class="clearBoth">

<?php 
if ($flag_any_out_of_stock) {
...
?>
      ...
      <table id="cartContentsDisplay">
        ...
        <th scope="col" id="ccQuantityHeading"><?php 
echo TABLE_HEADING_QUANTITY;
?></th>
        <th scope="col" id="ccProductsHeading"><?php 
...
if (sizeof($order->info['tax_groups']) > 1) {
    ?>
          <th scope="col" id="ccTaxHeading"><?php 
    echo HEADING_TAX;
    ?></th>
<?php 
}
...
for ($i = 0, $n = sizeof($order->products); $i < $n; $i++) {
    ...
    ?>
          <?php 
    echo !empty($stock_check[$i]) ? $stock_check[$i] : '';
    ?>
...
<?php 
if (is_array($payment_modules->modules)) {
    $process_button_options = $payment_modules->process_button_ajax();
    if (isset($process_button_options['ccFields']) && count($process_button_options['ccFields']) > 0) {
        foreach ($process_button_options['ccFields'] as $new_field => $old_field) {
            ?>
...
<?php 
    ...
    if (isset($process_button_options['extraFields']) && count($process_button_options['extraFields']) > 0) {
        foreach ($process_button_options['extraFields'] as $field_name => $field_value) {
            ?>
<input type="hidden" name="<?php 
            echo $field_name;
            ?>"  value="<?php 
...
?>
<div class="buttonRow forward confirm-order"><?php 
echo zen_image_submit(BUTTON_IMAGE_CONFIRM_ORDER, BUTTON_CONFIRM_ORDER_ALT, 'name="btn_submit" id="btn_submit"');
?></div>
</form>
<div class="buttonRow back"><?php 
echo '<strong>' . TITLE_CONTINUE_CHECKOUT_PROCEDURE . '</strong>' . '<br>' . TEXT_CONTINUE_CHECKOUT_PROCEDURE;
?></div>
