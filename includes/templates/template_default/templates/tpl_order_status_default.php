...
<div class="centerColumn" id="orderStatus">
    <h1 id="orderHistoryHeading"><?php 
echo HEADING_TITLE;
?></h1>
<?php 
if ($message_stack->size('order_status') > 0) {
    echo $message_stack->output('order_status');
}
if (isset($order)) {
    ?>
    <fieldset>
        <h2 id="orderHistoryDetailedOrder"><?php 
    echo SUB_HEADING_TITLE . ORDER_HEADING_DIVIDER . sprintf(HEADING_ORDER_NUMBER, $_POST['order_id']);
    ?></h2>
...
<?php 
    if ($display_products === true) {
        ...
        ?></th>
                <th scope="col" id="myAccountProducts"><?php 
        echo HEADING_PRODUCTS;
        ?></th>
<?php 
        if (count($order->info['tax_groups']) > 1) {
            ?>
                <th scope="col" id="myAccountTax"><?php 
            echo HEADING_TAX;
            ?></th>
<?php 
        }
...
<?php 
        ...
        foreach ($order->products as $current_product) {
            ...
            echo $current_product['qty'] . QUANTITY_SUFFIX;
            ?></td>
                <td class="accountProductDisplay"><?php 
            echo $current_product['name'];
...
<?php 
                foreach ($current_product['attributes'] as $current_attribute) {
                    ?>
                        <li><?php 
                    echo $current_attribute['option'] . TEXT_OPTION_DIVIDER . nl2br(zen_output_string_protected($current_attribute['value']), false);
                    ?></li>
<?php 
                }
                ?>
...
<?php 
            ...
            if (count($order->info['tax_groups']) > 1) {
                ...
                echo zen_display_tax_value($product_tax) . '%';
                ?></td>
<?php 
            }
            ?>
                <td class="accountTotalDisplay">
                    <?php 
            echo $currencies->format(zen_add_tax($current_product['final_price'], $product_tax) * $current_product['qty'], true, $currency, $currency_value) . ($current_product['onetime_charges'] != 0 ? '<br>' . $currencies->format(zen_add_tax($current_product['onetime_charges'], $product_tax), true, $currency, $currency_value) : '');
            ?></td>
            </tr>
<?php 
        }
...
<?php 
        foreach ($order->totals as $current_ot) {
            ...
            echo $current_ot['text'];
            ?></div>
            <div class="lineTitle larger forward"><?php 
            echo $current_ot['title'];
            ?></div>
...
<?php 
    ...
    if (!empty($status_array)) {
        ...
        <table id="myAccountOrdersStatus">
            <caption><h2 id="orderHistoryStatus"><?php 
        echo HEADING_ORDER_HISTORY;
        ?></h2></caption>
            <tr class="tableHeading">
                <th scope="col" id="myAccountStatusDate"><?php 
...
<?php 
        ...
        //
        $protected = true;
        foreach ($status_array as $statuses) {
            ?>
            <tr>
                <td><?php 
            ...
            ?></td>
                <td><?php 
            echo empty($statuses['comments']) ? '&nbsp;' : nl2br(zen_output_string($statuses['comments'], false, $protected), false);
            ?></td>
            </tr>
<?php 
            $protected = false;
...
?></label>
        <?php 
echo zen_draw_input_field('order_id', $order_id, 'size="10" id="order_id" required', 'number');
?> 
        <br>
...
?></label>
        <?php 
echo zen_draw_input_field('query_email_address', $query_email_address, 'size="35" id="query_email_address" required', 'email');
?> 
        <br>

        <?php 
echo zen_draw_input_field($spam_input_name, '', ' size="40" id="CUAS" style="visibility:hidden; display:none;" autocomplete="off"');
?>
        <?php 
echo $extra_validation_html;
?>
