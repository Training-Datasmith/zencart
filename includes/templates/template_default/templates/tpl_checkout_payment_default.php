...
if ($message_stack->size('checkout_payment') > 0) {
    echo $message_stack->output('checkout_payment');
}
...
if (!$payment_modules->in_special_checkout()) {
    ...
    if (MAX_ADDRESS_BOOK_ENTRIES >= 2) {
        ...
        echo zen_href_link(FILENAME_CHECKOUT_PAYMENT_ADDRESS, '', 'SSL');
        ?>"><?php 
        echo zen_image_button(BUTTON_IMAGE_CHANGE_ADDRESS, BUTTON_CHANGE_ADDRESS_ALT);
        ?></a></div>
            <?php 
    }
    ?>
            <address><?php 
    echo zen_address_label($_SESSION['customer_id'], $_SESSION['billto'], true, ' ', '<br>');
    ?></address>
        </div>

        <div class="floatingBox important forward"><?php 
    echo TEXT_SELECTED_BILLING_DESTINATION;
    ?></div>
        <br class="clearBoth">
        <br>
        <?php 
...
if (sizeof($selection) > 0) {
    for ($i = 0, $n = sizeof($selection); $i < $n; $i++) {
        if (isset($_GET['credit_class_error_code']) && ($_GET['credit_class_error_code'] == isset($selection[$i]['id']) ? $selection[$i]['id'] : 0)) {
            ...
            echo zen_output_string_protected($_GET['credit_class_error']);
            ?></div>
                <?php 
        }
...
if (!$payment_modules->in_special_checkout()) {
    ...
    for ($i = 0, $n = sizeof($selection); $i < $n; $i++) {
        ...
        if (!empty($selection[$i]['text'])) {
            ?>
                    <div class="ccinfoText"><?php 
            echo $selection[$i]['text'];
            ?></div>
                    <?php 
        }
        ?>
...
if (DISPLAY_CONDITIONS_ON_CHECKOUT === 'true') {
    ...
    ?></legend>
            <div><?php 
    echo TEXT_CONDITIONS_DESCRIPTION;
    ?></div>
            <?php 
    echo zen_draw_checkbox_field('conditions', '1', isset($_SESSION['conditions']) && $_SESSION['conditions'] === '1', 'id="conditions" required oninput="this.setCustomValidity(\'\')" oninvalid="this.setCustomValidity(\'' . ERROR_CONDITIONS_NOT_ACCEPTED . '\')"');
    ?>
            <label class="checkboxLabel" for="conditions"><?php 
    echo TEXT_CONDITIONS_CONFIRM;
    ?></label>
        </fieldset>
        <?php 
}
...
echo zen_image_submit(BUTTON_IMAGE_CONTINUE_CHECKOUT, BUTTON_CONTINUE_ALT, 'onclick="submitFunction(' . $gv_balance . ',' . $order->info['total'] . ')"');
?></div>
    <div class="buttonRow back"><strong><?php 
echo TITLE_CONTINUE_CHECKOUT_PROCEDURE;
?></strong><br><?php 
echo TEXT_CONTINUE_CHECKOUT_PROCEDURE;
?></div>
    <?php 
echo '</form>';
?>
