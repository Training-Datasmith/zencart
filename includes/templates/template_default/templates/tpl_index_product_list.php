...
<div id="categoryImgListing" class="categoryImg"><?php 
        echo zen_image(DIR_WS_IMAGES . $categories_image, '', CATEGORY_ICON_IMAGE_WIDTH, CATEGORY_ICON_IMAGE_HEIGHT);
        ?></div>
<?php 
    }
}
...
if ($do_filter_list || isset($_GET['alpha_filter_id']) || $check_for_alpha->record_count() > 0 && PRODUCT_LIST_ALPHA_SORTER == 'true') {
...
<?php 
    ...
    if (empty($getoption_set)) {
        echo zen_draw_hidden_field('cPath', $c_path);
    } else {
...
if ($error_categories == true) {
    ...
    $check_category = $db->Execute("select categories_id from " . TABLE_CATEGORIES . " where categories_id='" . $c_path . "'");
    if ($check_category->record_count() == 0) {
...
if ($error_categories == false and $show_display_category->record_count() > 0) {
    ?>
