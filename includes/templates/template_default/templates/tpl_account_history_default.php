...
<div class="centerColumn" id="accountHistoryDefault">
    ...
    <h1 id="accountHistoryDefaultHeading"><?php 
echo HEADING_TITLE;
?></h1>
...
if (!empty($account_history)) {
    ...
    <fieldset>
        ...
        ?></legend>
        <div class="notice forward"><?php 
        echo TEXT_ORDER_STATUS . $history['orders_status_name'];
        ?></div>
        <br class="clearBoth">
        ...
        <div class="content">
            <?php 
        echo '<strong>' . TEXT_ORDER_PRODUCTS . '</strong> ' . $history['product_count'] . '<br><strong>' . TEXT_ORDER_COST . '</strong> ' . strip_tags((string) $history['order_total']);
        ?>
        </div>
        <div class="content forward">
            <?php 
        echo '<a href="' . zen_href_link(FILENAME_ACCOUNT_HISTORY_INFO, (isset($_GET['page']) ? 'page=' . $_GET['page'] . '&' : '') . 'order_id=' . $history['orders_id'], 'SSL') . '">' . zen_image_button(BUTTON_IMAGE_VIEW_SMALL, BUTTON_VIEW_SMALL_ALT) . '</a>';
        ?>
        </div>
        <br class="clearBoth">
    </fieldset>
<?php 
    ...
    <div class="navSplitPagesLinks forward">
        <?php 
    echo TEXT_RESULT_PAGE . $history_split->display_links($max_display_page_links, zen_get_all_get_params(['page', 'info', 'x', 'y', 'main_page']), $paginate_as_ul);
    ?>
    </div>
