...
<h1 id="checkoutSuccessHeading"><?php 
echo HEADING_TITLE;
?></h1>
<div id="checkoutSuccessOrderNumber"><?php 
echo TEXT_YOUR_ORDER_NUMBER . $zv_orders_id;
?></div>
<?php 
...
if (isset($additional_payment_messages) && $additional_payment_messages != '') {
  ...
  <div class="content">
  <?php 
    echo $additional_payment_messages;
    ?>
  </div>
<?php 
}
...
?>" id="linkMyAccount"><?php 
echo zen_image_button(BUTTON_IMAGE_MY_ORDERS, BUTTON_MY_ORDERS_TEXT);
?></a>
    <a href="<?php 
echo zen_href_link(FILENAME_LOGOFF, '', 'SSL');
?>" id="linkLogoff"><?php 
echo zen_image_button(BUTTON_IMAGE_LOG_OFF, BUTTON_LOG_OFF_ALT);
?></a>
</div>

<div id="checkoutSuccessContactLink"><?php 
echo TEXT_CONTACT_STORE_OWNER;
?></div>
...
<legend><?php 
    ...
    ?></legend>
<?php 
    echo zen_draw_form('order', zen_href_link(FILENAME_ACCOUNT_NOTIFICATIONS, 'action=notify', 'SSL'));
    ?>

<?php 
    foreach ($notifications_array as $notifications) {
...
<label class="checkboxLabel" for="<?php 
        echo 'notify-' . $notifications['counter'];
        ?>"><?php 
        echo $notifications['products_name'];
        ?></label>
<br>
<?php 
    }
...
<h3 id="checkoutSuccessThanks" class="centeredContent"><?php 
