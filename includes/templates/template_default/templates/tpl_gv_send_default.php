...
<?php
        ...
        <div id="gvSendDefaultMessage" class="content"><?php echo stripslashes((string) $_POST['message']); ?></div>
<?php
        }
    ...
    echo zen_draw_hidden_field('to_name', stripslashes($to_name)) .
         ...
         zen_draw_hidden_field('message', stripslashes((string) $_POST['message']));
    ?>
...
<?php
    ...
    <?php echo zen_draw_form('gv_send_send', zen_href_link(FILENAME_GV_SEND, 'action=send', 'SSL', false)); ?>
        <fieldset>
            ...
            <?php echo zen_draw_textarea_field('message', 50, 10, (!empty($_POST['message']) ? stripslashes((string) $_POST['message']) : ''), 'id="message-area"'); ?>
        </fieldset>
