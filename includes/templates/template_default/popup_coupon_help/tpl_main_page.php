...
if ($coupon->fields['coupon_is_valid_for_sales'] == 0) {
    $text_coupon_help .= '<p>' . TEXT_NO_PROD_SALES . '</p>';
}
if ($coupon->fields['coupon_minimum_order'] > 0) {
    $text_coupon_help .= '<p>' . sprintf(TEXT_COUPON_HELP_MINORDER, $currencies->format($coupon->fields['coupon_minimum_order'])) . '</p>';
}
...
$cats = [];
if ($get_result->record_count() == 1 && $get_result->fields['category_id'] == '-1') {
    $cats[] = ['name' => '<p>' . TEXT_NO_CAT_TOP_ONLY_DENY . '</p>'];
} else {
    ...
    while (!$get_result->EOF) {
        ...
        if ($get_result->record_count() >= 1 and $get_result->fields['category_id'] != '-1') {
            ...
            $cats[] = ['validity' => $get_result->fields['coupon_restrict'] == 'N' ? 'A' : 'D', 'name' => $result->fields['categories_name'] . $restrict, 'link' => '<a href="' . zen_href_link(FILENAME_DEFAULT, 'cPath=' . (int) $result->fields['categories_id']) . '">' . $result->fields['categories_name'] . '</a>' . $restrict];
        }
        $get_result->move_next();
    }
    if (sizeof($cats) == 0) {
        $cats[] = ['name' => '<p>' . TEXT_NO_CAT_RESTRICTIONS . '</p>'];
    }
}
sort($cats);
$mycats = [];
...
while (!$get_result->EOF) {
    ...
    $prods[] = ['validity' => $get_result->fields['coupon_restrict'] == 'N' ? 'A' : 'D', 'name' => $result->fields['products_name'] . $restrict, 'link' => '<a href="' . zen_href_link(zen_get_info_page($result->fields['products_id']), 'cPath=' . zen_get_generated_category_path_rev($result->fields['master_categories_id']) . '&products_id=' . $result->fields['products_id']) . '">' . $result->fields['products_name'] . '</a>' . $restrict];
    $get_result->move_next();
}
if (sizeof($prods) == 0) {
    $prods[] = ['name' => '<p>' . TEXT_NO_PROD_RESTRICTIONS . '</p>'];
}
sort($prods);
$myprods = [];
