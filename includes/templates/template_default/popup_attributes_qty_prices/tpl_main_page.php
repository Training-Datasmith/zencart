...
while (!$products_options_names_lookup->EOF) {
    ...
    while (!$products_options_lookup->EOF) {
        ...
        if ($products_options_lookup->fields['attributes_qty_prices'] != '') {
            ...
            $attribute_table_cost = preg_split('/[:,]/', (string) $products_options_lookup->fields['attributes_qty_prices']);
        ...
        $products_options_lookup->move_next();
    }
    $products_options_names_lookup->move_next();
}
?>
</div>
...
<h2 class="pageHeading"><?php 
    echo TEXT_ATTRIBUTES_QTY_PRICES_ONETIME_HELP;
    ?></h2>
    ...
    while (!$products_options_names_lookup->EOF) {
        ...
        while (!$products_options_lookup->EOF) {
            ...
            if ($products_options_lookup->fields['attributes_qty_prices_onetime'] != '') {
                $attribute_quantity = '';
                $attribute_quantity_price = '';
                $attribute_table_cost = preg_split('/[:,]/', (string) $products_options_lookup->fields['attributes_qty_prices_onetime']);
                $size = sizeof($attribute_table_cost);
                for ($i = 0, $n = $size; $i < $n; $i += 2) {
