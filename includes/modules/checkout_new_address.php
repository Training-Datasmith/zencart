    ...
    if (!empty($_POST['firstname']) && !empty($_POST['lastname']) && !empty($_POST['street_address'])) {
        ...
        if (ACCOUNT_GENDER === 'true') {
            if ($gender !== 'm' && $gender !== 'f') {
                $error = true;
                $message_stack->add('checkout_address', ENTRY_GENDER_ERROR);
            }
        }
        if (mb_strlen($firstname) < ENTRY_FIRST_NAME_MIN_LENGTH) {
            $error = true;
            $message_stack->add('checkout_address', ENTRY_FIRST_NAME_ERROR);
        }
        if (mb_strlen($lastname) < ENTRY_LAST_NAME_MIN_LENGTH) {
            $error = true;
            $message_stack->add('checkout_address', ENTRY_LAST_NAME_ERROR);
        }
        if (mb_strlen($street_address) < ENTRY_STREET_ADDRESS_MIN_LENGTH) {
            $error = true;
            $message_stack->add('checkout_address', ENTRY_STREET_ADDRESS_ERROR);
        }
        if (mb_strlen($city) < ENTRY_CITY_MIN_LENGTH) {
            $error = true;
            $message_stack->add('checkout_address', ENTRY_CITY_ERROR);
        }
        if (ACCOUNT_STATE === 'true') {
            ...
            if ($entry_state_has_zones === true) {
                ...
                if ($found_exact_iso_match === true) {
                    ...
                    $message_stack->add('checkout_address', ENTRY_STATE_ERROR_SELECT);
                }
            } elseif (mb_strlen($state) < ENTRY_STATE_MIN_LENGTH) {
                $error = true;
                $error_state_input = true;
                $message_stack->add('checkout_address', ENTRY_STATE_ERROR);
            }
        }
        if (mb_strlen($postcode) < ENTRY_POSTCODE_MIN_LENGTH) {
            $error = true;
            $message_stack->add('checkout_address', ENTRY_POST_CODE_ERROR);
        }
        if (is_numeric($country) === false || $country < 1) {
            $error = true;
            $message_stack->add('checkout_address', ENTRY_COUNTRY_ERROR);
        }
        $zco_notifier->notify('NOTIFY_MODULE_CHECKOUT_NEW_ADDRESS_VALIDATION', [], $error);
        if ($error === false) {
            $sql_data_array = [['fieldName' => 'customers_id', 'value' => $_SESSION['customer_id'], 'type' => 'integer'], ['fieldName' => 'entry_firstname', 'value' => $firstname, 'type' => 'stringIgnoreNull'], ['fieldName' => 'entry_lastname', 'value' => $lastname, 'type' => 'stringIgnoreNull'], ['fieldName' => 'entry_street_address', 'value' => $street_address, 'type' => 'stringIgnoreNull'], ['fieldName' => 'entry_postcode', 'value' => $postcode, 'type' => 'stringIgnoreNull'], ['fieldName' => 'entry_city', 'value' => $city, 'type' => 'stringIgnoreNull'], ['fieldName' => 'entry_country_id', 'value' => $country, 'type' => 'integer']];
            if (ACCOUNT_GENDER === 'true') {
        ...
        switch ($address_type) {
            case 'billto':
                ...
                $check_address_query = $db->bind_vars($check_address_query, ':customersID', $_SESSION['customer_id'], 'integer');
                $check_address_query = $db->bind_vars($check_address_query, ':addressBookID', $_SESSION['billto'], 'integer');
            ...
            case 'shipto':
                ...
                $check_address_query = 'SELECT COUNT(*) AS total
                      ...
                      WHERE customers_id = :customersID
                        AND address_book_id = :addressBookID';
                $check_address_query = $db->bind_vars($check_address_query, ':customersID', $_SESSION['customer_id'], 'integer');
                $check_address_query = $db->bind_vars($check_address_query, ':addressBookID', $_SESSION['sendto'], 'integer');
                $check_address = $db->Execute($check_address_query);
                if ($check_address->fields['total'] === '1') {
