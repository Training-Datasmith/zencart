...
if ($best_sellers->record_count() >= MIN_DISPLAY_BESTSELLERS) {
    $rows = 0;
    $bestsellers_list = [];
    foreach ($best_sellers as $bestseller) {
        $bs_product_info = (new Product((int) $bestseller['products_id']))->with_default_language();
        $bestseller = array_merge($bestseller, $bs_product_info->get_data());
        $best_products_id = $bestseller['products_id'];
        $rows++;
        $bestsellers_list[$rows] = ['id' => $best_products_id, 'name' => $bestseller['products_name'], 'image' => zen_image(DIR_WS_IMAGES . $bestseller['products_image'], $bestseller['products_name'], SMALL_IMAGE_WIDTH, SMALL_IMAGE_HEIGHT), 'href' => zen_href_link(zen_get_info_page($best_products_id), 'cPath=' . zen_get_generated_category_path_rev($bestseller['master_categories_id']) . '&products_id=' . $best_products_id), 'price' => zen_get_products_display_price((int) $bestseller['products_id']), 'model' => $bestseller['products_model'], 'description' => $bestseller['products_description']];
    }
