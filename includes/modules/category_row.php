...
$num_categories = $categories->record_count();
...
foreach ($categories as $next_category) {
    ...
    $c_path_new = zen_get_path($next_category['categories_id']);
    // strip out 0_ from top level cats
    $c_path_new = str_replace('=' . (int) TOPMOST_CATEGORY_PARENT_ID . '_', '=', $c_path_new);
    ...
    if ($category_row_layout_style === 'fluid') {
        ...
        if (!isset($grid_category_classes_matrix)) {
            ...
            $grid_category_classes_matrix = ['12' => 'row row-cols-1 row-cols-sm-2 row-cols-md-3 row-cols-lg-4 row-cols-xl-6', '10' => 'row row-cols-1 row-cols-md-2 row-cols-lg-4 row-cols-xl-5', '9' => 'row row-cols-1 row-cols-md-3 row-cols-lg-4 row-cols-xl-5', '8' => 'row row-cols-1 row-cols-md-2 row-cols-lg-3 row-cols-xl-4', '6' => 'row row-cols-1 row-cols-md-2 row-cols-lg-2 row-cols-xl-3'];
        }
    ...
    $list_box_contents[$rows][] = ['params' => 'class="' . $grid_category_card_params . '"' . $style, 'text' => '<a href="' . zen_href_link(FILENAME_DEFAULT, $c_path_new) . '">' . zen_image(DIR_WS_IMAGES . $next_category['categories_image'], $next_category['categories_name'], SUBCATEGORY_IMAGE_WIDTH, SUBCATEGORY_IMAGE_HEIGHT, 'loading="lazy"') . '<br>' . $next_category['categories_name'] . '</a>', 'wrap_with_classes' => $grid_category_wrap_classes, 'card_type' => $category_row_layout_style];
    if ($category_row_layout_style === 'columns') {
        $columns++;
