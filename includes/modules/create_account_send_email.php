...
//
$language_loader->load_module_language_file('create_account_send_email.php', '');
$extra_welcome_text ??= '';
...
if (NEW_SIGNUP_DISCOUNT_COUPON !== '' && NEW_SIGNUP_DISCOUNT_COUPON !== '0') {
    ...
    $coupon = $db->Execute('SELECT * FROM ' . TABLE_COUPONS . ' WHERE coupon_id = ' . $coupon_id . ' LIMIT 1');
    $coupon_desc = $db->Execute('SELECT coupon_description FROM ' . TABLE_COUPONS_DESCRIPTION . ' WHERE coupon_id = ' . $coupon_id . ' AND language_id = ' . (int) $_SESSION['languages_id'] . ' LIMIT 1');
...
if (NEW_SIGNUP_GIFT_VOUCHER_AMOUNT > 0) {
    $coupon_code = Coupon::generate_random_coupon_code();
