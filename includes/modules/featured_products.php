...
if ($sql !== '') {
    $featured_products = $db->execute_random_multi($sql, MAX_DISPLAY_SEARCH_RESULTS_FEATURED);
}
$row = 0;
$col = 0;
$list_box_contents = [];
$title = '';
$num_products_count = $sql === '' ? 0 : $featured_products->record_count();
...
if ($num_products_count > 0) {
    ...
    while (!$featured_products->EOF) {
        ...
        $data = $product_info->get_data_for_language();
        ...
        $list_box_contents[$row][$col] = ['params' => 'class="centerBoxContentsFeatured centeredContent back"' . ' ' . 'style="width:' . $col_width . '%;"', 'text' => (($data['products_image'] === '' and PRODUCTS_IMAGE_NO_IMAGE_STATUS == 0) ? '' : '<a href="' . zen_href_link(zen_get_info_page($data['products_id']), 'cPath=' . $products_in_category[$data['products_id']] . '&products_id=' . $data['products_id']) . '">' . zen_image(DIR_WS_IMAGES . $data['products_image'], $data['products_name'], IMAGE_FEATURED_PRODUCTS_LISTING_WIDTH, IMAGE_FEATURED_PRODUCTS_LISTING_HEIGHT) . '</a><br>') . '<a href="' . zen_href_link(zen_get_info_page($data['products_id']), 'cPath=' . $products_in_category[$data['products_id']] . '&products_id=' . $data['products_id']) . '">' . $data['products_name'] . '</a><br>' . $products_price];
        $col++;
        if ($col > SHOW_PRODUCT_INFO_COLUMNS_FEATURED_PRODUCTS - 1) {
            $col = 0;
            $row++;
        }
        $featured_products->move_next_random();
    }
    if ($featured_products->record_count() > 0) {
