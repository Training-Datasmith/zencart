...
if ($pages_query->record_count() > 0) {
    ...
    foreach ($pages_query as $page_query) {
        ...
        $page_query_list[$rows]['link'] = $page_query_list[$rows]['altURL'] == '' ? zen_href_link(FILENAME_EZPAGES, 'id=' . $page_query['pages_id'] . ($page_query['toc_chapter'] > 0 ? '&chapter=' . $page_query['toc_chapter'] : ''), 'SSL') : $page_query_list[$rows]['altURL'];
        $page_query_list[$rows]['link'] .= $page_query['page_open_new_window'] == '1' ? '" rel="noreferrer noopener" target="_blank' : '';
    }
    $var_links_list = $page_query_list;
}
