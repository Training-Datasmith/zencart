...
$anti_spam_field_name = $_SESSION['antispam_fieldname'] ?? 'should_be_empty';
/**
 * Process form contents
 */
if (isset($_POST['action']) && $_POST['action'] === 'process' && !isset($login_page)) {
    $process = true;
    $anti_spam = !empty($_POST[$anti_spam_field_name]) ? 'spam' : '';
    ...
    $zco_notifier->notify('NOTIFY_CREATE_ACCOUNT_CAPTCHA_CHECK', $anti_spam_field_name, $anti_spam);
    $gender = false;
    ...
    if (DISPLAY_PRIVACY_CONDITIONS === 'true') {
        if (!isset($_POST['privacy_conditions']) || $_POST['privacy_conditions'] !== '1') {
            $error = true;
            $message_stack->add('create_account', ERROR_PRIVACY_STATEMENT_NOT_ACCEPTED, 'error');
        }
    }
    if (ACCOUNT_GENDER === 'true' && !in_array(strtolower($gender), ['m', 'f'])) {
        $error = true;
        $message_stack->add('create_account', ENTRY_GENDER_ERROR);
    }
    if (mb_strlen($firstname) < ENTRY_FIRST_NAME_MIN_LENGTH) {
        $error = true;
        $message_stack->add('create_account', ENTRY_FIRST_NAME_ERROR);
    }
    if (mb_strlen($lastname) < ENTRY_LAST_NAME_MIN_LENGTH) {
        $error = true;
        $message_stack->add('create_account', ENTRY_LAST_NAME_ERROR);
    }
    if (ACCOUNT_DOB === 'true') {
        if (ENTRY_DOB_MIN_LENGTH > 0 or !empty($_POST['dob'])) {
            if (strlen($dob) > 10 || zen_valid_date($dob) === false) {
                $error = true;
                $message_stack->add('create_account', ENTRY_DATE_OF_BIRTH_ERROR);
            }
        }
    }
    if (ACCOUNT_COMPANY === 'true') {
        if ((int) ENTRY_COMPANY_MIN_LENGTH > 0 && mb_strlen($company) < ENTRY_COMPANY_MIN_LENGTH) {
            $error = true;
            $message_stack->add('create_account', ENTRY_COMPANY_ERROR);
        }
    }
    $nick_error = false;
    if (mb_strlen($email_address) < ENTRY_EMAIL_ADDRESS_MIN_LENGTH) {
        $error = true;
        $message_stack->add('create_account', ENTRY_EMAIL_ADDRESS_ERROR);
    } elseif (zen_validate_email($email_address) == false) {
        $error = true;
        $message_stack->add('create_account', ENTRY_EMAIL_ADDRESS_CHECK_ERROR);
    } else {
        ...
        if ($already_exists) {
            $error = true;
            $message_stack->add('create_account', ENTRY_EMAIL_ADDRESS_ERROR_EXISTS);
        } else {
            $nick_error = false;
    ...
    if ($error === false && !empty($nick)) {
        ...
        $check_nick_query = $db->bind_vars($sql, ':nick:', $nick, 'string');
        $check_nick = $db->Execute($check_nick_query, 1);
        if (!$check_nick->EOF) {
            $error = true;
            $message_stack->add('create_account', ENTRY_NICK_DUPLICATE_ERROR);
        }
    }
    if (mb_strlen($street_address) < ENTRY_STREET_ADDRESS_MIN_LENGTH) {
        $error = true;
        $message_stack->add('create_account', ENTRY_STREET_ADDRESS_ERROR);
    }
    if (mb_strlen($city) < ENTRY_CITY_MIN_LENGTH) {
        $error = true;
        $message_stack->add('create_account', ENTRY_CITY_ERROR);
    }
    if (ACCOUNT_STATE === 'true') {
        ...
        if ($entry_state_has_zones === true) {
            ...
            $found_exact_iso_match = (int) $zone->record_count() === 1;
            if ((int) $zone->record_count() > 1) {
            ...
            if ($found_exact_iso_match === true) {
                ...
                $message_stack->add('create_account', ENTRY_STATE_ERROR_SELECT);
            }
        } elseif (mb_strlen($state) < ENTRY_STATE_MIN_LENGTH) {
            $error = true;
            $error_state_input = true;
            $message_stack->add('create_account', ENTRY_STATE_ERROR);
        }
    }
    if (mb_strlen($postcode) < ENTRY_POSTCODE_MIN_LENGTH) {
        $error = true;
        $message_stack->add('create_account', ENTRY_POST_CODE_ERROR);
    }
    if (is_numeric($country) === false || $country < 1) {
        $error = true;
        $message_stack->add('create_account', ENTRY_COUNTRY_ERROR);
    }
    if (strlen($telephone) < ENTRY_TELEPHONE_MIN_LENGTH) {
        $error = true;
        $message_stack->add('create_account', ENTRY_TELEPHONE_NUMBER_ERROR);
    }
    $zco_notifier->notify('NOTIFY_CREATE_ACCOUNT_VALIDATION_CHECK', [], $error, $send_welcome_email);
    if (strlen($password) < ENTRY_PASSWORD_MIN_LENGTH) {
        $error = true;
        $message_stack->add('create_account', ENTRY_PASSWORD_ERROR);
    } elseif ($password !== $confirmation) {
        $error = true;
        $message_stack->add('create_account', ENTRY_PASSWORD_ERROR_NOT_MATCHING);
    }
    if ($error === true) {
        // hook notifier class
        ...
        $message_stack->add_session('header', defined('ERROR_CREATE_ACCOUNT_SPAM_DETECTED') ? ERROR_CREATE_ACCOUNT_SPAM_DETECTED : 'Thank you, your account request has been submitted for review.', 'success');
        zen_redirect(zen_href_link(FILENAME_SHOPPING_CART));
    } else {
        $ip_address = zen_get_ip_address();
        $customer = new Customer();
        $data = compact('firstname', 'lastname', 'email_address', 'nick', 'email_format', 'telephone', 'fax', 'newsletter', 'password', 'customers_authorization', 'customers_referral', 'gender', 'dob', 'company', 'street_address', 'suburb', 'city', 'zone_id', 'state', 'postcode', 'country', 'ip_address');
        ...
        Customer::set_welcome_email_sent((int) $result['customers_id']);
