...
foreach ($products_discounts_query as $next_discount) {
    if ($column_count !== 0 && $next_discount['discount_qty'] != $show_qty) {
        if ($quantity_discounts[$column_count - 1]['show_qty'] < $next_discount['discount_qty'] - 1) {
            $quantity_discounts[$column_count - 1]['show_qty'] .= '-' . number_format($next_discount['discount_qty'] - 1);
        }
    }
    $column_count++;
    // -----
    ...
    $quantity_discounts[] = ['discounted_price' => $discounted_price, 'show_qty' => number_format($next_discount['discount_qty'])];
}
if ($column_count === 0) {
    $disc_cnt = 1;
} else {
    $disc_cnt = 0;
    $quantity_discounts[$column_count - 1]['show_qty'] .= '+';
}