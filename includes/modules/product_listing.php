...
$listing_split = new Split_Page_Results($listing_sql, $max_results, 'p.products_id', 'page');
...
if ($product_listing_layout_style === 'table' && !empty($show_table_header_row)) {
    ...
    for ($col = 0, $n = count($column_list); $col < $n; $col++) {
        ...
        $list_box_contents[0][$col] = ['align' => $lc_align, 'params' => 'class="productListing-heading"', 'text' => $lc_text];
    }
}
...
if ($num_products_count > 0) {
    ...
    foreach ($listing as $record) {
        $product_info = (new Product((int) $record['products_id']))->get_data_for_language((int) $_SESSION['languages_id']) ?? [];
        ...
        $records[] = array_merge($record, ['parent_category_name' => !empty($parent_category_name) ? $parent_category_name : $category_name, 'category_name' => $category_name], $product_info);
    }
    ...
    foreach ($records as $record) {
        ...
        $product_contents = [];
        $link_cpath = $record['master_categories_id'];
        if (!empty($_GET['cPath'])) {
            $link_cpath = $_GET['cPath'];
        }
        if (!empty($_GET['manufacturers_id']) && !empty($_GET['filter_id'])) {
            $link_cpath = $_GET['filter_id'];
        }
        for ($col = 0, $n = count($column_list); $col < $n; $col++) {
            $lc_align = '';
            $lc_text = '';
            $href = zen_href_link(zen_get_info_page($record['products_id']), 'cPath=' . zen_get_generated_category_path_rev($link_cpath) . '&products_id=' . $record['products_id']);
            ...
            $listing_qty_input_form = zen_draw_form('cart_quantity', zen_href_link($_GET['main_page'], zen_get_all_get_params(['action']) . 'action=add_product&products_id=' . $record['products_id']), 'post', 'enctype="multipart/form-data"') . '<input class="" type="text" name="cart_quantity" value="' . zen_get_buy_now_qty($record['products_id']) . '" maxlength="6" size="4" aria-label="' . ARIA_QTY_ADD_TO_CART . '">' . '<br>' . zen_draw_hidden_field('products_id', $record['products_id']) . zen_image_submit(BUTTON_IMAGE_IN_CART, BUTTON_IN_CART_ALT) . '</form>';
            $lc_button = '';
            if (zen_requires_attribute_selection($record['products_id']) || PRODUCT_LIST_PRICE_BUY_NOW == '0') {
                ...
                if ($record['products_qty_box_status'] != 0 && zen_get_products_allow_add_to_cart($record['products_id']) !== 'N' && $record['product_is_call'] == 0 && ($listing_quantity > 0 || SHOW_PRODUCTS_SOLD_OUT_IMAGE == 0)) {
                    $how_many++;
                }
                // hide quantity box
            ...
            } else if (PRODUCT_LIST_PRICE_BUY_NOW == '2' && $record['products_qty_box_status'] != 0) {
                $lc_button = '';
            ...
            // (used in column/fluid modes)
            if ($product_listing_layout_style === 'table') {
                $list_box_contents[$rows][] = ['align' => $lc_align, 'params' => 'class="productListing-data"', 'category' => $record['master_categories_id'], 'parent_category_name' => $record['parent_category_name'], 'category_name' => $record['category_name'], 'manufacturers_id' => $record['manufacturers_id'], 'manufacturers_name' => $listing_mfg_name, 'text' => $lc_text];
                //                // add description
                //                if (!empty($listing_description)) {
                //                    $rows++;
        ...
        if ($product_listing_layout_style === 'columns' || $product_listing_layout_style === 'fluid') {
            ...
            $list_box_contents[$rows][] = ['params' => 'class="' . $grid_product_card_params . '"' . $style, 'text' => $lc_text, 'wrap_with_classes' => $grid_product_wrap_classes, 'card_type' => $product_listing_layout_style, 'category' => $record['master_categories_id'], 'parent_category_name' => $record['parent_category_name'], 'category_name' => $record['category_name'], 'manufacturers_id' => $record['manufacturers_id'], 'manufacturers_name' => $listing_mfg_name];
            if ($product_listing_layout_style === 'columns') {
                $column++;
    ...
    $list_box_contents = [];
    $list_box_contents[0][] = ['params' => 'class="productListing-data"', 'text' => defined('TEXT_NO_PRODUCTS') ? TEXT_NO_PRODUCTS : 'No products to show.'];
