...
class ot_gv
{
    ...
    public function __construct()
    {
        ...
        if (!(isset($_SESSION['cot_gv']) && zen_not_null(ltrim((string) $_SESSION['cot_gv'], ' 0'))) || $_SESSION['cot_gv'] == '0') {
            $_SESSION['cot_gv'] = '0.00';
        }
        if (IS_ADMIN_FLAG !== true && zen_is_logged_in() && !zen_in_guest_checkout()) {
            $this->checkbox = $this->user_prompt . '<input type="text" size="6" onkeyup="submitFunction()" name="cot_gv" value="' . number_format($currencies->normalize_value($_SESSION['cot_gv']), 2) . '" onfocus="if (this.value == \'' . number_format($currencies->normalize_value($_SESSION['cot_gv']), 2) . '\') this.value = \'\';">' . ($this->user_has_gv_account($_SESSION['customer_id']) > 0 ? '<br>' . MODULE_ORDER_TOTAL_GV_USER_BALANCE . $currencies->format($this->user_has_gv_account($_SESSION['customer_id'])) : '');
        }
        $this->output = [];
     ...
     * @since ZC v1.0.3
     */
    public function selection_test(): bool
    {
     ...
     * @since ZC v1.0.3
     */
    public function pre_confirmation_check($order_total): float|int|array
    {
        global $order, $currencies, $message_stack;
        // clean out negative values and strip common currency symbols
        $_SESSION['cot_gv'] = preg_replace('/[^0-9.,%]/', '', (string) $_SESSION['cot_gv']);
        if ($_SESSION['cot_gv'] > 0) {
            // if cot_gv value contains any nonvalid characters, throw error
            if (preg_match('/[^0-9\,.]/', trim($_SESSION['cot_gv']))) {
                $message_stack->add_session('checkout_payment', TEXT_INVALID_REDEEM_AMOUNT, 'error');
    ...
    public function update_credit_account($i): void
    {
        ...
        if (preg_match('/^GIFT/', addslashes((string) $order->products[$i]['model']))) {
    ...
    public function credit_selection(): array
    {
        ...
        if (!$gv_query->EOF || $this->use_credit_amount()) {
            return ['id' => $this->code, 'module' => $this->title, 'redeem_instructions' => MODULE_ORDER_TOTAL_GV_REDEEM_INSTRUCTIONS, 'checkbox' => $this->use_credit_amount(), 'fields' => [['title' => MODULE_ORDER_TOTAL_GV_TEXT_ENTER_CODE, 'field' => zen_draw_input_field('gv_redeem_code', '', 'id="disc-' . $this->code . '" onkeyup="submitFunction(0,0)"'), 'tag' => 'disc-' . $this->code]]];
        }
        return $selection;
    }
    /**
    ...
    {
        global $db, $currencies, $message_stack;
        // if we have no GV amount selected, set it to 0
        // if requested redemption amount is greater than value of credits on account, throw error
        if ($_SESSION['cot_gv'] > $currencies->value($this->user_has_gv_account($_SESSION['customer_id']))) {
            $message_stack->add_session('checkout_payment', TEXT_INVALID_REDEEM_AMOUNT . ' - ' . number_format($_SESSION['cot_gv'], 2), 'error');
            $_SESSION['cot_gv'] = 0.0;
            zen_redirect(zen_href_link(FILENAME_CHECKOUT_PAYMENT, '', 'SSL'));
        }
        ...
        if (!empty($_POST['gv_redeem_code'])) {
            // check for validity
            $_POST['gv_redeem_code'] = preg_replace('/[^0-9a-zA-Z]/', '', (string) $_POST['gv_redeem_code']);
            ...
            if ($gv_result->record_count() > 0) {
                ...
                if ($redeem_query->record_count() > 0 && $gv_result->fields['coupon_type'] == 'G') {
                    $message_stack->add_session('checkout_payment', ERROR_NO_INVALID_REDEEM_GV, 'error');
                    zen_redirect(zen_href_link(FILENAME_CHECKOUT_PAYMENT, '', 'SSL'));
                }
            } else {
            ...
            if ($gv_result->fields['coupon_type'] == 'G') {
                ...
                if ($gv_amount_result->record_count() > 0) {
                ...
                $message_stack->add_session('redemptions', ERROR_REDEEMED_AMOUNT . $currencies->format($gv_amount), 'success');
                zen_redirect(zen_href_link(FILENAME_CHECKOUT_PAYMENT, '', 'SSL', true, false));
            }
        }
        if (isset($_POST['submit_redeem_x']) && $_POST['submit_redeem_x'] && $gv_result->fields['coupon_type'] == 'G') {
    ...
    public function calculate_credit($save_total_cost)
    {
        ...
        $gv_payment_amount = $currencies->normalize_value($_SESSION['cot_gv']);
     ...
     * @since ZC v1.3.8
     */
    public function calculate_deductions($order_total): array
    {
        global $db, $order;
        $od_amount = [];
    ...
    {
        global $db;
        $gv_result = $db->execute_no_cache('SELECT amount FROM ' . TABLE_COUPON_GV_CUSTOMER . " WHERE customer_id = '" . (int) $c_id . "'");
        if ($gv_result->record_count() > 0) {
            return $gv_result->fields['amount'];
        }
        return 0;
        // use 0 because 'false' was preventing checkout_payment from continuing
    }
    /**
     * Recalculates base order-total amount for use in deduction calculations
     * @since ZC v1.0.3
     */
    public function get_order_total(): float|int
    {
        global $order;
        ...
        for ($i = 0, $n = sizeof($chk_products); $i < $n; $i++) {
            if (preg_match('/^GIFT/', addslashes((string) $chk_products[$i]['model']))) {
                // determine how much GV was purchased
