...
$sql = 'SELECT c.categories_id, c.categories_image, cd.categories_name
        ...
        AND fc.status = 1';
$featured_categories = $db->execute_random_multi($sql, MAX_DISPLAY_SEARCH_RESULTS_FEATURED);
$row = 0;
$col = 0;
$list_box_contents = [];
$title = '';
$num_categories_count = $featured_categories->record_count();
...
if ($num_categories_count > 0) {
    ...
    while (!$featured_categories->EOF) {
        ...
        $data = $category_info->get_data_for_language();
        $list_box_contents[$row][$col] = ['params' => 'class="centerBoxContentsFeaturedCategories centeredContent back"' . ' ' . 'style="width:' . $col_width . '%;"', 'text' => ($data['categories_image'] === '' && PRODUCTS_IMAGE_NO_IMAGE_STATUS == 0 ? '' : '<a href="' . zen_href_link(FILENAME_DEFAULT, 'cPath=' . zen_get_generated_category_path_rev($data['categories_id'])) . '">' . zen_image(DIR_WS_IMAGES . $data['categories_image'], $data['categories_name'], IMAGE_PRODUCT_LISTING_WIDTH, IMAGE_PRODUCT_LISTING_HEIGHT) . '</a><br>') . '<a href="' . zen_href_link(FILENAME_DEFAULT, 'cPath=' . zen_get_generated_category_path_rev($data['categories_id'])) . '">' . $data['categories_name'] . '</a><br>'];
        $col++;
        if ($col > SHOW_PRODUCT_INFO_COLUMNS_FEATURED_PRODUCTS - 1) {
            $col = 0;
            $row++;
        }
        $featured_categories->move_next_random();
    }
