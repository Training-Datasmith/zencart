...
if (isset($_GET['action']) && $_GET['action'] == 'deleteconfirm' && isset($_POST['delete']) && is_numeric($_POST['delete'])) {
    ...
    $sql = $db->bind_vars($sql, ':delete', $_POST['delete'], 'integer');
    $db->Execute($sql);
    $zco_notifier->notify('NOTIFY_HEADER_ADDRESS_BOOK_DELETION_DONE');
    $message_stack->add_session('addressbook', SUCCESS_ADDRESS_BOOK_ENTRY_DELETED, 'success');
...
if (isset($_POST['action']) && ($_POST['action'] == 'process' || $_POST['action'] == 'update')) {
    ...
    if (ACCOUNT_GENDER == 'true') {
        if ($gender != 'm' && $gender != 'f') {
            $error = true;
            $message_stack->add('addressbook', ENTRY_GENDER_ERROR);
        }
    }
    if (mb_strlen($firstname) < ENTRY_FIRST_NAME_MIN_LENGTH) {
        $error = true;
        $message_stack->add('addressbook', ENTRY_FIRST_NAME_ERROR);
    }
    if (mb_strlen($lastname) < ENTRY_LAST_NAME_MIN_LENGTH) {
        $error = true;
        $message_stack->add('addressbook', ENTRY_LAST_NAME_ERROR);
    }
    if (mb_strlen($street_address) < ENTRY_STREET_ADDRESS_MIN_LENGTH) {
        $error = true;
        $message_stack->add('addressbook', ENTRY_STREET_ADDRESS_ERROR);
    }
    if (mb_strlen($city) < ENTRY_CITY_MIN_LENGTH) {
        $error = true;
        $message_stack->add('addressbook', ENTRY_CITY_ERROR);
    }
    if (ACCOUNT_STATE == 'true') {
        $check_query = 'SELECT count(*) AS total
                    FROM ' . TABLE_ZONES . '
                    WHERE zone_country_id = :zoneCountryID';
        $check_query = $db->bind_vars($check_query, ':zoneCountryID', $country, 'integer');
        ...
        if ($entry_state_has_zones == true) {
            $zone_query = 'SELECT distinct zone_id, zone_name, zone_code
                     FROM ' . TABLE_ZONES . '
                     WHERE zone_country_id = :zoneCountryID
                     AND ' . (trim($state) != '' && (int) $zone_id === 0 ? "(upper(zone_name) like ':zoneState%' OR upper(zone_code) like '%:zoneState%') OR " : '') . 'zone_id = :zoneID
                     ORDER BY zone_code ASC, zone_name';
            $zone_query = $db->bind_vars($zone_query, ':zoneCountryID', $country, 'integer');
            $zone_query = $db->bind_vars($zone_query, ':zoneState', strtoupper($state), 'noquotestring');
            $zone_query = $db->bind_vars($zone_query, ':zoneID', $zone_id, 'integer');
            $zone = $db->Execute($zone_query);
            //look for an exact match on zone ISO code
            $found_exact_iso_match = $zone->record_count() == 1;
            if ($zone->record_count() > 1) {
                while (!$zone->EOF && !$found_exact_iso_match) {
                    if (strtoupper((string) $zone->fields['zone_code']) == strtoupper($state)) {
            ...
            if ($found_exact_iso_match) {
                ...
                $message_stack->add('addressbook', ENTRY_STATE_ERROR_SELECT);
            }
        } else if (mb_strlen($state) < ENTRY_STATE_MIN_LENGTH) {
            $error = true;
            $error_state_input = true;
            $message_stack->add('addressbook', ENTRY_STATE_ERROR);
        }
    }
    if (mb_strlen($postcode) < ENTRY_POSTCODE_MIN_LENGTH) {
        $error = true;
        $message_stack->add('addressbook', ENTRY_POST_CODE_ERROR);
    }
    if (!is_numeric($country)) {
        $error = true;
        $message_stack->add('addressbook', ENTRY_COUNTRY_ERROR);
    }
    // -----
    // Give an observer the opportunity to check the data submitted and identify
    // an additional error.
    //
    $zco_notifier->notify('NOTIFY_ADDRESS_BOOK_PROCESS_VALIDATION', [], $error);
    if ($error == false) {
        $sql_data_array = [['fieldName' => 'entry_firstname', 'value' => $firstname, 'type' => 'stringIgnoreNull'], ['fieldName' => 'entry_lastname', 'value' => $lastname, 'type' => 'stringIgnoreNull'], ['fieldName' => 'entry_street_address', 'value' => $street_address, 'type' => 'stringIgnoreNull'], ['fieldName' => 'entry_postcode', 'value' => $postcode, 'type' => 'stringIgnoreNull'], ['fieldName' => 'entry_city', 'value' => $city, 'type' => 'stringIgnoreNull'], ['fieldName' => 'entry_country_id', 'value' => $country, 'type' => 'integer']];
        if (ACCOUNT_GENDER == 'true') {
        ...
        if ($_POST['action'] == 'update') {
            ...
            $where_clause = $db->bind_vars($where_clause, ':customersID', $_SESSION['customer_id'], 'integer');
            $where_clause = $db->bind_vars($where_clause, ':edit', $_GET['edit'], 'integer');
            $db->perform(TABLE_ADDRESS_BOOK, $sql_data_array, 'update', $where_clause);
            $zco_notifier->notify('NOTIFY_MODULE_ADDRESS_BOOK_UPDATED_ADDRESS_BOOK_RECORD', array_merge(['address_book_id' => $_GET['edit'], 'customers_id' => $_SESSION['customer_id']], $sql_data_array));
            ...
            if (isset($_POST['primary']) && $_POST['primary'] == 'on' || $_GET['edit'] == $_SESSION['customer_default_address_id']) {
                ...
                $sql_data_array = [['fieldName' => 'customers_firstname', 'value' => $firstname, 'type' => 'stringIgnoreNull'], ['fieldName' => 'customers_lastname', 'value' => $lastname, 'type' => 'stringIgnoreNull'], ['fieldName' => 'customers_default_address_id', 'value' => $_GET['edit'], 'type' => 'integer']];
                if (ACCOUNT_GENDER == 'true') {
                    $sql_data_array[] = ['fieldName' => 'customers_gender', 'value' => $gender, 'type' => 'enum:m|f'];
                }
                $where_clause = 'customers_id = :customersID';
                $where_clause = $db->bind_vars($where_clause, ':customersID', $_SESSION['customer_id'], 'integer');
                $db->perform(TABLE_CUSTOMERS, $sql_data_array, 'update', $where_clause);
                $zco_notifier->notify('NOTIFY_MODULE_ADDRESS_BOOK_UPDATED_CUSTOMER_RECORD', array_merge(['customers_id' => $_SESSION['customer_id']], $sql_data_array));
            }
        } else {
            $sql_data_array[] = ['fieldName' => 'customers_id', 'value' => $_SESSION['customer_id'], 'type' => 'integer'];
            $db->perform(TABLE_ADDRESS_BOOK, $sql_data_array);
            $new_address_book_id = $db->Insert_ID();
            $zco_notifier->notify('NOTIFY_MODULE_ADDRESS_BOOK_ADDED_ADDRESS_BOOK_RECORD', array_merge(['address_id' => $new_address_book_id], $sql_data_array));
            ...
            if (isset($_POST['primary']) && $_POST['primary'] == 'on') {
                ...
                $sql_data_array = [['fieldName' => 'customers_firstname', 'value' => $firstname, 'type' => 'stringIgnoreNull'], ['fieldName' => 'customers_lastname', 'value' => $lastname, 'type' => 'stringIgnoreNull']];
                if (ACCOUNT_GENDER == 'true') {
                ...
                $where_clause = $db->bind_vars($where_clause, ':customersID', $_SESSION['customer_id'], 'integer');
                $db->perform(TABLE_CUSTOMERS, $sql_data_array, 'update', $where_clause);
                $zco_notifier->notify('NOTIFY_MODULE_ADDRESS_BOOK_UPDATED_PRIMARY_CUSTOMER_RECORD', array_merge(['address_id' => $new_address_book_id, 'customers_id' => $_SESSION['customer_id']], $sql_data_array));
            }
        }
        $message_stack->add_session('addressbook', SUCCESS_ADDRESS_BOOK_ENTRY_UPDATED, 'success');
        zen_redirect(zen_href_link(FILENAME_ADDRESS_BOOK, '', 'SSL'));
    }
}
if (isset($_GET['edit']) && is_numeric($_GET['edit'])) {
    ...
    $entry_query = $db->bind_vars($entry_query, ':customersID', $_SESSION['customer_id'], 'integer');
    $entry_query = $db->bind_vars($entry_query, ':addressBookID', $_GET['edit'], 'integer');
    $entry = $db->Execute($entry_query);
    if ($entry->record_count() <= 0) {
        $message_stack->add_session('addressbook', ERROR_NONEXISTING_ADDRESS_BOOK_ENTRY);
        zen_redirect(zen_href_link(FILENAME_ADDRESS_BOOK, '', 'SSL'));
    }
    if (!isset($zone_name) || (int) $zone_name == 0) {
        $zone_name = zen_get_zone_name((int) $entry->fields['entry_country_id'], (int) $entry->fields['entry_zone_id'], $entry->fields['entry_state']);
    }
    ...
    if ($_GET['delete'] == $_SESSION['customer_default_address_id']) {
        $message_stack->add_session('addressbook', WARNING_PRIMARY_ADDRESS_DELETION, 'warning');
        zen_redirect(zen_href_link(FILENAME_ADDRESS_BOOK, '', 'SSL'));
    } else {
        $check_query = 'SELECT count(*) AS total
                    FROM ' . TABLE_ADDRESS_BOOK . '
                    ...
                    AND customers_id = :customersID';
        $check_query = $db->bind_vars($check_query, ':customersID', $_SESSION['customer_id'], 'integer');
        $check_query = $db->bind_vars($check_query, ':addressBookID', $_GET['delete'], 'integer');
        $check = $db->Execute($check_query);
...
if (!isset($_GET['delete']) && !isset($_GET['edit']) && count(zen_get_customer_address_book_entries($_SESSION['customer_id'])) >= MAX_ADDRESS_BOOK_ENTRIES) {
    $message_stack->add_session('addressbook', ERROR_ADDRESS_BOOK_FULL);
    zen_redirect(zen_href_link(FILENAME_ADDRESS_BOOK, '', 'SSL'));
}
