...
$product_info_query = $db->bind_vars($product_info_query, ':productsID', !empty($_GET['products_id']) ? $_GET['products_id'] : 0, 'integer');
$product_info_query = $db->bind_vars($product_info_query, ':languagesID', $_SESSION['languages_id'], 'integer');
$product_info = $db->Execute($product_info_query);
if (!$product_info->record_count()) {
    zen_redirect(zen_href_link(FILENAME_PRODUCT_REVIEWS, zen_get_all_get_params(['action'])));
}
...
$customer_query = $db->bind_vars($customer_query, ':customersID', $_SESSION['customer_id'], 'integer');
...
if (isset($_GET['action']) && $_GET['action'] == 'process') {
    ...
    if (mb_strlen($review_text) < REVIEW_TEXT_MIN_LENGTH) {
        $error = true;
        $message_stack->add('review_text', JS_REVIEW_TEXT);
    }
    if ($rating < 1 || $rating > 5) {
        $error = true;
        $message_stack->add('review_text', JS_REVIEW_RATING);
    }
    if ($error == false) {
        if ($anti_spam != '') {
            $zco_notifier->notify('NOTIFY_SPAM_DETECTED_DURING_WRITE_REVIEW');
            $message_stack->add_session('product_info', defined('ERROR_WRITE_REVIEW_SPAM_DETECTED') ? ERROR_WRITE_REVIEW_SPAM_DETECTED : TEXT_REVIEW_SUBMITTED_FOR_REVIEW, 'success');
        } else {
            $review_status = '1';
            ...
            $sql = $db->bind_vars($sql, ':productsID', !empty($_GET['products_id']) ? $_GET['products_id'] : 0, 'integer');
            $sql = $db->bind_vars($sql, ':customersID', $_SESSION['customer_id'], 'integer');
            $sql = $db->bind_vars($sql, ':customersName', $reviewer->fields['customers_firstname'] . ' ' . $reviewer->fields['customers_lastname'], 'string');
            $sql = $db->bind_vars($sql, ':rating', $rating, 'string');
            $db->Execute($sql);
            ...
            $sql = $db->bind_vars($sql, ':insertID', $insert_id, 'integer');
            $sql = $db->bind_vars($sql, ':languagesID', $_SESSION['languages_id'], 'integer');
            $sql = $db->bind_vars($sql, ':reviewText', $review_text, 'string');
            $db->Execute($sql);
            $email_text = '';
            ...
            // end send email
            $message_stack->add_session('product_info', REVIEWS_APPROVAL == '1' ? TEXT_REVIEW_SUBMITTED_FOR_REVIEW : TEXT_REVIEW_SUBMITTED, 'success');
        }
        zen_redirect(zen_href_link(FILENAME_PRODUCT_REVIEWS, zen_get_all_get_params(['action'])));
    }
}
