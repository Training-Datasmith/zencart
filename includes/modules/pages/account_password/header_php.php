...
if (isset($_POST['action']) && $_POST['action'] == 'process') {
    ...
    if (mb_strlen($password_new) < ENTRY_PASSWORD_MIN_LENGTH) {
        $error = true;
        $message_stack->add('account_password', ENTRY_PASSWORD_NEW_ERROR);
    } elseif ($password_new != $password_confirmation) {
        $error = true;
        $message_stack->add('account_password', ENTRY_PASSWORD_NEW_ERROR_NOT_MATCHING);
    }
    if ($error == false) {
        $check_customer_query = 'SELECT customers_password, customers_nick
                             FROM   ' . TABLE_CUSTOMERS . '
                             WHERE  customers_id = :customersID';
        $check_customer_query = $db->bind_vars($check_customer_query, ':customersID', $_SESSION['customer_id'], 'integer');
        $check_customer = $db->Execute($check_customer_query);
        if (zen_validate_password($password_current, $check_customer->fields['customers_password'])) {
            $customer = new Customer();
            //- Note, customer-id is set from session value
            $customer->set_password($password_new);
            ...
            $message_stack->add_session('account', SUCCESS_PASSWORD_UPDATED, 'success');
            zen_redirect(zen_href_link(FILENAME_ACCOUNT, '', 'SSL'));
        } else {
            $error = true;
            $message_stack->add('account_password', ERROR_CURRENT_PASSWORD_NOT_MATCHING);
        }
    }
}
