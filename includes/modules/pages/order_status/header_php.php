...
if (isset($_GET['action']) && $_GET['action'] === 'status') {
    ...
    if ($order_id < 1) {
        $error = true;
        $message_stack->add('order_status', ERROR_INVALID_ORDER);
    }
    ...
    if ($query_email_address === '' || !zen_validate_email($query_email_address)) {
        $error = true;
        $message_stack->add('order_status', ERROR_INVALID_EMAIL);
    }
    if ($error === false) {
        $customeremail = $db->Execute('SELECT orders_id FROM ' . TABLE_ORDERS . "\n              WHERE customers_email_address = '" . zen_db_input($query_email_address) . "'\n                AND orders_id = {$order_id}\n              LIMIT 1");
        if ($customeremail->EOF) {
            $error = true;
            $message_stack->add('order_status', ERROR_NO_MATCH);
        }
    }
    ...
    if ($error === true) {
        ...
        $statuses_query = $db->bind_vars($statuses_query, ':ordersID', $order_id, 'integer');
