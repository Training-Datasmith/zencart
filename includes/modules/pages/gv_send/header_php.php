...
$account_query = $db->bind_vars($account_query, ':customersID', $_SESSION['customer_id'], 'integer');
...
$gv_query = $db->bind_vars($gv_query, ':customersID', $_SESSION['customer_id'], 'integer');
...
if ($_GET['action'] == 'send') {
    ...
    if (!isset($_POST['to_name']) || trim($_POST['to_name'] == '')) {
        $error = true;
        $message_stack->add('gv_send', ERROR_ENTRY_TO_NAME_CHECK, 'error');
    }
    if (!zen_validate_email(trim((string) $_POST['email']))) {
        $error = true;
        $message_stack->add('gv_send', ERROR_ENTRY_EMAIL_ADDRESS_CHECK, 'error');
    }
    ...
    if (preg_match('/[^0-9\.,]/', $gv_amount)) {
        $error = true;
        $message_stack->add('gv_send', ERROR_ENTRY_AMOUNT_CHECK, 'error');
    }
    $gv_amount = $currencies->normalize_value($gv_amount);
...
if ($_GET['action'] == 'process') {
    if (!isset($_POST['back'])) {
        ...
        $id1 = Coupon::generate_random_coupon_code($account->fields['customers_email_address']);
        ...
        if ($new_amount < 0) {
            ...
            $gv_query = $db->bind_vars($gv_query, ':couponCode', $id1, 'string');
            $gv_query = $db->bind_vars($gv_query, ':amount', $currencies->value($_POST['amount'], true, DEFAULT_CURRENCY), 'currency');
            ...
            $gv_query = $db->bind_vars($gv_query, ':insertID', $insert_id, 'integer');
            $gv_query = $db->bind_vars($gv_query, ':customersID', $_SESSION['customer_id'], 'integer');
            $gv_query = $db->bind_vars($gv_query, ':firstname', $account->fields['customers_firstname'], 'string');
            $gv_query = $db->bind_vars($gv_query, ':lastname', $account->fields['customers_lastname'], 'string');
            $gv_query = $db->bind_vars($gv_query, ':email', $_POST['email'], 'string');
            $db->Execute($gv_query);
            // build email content:
            ...
            // include disclaimer
            $gv_email .= "\n\n" . EMAIL_ADVISORY . "\n\n";
            $html_msg['EMAIL_GV_FIXED_FOOTER'] = str_replace(["\r\n", "\n", "\r", '-----'], '', EMAIL_GV_FIXED_FOOTER);
