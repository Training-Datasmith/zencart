...
switch ($_GET['action']) {
    case 'product':
        if (!empty($_GET['products_id'])) {
            ...
            $sql = $db->bind_vars($sql, ':productId:', $_GET['products_id'], 'integer');
            ...
            if (!$result->EOF && $result->fields['products_url'] !== '') {
                ...
                $sql = 'SELECT products_url FROM ' . TABLE_PRODUCTS_DESCRIPTION . ' WHERE products_id = :productId: AND language_id = :languageId: LIMIT 1';
                $sql = $db->bind_vars($sql, ':productId:', $_GET['products_id'], 'integer');
                $sql = $db->bind_vars($sql, ':languageId:', $default_language_id, 'integer');
                $result = $db->Execute($sql);
                if (!$result->EOF && $result->fields['products_url'] !== '') {
    ...
    case 'music_artist':
        if (!empty($_GET['artists_id'])) {
            ...
            $sql = $db->bind_vars($sql, ':artistId:', $_GET['artists_id'], 'integer');
            $sql = $db->bind_vars($sql, ':languageId:', $_SESSION['languages_id'], 'integer');
            $result = $db->Execute($sql);
    ...
    case 'music_record_company':
        if (!empty($_GET['record_company_id'])) {
            ...
            $sql = $db->bind_vars($sql, ':rcId:', $_GET['record_company_id'], 'integer');
            $sql = $db->bind_vars($sql, ':languageId:', $_SESSION['languages_id'], 'integer');
            $result = $db->Execute($sql);
    ...
    case 'banner':
        if (!empty($_GET['goto'])) {
            ...
            $banner_query = $db->bind_vars($banner_query, ':bannersID', $_GET['goto'], 'integer');
            $banner = $db->Execute($banner_query);
            if (!$banner->EOF && $banner->fields['banners_url'] !== '') {
                $zco_notifier->notify('NOTIFY_BEFORE_REDIRECT_ACTION_BANNER', ['banners_id' => (int) $_GET['goto'], 'banners_url' => $banner->fields['banners_url']]);
    ...
    case 'manufacturer':
        if (!empty($_GET['manufacturers_id'])) {
            ...
            $sql = $db->bind_vars($sql, ':manufacturersID', $_GET['manufacturers_id'], 'integer');
            ...
            if (!$manufacturer->EOF && $manufacturer->fields['manufacturers_url'] !== '') {
                $zco_notifier->notify('NOTIFY_BEFORE_REDIRECT_ACTION_MANUFACTURER', ['manufacturers_id' => $_GET['manufacturers_id'], 'manufacturers_url' => $manufacturer->fields['manufacturers_url'], 'language_id' => $_SESSION['languages_id']]);
                ...
                $sql = 'SELECT manufacturers_url FROM ' . TABLE_MANUFACTURERS_INFO . ' WHERE manufacturers_id = :manufacturersID AND languages_id = :languagesID LIMIT 1';
                $sql = $db->bind_vars($sql, ':manufacturersID', $_GET['manufacturers_id'], 'integer');
                $sql = $db->bind_vars($sql, ':languagesID', $default_language_id, 'integer');
                $manufacturer = $db->Execute($sql);
                if (!$manufacturer->EOF && $manufacturer->fields['manufacturers_url'] !== '') {
                    $zco_notifier->notify('NOTIFY_BEFORE_REDIRECT_ACTION_MANUFACTURER', ['manufacturers_id' => $_GET['manufacturers_id'], 'manufacturers_url' => $manufacturer->fields['manufacturers_url'], 'language_id' => $default_language_id]);
                    $sql = 'UPDATE ' . TABLE_MANUFACTURERS_INFO . '
                            SET url_clicked = url_clicked+1, date_last_click = now()
                          WHERE manufacturers_id = :manufacturersID
                            AND languages_id = :languagesID
