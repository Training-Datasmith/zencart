...
if ($category_depth == 'nested') {
    ...
    if (isset($c_path) && strpos($c_path, '_')) {
        ...
        for ($i = 0, $n = sizeof($category_links); $i < $n; $i++) {
            ...
            if ($categories->fields['total'] < 1) {
                ...
                $categories_query = $db->bind_vars($categories_query, ':parentID', $category_links[$i], 'integer');
                $categories_query = $db->bind_vars($categories_query, ':languagesID', $_SESSION['languages_id'], 'integer');
                break;
        ...
        $categories_query = 'SELECT c.categories_id, cd.categories_name, c.categories_image, c.parent_id
                         FROM   ' . TABLE_CATEGORIES . ' c, ' . TABLE_CATEGORIES_DESCRIPTION . " cd\n                         WHERE      c.parent_id = :parentID\n                         AND        c.categories_id = cd.categories_id\n                         AND        cd.language_id = :languagesID\n                         AND        c.categories_status= '1'\n                         ORDER BY   sort_order, cd.categories_name";
        $categories_query = $db->bind_vars($categories_query, ':parentID', $current_category_id, 'integer');
        $categories_query = $db->bind_vars($categories_query, ':languagesID', $_SESSION['languages_id'], 'integer');
    }
    $categories = $db->Execute($categories_query);
    $number_of_categories = $categories->record_count();
    ...
    if (SHOW_PRODUCT_INFO_ALL_PRODUCTS == '1') {
        ...
        $new_products_category_id = $c_path;
    } else {
...
if ($categories_description_lookup->record_count() > 0) {
