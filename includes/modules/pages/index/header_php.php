...
if (isset($c_path) && zen_not_null($c_path)) {
    if ($c_path > 0) {
        $category_status_query = 'SELECT categories_status
             FROM ' . TABLE_CATEGORIES . '
             WHERE categories_id = :currentCategoryId
             LIMIT 1';
        $category_status_query = $db->bind_vars($category_status_query, ':currentCategoryId', $current_category_id, 'integer');
    ...
    $category_products_query = 'SELECT products_id
         FROM ' . TABLE_PRODUCTS_TO_CATEGORIES . '
         WHERE categories_id = :currentCategoryId
         LIMIT 1';
    $category_products_query = $db->bind_vars($category_products_query, ':currentCategoryId', $current_category_id, 'integer');
    ...
    if (!$category_products->EOF) {
        ...
        $category_parent_query = 'SELECT parent_id
             ...
             LIMIT 1';
        $category_parent_query = $db->bind_vars($category_parent_query, ':currentCategoryId', $current_category_id, 'integer');
    ...
    $zco_notifier->notify('NOTIFY_INDEX_CATEGORY_STATUS_CHECK', ['cPath' => $c_path, 'current_category_id' => $current_category_id], $category_redirect_handled, $current_category_not_found, $current_category_is_disabled, $current_category_has_products, $current_category_has_subcats, $category_depth);
    // -----
...
if (SKIP_SINGLE_PRODUCT_CATEGORIES == 'True' && !isset($_GET['filter_id']) && !isset($_GET['alpha_filter_id'])) {
    if ($listing->record_count() == 1) {
        zen_redirect(zen_href_link(zen_get_info_page($listing->fields['products_id']), ($c_path ? 'cPath=' . $c_path . '&' : '') . 'products_id=' . $listing->fields['products_id']));
    }
}
