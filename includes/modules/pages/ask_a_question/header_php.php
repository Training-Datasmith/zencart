...
if (isset($_GET['action']) && $_GET['action'] == 'send') {
    ...
    if ($zc_validate_email && !empty($enquiry) && !empty($name) && $error == false) {
        ...
        if ($anti_spam != '') {
            ...
            // default to EMAIL_FROM
            $send_to_name = trim(STORE_NAME);
            // default to STORE_NAME
            ...
            $html_msg['CONTACT_US_OFFICE_FROM'] = OFFICE_FROM . ' ' . $name . '<br>' . OFFICE_EMAIL . ' ' . $email_address . (!empty($telephone) ? '<br>' . OFFICE_LOGIN_PHONE . ' ' . $telephone : '');
        ...
        $error = true;
        if (empty($name)) {
            $message_stack->add('contact', ENTRY_EMAIL_NAME_CHECK_ERROR);
        }
        if ($zc_validate_email == false) {
            $message_stack->add('contact', ENTRY_EMAIL_ADDRESS_CHECK_ERROR);
        }
        if (empty($enquiry)) {
            $message_stack->add('contact', ENTRY_EMAIL_CONTENT_CHECK_ERROR);
        }
    }
}
// end action==send
