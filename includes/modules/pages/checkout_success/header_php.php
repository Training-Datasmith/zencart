...
if (!isset($_GET['action']) || $_GET['action'] !== 'confirm') {
    ...
    if (isset($_SESSION['order_number_created']) && $_SESSION['order_number_created'] >= 1) {
        ...
        $orders_query = $db->bind_vars($orders_query, ':customersID', $_SESSION['customer_id'], 'integer');
    ...
    $statuses_query = $db->bind_vars($statuses_query, ':ordersID', $orders_id, 'integer');
    $statuses_query = $db->bind_vars($statuses_query, ':languagesID', $_SESSION['languages_id'], 'integer');
    $statuses = $db->Execute($statuses_query);
    foreach ($statuses as $status) {
        $status_array[] = ['date_added' => $status['date_added'], 'orders_status_name' => $status['orders_status_name'], 'comments' => $status['comments']];
    }
    // get order details
    ...
    if ($flag_global_notifications !== '1') {
        ...
        $products_query = $db->bind_vars($products_query, ':ordersID', $orders_id, 'integer');
        $products = $db->Execute($products_query);
        foreach ($products as $product) {
            $notifications_array[] = ['counter' => $counter, 'products_id' => $product['products_id'], 'products_name' => $product['products_name']];
            $counter++;
        }
    }
    $flag_show_products_notification = CUSTOMERS_PRODUCTS_NOTIFICATION_STATUS === '1' && count($notifications_array) > 0 && $flag_global_notifications !== '1';
