...
$sql = $db->bind_vars($sql, ':downloadID', $_GET['id'], 'integer');
$sql = $db->bind_vars($sql, ':ordersID', $_GET['order'], 'integer');
if (isset($_SESSION['email_address'])) {
    $sql = $db->bind_vars($sql, ':emailAddress', $_SESSION['email_address'], 'string');
} else {
...
if (str_contains($browser_filename, '/')) {
    $browser_filename = substr($browser_filename, strrpos($browser_filename, '/') + 1);
}
if (str_contains($browser_filename, '\\')) {
    $browser_filename = substr($browser_filename, strrpos($browser_filename, '\\') + 1);
}
if (!str_ends_with($source_directory, '/')) {
    $source_directory .= '/';
}
$file_exists = file_exists($source_directory . $origin_filename);
$download_filesize = (int) @filesize($source_directory . $origin_filename);
// calculate days
[$dt_year, $dt_month, $dt_day] = explode('-', (string) $downloads->fields['date_purchased_day']);
...
$remaining_count = $downloads->fields['download_count'];
$is_expired = !$unlimited && ($download_timestamp <= time() || $remaining_count <= 0);
$zco_notifier->notify('NOTIFY_CHECK_DOWNLOAD_HANDLER', $downloads, $downloads->fields, $origin_filename, $browser_filename, $source_directory, $file_exists, $service, $is_expired, $download_timestamp);
if ($is_expired) {
...
if (!isset($downloads_should_decrement) || $downloads_should_decrement === true) {
...
if ($download_filesize < 1 && $service == 'local') {
 ...
 */
$detected_browser = '';
preg_match('/msie (.*?)/i', (string) $_SERVER['HTTP_USER_AGENT'], $matches);
if (count($matches) < 2) {
    preg_match('/Trident\/\d{1,2}.\d{1,2}; rv:([0-9]*)/', (string) $_SERVER['HTTP_USER_AGENT'], $matches);
    $version = count($matches) > 1 ? $matches[1] : 0;
    if ($version > 5) {
        $detected_browser = 'IE' . (int) $version;
    }
}
$zco_notifier->notify('NOTIFY_DOWNLOAD_BROWSER_DETECTION', [], $detected_browser, $_SERVER['HTTP_USER_AGENT'], $version, $browser_headers_override, $browser_extra_headers);
/**
 * Do we need to transform something?
 * An observer class could stamp PDFs or do other pre-processing of the download media.
 */
$zco_notifier->notify('NOTIFY_DOWNLOAD_BEFORE_START', $_SESSION['customers_host_address'], $service, $origin_filename, $browser_filename, $source_directory, $download_filesize, $mime_type, $downloads->fields, $browser_headers_override);
$zco_notifier->notify('NOTIFY_DOWNLOAD_READY_TO_START', $_SESSION['customers_host_address'], $service, $origin_filename, $browser_filename, $source_directory, $download_filesize, $mime_type, $downloads->fields, $browser_headers_override);
/**
...
$zco_notifier->notify('NOTIFY_DOWNLOAD_READY_TO_REDIRECT', [], $service, $origin_filename, $browser_filename, $source_directory, $link_create_status);
...
$zco_notifier->notify('NOTIFY_DOWNLOAD_READY_TO_STREAM', [], $service, $origin_filename, $browser_filename, $source_directory, $download_filesize);
