    ...
    if ($coupon->record_count() < 1) {
        ...
        if ($coupon->fields['coupon_active'] == 'N') {
            ...
            if ($get_result->record_count() == 1 && $get_result->fields['category_id'] == '-1') {
                $cats[] = ['link' => TEXT_NO_CAT_TOP_ONLY_DENY];
            } else {
                ...
                while (!$get_result->EOF) {
                    ...
                    if ($get_result->record_count() >= 1 and $get_result->fields['category_id'] != '-1') {
                        ...
                        $cats[] = ['validity' => $get_result->fields['coupon_restrict'] == 'N' ? 'A' : 'D', 'name' => $result->fields['categories_name'], 'link' => '<a href="' . zen_href_link(FILENAME_DEFAULT, 'cPath=' . (int) $result->fields['categories_id']) . '">' . $result->fields['categories_name'] . '</a>' . $restrict];
                    }
                    $get_result->move_next();
                }
                //echo 'CAT SIZE: ' . sizeof($cats) . ' - ' . ' $restrict: ' . $restrict . '<br>';
                if (sizeof($cats) == 0) {
                    $cats[] = ['link' => TEXT_NO_CAT_RESTRICTIONS];
                }
            }
            sort($cats);
            $mycats = [];
            ...
            while (!$get_result->EOF) {
                ...
                $prods[] = ['validity' => $get_result->fields['coupon_restrict'] == 'N' ? 'A' : 'D', 'name' => $result->fields['products_name'], 'link' => '<a href="' . zen_href_link(zen_get_info_page($result->fields['products_id']), 'cPath=' . zen_get_generated_category_path_rev($result->fields['master_categories_id']) . '&products_id=' . $result->fields['products_id']) . '">' . $result->fields['products_name'] . '</a>' . $restrict];
                $get_result->move_next();
            }
            if (sizeof($prods) == 0) {
                $prods[] = ['link' => TEXT_NO_PROD_RESTRICTIONS];
            }
            sort($prods);
            $myprods = [];
