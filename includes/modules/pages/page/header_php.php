...
$var_page_details = $db->Execute($sql);
// redirect to home page if page not found (or deactivated/deleted):
if ($var_page_details->EOF) {
    require DIR_WS_MODULES . zen_get_module_directory('require_languages.php');
    $message_stack->add_session('header', ERROR_PAGE_NOT_FOUND, 'caution');
...
$pages_order_query = $db->bind_vars($pages_order_query, ':chapterID', $chapter_id, 'integer');
$pages_ordering = $db->execute($pages_order_query);
foreach ($pages_ordering as $page_order) {
    $vert_links[] = $page_order['pages_id'];
    $toc_links[] = ['pages_id' => $page_order['pages_id'], 'pages_title' => $page_order['pages_title']];
}
...
define('NAVBAR_TITLE', $var_page_details->fields['pages_title']);
define('HEADING_TITLE', $var_page_details->fields['pages_title']);
$breadcrumb->add($var_page_details->fields['pages_title']);
require DIR_WS_MODULES . zen_get_module_directory('require_languages.php');
