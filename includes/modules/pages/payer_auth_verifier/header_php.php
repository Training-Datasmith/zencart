...
if ($_SESSION['3Dsecure_transactionId'] == '') {
    ...
    if ($_SESSION['3Dsecure_requires_lookup'] == true && strcasecmp('Y', (string) $_SESSION['3Dsecure_enroll_lookup_attempted']) != 0) {
        ...
        $message_stack->add_session('checkout_payment', $error . '<!-- [' . $payment_module->code . '] -->', 'error');
        $redirect_page = zen_href_link(FILENAME_CHECKOUT_PAYMENT, '', 'SSL', true, false);
    } else {
        // enrollment lookup was either not required for the card type or was required and completed
        $redirect_page = zen_href_link(FILENAME_CHECKOUT_PROCESS, '', 'SSL', true);
    }
} else if (isset($_POST['PaRes']) && $_POST['PaRes'] != '') {
    $authenticate_data_array = ['transaction_id' => $_SESSION['3Dsecure_transactionId'], 'payload' => $_POST['PaRes']];
    if (is_object(${$payment_module})) {
        $authenticate_resp_array = ${$payment_module}->get3d_secure_authenticate_response($authenticate_data_array);
    }
    $should_continue = $authenticate_resp_array['continue_flag'];
    ...
    $merchant_data = unserialize($_SESSION['3Dsecure_merchantData'], ['allowed_classes' => false]);
    $_POST['paypalwpp_cc_issue_month'] = $merchant_data['im'];
    $_POST['paypalwpp_cc_issue_year'] = $merchant_data['iy'];
    $_POST['paypalwpp_cc_issuenumber'] = $merchant_data['in'];
    $_POST['paypalwpp_cc_firstname'] = $merchant_data['fn'];
    $_POST['paypalwpp_cc_lastname'] = $merchant_data['ln'];
    ...
    /////////////////////////////////////////////////////////////////////////////////////////
    if (strcasecmp('Y', (string) $should_continue) == 0) {
        ////////////////////////////////////////////////////////////////////
        // Business rules are set to continue to authorization
        ////////////////////////////////////////////////////////////////////
        $redirect_page = zen_href_link(FILENAME_CHECKOUT_PROCESS, '', 'SSL', false);
    } else {
        ////////////////////////////////////////////////////////////////////
    ...
    // Business rules are set to continue to authorization
    ////////////////////////////////////////////////////////////////////
    $redirect_page = zen_href_link(FILENAME_CHECKOUT_PROCESS, '', 'SSL', true, false);
}
header('Cache-Control: max-age=1');
// stores for only 1 second, which prevents page from being re-displayed
?>
<html>
<head>
...
<center>
<form name="frmResultPage" target="_top" method="post" action="<?php 
echo $redirect_page;
?>">
<br>Processing. Please wait.
<br><br><em>This may take a few moments...</em>
<br><br><?php 
echo zen_image(DIR_WS_IMAGES . '3ds/pleasewait.gif');
?>
