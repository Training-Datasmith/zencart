...
$_GET['gv_no'] ??= '';
$_GET['goback'] = '';
$_GET['gv_no'] = zen_sanitize_string(trim((string) $_GET['gv_no']));
// if the customer is not logged on, redirect them to the login page
if (!zen_is_logged_in() || zen_in_guest_checkout()) {
    $_SESSION['navigation']->set_snapshot();
    $message_stack->add_session('login', ERROR_GV_CREATE_ACCOUNT, 'error');
...
if (isset($_GET['gv_no'])) {
    ...
    $gv_query = $db->bind_vars($gv_query, ':couponCode', $_GET['gv_no'], 'string');
    $coupon = $db->Execute($gv_query);
    if ($coupon->record_count() > 0) {
        $redeem_query = 'SELECT coupon_id
                     FROM ' . TABLE_COUPON_REDEEM_TRACK . '
                     WHERE coupon_id = :couponID';
        $redeem_query = $db->bind_vars($redeem_query, ':couponID', $coupon->fields['coupon_id'], 'integer');
        $redeem = $db->Execute($redeem_query);
        if ($redeem->record_count() == 0) {
...
if (!$error) {
    ...
    $gv_query = $db->bind_vars($gv_query, ':couponID', $coupon->fields['coupon_id'], 'integer');
    $gv_query = $db->bind_vars($gv_query, ':remoteADDR', zen_get_ip_address(), 'string');
    $db->Execute($gv_query);
    $gv_update = 'UPDATE ' . TABLE_COUPONS . "\n                SET coupon_active = 'N'\n                WHERE coupon_id = :couponID";
    $gv_update = $db->bind_vars($gv_update, ':couponID', $coupon->fields['coupon_id'], 'integer');
