...
$review_query_raw = $db->bind_vars($review_query_raw, ':productsID', $_GET['products_id'], 'integer');
$review_query_raw = $db->bind_vars($review_query_raw, ':languagesID', $_SESSION['languages_id'], 'integer');
...
$reviews_query_raw = $db->bind_vars($reviews_query_raw, ':productsID', $_GET['products_id'], 'integer');
$reviews_query_raw = $db->bind_vars($reviews_query_raw, ':languagesID', $_SESSION['languages_id'], 'integer');
$reviews_split = new Split_Page_Results($reviews_query_raw, MAX_DISPLAY_NEW_REVIEWS);
$reviews = $db->Execute($reviews_split->sql_query);
$reviews_array = [];
while (!$reviews->EOF) {
    $reviews_array[] = ['id' => $reviews->fields['reviews_id'], 'customersName' => $reviews->fields['customers_name'], 'dateAdded' => $reviews->fields['date_added'], 'reviewsText' => $reviews->fields['reviews_text'], 'reviewsRating' => $reviews->fields['reviews_rating']];
    $reviews->move_next();
}
