...
if (($_GET['action'] ?? '') === 'process') {
    ...
    // END SLAM PREVENTION
    if (empty($_POST['email_address'])) {
        $message_stack->add_session('password_forgotten', ENTRY_EMAIL_ADDRESS_ERROR, 'error');
        zen_redirect(zen_href_link(FILENAME_PASSWORD_FORGOTTEN, '', 'SSL'));
    }
    $session_message = SUCCESS_PASSWORD_RESET_SENT;
    ...
    //
    $check_token_sent = Customer::get_password_reset_token_for_email($email_address);
    if ($check_token_sent !== false) {
        $max_minutes_token_valid = Customer::get_password_reset_token_minutes_valid();
        if (strtotime((string) $check_token_sent['created_at']) + $max_minutes_token_valid / 2 < time()) {
            ...
            $zco_notifier->notify('NOTIFY_PASSWORD_FORGOTTEN_ALREADY_SENT', $check_token_sent, $session_message, $continue_with_reset_email);
            if ((bool) $continue_with_reset_email === false) {
                $message_stack->add_session('login', $session_message, 'success');
                zen_redirect(zen_href_link(FILENAME_LOGIN, '', 'SSL'));
            }
        }
    }
    $check_customer = Customer::create_password_reset_token($email_address);
    if ($check_customer === false) {
        $zco_notifier->notify('NOTIFY_PASSWORD_FORGOTTEN_NOT_FOUND', $email_address, $session_message);
    } else {
        $zco_notifier->notify('NOTIFY_PASSWORD_FORGOTTEN_VALIDATED', $email_address, $session_message);
