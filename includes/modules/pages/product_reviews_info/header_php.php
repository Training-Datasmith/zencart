...
if (!empty($_GET['reviews_id']) && !empty($_GET['products_id'])) {
    ...
    $products_reviews_check_query = $db->bind_vars($products_reviews_check_query, ':productsID', $_GET['products_id'], 'integer');
    ...
    $reviews_count_query = $db->bind_vars($reviews_count_query, ':productsID', $_GET['products_id'], 'integer');
    $reviews_count_query = $db->bind_vars($reviews_count_query, ':languagesID', $_SESSION['languages_id'], 'integer');
    ...
    $review_info_check_query = 'SELECT count(*) AS total
                                ...
                                AND r.reviews_id = rd.reviews_id
                                AND rd.languages_id = :languagesID ' . $review_status;
    $review_info_check_query = $db->bind_vars($review_info_check_query, ':reviewsID', $_GET['reviews_id'], 'integer');
    $review_info_check_query = $db->bind_vars($review_info_check_query, ':productsID', $_GET['products_id'], 'integer');
    $review_info_check_query = $db->bind_vars($review_info_check_query, ':languagesID', $_SESSION['languages_id'], 'integer');
    $review_info_check = $db->Execute($review_info_check_query);
    if ($review_info_check->fields['total'] < 1) {
        zen_redirect(zen_href_link(FILENAME_PRODUCT_REVIEWS, zen_get_all_get_params(['reviews_id'])));
    }
} else {
    zen_redirect(zen_href_link(FILENAME_PRODUCT_REVIEWS, zen_get_all_get_params(['reviews_id'])));
}
...
$sql = $db->bind_vars($sql, ':reviewsID', $_GET['reviews_id'], 'integer');
$db->Execute($sql);
...
$review_info_query = $db->bind_vars($review_info_query, ':reviewsID', $_GET['reviews_id'], 'integer');
$review_info_query = $db->bind_vars($review_info_query, ':languagesID', $_SESSION['languages_id'], 'integer');
