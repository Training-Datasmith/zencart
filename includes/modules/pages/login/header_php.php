...
if (($_GET['action'] ?? '') === 'process') {
    ...
    if (isset($_GET['hmac'])) {
        ...
        if (!zen_validate_hmac_timestamp() || !$admin_id = zen_validate_hmac_admin_id($_POST['aid'])) {
        ...
        $_SESSION['emp_admin_id'] = $admin_id;
    ...
    $login_attempt = $customer->do_login_lookup_by_email($email_address);
    if ($login_attempt === false) {
        $error = true;
        $message_stack->add('login', TEXT_LOGIN_ERROR);
    } elseif ($login_attempt['customers_authorization'] == '4') {
        // this account is banned
        $zco_notifier->notify('NOTIFY_LOGIN_BANNED');
        $message_stack->add('login', TEXT_LOGIN_BANNED);
    } else {
        if (!$login_authorized) {
            $db_password = $login_attempt['customers_password'];
            // Check whether the password is good
            if (zen_validate_password($password, $db_password)) {
                $login_authorized = true;
                if (password_needs_rehash($db_password, PASSWORD_DEFAULT)) {
                    $customer->set_password_using_email_address($password, $email_address);
                }
            } else {
                $login_authorized = zen_validate_storefront_admin_login($password, $email_address);
            }
        }
        $zco_notifier->notify('NOTIFY_PROCESS_3RD_PARTY_LOGINS', $email_address, $password, $login_authorized);
        if (!$login_authorized) {
            $error = true;
            $message_stack->add('login', TEXT_LOGIN_ERROR);
        } else {
            ...
            if ($customer->get_data('activation_required')) {
...
$ec_button_enabled = $paypalec_enabled && $_SESSION['cart']->count_contents() > 0 && $_SESSION['cart']->total > 0 && paypal_usd_check($_SESSION['cart']->total) === true;
