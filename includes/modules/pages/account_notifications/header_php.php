...
$global_query = $db->bind_vars($global_query, ':customersID', $_SESSION['customer_id'], 'integer');
$global = $db->Execute($global_query);
//$flag_global_notifications = $global->fields['global_product_notifications'];
$flag_global_notifications = 0;
// @TODO - fix template layout, and then use the flag value from db, above.
if (isset($_POST['action']) && $_POST['action'] == 'process') {
    ...
    if ($product_global != $global->fields['global_product_notifications']) {
        ...
        $sql = 'UPDATE ' . TABLE_CUSTOMERS_INFO . '
            ...
            WHERE  customers_info_id = :customersID';
        $sql = $db->bind_vars($sql, ':globalProductNotifications', $product_global, 'integer');
        ...
        if (sizeof($products_parsed) > 0) {
            ...
            $check_query = $db->bind_vars($check_query, ':productsParsed', implode(',', $products_parsed), 'csv');
            ...
            if ($check->fields['total'] > 0) {
                ...
                AND         products_id NOT IN (:productsParsed)';
                $sql = $db->bind_vars($sql, ':customersID', $_SESSION['customer_id'], 'integer');
                $sql = $db->bind_vars($sql, ':productsParsed', implode(',', $products_parsed), 'csv');
                $db->Execute($sql);
            }
        }
    } else {
    ...
    $message_stack->add_session('account', SUCCESS_NOTIFICATIONS_UPDATED, 'success');
...
$products_query = $db->bind_vars($products_query, ':customersID', $_SESSION['customer_id'], 'integer');
$products_query = $db->bind_vars($products_query, ':languagesID', $_SESSION['languages_id'], 'integer');
$products = $db->Execute($products_query);
while (!$products->EOF) {
    $notifications_array[] = ['counter' => $counter, 'products_id' => $products->fields['products_id'], 'products_name' => $products->fields['products_name']];
    $counter++;
    $products->move_next();
}
$flag_products_check = sizeof($notifications_array);
