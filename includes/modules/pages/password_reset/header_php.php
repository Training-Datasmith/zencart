...
$result = Customer::get_password_reset_token_info($reset_token);
if ($result === false) {
    // no matching token found within date range of unexpired tokens
    $message_stack->add('reset_password', PASSWORD_RESET_ENTRY_PASSWORD_TOKEN_ERROR);
    $error = true;
    $token_error = true;
} else {
...
if (isset($_POST['action']) && $_POST['action'] === 'process' && !empty($customer_id)) {
    ...
    if (mb_strlen($password_new) < ENTRY_PASSWORD_MIN_LENGTH) {
        $error = true;
        $message_stack->add('reset_password', ENTRY_PASSWORD_NEW_ERROR);
    } elseif ($password_new !== $password_confirmation) {
        $error = true;
        $message_stack->add('reset_password', ENTRY_PASSWORD_NEW_ERROR_NOT_MATCHING);
    }
    if ($error === false) {
        $customer = new Customer($customer_id);
        $customer->set_password($password_new);
        $message_stack->add_session('login', PASSWORD_RESET_SUCCESS_PASSWORD_UPDATED, 'success');
