...
$newsletter_query = $db->bind_vars($newsletter_query, ':customersID', $_SESSION['customer_id'], 'integer');
...
if (isset($_POST['action']) && $_POST['action'] == 'process') {
    ...
    if ($newsletter_general != $newsletter->fields['customers_newsletter']) {
        ...
        $sql = $db->bind_vars($sql, ':customersNewsletter', $newsletter_general, 'integer');
        $db->Execute($sql);
    }
    $zco_notifier->notify('NOTIFY_HEADER_ACCOUNT_NEWSLETTER_UPDATED', $newsletter_general);
    $message_stack->add_session('newsletter', SUCCESS_NEWSLETTER_UPDATED, 'success');
