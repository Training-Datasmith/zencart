...
if ($_SESSION['valid_to_checkout'] == false) {
    $message_stack->add_session('header', ERROR_CART_UPDATE, 'error');
...
if (isset($_SESSION['cart']->cart_id)) {
    if (!isset($_SESSION['cartID']) || $_SESSION['cart']->cart_id != $_SESSION['cartID']) {
        ...
        if (isset($_POST['shipping']) && strpos($_POST['shipping'], '_')) {
            ...
            if (isset(${$module}) && is_object(${$module}) || $_POST['shipping'] == 'free_free') {
                ...
                if (isset($quote[0]['error'])) {
                    ...
                    $_SESSION['shipping'] = ['id' => $_POST['shipping'], 'title' => $free_shipping == true ? $quote[0]['methods'][0]['title'] : $quote[0]['module'] . ' (' . $quote[0]['methods'][0]['title'] . ')', 'cost' => $quote[0]['methods'][0]['cost']];
                    zen_redirect(zen_href_link(FILENAME_CHECKOUT_PAYMENT, '', 'SSL'));
                }
            } else {
                unset($_SESSION['shipping']);
...
if (isset($_SESSION['shipping']['id'])) {
    ...
    if (!in_array($checkval, $checklist)) {
        $message_stack->add('checkout_shipping', ERROR_PLEASE_RESELECT_SHIPPING_METHOD, 'error');
        unset($_SESSION['shipping']);
        // Prepare $_SESSION to determine lowest available price/force a default selection mc12345678 2018-04-03
    }
}
...
$display_address_edit = MAX_ADDRESS_BOOK_ENTRIES >= 2;
// if shipping-edit button should be overridden, do so
$edit_shipping_button_link = zen_href_link(FILENAME_CHECKOUT_SHIPPING_ADDRESS, '', 'SSL');
