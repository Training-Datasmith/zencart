 ...
 * @since ZC v1.3.7
 */
function ipn_lookup_transaction(array $postArray): array
{
    global $db;
    ...
    return ['order_id' => $ordersID, 'paypal_ipn_id' => $paypalipnID, 'txn_type' => $transType];
}
/**
 * IPN Validation
 ...
 * @since ZC v1.3.0
 */
function ipn_validate_transaction(string $info, array $postArray, $mode = 'IPN'): bool
{
    ...
    if (defined('MODULE_PAYMENT_PAYPAL_BUSINESS_ID')) {
        if (strtolower(trim((string) $postArray['business'])) == strtolower(trim((string) MODULE_PAYMENT_PAYPAL_BUSINESS_ID))) {
            $ppBusEmail = true;
        }
        if (strtolower(trim((string) $postArray['receiver_email'])) == strtolower(trim((string) MODULE_PAYMENT_PAYPAL_BUSINESS_ID))) {
            $ppRecEmail = true;
        }
 ...
 * @since ZC v1.3.7
 */
function ipn_determine_txn_type(array $postArray, $txn_type = 'unknown')
{
    global $parentLookup;
...
{
    $sql_data_array = ['payment_type' => $_POST['payment_type'],
                            'txn_type' => $txn_type,
                            'parent_txn_id' => $_POST['parent_txn_id'],
                            'payment_status' => $_POST['payment_status'],
                            'pending_reason' => $_POST['pending_reason'],
 ...
 * @since ZC v1.3.0
 */
function getRequestBodyContents(&$handle): string|false
{
    if ($handle) {
        $line = '';
        while (!feof($handle)) {
 ...
 * @since ZC v1.3.7
 */
function ipn_postback($mode = 'IPN', $pdtTX = ''): false|string|array
{
    $postdata = '';
    $postback = '';
    $postback_array = [];
 ...
 * @since ZC v1.3.9a
 */
function doPayPalIPNFsockopenPostback(array $web, string $postback, $postback_array, $ssl, $mode = 'IPN'): array|string
{
 ...
 * @since ZC v1.3.9a
 */
function doPayPalIPNCurlPostback(array $url, $vars, $varsArray, $mode = 'IPN')
{
    ...
    $curlOpts = [CURLOPT_URL => 'https://' . $url['host'] . $url['path'],
                      CURLOPT_POST => true,
 ...
 * @since ZC v1.3.7
 */
function ipn_update_orders_status_and_history($ordersID, $new_status = 1, string $txn_type = ''): void
{
    global $db;
    ...
    if ($txn_type == 'echeck-cleared' || $txn_type == 'express-checkout-cleared' || str_starts_with($txn_type, 'cleared-')) {
        ...
        ipn_debug_email('IPN NOTICE :: Updating order #' . (int)$ordersID . ' downloads (if any).  New max days: ' . $zc_max_days . ', New count: ' . (int)DOWNLOAD_MAX_COUNT);
        $update_downloads_query = 'UPDATE ' . TABLE_ORDERS_PRODUCTS_DOWNLOAD . " SET download_maxdays='" . $zc_max_days . "', download_count='" . (int)DOWNLOAD_MAX_COUNT . "' WHERE orders_id='" . (int)$ordersID . "'";
 ...
 * @return mixed[]
 */
function ipn_getLineItemDetails($restrictedCurrency): array
{
    ...
    for ($i = 0, $n = sizeof($order->products), $k = 0; $i < $n; $i++) {
        ...
        if ($order->products[$i]['onetime_charges'] != 0) {
            $k++;
            $optionsLI["item_name_$k"]   = MODULES_PAYMENT_PAYPALSTD_LINEITEM_TEXT_ONETIME_CHARGES_PREFIX . substr(htmlentities((string) $order->products[$i]['name'], ENT_QUOTES, 'UTF-8'), 0, 120);
    ...
    for ($k = 1, $n = $numberOfLineItemsProcessed + 1; $k < $n; $k++) {
        $optionsLI["item_name_$k"] = str_replace(['&','=','%'], '*', $optionsLI["item_name_$k"]);
        ...
        if (isset($optionsLI["item_number_$k"])) {
            if ($optionsLI["item_number_$k"] == '') {
                ...
                $optionsLI["item_number_$k"] = str_replace(['&','=','%'], '*', $optionsLI["item_number_$k"]);
