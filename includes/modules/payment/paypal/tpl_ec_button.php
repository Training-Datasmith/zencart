...
$paypalec_enabled = defined('MODULE_PAYMENT_PAYPALWPP_STATUS') && MODULE_PAYMENT_PAYPALWPP_STATUS == 'True' && paypal_usd_check($_SESSION['cart']->total) === true;
...
if ($paypalec_enabled) {
    ...
    if ((int) MODULE_PAYMENT_PAYPALWPP_ZONE > 0 && zen_is_logged_in()) {
        $cust_country_check = isset($order) ? $order->billing['country']['id'] : $_SESSION['customer_country_id'];
        $cust_zone_check = isset($order) ? $order->billing['zone_id'] : $_SESSION['customer_zone_id'];
        $check_flag = false;
        $sql = 'SELECT zone_id
            ...
            ORDER BY zone_id';
        $sql = $db->bind_vars($sql, ':zoneId', (int) MODULE_PAYMENT_PAYPALWPP_ZONE, 'integer');
        $sql = $db->bind_vars($sql, ':countryId', $cust_country_check, 'integer');
        $result = $db->Execute($sql);
        while (!$result->EOF) {
            if ($result->fields['zone_id'] < 1 || $result->fields['zone_id'] == $cust_zone_check) {
                $check_flag = true;
                break;
            }
            $result->move_next();
