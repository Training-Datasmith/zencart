...
class authorizenet_aim extends base
{
     ...
     */
    public $display_specific_failure_reason_for_cvv_and_date = false;
    /**
     ...
     */
    protected $authorize = '';
    protected $comm_err_no = 0;
    protected $comm_error = '';
    ...
    {
        global $order, $message_stack;
        $this->code = 'authorizenet_aim';
        $this->enabled = defined('MODULE_PAYMENT_AUTHORIZENET_AIM_STATUS') && MODULE_PAYMENT_AUTHORIZENET_AIM_STATUS == 'True';
        ...
        if (IS_ADMIN_FLAG === true) {
            ...
            if ($this->enabled) {
                ...
                if (!function_exists('curl_init')) {
                    $message_stack->add_session(MODULE_PAYMENT_AUTHORIZENET_AIM_TEXT_ERROR_CURL_NOT_FOUND, 'error');
                }
            }
        } else {
        ...
        $this->description = MODULE_PAYMENT_AUTHORIZENET_AIM_TEXT_DESCRIPTION;
        // Descriptive Info about module in Admin
        $this->sort_order = defined('MODULE_PAYMENT_AUTHORIZENET_AIM_SORT_ORDER') ? MODULE_PAYMENT_AUTHORIZENET_AIM_SORT_ORDER : null;
        ...
        $this->_log_dir = defined('DIR_FS_LOGS') ? DIR_FS_LOGS : DIR_FS_SQL_CACHE;
        if (is_object($order)) {
            $this->update_status();
        }
    ...
    public function update_status(): void
    {
        ...
        if (MODULE_PAYMENT_AUTHORIZENET_AIM_TESTMODE == 'Production' && substr(HTTP_SERVER, 6) != 'https:' && (!defined('ENABLE_SSL') || ENABLE_SSL != 'true')) {
            $this->enabled = false;
        }
    ...
    public function selection(): array
    {
        ...
        $selection = ['id' => $this->code, 'module' => MODULE_PAYMENT_AUTHORIZENET_AIM_TEXT_CATALOG_TITLE, 'fields' => [['title' => MODULE_PAYMENT_AUTHORIZENET_AIM_TEXT_CREDIT_CARD_OWNER, 'field' => zen_draw_input_field('authorizenet_aim_cc_owner', $order->billing['firstname'] . ' ' . $order->billing['lastname'], 'id="' . $this->code . '-cc-owner"' . $on_focus . ' autocomplete="off"'), 'tag' => $this->code . '-cc-owner'], ['title' => MODULE_PAYMENT_AUTHORIZENET_AIM_TEXT_CREDIT_CARD_NUMBER, 'field' => zen_draw_input_field('authorizenet_aim_cc_number', '', 'id="' . $this->code . '-cc-number"' . $on_focus . ' autocomplete="off"'), 'tag' => $this->code . '-cc-number'], ['title' => MODULE_PAYMENT_AUTHORIZENET_AIM_TEXT_CREDIT_CARD_EXPIRES, 'field' => zen_draw_pull_down_menu('authorizenet_aim_cc_expires_month', $expires_month, $zc_date->output('%m'), 'id="' . $this->code . '-cc-expires-month"' . $on_focus) . '&nbsp;' . zen_draw_pull_down_menu('authorizenet_aim_cc_expires_year', $expires_year, '', 'id="' . $this->code . '-cc-expires-year"' . $on_focus), 'tag' => $this->code . '-cc-expires-month']]];
        if (MODULE_PAYMENT_AUTHORIZENET_AIM_USE_CVV == 'True') {
            $selection['fields'][] = ['title' => MODULE_PAYMENT_AUTHORIZENET_AIM_TEXT_CVV, 'field' => zen_draw_input_field('authorizenet_aim_cc_cvv', '', 'size="4" maxlength="4"' . ' id="' . $this->code . '-cc-cvv"' . $on_focus . ' autocomplete="off"') . ' ' . '<a href="javascript:popupWindow(\'' . zen_href_link(FILENAME_POPUP_CVV_HELP) . '\')">' . MODULE_PAYMENT_AUTHORIZENET_AIM_TEXT_POPUP_CVV_LINK . '</a>', 'tag' => $this->code . '-cc-cvv'];
        }
        return $selection;
    }
    /**
    ...
    public function pre_confirmation_check(): void
    {
        ...
        if ($result == false || $result < 1) {
            $message_stack->add_session('checkout_payment', $error . '<!-- [' . $this->code . '] -->', 'error');
    ...
    {
        global $zc_date;
        return ['fields' => [['title' => MODULE_PAYMENT_AUTHORIZENET_AIM_TEXT_CREDIT_CARD_TYPE, 'field' => $this->cc_card_type], ['title' => MODULE_PAYMENT_AUTHORIZENET_AIM_TEXT_CREDIT_CARD_OWNER, 'field' => $_POST['authorizenet_aim_cc_owner']], ['title' => MODULE_PAYMENT_AUTHORIZENET_AIM_TEXT_CREDIT_CARD_NUMBER, 'field' => substr((string) $this->cc_card_number, 0, 4) . str_repeat('X', strlen((string) $this->cc_card_number) - 8) . substr((string) $this->cc_card_number, -4)], ['title' => MODULE_PAYMENT_AUTHORIZENET_AIM_TEXT_CREDIT_CARD_EXPIRES, 'field' => $zc_date->output('%B, %Y', mktime(0, 0, 0, $_POST['authorizenet_aim_cc_expires_month'], 1, '20' . $_POST['authorizenet_aim_cc_expires_year']))]]];
    }
    /**
    ...
    {
        $process_button_string = zen_draw_hidden_field('cc_owner', $_POST['authorizenet_aim_cc_owner']) . zen_draw_hidden_field('cc_expires', $this->cc_expiry_month . substr((string) $this->cc_expiry_year, -2)) . zen_draw_hidden_field('cc_type', $this->cc_card_type) . zen_draw_hidden_field('cc_number', $this->cc_card_number);
        if (MODULE_PAYMENT_AUTHORIZENET_AIM_USE_CVV == 'True') {
    ...
    {
        return ['ccFields' => ['cc_number' => 'authorizenet_aim_cc_number', 'cc_owner' => 'authorizenet_aim_cc_owner', 'cc_cvv' => 'authorizenet_aim_cc_cvv', 'cc_expires' => ['name' => 'concatExpiresFields', 'args' => "['authorizenet_aim_cc_expires_month','authorizenet_aim_cc_expires_year']"], 'cc_expires_month' => 'authorizenet_aim_cc_expires_month', 'cc_expires_year' => 'authorizenet_aim_cc_expires_year', 'cc_type' => $this->cc_card_type], 'extraFields' => [zen_session_name() => zen_session_id()]];
    }
    /**
    ...
    {
        global $response, $db, $order, $message_stack;
        $order->info['cc_owner'] = $_POST['cc_owner'];
        $order->info['cc_number'] = str_pad(substr((string) $_POST['cc_number'], -4), strlen((string) $_POST['cc_number']), 'X', STR_PAD_LEFT);
        $order->info['cc_expires'] = '';
        // $_POST['cc_expires'];
        $order->info['cc_cvv'] = '***';
        $sess_id = zen_session_id();
        ...
        $submit_data = [
            ...
            // AIM uses direct response, not relay response
            'x_delim_char' => $this->delimiter,
            // The default delimiter is a comma
            'x_encap_char' => $this->encap_char,
            // The divider to encapsulate response fields
            'x_version' => '3.1',
            // 3.1 is required to use CVV codes
            'x_method' => 'CC',
            ...
            // unable to accept partial authorizations at this time
        ...
        unset($response);
        $response = $this->_send_request($submit_data);
        ...
        $response_msg_to_customer = $response_text . ($this->comm_error == '' ? '' : ' Communications Error - Please notify webmaster.');
        if ($this->display_specific_failure_reason_for_cvv_and_date) {
            if ($response_code == '2' && in_array($response[2], ['44', '45', '65']) || $response_code == '3' && $response[2] == '78') {
                $response_msg_to_customer = 'CVV problem ';
            }
            if ($response_code == '3' && in_array($response[2], ['7', '8'])) {
        ...
        if ($response[0] == '3' && $response[2] == '78') {
            $response['ErrorDetails'] = 'Failed due to incorrect CVV format.';
        }
        if ($response[0] == '3' && $response[2] == '7') {
            $response['ErrorDetails'] = 'Expired card.';
        }
        if ($response[0] == '3' && $response[2] == '8') {
            $response['ErrorDetails'] = 'Expiry date invalid.';
        }
        ...
        if ($response[0] == '4' && in_array($response[2], ['193', '252', '253'])) {
            $this->order_status = 1;
        ...
        $this->_debug_actions($response, $order_time, $sess_id);
        if ($this->comm_error != '') {
            $message_stack->add_session('checkout_payment', MODULE_PAYMENT_AUTHORIZENET_AIM_TEXT_COMM_ERROR . ' (' . $this->comm_err_no . ')', 'caution');
            zen_redirect(zen_href_link(FILENAME_CHECKOUT_PAYMENT, '', 'SSL', true, false));
        }
        ...
        if ($response_code != '1' && $response_code != '4') {
            $message_stack->add_session('checkout_payment', $response_msg_to_customer . ' - ' . MODULE_PAYMENT_AUTHORIZENET_AIM_TEXT_DECLINED_MESSAGE, 'error');
            zen_redirect(zen_href_link(FILENAME_CHECKOUT_PAYMENT, '', 'SSL', true, false));
        }
        if ($response_code == '4') {
    ...
    {
        return ['title' => MODULE_PAYMENT_AUTHORIZENET_AIM_TEXT_ERROR, 'error' => htmlspecialchars(stripslashes(urldecode((string) $_GET['error'])), ENT_QUOTES, 'UTF-8')];
    }
    /**
    ...
    public function keys(): array
    {
        ...
        return ['MODULE_PAYMENT_AUTHORIZENET_AIM_STATUS', 'MODULE_PAYMENT_AUTHORIZENET_AIM_LOGIN', 'MODULE_PAYMENT_AUTHORIZENET_AIM_TXNKEY', 'MODULE_PAYMENT_AUTHORIZENET_AIM_TESTMODE', 'MODULE_PAYMENT_AUTHORIZENET_AIM_CURRENCY', 'MODULE_PAYMENT_AUTHORIZENET_AIM_AUTHORIZATION_TYPE', 'MODULE_PAYMENT_AUTHORIZENET_AIM_STORE_DATA', 'MODULE_PAYMENT_AUTHORIZENET_AIM_EMAIL_CUSTOMER', 'MODULE_PAYMENT_AUTHORIZENET_AIM_EMAIL_MERCHANT', 'MODULE_PAYMENT_AUTHORIZENET_AIM_USE_CVV', 'MODULE_PAYMENT_AUTHORIZENET_AIM_SORT_ORDER', 'MODULE_PAYMENT_AUTHORIZENET_AIM_ZONE', 'MODULE_PAYMENT_AUTHORIZENET_AIM_ORDER_STATUS_ID', 'MODULE_PAYMENT_AUTHORIZENET_AIM_REFUNDED_ORDER_STATUS_ID', 'MODULE_PAYMENT_AUTHORIZENET_AIM_REVIEW_ORDER_STATUS_ID', 'MODULE_PAYMENT_AUTHORIZENET_AIM_DEBUGGING'];
    }
    /**
     * Send communication request
     * @since ZC v1.3.8
     */
    public function _send_request($submit_data): array
    {
        global $request_type;
        // Populate an array that contains all of the data to be sent to Authorize.net
        ...
        foreach ($submit_data as $key => $value) {
            if ($key != 'x_delim_char' && $key != 'x_encap_char' && !empty($value)) {
                $value = str_replace([$this->delimiter, $this->encap_char, '"', "'", '&amp;', '&', '='], '', $value);
            }
            $data .= $key . '=' . (!empty($value) ? urlencode((string) $value) : '') . '&';
        }
        ...
        if (isset($this->reportable_submit_data['x_card_num'])) {
            $this->reportable_submit_data['x_card_num'] = str_repeat('X', strlen((string) $this->reportable_submit_data['x_card_num']) - 4) . substr((string) $this->reportable_submit_data['x_card_num'], -4);
        }
        ...
        $this->comm_error = curl_error($ch);
        $this->comm_err_no = curl_errno($ch);
        if ($this->comm_err_no == 35) {
        ...
        $this->comm_info = @curl_getinfo($ch);
        ...
        if (defined('AUTHORIZENET_DEVELOPER_MODE') && AUTHORIZENET_DEVELOPER_MODE == 'echo' || MODULE_PAYMENT_AUTHORIZENET_AIM_DEBUGGING == 'echo') {
            echo $this->authorize . ($this->comm_err_no != 0 ? '<br>' . $this->comm_err_no . ' ' . $this->comm_error : '') . '<br>';
        ...
        $string_to_parse = $this->authorize;
        if (substr((string) $string_to_parse, 0, 1) == $this->encap_char) {
            $string_to_parse = substr((string) $string_to_parse, 1);
        }
        $string_to_parse = preg_replace('/.{*}' . $this->encap_char . '$/', '', (string) $string_to_parse);
        return explode($this->encap_char . $this->delimiter . $this->encap_char, (string) $string_to_parse);
    }
    /**
     * Used to do any debug logging / tracking / storage as required.
     * @since ZC v1.3.8
     */
    public function _debug_actions(array $response, $order_time = '', $sess_id = ''): void
    {
        global $db;
        if ($order_time == '') {
            $order_time = date('F j, Y, g:i a');
        ...
        // DEBUG LOGGING
        $error_message = date('M-d-Y h:i:s') . "\n=================================\n\n" . ($this->comm_error != '' ? 'Comm results: ' . $this->comm_err_no . ' ' . $this->comm_error . "\n\n" : '') . 'Response Code: ' . ($response[0] ?? 'Unknown') . ".\nResponse Text: " . ($response[3] ?? '') . "\n\n" . 'Sending to Authorizenet: ' . print_r($this->reportable_submit_data, true) . "\n\n" . 'Results Received back from Authorizenet: ' . print_r($resp_output, true) . "\n\n" . 'CURL communication info: ' . print_r($this->comm_info, true) . "\n";
        if (CURL_PROXY_REQUIRED == 'True') {
            $error_message .= 'Using CURL Proxy: [' . CURL_PROXY_SERVER_DETAILS . ']  with Proxy Tunnel: ' . ($this->proxy_tunnel_flag ? 'On' : 'Off') . "\n";
        }
        $error_message .= "\nRAW data received: \n" . $this->authorize . "\n\n";
        if (strstr(MODULE_PAYMENT_AUTHORIZENET_AIM_DEBUGGING, 'Log') || strstr(MODULE_PAYMENT_AUTHORIZENET_AIM_DEBUGGING, 'All') || MODULE_PAYMENT_AUTHORIZENET_AIM_TESTMODE == 'Sandbox' || defined('AUTHORIZENET_DEVELOPER_MODE') && AUTHORIZENET_DEVELOPER_MODE == 'certify') {
            ...
            $file = $this->_log_dir . '/' . 'AIM_Debug_' . $key . '.log';
            if ($fp = @fopen($file, 'a')) {
        ...
        if ($response[0] != '1' && stristr(MODULE_PAYMENT_AUTHORIZENET_AIM_DEBUGGING, 'Alerts') || strstr(MODULE_PAYMENT_AUTHORIZENET_AIM_DEBUGGING, 'Email')) {
            zen_mail(STORE_NAME, STORE_OWNER_EMAIL_ADDRESS, 'AuthorizenetAIM Alert ' . $response[7] . ' ' . date('M-d-Y h:i:s') . ' ' . $response[6], $error_message, STORE_OWNER, STORE_OWNER_EMAIL_ADDRESS, ['EMAIL_MESSAGE_HTML' => nl2br($error_message)], 'debug');
        }
        // DATABASE SECTION
        ...
        if (MODULE_PAYMENT_AUTHORIZENET_AIM_STORE_DATA == 'True' && !empty($response_order_id)) {
            $db_response_text = ($response[3] ?? 'Unknown') . ($this->comm_error != '' ? ' - Comm results: ' . $this->comm_err_no . ' ' . $this->comm_error : '');
            ...
            $sql = $db->bind_vars($sql, ':custID', $_SESSION['customer_id'] ?? '-1', 'integer');
            $sql = $db->bind_vars($sql, ':orderID', preg_replace('/[^0-9]/', '', (string) $response_order_id), 'integer');
            $sql = $db->bind_vars($sql, ':respCode', $response[0], 'integer');
            $sql = $db->bind_vars($sql, ':respText', $db_response_text, 'string');
            $sql = $db->bind_vars($sql, ':authType', $response[11] ?? '', 'string');
            if (!empty($this->transaction_id)) {
                $sql = $db->bind_vars($sql, ':transID', substr((string) $this->transaction_id, 0, strpos((string) $this->transaction_id, ' ')), 'string');
            } else {
                $sql = $db->bind_vars($sql, ':transID', 'NULL', 'passthru');
            }
            $sql = $db->bind_vars($sql, ':sentData', print_r($this->reportable_submit_data, true), 'stringIgnoreNull');
            $sql = $db->bind_vars($sql, ':recvData', print_r($response, true), 'string');
            $sql = $db->bind_vars($sql, ':orderTime', $order_time, 'string');
            $sql = $db->bind_vars($sql, ':sessID', $sess_id, 'string');
            $db->Execute($sql);
        }
    }
    /**
    ...
    {
        global $db, $sniffer;
        $field_okay1 = method_exists($sniffer, 'field_type') ? $sniffer->field_type(TABLE_AUTHORIZENET, 'transaction_id', 'varchar(64)', false) : false;
        if ($field_okay1 !== true) {
     ...
     * @since ZC v1.3.8
     */
    public function _do_refund($o_id, $amount = 0): bool
    {
        global $message_stack;
        ...
        if (isset($_POST['refconfirm']) && $_POST['refconfirm'] !== 'on') {
            $message_stack->add_session(MODULE_PAYMENT_AUTHORIZENET_AIM_TEXT_REFUND_CONFIRM_ERROR, 'error');
        ...
        if (isset($_POST['buttonrefund']) && $_POST['buttonrefund'] === MODULE_PAYMENT_AUTHORIZENET_AIM_ENTRY_REFUND_BUTTON_TEXT) {
            ...
            if ($refund_amt == 0) {
                $message_stack->add_session(MODULE_PAYMENT_AUTHORIZENET_AIM_TEXT_INVALID_REFUND_AMOUNT, 'error');
                $proceed_to_refund = false;
            }
        }
        if (isset($_POST['cc_number']) && trim((string) $_POST['cc_number']) === '') {
            $message_stack->add_session(MODULE_PAYMENT_AUTHORIZENET_AIM_TEXT_CC_NUM_REQUIRED_ERROR, 'error');
            $proceed_to_refund = false;
        }
        if (isset($_POST['trans_id']) && trim((string) $_POST['trans_id']) === '') {
            $message_stack->add_session(MODULE_PAYMENT_AUTHORIZENET_AIM_TEXT_TRANS_ID_REQUIRED_ERROR, 'error');
            $proceed_to_refund = false;
        }
        /**
         * Submit refund request to gateway
         */
        if ($proceed_to_refund === true) {
            $submit_data = ['x_type' => 'CREDIT', 'x_card_num' => trim((string) $_POST['cc_number']), 'x_amount' => round($refund_amt, 2), 'x_trans_id' => trim((string) $_POST['trans_id'])];
            unset($response);
            $response = $this->_send_request($submit_data);
            $response_code = $response[0];
            $response_text = $response[3];
            $response_alert = $response_text . ($this->comm_error == '' ? '' : ' Communications Error - Please notify webmaster.');
            $this->reportable_submit_data['Note'] = $refund_note;
            $this->_debug_actions($response);
            if ($response_code != '1') {
                $message_stack->add_session($response_alert, 'error');
            } else {
                ...
                $message_stack->add_session(sprintf(MODULE_PAYMENT_AUTHORIZENET_AIM_TEXT_REFUND_INITIATED, $response[9], $response[6]), 'success');
                return true;
            }
        }
        return false;
     ...
     * @since ZC v1.3.8
     */
    public function _do_capt($o_id, $amt = 0, $currency = 'USD'): bool
    {
        global $message_stack;
        ...
        if (!isset($_POST['captconfirm']) || $_POST['captconfirm'] !== 'on') {
            $message_stack->add_session(MODULE_PAYMENT_AUTHORIZENET_AIM_TEXT_CAPTURE_CONFIRM_ERROR, 'error');
        ...
        if (!isset($_POST['captauthid']) || trim((string) $_POST['captauthid']) === '') {
            $message_stack->add_session(MODULE_PAYMENT_AUTHORIZENET_AIM_TEXT_TRANS_ID_REQUIRED_ERROR, 'error');
            $proceed_to_capture = false;
        }
        /**
         ...
         */
        if ($proceed_to_capture === true) {
            // Populate an array that contains all of the data to be sent to Authorize.net
            $submit_data = ['x_type' => 'PRIOR_AUTH_CAPTURE', 'x_amount' => round($capture_amt, 2), 'x_trans_id' => strip_tags(trim((string) $_POST['captauthid']))];
            $response = $this->_send_request($submit_data);
            $response_code = $response[0];
            $response_text = $response[3];
            $response_alert = $response_text . ($this->comm_error == '' ? '' : ' Communications Error - Please notify webmaster.');
            $this->reportable_submit_data['Note'] = $capture_note;
            $this->_debug_actions($response);
            if ($response_code != '1' || $response[0] == 1 && $response[2] == 311) {
                $message_stack->add_session($response_alert, 'error');
            } else {
                // Success, so save the results
                $comments = 'FUNDS COLLECTED. Auth Code: ' . $response[4] . "\n" . 'Trans ID: ' . $response[6] . "\n" . ' Amount: ' . ($response[9] == 0.0 ? 'Full Amount' : $response[9]) . "\n" . 'Time: ' . date('Y-m-D h:i:s') . "\n" . $capture_note;
                zen_update_orders_history($o_id, $comments, null, $new_order_status, 1);
                $message_stack->add_session(sprintf(MODULE_PAYMENT_AUTHORIZENET_AIM_TEXT_CAPT_INITIATED, $response[9] == 0.0 ? 'Full Amount' : $response[9], $response[6], $response[4]), 'success');
                return true;
            }
        }
        return false;
     ...
     * @since ZC v1.3.8
     */
    public function _do_void($o_id, string $note = ''): bool
    {
        global $message_stack;
        $new_order_status = (int) MODULE_PAYMENT_AUTHORIZENET_AIM_REFUNDED_ORDER_STATUS_ID;
        if ($new_order_status === 0) {
        ...
        $void_note = strip_tags(zen_db_input($_POST['voidnote'] . $note));
        ...
        if (isset($_POST['ordervoid']) && $_POST['ordervoid'] === MODULE_PAYMENT_AUTHORIZENET_AIM_ENTRY_VOID_BUTTON_TEXT) {
            if (isset($_POST['voidconfirm']) && $_POST['voidconfirm'] !== 'on') {
                $message_stack->add_session(MODULE_PAYMENT_AUTHORIZENET_AIM_TEXT_VOID_CONFIRM_ERROR, 'error');
        ...
        $submit_data = ['x_type' => 'VOID', 'x_trans_id' => trim($void_auth_id)];
        /**
         * Submit void request to Gateway
         */
        if ($proceed_to_void === true) {
            $response = $this->_send_request($submit_data);
            $response_code = $response[0];
            $response_text = $response[3];
            $response_alert = $response_text . ($this->comm_error == '' ? '' : ' Communications Error - Please notify webmaster.');
            $this->reportable_submit_data['Note'] = $void_note;
            $this->_debug_actions($response);
            if ($response_code != '1' || $response[0] == 1 && $response[2] == 310) {
                ...
                zen_update_orders_history($o_id, $comments, null, $new_order_status, 1);
                $message_stack->add_session(sprintf(MODULE_PAYMENT_AUTHORIZENET_AIM_TEXT_VOID_INITIATED, $response[6], $response[4]), 'success');
                return true;
            }
        }
        return false;
