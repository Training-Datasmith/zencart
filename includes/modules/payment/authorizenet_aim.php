...
class authorizenet_aim extends base
{
    ...
    public function selection(): array
    {
        ...
        $selection = ['id' => $this->code,
                           ...
                           'fields' => [['title' => MODULE_PAYMENT_AUTHORIZENET_AIM_TEXT_CREDIT_CARD_OWNER,
        ...
        if (MODULE_PAYMENT_AUTHORIZENET_AIM_USE_CVV == 'True') {
            $selection['fields'][] = ['title' => MODULE_PAYMENT_AUTHORIZENET_AIM_TEXT_CVV,
    ...
    public function confirmation(): array
    {
        ...
        return ['fields' => [['title' => MODULE_PAYMENT_AUTHORIZENET_AIM_TEXT_CREDIT_CARD_TYPE,
                                                ...
                                                ['title' => MODULE_PAYMENT_AUTHORIZENET_AIM_TEXT_CREDIT_CARD_NUMBER,
                                                      'field' => substr((string) $this->cc_card_number, 0, 4) . str_repeat('X', (strlen((string) $this->cc_card_number) - 8)) . substr((string) $this->cc_card_number, -4)],
                                                ['title' => MODULE_PAYMENT_AUTHORIZENET_AIM_TEXT_CREDIT_CARD_EXPIRES,
    ...
    public function process_button(): string
    {
        $process_button_string = zen_draw_hidden_field('cc_owner', $_POST['authorizenet_aim_cc_owner']) .
                                 zen_draw_hidden_field('cc_expires', $this->cc_expiry_month . substr((string) $this->cc_expiry_year, -2)) .
     ...
     * @since ZC v1.3.8
     */
    public function _sendRequest($submit_data): array
    {
        global $request_type;
        ...
        if (substr((string) $stringToParse, 0, 1) == $this->encapChar) {
            $stringToParse = substr((string) $stringToParse, 1);
        }
     ...
     * @since ZC v1.3.8
     */
    public function _debugActions(array $response, $order_time = '', $sessID = ''): void
    {
        global $db;
        if ($order_time == '') {
            $order_time = date('F j, Y, g:i a');
        ...
        if (($response[0] != '1' && stristr(MODULE_PAYMENT_AUTHORIZENET_AIM_DEBUGGING, 'Alerts')) || strstr(MODULE_PAYMENT_AUTHORIZENET_AIM_DEBUGGING, 'Email')) {
            zen_mail(STORE_NAME, STORE_OWNER_EMAIL_ADDRESS, 'AuthorizenetAIM Alert ' . $response[7] . ' ' . date('M-d-Y h:i:s') . ' ' . $response[6], $errorMessage, STORE_OWNER, STORE_OWNER_EMAIL_ADDRESS, ['EMAIL_MESSAGE_HTML' => nl2br($errorMessage)], 'debug');
        }
        ...
        if (MODULE_PAYMENT_AUTHORIZENET_AIM_STORE_DATA == 'True' && !empty($response_order_id)) {
            ...
            $sql = $db->bindVars($sql, ':orderID', preg_replace('/[^0-9]/', '', (string) $response_order_id), 'integer');
            ...
            if (!empty($this->transaction_id)) {
                $sql = $db->bindVars($sql, ':transID', substr((string) $this->transaction_id, 0, strpos((string) $this->transaction_id, ' ')), 'string');
            } else {
    ...
    public function _doRefund($oID, $amount = 0): bool
    {
        ...
        if ($proceedToRefund === true) {
            $submit_data = [
                'x_type' => 'CREDIT',
                'x_card_num' => trim((string) $_POST['cc_number']),
                'x_amount' => round($refundAmt, 2),
                'x_trans_id' => trim((string) $_POST['trans_id']),
            ];
            unset($response);
            $response = $this->_sendRequest($submit_data);
            $response_code = $response[0];
    ...
    public function _doCapt($oID, $amt = 0, $currency = 'USD'): bool
    {
        ...
        if ($proceedToCapture === true) {
            ...
            $submit_data = [
                ...
                'x_trans_id' => strip_tags(trim((string) $_POST['captauthid'])),
     ...
     * @since ZC v1.3.8
     */
    public function _doVoid($oID, string $note = ''): bool
    {
        global $messageStack;
