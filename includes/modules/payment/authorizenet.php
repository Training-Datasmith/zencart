...
class authorizenet extends base
{
    ...
    public function selection()
    {
        ...
        if ($this->gateway_mode == 'offsite') {
            ...
            $selection = ['id' => $this->code,
                               'module' => $this->title,
                               'fields' => [['title' => MODULE_PAYMENT_AUTHORIZENET_TEXT_CREDIT_CARD_OWNER,
...
//      'x_color_text' => ''
        ...
        $submit_data_onsite = [
          ...
          'x_exp_date' => $this->cc_expiry_month . substr((string) $this->cc_expiry_year, -2)];
...
//       && $this->authorize['x_description'] == $this->submit_data['x_description']
     ...
     * @since ZC v1.5.6b
     */
    public function hashingMode(): string|false
    {
     ...
     * @since ZC v1.5.6b
     */
    public function getHmacFingerprintFields($amount, string $sequence, string $currency = ''): array
    {
        $timestamp = time();
     ...
     * @since ZC v1.5.6b
     */
    public function getHashForResponseData(array $data): string
    {
        $dataArray = [
            $data['x_trans_id'],
            $data['x_test_request'],
     ...
     * @since ZC v1.3.8
     */
    public function _debugActions(array $response, $mode, $order_time = '', $sessID = ''): void
    {
        ...
        if ((isset($response['x_response_code']) && $response['x_response_code'] != '1' && stristr(MODULE_PAYMENT_AUTHORIZENET_DEBUGGING, 'Alerts')) || stristr(MODULE_PAYMENT_AUTHORIZENET_DEBUGGING, 'Email')) {
            zen_mail(STORE_NAME, STORE_OWNER_EMAIL_ADDRESS, 'Authorizenet-SIM Alert ' . $response['x_invoice_num'] . ' ' . date('M-d-Y h:i:s') . ' ' . $response['x_trans_id'], $errorMessage, STORE_OWNER, STORE_OWNER_EMAIL_ADDRESS, ['EMAIL_MESSAGE_HTML' => nl2br($errorMessage)], 'debug');
        }
        ...
        if (MODULE_PAYMENT_AUTHORIZENET_STORE_DATA == 'True' && $mode == 'Response-Data') {
            ...
            $sql = $db->bindVars($sql, ':orderID', preg_replace('/[^0-9]/', '', (string) $insert_id), 'integer');
