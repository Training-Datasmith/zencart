...
class authorizenet extends base
{
    ...
    /**
     * @var 'https://secure2.authorize.net/gateway/transact.dll'|'https://test.authorize.net/gateway/transact.dll'|'https://developer.authorize.net/param_dump.asp'|'https://certification.authorize.net/gateway/transact.dll'
     */
    ...
    {
        global $order;
        $this->title = MODULE_PAYMENT_AUTHORIZENET_TEXT_CATALOG_TITLE;
        // Payment module title in Catalog
        $this->description = MODULE_PAYMENT_AUTHORIZENET_TEXT_DESCRIPTION;
        if (IS_ADMIN_FLAG === true) {
            $this->title = MODULE_PAYMENT_AUTHORIZENET_TEXT_ADMIN_TITLE;
            // Payment module title in Admin
        }
        $this->sort_order = defined('MODULE_PAYMENT_AUTHORIZENET_SORT_ORDER') ? MODULE_PAYMENT_AUTHORIZENET_SORT_ORDER : null;
        $this->enabled = defined('MODULE_PAYMENT_AUTHORIZENET_STATUS') && MODULE_PAYMENT_AUTHORIZENET_STATUS == 'True' && $this->hashing_mode() !== false;
        if (null === $this->sort_order) {
            return;
        }
        if (IS_ADMIN_FLAG === true) {
            if (MODULE_PAYMENT_AUTHORIZENET_STATUS == 'True' && (MODULE_PAYMENT_AUTHORIZENET_LOGIN == 'testing' || MODULE_PAYMENT_AUTHORIZENET_TXNKEY == 'Test' || $this->hashing_mode() === false)) {
        ...
        if (defined('AUTHORIZNET_DEVELOPER_MODE') && AUTHORIZENET_DEVELOPER_MODE == 'echo' || MODULE_PAYMENT_AUTHORIZENET_DEBUGGING == 'echo') {
            $this->form_action_url = 'https://developer.authorize.net/param_dump.asp';
        }
        if (defined('AUTHORIZNET_DEVELOPER_MODE') && AUTHORIZENET_DEVELOPER_MODE == 'certify') {
    ...
    public function selection()
    {
        ...
        if ($this->gateway_mode == 'offsite') {
            $selection = ['id' => $this->code, 'module' => $this->title];
        } else {
            $selection = ['id' => $this->code, 'module' => $this->title, 'fields' => [['title' => MODULE_PAYMENT_AUTHORIZENET_TEXT_CREDIT_CARD_OWNER, 'field' => zen_draw_input_field('authorizenet_cc_owner', $order->billing['firstname'] . ' ' . $order->billing['lastname'], 'id="' . $this->code . '-cc-owner"' . $on_focus . ' autocomplete="off"'), 'tag' => $this->code . '-cc-owner'], ['title' => MODULE_PAYMENT_AUTHORIZENET_TEXT_CREDIT_CARD_NUMBER, 'field' => zen_draw_input_field('authorizenet_cc_number', '', 'id="' . $this->code . '-cc-number"' . $on_focus . ' autocomplete="off"'), 'tag' => $this->code . '-cc-number'], ['title' => MODULE_PAYMENT_AUTHORIZENET_TEXT_CREDIT_CARD_EXPIRES, 'field' => zen_draw_pull_down_menu('authorizenet_cc_expires_month', $expires_month, $zc_date->output('%m'), 'id="' . $this->code . '-cc-expires-month"' . $on_focus) . '&nbsp;' . zen_draw_pull_down_menu('authorizenet_cc_expires_year', $expires_year, '', 'id="' . $this->code . '-cc-expires-year"' . $on_focus), 'tag' => $this->code . '-cc-expires-month']]];
            if (MODULE_PAYMENT_AUTHORIZENET_USE_CVV == 'True') {
                $selection['fields'][] = ['title' => MODULE_PAYMENT_AUTHORIZENET_TEXT_CVV, 'field' => zen_draw_input_field('authorizenet_cc_cvv', '', 'size="4" maxlength="4"' . ' id="' . $this->code . '-cc-cvv"' . $on_focus . ' autocomplete="off"') . ' ' . '<a href="javascript:popupWindow(\'' . zen_href_link(FILENAME_POPUP_CVV_HELP) . '\')">' . MODULE_PAYMENT_AUTHORIZENET_TEXT_POPUP_CVV_LINK . '</a>', 'tag' => $this->code . '-cc-cvv'];
            }
        }
        return $selection;
    }
    ...
    public function confirmation(): array
    {
        global $zc_date;
        if (isset($_POST['authorizenet_cc_number'])) {
            $confirmation = ['title' => $this->title . ': ' . $this->cc_card_type, 'fields' => [['title' => MODULE_PAYMENT_AUTHORIZENET_TEXT_CREDIT_CARD_OWNER, 'field' => $_POST['authorizenet_cc_owner']], ['title' => MODULE_PAYMENT_AUTHORIZENET_TEXT_CREDIT_CARD_NUMBER, 'field' => substr((string) $this->cc_card_number, 0, 4) . str_repeat('X', strlen((string) $this->cc_card_number) - 8) . substr((string) $this->cc_card_number, -4)], ['title' => MODULE_PAYMENT_AUTHORIZENET_TEXT_CREDIT_CARD_EXPIRES, 'field' => $zc_date->output('%B, %Y', mktime(0, 0, 0, $_POST['authorizenet_cc_expires_month'], 1, '20' . $_POST['authorizenet_cc_expires_year']))]]];
        } else {
            $confirmation = [];
    ...
    public function process_button(): string
    {
        ...
        $submit_data_security = $this->get_hmac_fingerprint_fields($submit_data_core['x_amount'], $sequence, $submit_data_core['x_currency_code']);
        $submit_data_offline = ['x_show_form' => 'PAYMENT_FORM', 'x_receipt_link_method' => 'POST', 'x_receipt_link_text' => 'Click here to complete your order.', 'x_receipt_link_url' => zen_href_link(FILENAME_CHECKOUT_PROCESS, '', 'SSL', false)];
        ...
        $submit_data_onsite = ['x_card_num' => $this->cc_card_number, 'x_exp_date' => $this->cc_expiry_month . substr((string) $this->cc_expiry_year, -2)];
        if (MODULE_PAYMENT_AUTHORIZENET_USE_CVV == 'True') {
        ...
        $this->_debug_actions($this->reportable_submit_data, 'Submit-Data', '', zen_session_id());
    ...
    public function before_process(): void
    {
        ...
        $this->authorize['HashValidationValue'] = $this->get_hash_for_response_data($this->authorize);
        ...
        $this->_debug_actions($this->authorize, 'Response-Data', '', zen_session_id());
        ...
        if ($this->authorize['x_response_code'] == '1' && $this->authorize['HashMatchStatus'] == 'PASS') {
        ...
        if ($this->authorize['x_response_code'] == '2') {
            $message_stack->add_session('checkout_payment', $this->authorize['x_response_reason_text'] . MODULE_PAYMENT_AUTHORIZENET_TEXT_DECLINED_MESSAGE, 'error');
            zen_redirect(zen_href_link(FILENAME_CHECKOUT_PAYMENT, '', 'SSL', true, false));
        }
        // Code 3 or anything else is an error
        $message_stack->add_session('checkout_payment', MODULE_PAYMENT_AUTHORIZENET_TEXT_ERROR_MESSAGE, 'error');
        zen_redirect(zen_href_link(FILENAME_CHECKOUT_PAYMENT, '', 'SSL', true, false));
    }
    /**
    ...
    public function install()
    {
        ...
        $db->Execute('INSERT INTO ' . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, date_added) VALUES ('Transaction Method', 'MODULE_PAYMENT_AUTHORIZENET_METHOD', 'Credit Card', 'Transaction method used for processing orders', '6', '0', 'zen_cfg_select_option(array(\\'Credit Card\\'), ', now())");
        //, \'eCheck\'
     ...
     * @since ZC v1.5.6b
     */
    public function get_hmac_fingerprint_fields($amount, string $sequence, string $currency = ''): array
    {
        $timestamp = time();
        $key = hex2bin(strtoupper(MODULE_PAYMENT_AUTHORIZENET_SECURITYKEY));
        $hash_string = hash_hmac('sha512', MODULE_PAYMENT_AUTHORIZENET_LOGIN . '^' . $sequence . '^' . $timestamp . '^' . $amount . '^' . $currency, $key);
        return ['x_fp_sequence' => $sequence, 'x_fp_timestamp' => $timestamp, 'x_fp_hash' => $hash_string];
    }
    /**
     ...
     * @since ZC v1.5.6b
     */
    public function get_hash_for_response_data(array $data): string
    {
        $data_array = [$data['x_trans_id'], $data['x_test_request'], $data['x_response_code'], $data['x_auth_code'], $data['x_cvv2_resp_code'], $data['x_cavv_response'], $data['x_avs_code'], $data['x_method'], $data['x_account_number'], $data['x_amount'], $data['x_company'], $data['x_first_name'], $data['x_last_name'], $data['x_address'], $data['x_city'], $data['x_state'], $data['x_zip'], $data['x_country'], $data['x_phone'], $data['x_fax'], $data['x_email'], $data['x_ship_to_company'], $data['x_ship_to_first_name'], $data['x_ship_to_last_name'], $data['x_ship_to_address'], $data['x_ship_to_city'], $data['x_ship_to_state'], $data['x_ship_to_zip'], $data['x_ship_to_country'], $data['x_invoice_num']];
        $string = '^' . implode('^', $data_array) . '^';
     ...
     * @since ZC v1.3.8
     */
    public function _debug_actions(array $response, $mode, $order_time = '', $sess_id = ''): void
    {
        global $db, $message_stack, $insert_id;
        if ($order_time == '') {
        ...
        if (isset($response['x_response_code']) && $response['x_response_code'] != '1' && stristr(MODULE_PAYMENT_AUTHORIZENET_DEBUGGING, 'Alerts') || stristr(MODULE_PAYMENT_AUTHORIZENET_DEBUGGING, 'Email')) {
            zen_mail(STORE_NAME, STORE_OWNER_EMAIL_ADDRESS, 'Authorizenet-SIM Alert ' . $response['x_invoice_num'] . ' ' . date('M-d-Y h:i:s') . ' ' . $response['x_trans_id'], $error_message, STORE_OWNER, STORE_OWNER_EMAIL_ADDRESS, ['EMAIL_MESSAGE_HTML' => nl2br($error_message)], 'debug');
        }
        // DATABASE SECTION
        ...
        if (MODULE_PAYMENT_AUTHORIZENET_STORE_DATA == 'True' && $mode == 'Response-Data') {
            ...
            $sql = $db->bind_vars($sql, ':orderID', preg_replace('/[^0-9]/', '', (string) $insert_id), 'integer');
            $sql = $db->bind_vars($sql, ':respCode', $response['x_response_code'], 'integer');
            $sql = $db->bind_vars($sql, ':respText', $db_response_text, 'string');
            $sql = $db->bind_vars($sql, ':authType', $response['x_type'], 'string');
            if (isset($response['x_trans_id']) && trim($response['x_trans_id']) != '') {
                $sql = $db->bind_vars($sql, ':transID', $response['x_trans_id'], 'string');
            } else {
                $sql = $db->bind_vars($sql, ':transID', 'NULL', 'passthru');
            }
            $sql = $db->bind_vars($sql, ':sentData', print_r($this->reportable_submit_data, true), 'string');
    ...
    {
        global $db, $sniffer;
        $field_okay1 = method_exists($sniffer, 'field_type') ? $sniffer->field_type(TABLE_AUTHORIZENET, 'transaction_id', 'varchar(64)', true) : -1;
        if ($field_okay1 !== true) {
