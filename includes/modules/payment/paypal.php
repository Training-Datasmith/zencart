...
class paypal extends base
{
    ...
    public function __construct()
    {
        ...
        if (IS_ADMIN_FLAG === true) {
            ...
            if (IS_ADMIN_FLAG === true && defined('MODULE_PAYMENT_PAYPAL_TESTING') && MODULE_PAYMENT_PAYPAL_TESTING == 'Test') {
                $this->title .= '<span class="alert"> (dev/test mode active)</span>';
            }
        } else {
            $this->title = MODULE_PAYMENT_PAYPAL_TEXT_CATALOG_TITLE;
    ...
    {
        return ['id' => $this->code, 'module' => MODULE_PAYMENT_PAYPAL_TEXT_CATALOG_LOGO, 'icon' => MODULE_PAYMENT_PAYPAL_TEXT_CATALOG_LOGO];
    }
    /**
    ...
    public function process_button(): string
    {
        ...
        $telephone = preg_replace('/\D/', '', (string) $order->customer['telephone']);
        if ($telephone != '') {
            $options_phone['H_PhoneNumber'] = $telephone;
            if (in_array($order->customer['country']['iso_code_2'], ['US', 'CA'])) {
                $options_phone['night_phone_a'] = substr($telephone, 0, 3);
                $options_phone['night_phone_b'] = substr($telephone, 3, 3);
                $options_phone['night_phone_c'] = substr($telephone, 6, 4);
                $options_phone['day_phone_a'] = substr($telephone, 0, 3);
                $options_phone['day_phone_b'] = substr($telephone, 3, 3);
                $options_phone['day_phone_c'] = substr($telephone, 6, 4);
            } else {
                $options_phone['night_phone_b'] = $telephone;
                $options_phone['day_phone_b'] = $telephone;
            }
        }
        $options_core = [
            'lc' => $this->get_language_code(),
        ...
        if (sizeof($options_line_items) > 0) {
            ...
            unset($options_line_items['creditsExist']);
        }
        $options_aggregate = [
            'cmd' => '_ext-enter',
        ...
        $options_aggregate['tax'] = '0.00';
        $options_aggregate['tax_cart'] = '0.00';
        $options_trans = ['upload' => (int) (sizeof($order->products) > 0), 'currency_code' => $my_currency];
        // if line-item info is invalid, use aggregate:
        if (sizeof($options_line_items) > 0) {
            $options_aggregate = $options_line_items;
        }
        if (defined('MODULE_PAYMENT_PAYPAL_LOGO_IMAGE')) {
            $options_core['cpp_logo_image'] = urlencode(MODULE_PAYMENT_LOGO_IMAGE);
        }
        if (defined('MODULE_PAYMENT_PAYPAL_CART_BORDER_COLOR')) {
            $options_core['cpp_cart_border_color'] = MODULE_PAYMENT_PAYPAL_CART_BORDER_COLOR;
        }
        // prepare submission
        $options = array_merge($options_core, $options_cust, $options_phone, $options_ship, $options_trans, $options_aggregate);
        ...
        foreach ($options as $name => $value) {
            ...
            if (preg_match('/[^a-zA-Z_0-9]/', (string) $name)) {
            ...
            $button_array[] = zen_draw_hidden_field($name, $value);
        }
        $process_button_string = "\n" . implode("\n", $button_array) . "\n";
        $_SESSION['paypal_transaction_info'] = [$this->transaction_amount, $this->transaction_currency];
    ...
    public function get_language_code()
    {
        ...
        $shipping_iso = zen_get_countries((int) $order->delivery['country']['id'], true);
        $user_locale_info[] = strtoupper((string) $shipping_iso['countries_iso_code_2']);
        $billing_iso = zen_get_countries((int) $order->billing['country']['id'], true);
        $user_locale_info[] = strtoupper((string) $billing_iso['countries_iso_code_2']);
        $cust_iso = zen_get_countries((int) $order->customer['country']['id'], true);
        $user_locale_info[] = strtoupper((string) $cust_iso['countries_iso_code_2']);
        $store_iso = zen_get_countries((int) STORE_COUNTRY, true);
        ...
        foreach ($user_locale_info as $val) {
            if (in_array(strtoupper((string) $val), $to_match)) {
                if (strtoupper((string) $val) == 'EN' && isset($locales) && $locales[0] == 'en_GB') {
                    $val = 'GB';
                }
    ...
    public function before_process()
    {
        ...
        if (isset($_GET['referer']) && $_GET['referer'] == 'paypal') {
            ...
            if (defined('MODULE_PAYMENT_PAYPAL_PDTTOKEN') && MODULE_PAYMENT_PAYPAL_PDTTOKEN != '' && isset($_GET['tx']) && $_GET['tx'] != '') {
                $pdt_status = $this->_get_pd_tresults($this->transaction_amount, $this->transaction_currency, $_GET['tx']);
            } else {
                $pdt_status = false;
            }
            if ($pdt_status == false) {
     ...
     * @since ZC v1.2.2d
     */
    public function check_referrer($zf_domain): bool
    {
        return true;
    }
    /**
    ...
    public function admin_notification($zf_order_id): string
    {
        ...
        $ipn = $db->Execute($sql);
        if ($ipn->record_count() > 0 && file_exists(DIR_FS_CATALOG . DIR_WS_MODULES . 'payment/paypal/paypal_admin_notification.php')) {
    ...
    public function after_process()
    {
        ...
        unset($_SESSION['paypal_transaction_PDT_passed']);
        $comments = 'PayPal status: ' . $this->pdt_data['payment_status'] . ' ' . ' @ ' . $this->pdt_data['payment_date'] . "\n" . ' Trans ID:' . $this->pdt_data['txn_id'] . "\n" . ' Amount: ' . $this->pdt_data['mc_gross'] . ' ' . $this->pdt_data['mc_currency'] . '.';
        ...
        $sql_data_array = [
            'order_id' => $insert_id,
            'txn_type' => $this->pdt_data['txn_type'],
            'module_name' => $this->code . ' ' . $this->code_version,
            'module_mode' => 'PDT',
            'reason_code' => $this->pdt_data['reasoncode'],
            'payment_type' => $this->pdt_data['payment_type'],
            'payment_status' => $this->pdt_data['payment_status'],
            'pending_reason' => $this->pdt_data['pendingreason'],
            'invoice' => $this->pdt_data['invoice'],
            'first_name' => $this->pdt_data['first_name'],
            'last_name' => $this->pdt_data['last_name'],
            ...
            'address_country' => $this->pdt_data['residence_country'],
            // $order->billing['country']
            'address_status' => $this->pdt_data['address_status'],
            'payer_email' => $this->pdt_data['payer_email'],
            'payer_id' => $this->pdt_data['payer_id'],
            'payer_status' => $this->pdt_data['payer_status'],
            'payment_date' => datetime_to_sql_format($this->pdt_data['payment_date']),
            'business' => $this->pdt_data['business'],
            'receiver_email' => $this->pdt_data['receiver_email'],
            'receiver_id' => $this->pdt_data['receiver_id'],
            'txn_id' => $this->pdt_data['txn_id'],
            'parent_txn_id' => $this->pdt_data['parent_txn_id'],
            'num_cart_items' => (float) $this->pdt_data['num_cart_items'],
            'mc_gross' => (float) $this->pdt_data['mc_gross'],
            'mc_fee' => (float) $this->pdt_data['mc_fee'],
            'mc_currency' => $this->pdt_data['mc_currency'],
            'settle_amount' => (float) $this->pdt_data['settle_amount'],
            'settle_currency' => $this->pdt_data['settle_currency'],
            'exchange_rate' => $this->pdt_data['exchange_rate'] > 0 ? $this->pdt_data['exchange_rate'] : 1.0,
            'notify_version' => (float) $this->pdt_data['notify_version'],
            'verify_sign' => $this->pdt_data['verify_sign'],
            'date_added' => 'now()',
    ...
    public function check()
    {
        ...
        if (defined('MODULE_PAYMENT_PAYPAL_HANDLER') && !in_array(MODULE_PAYMENT_PAYPAL_HANDLER, ['live', 'sandbox'])) {
            $val = stristr((string) MODULE_PAYMENT_PAYPAL_HANDLER, 'sand') ? 'sandbox' : 'live';
    ...
    public function install()
    {
        ...
        if (defined('MODULE_PAYMENT_PAYPALWPP_STATUS')) {
            $message_stack->add_session('NOTE: PayPal Express Checkout module already installed. You don\'t need Standard if you have Express installed.', 'error');
    ...
    {
        $keys_list = ['MODULE_PAYMENT_PAYPAL_STATUS', 'MODULE_PAYMENT_PAYPAL_BUSINESS_ID', 'MODULE_PAYMENT_PAYPAL_PDTTOKEN', 'MODULE_PAYMENT_PAYPAL_CURRENCY', 'MODULE_PAYMENT_PAYPAL_ZONE', 'MODULE_PAYMENT_PAYPAL_PROCESSING_STATUS_ID', 'MODULE_PAYMENT_PAYPAL_ORDER_STATUS_ID', 'MODULE_PAYMENT_PAYPAL_REFUND_ORDER_STATUS_ID', 'MODULE_PAYMENT_PAYPAL_SORT_ORDER', 'MODULE_PAYMENT_PAYPAL_DETAILED_CART', 'MODULE_PAYMENT_PAYPAL_ADDRESS_OVERRIDE', 'MODULE_PAYMENT_PAYPAL_ADDRESS_REQUIRED', 'MODULE_PAYMENT_PAYPAL_PAGE_STYLE', 'MODULE_PAYMENT_PAYPAL_HANDLER', 'MODULE_PAYMENT_PAYPAL_IPN_DEBUG'];
        // Paypal testing/debug options go here:
        if (IS_ADMIN_FLAG === true) {
            if (isset($_GET['debug']) && $_GET['debug'] == 'on') {
                $keys_list[] = 'MODULE_PAYMENT_PAYPAL_DEBUG_EMAIL_ADDRESS';
                /* this defaults to store-owner-email-address */
            }
        }
        return $keys_list;
    }
     ...
     * @since ZC v1.3.7.1
     */
    public function _get_pd_tresults($order_amount, $my_currency, $pdt_tx)
    {
        global $db;
        $ipn_data = ipn_postback('PDT', $pdt_tx);
        $respdata = $ipn_data['info'];
        // parse the data
        $lines = explode("\n", (string) $respdata);
        $this->pdt_data = [];
        for ($i = 1; $i < count($lines); $i++) {
            ...
            $this->pdt_data[urldecode($key)] = urldecode($val);
        }
        if ($this->pdt_data['txn_id'] == '' || $this->pdt_data['payment_status'] == '') {
            ipn_debug_email('PDT Returned INVALID Data. Must wait for IPN to process instead. ' . "\n" . print_r($this->pdt_data, true));
            return false;
        }
        ipn_debug_email('PDT Returned Data ' . print_r($this->pdt_data, true));
        $_POST['mc_gross'] = $this->pdt_data['mc_gross'];
        $_POST['mc_currency'] = $this->pdt_data['mc_currency'];
        $_POST['business'] = $this->pdt_data['business'];
        $_POST['receiver_email'] = $this->pdt_data['receiver_email'];
        $pd_tstatus = ipn_validate_transaction($respdata, $this->pdt_data, 'PDT') && valid_payment($order_amount, $my_currency, 'PDT') && $this->pdt_data['payment_status'] == 'Completed';
        if ($this->pdt_data['payment_status'] != '' && $this->pdt_data['payment_status'] != 'Completed') {
            ipn_debug_email('PDT WARNING :: Order not marked as "Completed".  Check for Pending reasons or wait for IPN to complete.' . "\n" . '[payment_status] => ' . $this->pdt_data['payment_status'] . "\n" . '[pending_reason] => ' . $this->pdt_data['pending_reason']);
        }
        ...
        $sql = $db->bind_vars($sql, ':transactionID', $this->pdt_data['txn_id'], 'string');
        ...
        if ($ipn_id->record_count() != 0) {
            ...
            $pdt_txn_is_unique = false;
        } else {
            $pdt_txn_is_unique = true;
        }
        $pd_tstatus = $pdt_txn_is_unique && $pd_tstatus;
        if ($pd_tstatus == true) {
            $this->transaction_id = $this->pdt_data['txn_id'];
        }
        return $pd_tstatus;
    }
    /**
    ...
    {
        return ['body' => '<a href="https://docs.zen-cart.com/user/payment/paypal/" target="_blank" rel="noreferrer noopener">' . TEXT_DOCS_HELP . '</a><br>' . MODULES_PAYMENT_PAYPALSTD_NOT_RECOMMENDED . '<br>'];
    }
}