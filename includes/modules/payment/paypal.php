...
//                   'lc' => $order->customer['country']['iso_code_2'],
        ...
        if ($order->customer['country']['iso_code_2'] == 'JP') {
            $optionsCust['zip'] = substr((string) $order->customer['postcode'], 0, 3) . '-' . substr((string) $order->customer['postcode'], 3);
        }
        if (MODULE_PAYMENT_PAYPAL_ADDRESS_REQUIRED == 2) {
            ...
            if ($order->delivery['country']['iso_code_2'] == 'JP') {
                $optionsCust['zip'] = substr((string) $order->delivery['postcode'], 0, 3) . '-' . substr((string) $order->delivery['postcode'], 3);
            }
        }
...
//                   'paypal_order_id' => $paypal_order_id,
    ...
    public function getLanguageCode()
    {
        ...
        $user_locale_info[] = strtoupper((string) $shippingISO['countries_iso_code_2']);
    ...
    public function after_process()
    {
        ...
        $sql_data_array = ['order_id' => $insert_id,
