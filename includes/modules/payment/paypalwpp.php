...
class paypalwpp extends base
{
        ...
        if (IS_ADMIN_FLAG === true) {
            ...
            if ($this->enabled) {
                if ((MODULE_PAYMENT_PAYPALWPP_MODULE_MODE == 'PayPal' && (MODULE_PAYMENT_PAYPALWPP_APISIGNATURE == '' || MODULE_PAYMENT_PAYPALWPP_APIUSERNAME == '' || MODULE_PAYMENT_PAYPALWPP_APIPASSWORD == ''))
                  || (str_starts_with(MODULE_PAYMENT_PAYPALWPP_MODULE_MODE, 'Payflow') && (MODULE_PAYMENT_PAYPALWPP_PFPARTNER == '' || MODULE_PAYMENT_PAYPALWPP_PFVENDOR == '' || MODULE_PAYMENT_PAYPALWPP_PFUSER == '' || MODULE_PAYMENT_PAYPALWPP_PFPASSWORD == ''))
                ) {
    ...
    public function before_process(): void
    {
        ...
        $this->_errorHandler($response, 'DoExpressCheckoutPayment');
        ...
        //
        if ($response[$this->infoPrefix . 'PAYMENTTYPE'] != '') {
            $this->payment_type .=  ' (' . urldecode((string) $response[$this->infoPrefix . 'PAYMENTTYPE']) . ')';
        }
        ...
        if (empty($response[$this->infoPrefix . 'PENDINGREASON']) ||
            ...
            if ($response[$this->infoPrefix . 'PAYMENTSTATUS'] == 'Pending') {
                ...
                if ($response['L_ERRORCODE0'] == 11610 && $response[$this->infoPrefix . 'PENDINGREASON'] == '') {
                    $response[$this->infoPrefix . 'PENDINGREASON'] = 'Pending FMF Review by Storeowner';
                }
            }
        ...
        $this->payment_time = urldecode((string) $response[$this->infoPrefix . 'ORDERTIME']);
        $this->feeamt = empty($response[$this->infoPrefix . 'FEEAMT']) ? 0 : urldecode((string) $response[$this->infoPrefix . 'FEEAMT']);
    ...
    public function after_process(): void
    {
        ...
        $paypal_order = ['order_id' => $insert_id,
                              ...
                              'payment_date' => convertToLocalTimeZone(trim((string) preg_replace('/[^0-9-:]/', ' ', (string) $this->payment_time))),
                              'business' => '',
                              'receiver_email' => (str_starts_with(MODULE_PAYMENT_PAYPALWPP_MODULE_MODE, 'Payflow') ? MODULE_PAYMENT_PAYPALWPP_PFVENDOR : str_replace('_api1', '', MODULE_PAYMENT_PAYPALWPP_APIUSERNAME)),
                              'receiver_id' => '',
                              'txn_id' => $this->transaction_id,
                              'parent_txn_id' => '',
                              'num_cart_items' => (float)$this->numitems,
                              'mc_gross' => (float)$this->amt,
                              'mc_fee' => (float)urldecode((string) $this->feeamt),
     ...
     * @since ZC v1.3.7
     */
    public function zcLog(string $stage, string $message): void
    {
        static $tokenHash;
        if ($tokenHash == '') {
     ...
     * @since ZC v1.3.7
     */
    public function _doDebug($subject = 'PayPal debug data', $data = '', $useSession = true): void
    {
    ...
    public function paypal_init()
    {
        ...
        if (str_starts_with(MODULE_PAYMENT_PAYPALWPP_MODULE_MODE, 'Payflow')) {
            $doPayPal = new paypal_curl(['mode' => 'payflow',
            ...
            $doPayPal->_endpoints = ['live'    => 'https://payflowpro.paypal.com/transaction',
                                          'sandbox' => 'https://pilot-payflowpro.paypal.com/transaction'];

            $this->requestPrefix = '';
            $this->infoPrefix = '';
        } else {
            $doPayPal = new paypal_curl(['mode' => 'nvp',
                                              ...
                                              'version' => '124.0',
                                              'server' => MODULE_PAYMENT_PAYPALWPP_SERVER]);
            $doPayPal->_endpoints = ['live'    => 'https://api-3t.paypal.com/nvp',
     ...
     * @since ZC v1.3.7
     */
    public function getPayPalLoginServer(): string
    {
    ...
    public function _doRefund($oID, $amount = 'Full', $note = '')
    {
        ...
        if ($proceedToRefund) {
            ...
            if (!$error) {
                ...
                $messageStack->add_session(sprintf(MODULE_PAYMENT_PAYPALWPP_TEXT_REFUND_INITIATED, urldecode((string) $response['GROSSREFUNDAMT']), urldecode((string) $response['REFUNDTRANSACTIONID']). ($response['PNREF'] ?? '')), 'success');
                return true;
            }
        }
    }
     ...
     * @since ZC v1.3.7
     */
    public function _doAuth($oID, $amt, string $currency = 'USD')
    {
        global $db, $doPayPal, $messageStack;
        $doPayPal = $this->paypal_init();
        $authAmt = $amt;
        ...
        if ($proceedToAuth) {
            ...
            if (!$error) {
                ...
                $messageStack->add_session(sprintf(MODULE_PAYMENT_PAYPALWPP_TEXT_AUTH_INITIATED, urldecode((string) $response['AMT'])), 'success');
                return true;
            }
        }
    }
     ...
     * @since ZC v1.3.7
     */
    public function _doCapt($oID, $captureType = 'Complete', $amt = 0, string $currency = 'USD', $note = '')
    {
        global $db, $doPayPal, $messageStack;
        $doPayPal = $this->paypal_init();
        ...
        if ($proceedToCapture) {
            ...
            if (!$error) {
                ...
                $messageStack->add_session(sprintf(MODULE_PAYMENT_PAYPALWPP_TEXT_CAPT_INITIATED, urldecode((string) $response['AMT']), urldecode(!empty($response['AUTHCODE']) ? $response['AUTHCODE'] : $response['CORRELATIONID']). ($response['PNREF'] ?? '')), 'success');
                return true;
            }
        }
    }
    ...
    public function _doVoid($oID, $note = '')
    {
        ...
        if ($proceedToVoid) {
            ...
            if (!$error) {
                ...
                $messageStack->add_session(sprintf(MODULE_PAYMENT_PAYPALWPP_TEXT_VOID_INITIATED, urldecode((string) $response['AUTHORIZATIONID']) . ($response['PNREF'] ?? '')), 'success');
                return true;
            }
        }
    }
    ...
    public function getLanguageCode($mode = 'ec')
    {
        ...
        if (isset($order->billing['country']['id'])) {
            ...
            $user_locale_info[] = strtoupper((string) $billingISO['countries_iso_code_2']);
        }

        if (isset($order->customer['country']['id'])) {
            $custISO = zen_get_countries_with_iso_codes($order->customer['country']['id']);
            $user_locale_info[] = strtoupper((string) $custISO['countries_iso_code_2']);
        }

        $storeISO = zen_get_countries_with_iso_codes(STORE_COUNTRY);
        $user_locale_info[] = strtoupper((string) $storeISO['countries_iso_code_2']);
     ...
     * @since ZC v1.3.7
     */
    public function calc_order_amount($amount, $paypalCurrency, $applyFormatting = false): float|int
    {
        global $currencies;
     ...
     * @since ZC v1.3.7
     */
    public function setStateAndCountry(array &$info): void
    {
        global $db, $messageStack;
     ...
     * @return mixed[]
     */
    public function getLineItemDetails($restrictedCurrency): array
    {
        ...
        for ($i = 0, $n = count($order->products), $k = -1; $i < $n; $i++) {
            ...
            if ($order->products[$i]['onetime_charges'] != 0) {
                $k++;
                $optionsLI["L_PAYMENTREQUEST_0_NAME$k"]   = MODULES_PAYMENT_PAYPALWPP_LINEITEM_TEXT_ONETIME_CHARGES_PREFIX . substr(htmlentities((string) $order->products[$i]['name'], ENT_QUOTES, 'UTF-8'), 0, 120);
        ...
        if (!defined('PAYPALWPP_SKIP_LINE_ITEM_DETAIL_FORMATTING') || PAYPALWPP_SKIP_LINE_ITEM_DETAIL_FORMATTING != 'true' || in_array($order->info['currency'], ['JPY', 'NOK', 'HUF', 'TWD'])) {
            ...
            if (is_array($optionsLI)) {
                foreach ($optionsLI as $key => $value) {
                    if (str_ends_with((string) $key, 'TAXAMT') && ($optionsLI[$key] == '' || $optionsLI[$key] == 0)) {
                        unset($optionsLI[$key]);
                    } else {
                        if (strstr((string) $key, 'AMT')) {
                            $optionsLI[$key] = round($value, ((int)$currencies->get_decimal_places($restrictedCurrency) == 0 ? 0 : 2));
                        }
                    }
                }
            }
    ...
    public function ec_step2(): void
    {
        ...
        $step2_payerinfo = [
            ...
            'payer_email' => urldecode((string) $response['EMAIL']),
            'payer_salutation' => '',
            'payer_gender' => '',
            'payer_firstname' => urldecode((string) $response['FIRSTNAME']),
            'payer_lastname'  => urldecode((string) $response['LASTNAME']),
            'payer_business'  => urldecode((string) $response['BUSINESS']),
            'payer_status'    => $response['PAYERSTATUS'],
            'ship_country_code' => urldecode((string) $response[$this->requestPrefix . 'SHIPTOCOUNTRYCODE']),
            'ship_address_status' => (empty($response[$this->requestPrefix . 'ADDRESSSTATUS'])) ? 'NONE' : urldecode((string) $response[$this->requestPrefix . 'ADDRESSSTATUS']),
            ...
            'order_comment' => (isset($response['NOTE'], $response[$this->requestPrefix . 'NOTETEXT']) ? urldecode((string) $response['NOTE']) . ' ' . urldecode((string) $response[$this->requestPrefix . 'NOTETEXT']) : ''),
        ];
        ...
        $step2_shipto = [
            'ship_name' => urldecode((string) $response[$this->requestPrefix . 'SHIPTONAME']),
            'ship_street_1' => urldecode((string) $response[$this->requestPrefix . 'SHIPTOSTREET']),
            'ship_street_2' => urldecode((string) $response[$this->requestPrefix . 'SHIPTOSTREET2']),
            'ship_city' => urldecode((string) $response[$this->requestPrefix . 'SHIPTOCITY']),
            'ship_state' => (($response[$this->requestPrefix . 'SHIPTOSTATE'] ?? '') !== '') ? urldecode((string) $response[$this->requestPrefix . 'SHIPTOSTATE']) : urldecode((string) $response[$this->requestPrefix . 'SHIPTOCITY']),
            'ship_postal_code' => urldecode((string) $response[$this->requestPrefix . 'SHIPTOZIP']),
            'ship_country_code' => urldecode((string) $response[$this->requestPrefix . 'SHIPTOCOUNTRYCODE']),
            'ship_country_name' => urldecode((string) ($response['SHIPTOCOUNTRY'] ?? $response['SHIPTOCOUNTRYNAME'] ?? '')),
        ];
        ...
        if ($order->info['total'] < 0.01 && urldecode((string) $response[$this->requestPrefix . 'AMT']) > 0) {
            $order->info['total'] = urldecode((string) $response[$this->requestPrefix . 'AMT']);
        }
     ...
     * @since ZC v1.3.7
     */
    public function findMatchingAddressBookEntry($customer_id, array $address_question_arr)
    {
        global $db;
        ...
        if (!$answers_arr->EOF) {
            ...
            while (!$answers_arr->EOF) {
                ...
                if (strlen($fromDb) == strlen($matchQuestion)) {
                    ...
                    if (str_starts_with($fromDb, $matchQuestion)) {
                    ...
                    if (str_starts_with($matchQuestion, $fromDb)) {
     ...
     * @since ZC v1.3.7
     */
    public function addAddressBookEntry($customer_id, array $address_question_arr, $make_default = false)
    {
        global $db;
        ...
        $sql_data_array = [['fieldName' => 'entry_firstname', 'value' => $fname, 'type' => 'string'],
     ...
     * @since ZC v1.3.7
     */
    public function terminateEC($error_msg = '', $kill_sess_vars = false, ?string $goto_page = ''): void
    {
     ...
     * @since ZC v1.3.7
     */
    public function _errorHandler(array $response, string $operation = '', $ignore_codes = '')
    {
        ...
        if (in_array($operation, ['DoExpressCheckoutPayment', 'DoDirectPayment'])
            ...
            $this->fmfResponse = urldecode((string) $response['L_SHORTMESSAGE0']);
            $this->fmfErrors = [];
        ...
        switch ($operation) {
            case 'SetExpressCheckout':
                if ($basicError) {
                    ...
                    if ($detailedEmailMessage != '') {
                        zen_mail(STORE_NAME, STORE_OWNER_EMAIL_ADDRESS, MODULE_PAYMENT_PAYPALWPP_TEXT_EMAIL_ERROR_SUBJECT . ' (' . $this->uncomment($errorNum) . ')', $this->uncomment($detailedEmailMessage), STORE_OWNER, STORE_OWNER_EMAIL_ADDRESS, ['EMAIL_MESSAGE_HTML' => $this->uncomment($detailedMessage)], 'paymentalert');
                    }
            ...
            case 'DoExpressCheckoutPayment':
                if ($basicError || $_SESSION['paypal_ec_token'] != urldecode((string) $response['TOKEN'])) {
                    ...
                    if ($response['L_ERRORCODE0'] == 10474) {
                        $errorText .= urldecode((string) $response['L_LONGMESSAGE0']);
                    }
            ...
            case 'DoRefund':
                if ($basicError || (!isset($response['RESPMSG']) && !isset($response['REFUNDTRANSACTIONID']))) {
                    ...
                    $errorText .= ' (' . urldecode((string) $response['L_SHORTMESSAGE0']) . ') ' . $response['L_ERRORCODE0'];
                    $messageStack->add_session($errorText, 'error');
                    return true;
                }
                break;
            ...
            default:
                if ($basicError) {
                    ...
                    $detailedEmailMessage = ($detailedMessage == '') ? '' : MODULE_PAYMENT_PAYPALWPP_TEXT_EMAIL_ERROR_MESSAGE .  ' ' . $response['RESPMSG'] . urldecode($response['L_ERRORCODE0'] . "\n" . $response['L_SHORTMESSAGE0'] . "\n" . $response['L_LONGMESSAGE0'] . $response['L_ERRORCODE1'] . "\n" . $response['L_SHORTMESSAGE1'] . "\n" . $response['L_LONGMESSAGE1'] . $response['L_ERRORCODE2'] . "\n" . $response['L_SHORTMESSAGE2'] . "\n" . $response['L_LONGMESSAGE2'] . ($response['CURL_ERRORS'] != '' ? "\n" . $response['CURL_ERRORS'] : '') . "\n\n" . 'Zen Cart message: ' . $detailedMessage . "\n\n" . 'Transaction Response Details: ' . print_r($response, true) . "\n\n" . 'Transaction Submission: ' . urldecode((string) $doPayPal->_sanitizeLog($doPayPal->_parseNameValueList($doPayPal->lastParamList), true)));
                    if ($detailedEmailMessage != '') {
                        zen_mail(STORE_NAME, STORE_OWNER_EMAIL_ADDRESS, MODULE_PAYMENT_PAYPALWPP_TEXT_EMAIL_ERROR_SUBJECT . ' (' . $this->uncomment($errorNum) . ')', $this->uncomment($detailedMessage), STORE_OWNER, STORE_OWNER_EMAIL_ADDRESS, ['EMAIL_MESSAGE_HTML' => nl2br($this->uncomment($detailedEmailMessage))], 'paymentalert');
                    }
     ...
     * @since v1.3.7.1
     */
    public function tableCheckup(): void
    {
        global $db, $sniffer;
