...
class paypalwpp extends base
{
     ...
     * @var boolean
     */
    public $show_payment_page = false;
    public $flag_disable_payment_address_change = false;
    /**
     ...
     * @var string
     */
    public $button_source_ec = 'ZenCart-EC_us';
    /**
     * @var 'ZenCart-DP_us'|'ZenCart-DP_uk'|'ZenCart-GW_us'
     */
    public $button_source_dp;
    /**
    ...
    // used in order class
    public $ot_merge;
    //-Public, since might be referenced by an observer.
    protected $request_prefix;
    protected $info_prefix;
    /**
     * Debug tools
     */
    protected $_log_dir = DIR_FS_LOGS;
    ...
    public function __construct()
    {
        global $order;
        $this->code = 'paypalwpp';
        $this->code_title = MODULE_PAYMENT_PAYPALWPP_TEXT_ADMIN_TITLE_EC;
        $this->code_version = '1.5.8';
        ...
        if (IS_ADMIN_FLAG === true) {
            $this->description = sprintf(MODULE_PAYMENT_PAYPALWPP_TEXT_ADMIN_DESCRIPTION, ' (rev' . $this->code_version . ')');
        ...
        $this->enable_debugging = MODULE_PAYMENT_PAYPALWPP_DEBUGGING == 'Log File' || MODULE_PAYMENT_PAYPALWPP_DEBUGGING == 'Log and Email';
        $this->email_alerts = MODULE_PAYMENT_PAYPALWPP_DEBUGGING == 'Log File' || MODULE_PAYMENT_PAYPALWPP_DEBUGGING == 'Log and Email' || MODULE_PAYMENT_PAYPALWPP_DEBUGGING == 'Alerts Only';
        $this->show_payment_page = MODULE_PAYMENT_PAYPALWPP_SKIP_PAYMENT_PAGE == 'No' ? true : false;
        $this->button_source_ec = 'ZenCart-EC_us';
        $this->button_source_dp = 'ZenCart-DP_us';
        if (MODULE_PAYMENT_PAYPALWPP_MODULE_MODE == 'Payflow-UK') {
            $this->button_source_ec = 'ZenCart-EC_uk';
            $this->button_source_dp = 'ZenCart-DP_uk';
        }
        if (MODULE_PAYMENT_PAYPALWPP_MODULE_MODE == 'Payflow-US') {
            $this->button_source_ec = 'ZenCart-ECGW_us';
            $this->button_source_dp = 'ZenCart-GW_us';
        }
    ...
    public function update_status(): void
    {
        ...
        if ($this->enabled && (int) $this->zone > 0 && isset($order->billing['country']['id'])) {
            ...
            $sql = 'SELECT zone_id
              ...
              ORDER BY zone_id';
            $sql = $db->bind_vars($sql, ':zoneId', $this->zone, 'integer');
            $sql = $db->bind_vars($sql, ':countryId', $order->billing['country']['id'], 'integer');
            ...
            if (!$check_flag) {
                $this->enabled = false;
                $this->zc_log('update_status', 'Module disabled due to zone restriction. Billing address is not within the Payment Zone selected in the module settings.');
            }
        }
        // module cannot be used for purchase > 1000000 JPY
        if ($order->info['total'] > 1000000 && $order->info['currency'] == 'JPY') {
            $this->enabled = false;
            $this->zc_log('update_status', 'Module disabled because purchase price (' . $order->info['total'] . ') exceeds PayPal-imposed maximum limit of 1000000 JPY.');
        }
        ...
        if (paypal_usd_check($order->info['total']) === false) {
            $this->enabled = false;
            $this->zc_log('update_status', 'Module disabled because purchase price (' . $order->info['total'] . ') exceeds PayPal-imposed maximum limit of 10,000 USD or equivalent.');
        }
        if ($order->info['total'] == 0) {
            $this->enabled = false;
            $this->zc_log('update_status', 'Module disabled because purchase amount is set to 0.00.' . "\n" . print_r($order, true));
        }
        // other status checks?
        if ($this->enabled) {
            // other checks here
    ...
    public function selection(): array
    {
         ...
         * @since ZC v1.3.7
         */
        return ['id' => $this->code, 'module' => '<img src="' . MODULE_PAYMENT_PAYPALEC_MARK_BUTTON_IMG . '" alt="' . MODULE_PAYMENT_PAYPALWPP_TEXT_BUTTON_ALTTEXT . '"><span style="font-size:11px; font-family: Arial, Verdana;"> ' . MODULE_PAYMENT_PAYPALWPP_MARK_BUTTON_TXT . '</span>'];
    }
    ...
    {
        global $order, $do_pay_pal, $message_stack;
        $options = $this->get_line_item_details($this->select_currency($order->info['currency']));
        ...
        $do_pay_pal = $this->paypal_init();
        $this->zc_log('before_process - EC-1', 'Beginning EC mode');
        /****************************************
         * Do EC checkout
         ****************************************/
        // do not allow blank address to be sent to PayPal
        if ($_SESSION['paypal_ec_payer_info']['ship_street_1'] != '' && strtoupper((string) $_SESSION['paypal_ec_payer_info']['ship_address_status']) != 'NONE') {
            $options = array_merge($options, ['PAYMENTREQUEST_0_SHIPTONAME' => $_SESSION['paypal_ec_payer_info']['ship_name'], 'PAYMENTREQUEST_0_SHIPTOSTREET' => $_SESSION['paypal_ec_payer_info']['ship_street_1'], 'PAYMENTREQUEST_0_SHIPTOSTREET2' => $_SESSION['paypal_ec_payer_info']['ship_street_2'], 'PAYMENTREQUEST_0_SHIPTOCITY' => $_SESSION['paypal_ec_payer_info']['ship_city'], 'PAYMENTREQUEST_0_SHIPTOSTATE' => $_SESSION['paypal_ec_payer_info']['ship_state'], 'PAYMENTREQUEST_0_SHIPTOZIP' => $_SESSION['paypal_ec_payer_info']['ship_postal_code'], 'PAYMENTREQUEST_0_SHIPTOCOUNTRYCODE' => $_SESSION['paypal_ec_payer_info']['ship_country_code']]);
            $this->zc_log('before_process - EC-2', 'address overrides added:' . "\n" . print_r($options, true));
        }
        $this->zc_log('before_process - EC-3', 'address info added:' . "\n" . print_r($options, true));
        $options['BUTTONSOURCE'] = $this->button_source_ec;
        ...
        if ($order->delivery['street_address'] != '' && $order->delivery['street_address'] != $_SESSION['paypal_ec_payer_info']['ship_street_1'] && $_SESSION['paypal_ec_payer_info']['ship_street_1'] != '') {
            $_GET['markflow'] = 2;
            if (($address_arr = $this->get_override_address()) !== false) {
        ...
        $options['PAYMENTREQUEST_0_INVNUM'] = (int) $_SESSION['customer_id'] . '-' . time() . '-[' . substr(preg_replace('/[^a-zA-Z0-9_]/', '', STORE_NAME), 0, 30) . ']';
        // (cannot send actual invoice number because it's not assigned until after payment is completed)
        $options['PAYMENTREQUEST_0_CURRENCYCODE'] = $this->select_currency($order->info['currency']);
        ...
        if (false) {
            // disabled until clarification is received about coupons in PayPal Wallet
            ...
            if (sizeof($order_totals)) {
                for ($i = 0, $n = sizeof($order_totals); $i < $n; $i++) {
                    if (in_array($order_totals[$i]['code'], ['ot_coupon', 'ot_gv'])) {
                        ...
                        $options["L_PAYMENTREQUEST_0_REDEEMEDOFFERNAME{$j}"] = substr((string) $order_totals[$i]['title'], 0, 127);
                        $options["L_PAYMENTREQUEST_0_REDEEMEDOFFERDESCRIPTION{$j}"] = substr((string) $order_totals[$i]['title'], 0, 127);
        ...
        $this->ec_redirect_url = $this->get_pay_pal_login_server() . '?cmd=_express-checkout&token=' . $_SESSION['paypal_ec_token'] . '&useraction=continue';
        $response = $do_pay_pal->do_express_checkout_payment($_SESSION['paypal_ec_token'], $_SESSION['paypal_ec_payer_id'], $options);
        $this->zc_log('before_process - EC-5', 'resultset:' . "\n" . urldecode(print_r($response, true)));
        // CHECK RESPONSE -- if error, actions are taken in the errorHandler
        $this->_error_handler($response, 'DoExpressCheckoutPayment');
        if ($this->fmf_response != '') {
        ...
        //
        if ($response[$this->info_prefix . 'PAYMENTTYPE'] != '') {
            $this->payment_type .= ' (' . urldecode((string) $response[$this->info_prefix . 'PAYMENTTYPE']) . ')';
        }
        $this->transaction_id = trim(($response['PNREF'] ?? '') . ' ' . $response[$this->info_prefix . 'TRANSACTIONID']);
        if (empty($response[$this->info_prefix . 'PENDINGREASON']) || $response[$this->info_prefix . 'PENDINGREASON'] == 'none' || $response[$this->info_prefix . 'PENDINGREASON'] == 'completed' || $response[$this->info_prefix . 'PAYMENTSTATUS'] == 'Completed') {
            $this->payment_status = 'Completed';
            if ($this->order_status > 0) {
                $order->info['order_status'] = $this->order_status;
            }
        } else {
            if ($response[$this->info_prefix . 'PAYMENTSTATUS'] == 'Pending') {
                $response['L_ERRORCODE0'] = empty($response['L_ERRORCODE0']) ? 0 : $response['L_ERRORCODE0'];
                if ($response['L_ERRORCODE0'] == 11610 && $response[$this->info_prefix . 'PENDINGREASON'] == '') {
                    $response[$this->info_prefix . 'PENDINGREASON'] = 'Pending FMF Review by Storeowner';
                }
            }
            $this->payment_status = 'Pending (' . $response[$this->info_prefix . 'PENDINGREASON'] . ')';
            $order->info['order_status'] = $this->order_pending_status;
        }
        $this->avs = 'N/A';
        $this->cvv2 = 'N/A';
        $this->correlationid = $response['CORRELATIONID'];
        $this->transactiontype = $response[$this->info_prefix . 'TRANSACTIONTYPE'];
        $this->payment_time = urldecode((string) $response[$this->info_prefix . 'ORDERTIME']);
        $this->feeamt = empty($response[$this->info_prefix . 'FEEAMT']) ? 0 : urldecode((string) $response[$this->info_prefix . 'FEEAMT']);
        $this->taxamt = urldecode($response[$this->info_prefix . 'TAXAMT'] ?? '');
        $this->pendingreason = $response[$this->info_prefix . 'PENDINGREASON'];
        $this->reasoncode = $response[$this->info_prefix . 'REASONCODE'];
        $this->numitems = count($order->products);
        $this->amt = urldecode($response[$this->info_prefix . 'AMT'] . ' ' . $response[$this->info_prefix . 'CURRENCYCODE']);
        $this->auth_code = $response['AUTHCODE'] ?? $response['TOKEN'];
    ...
    public function after_process(): void
    {
        ...
        // FMF
        if ($this->fmf_response != '') {
            $detailed_message = $insert_id . "\n" . $this->fmf_response . "\n" . MODULES_PAYMENT_PAYPALDP_TEXT_EMAIL_FMF_INTRO . "\n" . print_r($this->fmf_errors, true);
            zen_mail(STORE_NAME, STORE_OWNER_EMAIL_ADDRESS, MODULES_PAYMENT_PAYPALDP_TEXT_EMAIL_FMF_SUBJECT . ' (' . $insert_id . ')', $detailed_message, STORE_OWNER, STORE_OWNER_EMAIL_ADDRESS, ['EMAIL_MESSAGE_HTML' => nl2br($detailed_message)], 'paymentalert');
        }
        // add a new OSH record for this order's PP details
        $comment_string = 'Transaction ID: ' . $this->transaction_id . (isset($this->responsedata['PPREF']) ? "\nPPRef: " . $this->responsedata['PPREF'] : '') . (isset($this->responsedata['AUTHCODE']) ? "\nAuthCode: " . $this->responsedata['AUTHCODE'] : '') . "\nPayment Type: " . $this->payment_type . ($this->payment_time != '' ? "\nTimestamp: " . $this->payment_time : '') . "\nPayment Status: " . $this->payment_status . (isset($this->responsedata['auth_exp']) ? "\nAuth-Exp: " . $this->responsedata['auth_exp'] : '') . ($this->avs != 'N/A' ? "\nAVS Code: " . $this->avs . "\nCVV2 Code: " . $this->cvv2 : '') . (trim((string) $this->amt) != '' ? "\nAmount: " . $this->amt : '');
        zen_update_orders_history($insert_id, $comment_string, null, $order->info['order_status'], 0);
        // store the PayPal order meta data -- used for later matching and back-end processing activities
        $paypal_order = ['order_id' => $insert_id, 'txn_type' => $this->transactiontype, 'module_name' => $this->code, 'module_mode' => MODULE_PAYMENT_PAYPALWPP_MODULE_MODE, 'reason_code' => $this->reasoncode, 'payment_type' => $this->payment_type, 'payment_status' => $this->payment_status, 'pending_reason' => $this->pendingreason, 'invoice' => urldecode($_SESSION['paypal_ec_token'] . ($this->responsedata['PPREF'] ?? '')), 'first_name' => $_SESSION['paypal_ec_payer_info']['payer_firstname'], 'last_name' => $_SESSION['paypal_ec_payer_info']['payer_lastname'], 'payer_business_name' => $_SESSION['paypal_ec_payer_info']['payer_business'], 'address_name' => $_SESSION['paypal_ec_payer_info']['ship_name'], 'address_street' => $_SESSION['paypal_ec_payer_info']['ship_street_1'], 'address_city' => $_SESSION['paypal_ec_payer_info']['ship_city'], 'address_state' => $_SESSION['paypal_ec_payer_info']['ship_state'], 'address_zip' => $_SESSION['paypal_ec_payer_info']['ship_postal_code'], 'address_country' => $_SESSION['paypal_ec_payer_info']['ship_country_name'], 'address_status' => $_SESSION['paypal_ec_payer_info']['ship_address_status'], 'payer_email' => $_SESSION['paypal_ec_payer_info']['payer_email'], 'payer_id' => $_SESSION['paypal_ec_payer_id'], 'payer_status' => $_SESSION['paypal_ec_payer_info']['payer_status'], 'payment_date' => convert_to_local_time_zone(trim((string) preg_replace('/[^0-9-:]/', ' ', (string) $this->payment_time))), 'business' => '', 'receiver_email' => str_starts_with(MODULE_PAYMENT_PAYPALWPP_MODULE_MODE, 'Payflow') ? MODULE_PAYMENT_PAYPALWPP_PFVENDOR : str_replace('_api1', '', MODULE_PAYMENT_PAYPALWPP_APIUSERNAME), 'receiver_id' => '', 'txn_id' => $this->transaction_id, 'parent_txn_id' => '', 'num_cart_items' => (float) $this->numitems, 'mc_gross' => (float) $this->amt, 'mc_fee' => (float) urldecode((string) $this->feeamt), 'mc_currency' => $this->responsedata[$this->info_prefix . 'CURRENCYCODE'], 'settle_amount' => (float) (isset($this->responsedata[$this->info_prefix . 'SETTLEAMT']) ? urldecode($this->responsedata[$this->info_prefix . 'SETTLEAMT']) : $this->amt), 'settle_currency' => $this->responsedata[$this->info_prefix . 'CURRENCYCODE'], 'exchange_rate' => isset($this->responsedata[$this->info_prefix . 'EXCHANGERATE']) && urldecode($this->responsedata[$this->info_prefix . 'EXCHANGERATE']) > 0 ? urldecode($this->responsedata[$this->info_prefix . 'EXCHANGERATE']) : 1.0, 'notify_version' => '0', 'verify_sign' => '', 'date_added' => 'now()', 'memo' => sizeof($this->fmf_errors) > 0 ? 'FMF Details ' . print_r($this->fmf_errors, true) : '{Record generated by payment module}'];
    ...
    public function admin_notification($zf_order_id): string
    {
        ...
        $module = $this->code;
        $output = '';
        $response = $this->_get_transaction_details($zf_order_id);
        ...
        $sql = $db->bind_vars($sql, ':orderID', $zf_order_id, 'integer');
        $ipn = $db->Execute($sql);
     ...
     * @since ZC v1.3.7
     */
    public function _get_transaction_details($o_id, $last_txn = true)
    {
        if ($o_id == '' || $o_id < 1) {
            return false;
        }
        global $db, $message_stack, $do_pay_pal;
        $do_pay_pal = $this->paypal_init();
        ...
        $sql = $db->bind_vars($sql, ':orderID', $o_id, 'integer');
        $zc_pp_hist = $db->Execute($sql);
        if ($zc_pp_hist->record_count() == 0) {
            return false;
        }
        $txn_id = $zc_pp_hist->fields['txn_id'];
        if ($txn_id == '' || $txn_id === 0) {
            return false;
        }
        /**
         ...
         */
        $response = $do_pay_pal->get_transaction_details($txn_id);
        // $this->zcLog("_GetTransactionDetails($oID):", print_r($response, true));
        $error = $this->_error_handler($response, 'GetTransactionDetails', 10007);
        if ($error === false) {
            return false;
        }
        return $response;
     ...
     * @since ZC v1.3.7.1
     */
    public function _transaction_search($start_date = '', $o_id = '', $criteria = '')
    {
        global $db, $message_stack, $do_pay_pal;
        $do_pay_pal = $this->paypal_init();
        // look up history on this order from PayPal table
        ...
        $txn_id = $zc_pp_hist->fields['txn_id'];
        $start_date = $zc_pp_hist->fields['payment_date'];
        $timeval = time();
        if ($start_date == '') {
            $start_date = date('Y-m-d', $timeval) . 'T' . date('h:i:s', $timeval) . 'Z';
        }
        /**
         * Read data from PayPal
         */
        $response = $do_pay_pal->transaction_search($start_date, $txn_id, $email, $criteria);
        //$this->zcLog("_TransactionSearch($startDate, $oID, $criteria):", print_r($response, true));
        $error = $this->_error_handler($response, 'TransactionSearch');
        if ($error === false) {
            return false;
        }
        return $response;
    ...
    public function install()
    {
        ...
        if (defined('MODULE_PAYMENT_PAYPAL_STATUS')) {
            $message_stack->add_session('NOTE: You already have the PayPal Website Payments Standard module installed. You should REMOVE it if you want to install Express Checkout.', 'error');
    ...
    public function remove()
    {
        ...
        if (defined('MODULE_PAYMENT_PAYPALDP_STATUS')) {
            ...
            $message_stack->add_session('<strong>Sorry, you must remove PayPal Payments Pro (paypaldp) first.</strong> PayPal Payments Pro (Website Payments Pro) requires that you offer Express Checkout to your customers.<br><a href="' . zen_href_link('modules.php?set=payment&module=paypaldp', '', 'NONSSL') . '">Click here to edit or remove your PayPal Payments Pro module.</a>', 'error');
            zen_redirect(zen_href_link(FILENAME_MODULES, 'set=payment&module=paypalwpp', 'NONSSL'));
            return 'failed';
        }
        global $db;
    ...
    public function in_special_checkout()
    {
        if (defined('MODULE_PAYMENT_PAYPALWPP_STATUS') && MODULE_PAYMENT_PAYPALWPP_STATUS == 'True' && !empty($_SESSION['paypal_ec_token']) && !empty($_SESSION['paypal_ec_payer_id']) && !empty($_SESSION['paypal_ec_payer_info'])) {
            $this->flag_disable_payment_address_change = true;
            return true;
        }
    }
    /**
     ...
     * @since ZC v1.3.7
     */
    public function alter_shipping_edit_button(): bool
    {
        return false;
    }
    /**
     ...
     * @since ZC v1.3.7
     */
    public function zc_log(string $stage, string $message): void
    {
        static $token_hash;
        if ($token_hash == '') {
        ...
        if (MODULE_PAYMENT_PAYPALWPP_DEBUGGING == 'Log and Email' || MODULE_PAYMENT_PAYPALWPP_DEBUGGING == 'Log File') {
            $token = $_SESSION['paypal_ec_token'] ?? (isset($_GET['token']) ? preg_replace('/[^0-9.A-Z\-]/', '', (string) $_GET['token']) : '');
            $token = $token == '' ? date('m-d-Y-H-i') : $token;
            // or time()
            $token .= $token_hash;
            $file = $this->_log_dir . '/' . $this->code . '_Paypal_Action_' . $token . '.log';
            if (defined('PAYPAL_DEV_MODE') && PAYPAL_DEV_MODE == 'true') {
                $file = $this->_log_dir . '/' . $this->code . '_Paypal_Debug_' . $token . '.log';
            }
            $fp = @fopen($file, 'a');
            @fwrite($fp, date('M-d-Y H:i:s') . ' (' . time() . ')' . "\n" . $stage . "\n" . $message . "\n=================================\n\n");
            @fclose($fp);
        }
        $this->_do_debug($stage, $message, false);
    }
    /**
     * Debug Emailing support
     * @since ZC v1.3.7
     */
    public function _do_debug($subject = 'PayPal debug data', $data = '', $use_session = true): void
    {
    ...
    public function paypal_init()
    {
        if (!defined('MODULE_PAYMENT_PAYPALWPP_STATUS') || !defined('MODULE_PAYMENT_PAYPALWPP_SERVER')) {
            return new paypal_curl(['mode' => 'NOTCONFIGURED']);
        }
        defined('MODULE_PAYMENT_PAYPALWPP_PRO20_EC_METHOD') && MODULE_PAYMENT_PAYPALWPP_PRO20_EC_METHOD == 'Payflow' ? true : false;
        // -----
        ...
        //
        if (str_starts_with(MODULE_PAYMENT_PAYPALWPP_MODULE_MODE, 'Payflow')) {
            $do_pay_pal = new paypal_curl(['mode' => 'payflow', 'user' => trim(MODULE_PAYMENT_PAYPALWPP_PFUSER), 'vendor' => trim(MODULE_PAYMENT_PAYPALWPP_PFVENDOR), 'partner' => trim(MODULE_PAYMENT_PAYPALWPP_PFPARTNER), 'pwd' => trim(MODULE_PAYMENT_PAYPALWPP_PFPASSWORD), 'server' => MODULE_PAYMENT_PAYPALWPP_SERVER]);
            $do_pay_pal->_endpoints = ['live' => 'https://payflowpro.paypal.com/transaction', 'sandbox' => 'https://pilot-payflowpro.paypal.com/transaction'];
            $this->request_prefix = '';
            $this->info_prefix = '';
        } else {
            $do_pay_pal = new paypal_curl(['mode' => 'nvp', 'user' => trim(MODULE_PAYMENT_PAYPALWPP_APIUSERNAME), 'pwd' => trim(MODULE_PAYMENT_PAYPALWPP_APIPASSWORD), 'signature' => trim(MODULE_PAYMENT_PAYPALWPP_APISIGNATURE), 'version' => '124.0', 'server' => MODULE_PAYMENT_PAYPALWPP_SERVER]);
            $do_pay_pal->_endpoints = ['live' => 'https://api-3t.paypal.com/nvp', 'sandbox' => 'https://api-3t.sandbox.paypal.com/nvp'];
            $this->request_prefix = 'PAYMENTREQUEST_0_';
            $this->info_prefix = 'PAYMENTINFO_0_';
        }
        // set logging options
        $do_pay_pal->_log_dir = $this->_log_dir;
        $do_pay_pal->_log_level = $this->_log_level;
        ...
        if (CURL_PROXY_REQUIRED == 'True' && CURL_PROXY_SERVER_DETAILS != '') {
            ...
            $do_pay_pal->set_curl_option(CURLOPT_HTTPPROXYTUNNEL, $proxy_tunnel_flag);
            $do_pay_pal->set_curl_option(CURLOPT_PROXYTYPE, CURLPROXY_HTTP);
            $do_pay_pal->set_curl_option(CURLOPT_PROXY, CURL_PROXY_SERVER_DETAILS);
        }
        // transaction processing mode
        $do_pay_pal->_trxtype = in_array(MODULE_PAYMENT_PAYPALWPP_TRANSACTION_MODE, ['Auth Only', 'Order']) ? 'A' : 'S';
        return $do_pay_pal;
    }
    /**
     ...
     * @since ZC v1.3.7
     */
    public function get_pay_pal_login_server(): string
    {
     ...
     * @since ZC v1.3.7
     */
    public function _do_refund($o_id, $amount = 'Full', $note = '')
    {
        global $db, $do_pay_pal, $message_stack;
        $new_order_status = (int) MODULE_PAYMENT_PAYPALWPP_REFUNDED_STATUS_ID;
        $do_pay_pal = $this->paypal_init();
        $proceed_to_refund = false;
        $refund_note = strip_tags(zen_db_input($_POST['refnote']));
        if (isset($_POST['fullrefund']) && $_POST['fullrefund'] == MODULE_PAYMENT_PAYPAL_ENTRY_REFUND_BUTTON_TEXT_FULL) {
            ...
            if (isset($_POST['reffullconfirm']) && $_POST['reffullconfirm'] == 'on') {
                ...
                $message_stack->add_session(MODULE_PAYMENT_PAYPALWPP_TEXT_REFUND_FULL_CONFIRM_ERROR, 'error');
            }
        }
        if (isset($_POST['partialrefund']) && $_POST['partialrefund'] == MODULE_PAYMENT_PAYPAL_ENTRY_REFUND_BUTTON_TEXT_PARTIAL) {
            $refund_amt = (float) $_POST['refamt'];
            $proceed_to_refund = true;
            if ($refund_amt == 0) {
                $message_stack->add_session(MODULE_PAYMENT_PAYPALWPP_TEXT_INVALID_REFUND_AMOUNT, 'error');
                $proceed_to_refund = false;
            }
        }
        // look up history on this order from PayPal table
        ...
        $txn_id = $zc_pp_hist->fields['txn_id'];
        $cur_code = $zc_pp_hist->fields['mc_currency'];
        $p_famt = $zc_pp_hist->fields['mc_gross'];
        if ($do_pay_pal->_mode == 'payflow' && $refund_amt == 'Full') {
            $refund_amt = $p_famt;
        }
        /**
        ...
        if ($proceed_to_refund) {
            $response = $do_pay_pal->refund_transaction($o_id, $txn_id, $refund_amt, $refund_note, $cur_code);
            //$this->zcLog("_doRefund($oID, $amount, $note):", print_r($response, true));
            $error = $this->_error_handler($response, 'DoRefund');
            $new_order_status = $new_order_status > 0 ? $new_order_status : 1;
            if (!$error) {
                if (!isset($response['GROSSREFUNDAMT'])) {
                    $response['GROSSREFUNDAMT'] = $refund_amt;
                }
                // Success, so save the results
                $comments = 'REFUND INITIATED. Trans ID:' . $response['REFUNDTRANSACTIONID'] . ($response['PNREF'] ?? '') . "\n" . ' Gross Refund Amt: ' . urldecode((string) $response['GROSSREFUNDAMT']) . (isset($response['PPREF']) ? "\nPPRef: " . $response['PPREF'] : '') . "\n" . $refund_note;
                zen_update_orders_history($o_id, $comments, null, $new_order_status, 0);
                $message_stack->add_session(sprintf(MODULE_PAYMENT_PAYPALWPP_TEXT_REFUND_INITIATED, urldecode((string) $response['GROSSREFUNDAMT']), urldecode((string) $response['REFUNDTRANSACTIONID']) . ($response['PNREF'] ?? '')), 'success');
                return true;
            }
        }
    }
     ...
     * @since ZC v1.3.7
     */
    public function _do_auth($o_id, $amt, string $currency = 'USD')
    {
        global $db, $do_pay_pal, $message_stack;
        $do_pay_pal = $this->paypal_init();
        $auth_amt = $amt;
        ...
        if (isset($_POST['orderauth']) && $_POST['orderauth'] == MODULE_PAYMENT_PAYPAL_ENTRY_AUTH_BUTTON_TEXT_PARTIAL) {
            $auth_amt = (float) $_POST['authamt'];
            $new_order_status = MODULE_PAYMENT_PAYPALWPP_ORDER_STATUS_ID;
            if (isset($_POST['authconfirm']) && $_POST['authconfirm'] == 'on') {
                $proceed_to_auth = true;
            } else {
                $message_stack->add_session(MODULE_PAYMENT_PAYPALWPP_TEXT_AUTH_CONFIRM_ERROR, 'error');
                $proceed_to_auth = false;
            }
            if ($auth_amt == 0) {
                $message_stack->add_session(MODULE_PAYMENT_PAYPALWPP_TEXT_INVALID_AUTH_AMOUNT, 'error');
                $proceed_to_auth = false;
            }
        }
        // look up history on this order from PayPal table
         ...
         */
        if ($proceed_to_auth) {
            $response = $do_pay_pal->do_authorization($txn_id, $auth_amt, $currency);
            //$this->zcLog("_doAuth($oID, $amt, $currency):", print_r($response, true));
            $error = $this->_error_handler($response, 'DoAuthorization');
            $new_order_status = $new_order_status > 0 ? $new_order_status : 1;
            if (!$error) {
                // Success, so save the results
                $comments = 'AUTHORIZATION ADDED. Trans ID: ' . urldecode((string) $response['TRANSACTIONID']) . "\n" . ' Amount:' . urldecode((string) $response['AMT']) . ' ' . $currency;
                zen_update_orders_history($o_id, $comments, null, $new_order_status, -1);
                $message_stack->add_session(sprintf(MODULE_PAYMENT_PAYPALWPP_TEXT_AUTH_INITIATED, urldecode((string) $response['AMT'])), 'success');
                return true;
            }
        }
    }
     ...
     * @since ZC v1.3.7
     */
    public function _do_capt($o_id, $capture_type = 'Complete', $amt = 0, string $currency = 'USD', $note = '')
    {
        global $db, $do_pay_pal, $message_stack;
        $do_pay_pal = $this->paypal_init();
        ...
        $capture_note = strip_tags(zen_db_input($_POST['captnote']));
        if (isset($_POST['captfullconfirm']) && $_POST['captfullconfirm'] == 'on') {
            $proceed_to_capture = true;
        } else {
            $message_stack->add_session(MODULE_PAYMENT_PAYPALWPP_TEXT_CAPTURE_FULL_CONFIRM_ERROR, 'error');
        }
        ...
        if (isset($_POST['btndocapture']) && $_POST['btndocapture'] == MODULE_PAYMENT_PAYPAL_ENTRY_CAPTURE_BUTTON_TEXT_FULL) {
            $capture_amt = (float) $_POST['captamt'];
            if ($capture_amt == 0) {
                $message_stack->add_session(MODULE_PAYMENT_PAYPALWPP_TEXT_INVALID_CAPTURE_AMOUNT, 'error');
                $proceed_to_capture = false;
            }
        }
        // look up history on this order from PayPal table
        ...
        if ($proceed_to_capture) {
            $response = $do_pay_pal->do_capture($txn_id, $capture_amt, $currency, $capture_type, '', $capture_note);
            //$this->zcLog("_doCapt($oID, $captureType, $amt, $currency, $note):", print_r($response, true));
            $error = $this->_error_handler($response, 'DoCapture');
            $new_order_status = $new_order_status > 0 ? $new_order_status : 1;
            if (!$error) {
                if (isset($response['PNREF'])) {
                    if (!isset($response['AMT'])) {
                        $response['AMT'] = $capture_amt;
                    }
                ...
                // Success, so save the results
                $comments = 'FUNDS CAPTURED. Trans ID: ' . urldecode((string) $response['TRANSACTIONID']) . ($response['PNREF'] ?? '') . "\n" . ' Amount: ' . urldecode((string) $response['AMT']) . ' ' . $currency . "\n" . 'Time: ' . urldecode((string) $response['ORDERTIME']) . "\n" . 'Auth Code: ' . (!empty($response['AUTHCODE']) ? $response['AUTHCODE'] : $response['CORRELATIONID']) . (isset($response['PPREF']) ? "\nPPRef: " . $response['PPREF'] : '') . "\n" . $capture_note;
                zen_update_orders_history($o_id, $comments, null, $new_order_status, 0);
                $message_stack->add_session(sprintf(MODULE_PAYMENT_PAYPALWPP_TEXT_CAPT_INITIATED, urldecode((string) $response['AMT']), urldecode(!empty($response['AUTHCODE']) ? $response['AUTHCODE'] : $response['CORRELATIONID']) . ($response['PNREF'] ?? '')), 'success');
                return true;
            }
        }
    }
     ...
     * @since ZC v1.3.7
     */
    public function _do_void($o_id, $note = '')
    {
        global $db, $do_pay_pal, $message_stack;
        $new_order_status = (int) MODULE_PAYMENT_PAYPALWPP_REFUNDED_STATUS_ID;
        $do_pay_pal = $this->paypal_init();
        $void_note = strip_tags(zen_db_input($_POST['voidnote']));
        $void_auth_id = trim(strip_tags(zen_db_input($_POST['voidauthid'])));
        ...
        if (isset($_POST['ordervoid']) && $_POST['ordervoid'] == MODULE_PAYMENT_PAYPAL_ENTRY_VOID_BUTTON_TEXT_FULL) {
            if (isset($_POST['voidconfirm']) && $_POST['voidconfirm'] == 'on') {
                ...
                $message_stack->add_session(MODULE_PAYMENT_PAYPALWPP_TEXT_VOID_CONFIRM_ERROR, 'error');
            }
        }
        // look up history on this order from PayPal table
        $sql = 'SELECT * FROM ' . TABLE_PAYPAL . " WHERE order_id = :orderID  AND parent_txn_id = '' ";
        $sql = $db->bind_vars($sql, ':orderID', $o_id, 'integer');
        $sql = $db->bind_vars($sql, ':transID', $void_auth_id, 'string');
        $zc_pp_hist = $db->Execute($sql);
        if ($zc_pp_hist->record_count() == 0) {
            return false;
        }
         ...
         */
        if ($proceed_to_void) {
            $response = $do_pay_pal->do_void($void_auth_id, $void_note);
            //$this->zcLog("_doVoid($oID, $note):", print_r($response, true));
            $error = $this->_error_handler($response, 'DoVoid');
            $new_order_status = $new_order_status > 0 ? $new_order_status : 1;
            if (!$error) {
                // Success, so save the results
                $comments = 'VOIDED. Trans ID: ' . urldecode((string) $response['AUTHORIZATIONID']) . ($response['PNREF'] ?? '') . (isset($response['PPREF']) ? "\nPPRef: " . $response['PPREF'] : '') . "\n" . $void_note;
                zen_update_orders_history($o_id, $comments, null, $new_order_status, 0);
                $message_stack->add_session(sprintf(MODULE_PAYMENT_PAYPALWPP_TEXT_VOID_INITIATED, urldecode((string) $response['AUTHORIZATIONID']) . ($response['PNREF'] ?? '')), 'success');
                return true;
            }
        }
    }
     ...
     * @since ZC v1.3.7
     */
    public function get_language_code($mode = 'ec')
    {
        ...
        $user_locale_info[] = strtoupper((string) $_SESSION['languages_code']);
        if (isset($order->delivery['country']['id'])) {
            $shipping_iso = zen_get_countries_with_iso_codes($order->delivery['country']['id']);
            $user_locale_info[] = strtoupper($shipping_iso['countries_iso_code_2'] ?? '');
        }
        if (isset($order->billing['country']['id'])) {
            $billing_iso = zen_get_countries_with_iso_codes($order->billing['country']['id']);
            $user_locale_info[] = strtoupper((string) $billing_iso['countries_iso_code_2']);
        }
        if (isset($order->customer['country']['id'])) {
            $cust_iso = zen_get_countries_with_iso_codes($order->customer['country']['id']);
            $user_locale_info[] = strtoupper((string) $cust_iso['countries_iso_code_2']);
        }
        $store_iso = zen_get_countries_with_iso_codes(STORE_COUNTRY);
        $user_locale_info[] = strtoupper((string) $store_iso['countries_iso_code_2']);
        $to_match = array_map(strtoupper(...), array_merge($allowed_country_codes, $allowed_language_codes));
        foreach ($user_locale_info as $val) {
            if (in_array(strtoupper((string) $val), $to_match)) {
                if (strtoupper((string) $val) === 'EN') {
                    return isset($locales) && $locales[0] === 'en_GB' ? 'GB' : 'US';
                }
                return $val;
            }
        }
     ...
     * @since ZC v1.3.7
     */
    public function select_currency($val = '', $subset = 'EC')
    {
        ...
        $paypal_supported_currencies = $subset == 'EC' ? $ec_currencies : $dp_currencies;
        // if using Pro 2.0 (UK), only the 6 currencies are supported.
        $paypal_supported_currencies = MODULE_PAYMENT_PAYPALWPP_MODULE_MODE == 'Payflow-UK' ? $dp_currencies : $paypal_supported_currencies;
        ...
        if (!in_array($my_currency, $paypal_supported_currencies)) {
            return MODULE_PAYMENT_PAYPALWPP_MODULE_MODE == 'Payflow-UK' ? 'GBP' : 'USD';
        }
        return $my_currency;
    }
    /**
     ...
     * @since ZC v1.3.7
     */
    public function calc_order_amount($amount, $paypal_currency, $apply_formatting = false): float|int
    {
        global $currencies;
        $amount = $amount * $currencies->get_value($paypal_currency);
        if (in_array($paypal_currency, ['JPY', 'HUF', 'TWD']) || (int) $currencies->get_decimal_places($paypal_currency) == 0) {
            $amount = (int) $amount;
            $apply_formatting = false;
        }
        return $apply_formatting ? round($amount, $currencies->get_decimal_places($paypal_currency)) : $amount;
    }
    /**
     ...
     * @since ZC v1.3.7
     */
    public function set_state_and_country(array &$info): void
    {
        global $db, $message_stack;
        switch ($info['country']['iso_code_2']) {
            ...
            case 'CA':
                ...
                if (strlen((string) $info['state']) > 2) {
                    $sql = 'SELECT zone_code FROM ' . TABLE_ZONES . ' WHERE zone_name = :zoneName';
                    $sql = $db->bind_vars($sql, ':zoneName', $info['state'], 'string');
                    $state = $db->Execute($sql);
                    if (!$state->EOF) {
                        $info['state'] = $state->fields['zone_code'];
                    } else {
                        $message_stack->add_session('header', MODULE_PAYMENT_PAYPALWPP_TEXT_STATE_ERROR, 'error');
                        $this->terminate_ec(MODULE_PAYMENT_PAYPALWPP_TEXT_STATE_ERROR);
                    }
                }
                break;
            case 'AT':
     ...
     * @return mixed[]
     */
    public function get_line_item_details($restricted_currency): array
    {
        ...
        if (DEFAULT_CURRENCY != $order->info['currency'] || $restricted_currency != DEFAULT_CURRENCY) {
            $this->zc_log('getLineItemDetails 1', 'Not using default currency. Thus, no line-item details can be submitted.');
            return [];
        }
        if ($currencies->currencies[$_SESSION['currency']]['value'] != 1 || $currencies->currencies[$order->info['currency']]['value'] != 1) {
            $this->zc_log('getLineItemDetails 2', 'currency val not equal to 1.0000 - cannot proceed without coping with currency conversions. Aborting line-item details.');
            return [];
        }
        $options_st = [];
        $options_li = [];
        ...
        $sum_of_line_items = 0;
        $sum_of_line_tax = 0;
        $options_st['PAYMENTREQUEST_0_AMT'] = 0;
        $options_st['PAYMENTREQUEST_0_ITEMAMT'] = 0;
        $options_st['PAYMENTREQUEST_0_TAXAMT'] = 0;
        $options_st['PAYMENTREQUEST_0_SHIPPINGAMT'] = 0;
        $options_st['PAYMENTREQUEST_0_SHIPDISCAMT'] = 0;
        $options_st['PAYMENTREQUEST_0_HANDLINGAMT'] = 0;
        $options_st['PAYMENTREQUEST_0_INSURANCEAMT'] = 0;
        ...
        if (sizeof($order_totals)) {
            ...
            for ($i = 0, $n = sizeof($order_totals); $i < $n; $i++) {
                ...
                if (in_array($order_totals[$i]['code'], ['ot_total', 'ot_subtotal', 'ot_tax', 'ot_shipping']) || strstr((string) $order_totals[$i]['code'], 'insurance')) {
                    if ($order_totals[$i]['code'] == 'ot_shipping') {
                        $options_st['PAYMENTREQUEST_0_SHIPPINGAMT'] = round($order_totals[$i]['value'], 2);
                    }
                    if ($order_totals[$i]['code'] == 'ot_total') {
                        $options_st['PAYMENTREQUEST_0_AMT'] = round($order_totals[$i]['value'], 2);
                    }
                    if ($order_totals[$i]['code'] == 'ot_tax') {
                        $options_st['PAYMENTREQUEST_0_TAXAMT'] += round($order_totals[$i]['value'], 2);
                    }
                    if ($order_totals[$i]['code'] == 'ot_subtotal') {
                        $options_st['PAYMENTREQUEST_0_ITEMAMT'] = round($order_totals[$i]['value'], 2);
                    }
                    if (strstr((string) $order_totals[$i]['code'], 'insurance')) {
                        $options_st['PAYMENTREQUEST_0_INSURANCEAMT'] += round($order_totals[$i]['value'], 2);
                    }
            ...
            if (sizeof($this->ot_merge)) {
                $options_st = array_merge($options_st, $this->ot_merge);
            }
            if ($credits_applied > 0) {
                $options_st['PAYMENTREQUEST_0_ITEMAMT'] -= $credits_applied;
            }
            if ($surcharges > 0) {
                $options_st['PAYMENTREQUEST_0_ITEMAMT'] += $surcharges;
            }
            ...
            if (!empty($order->info['shipping_method']) && DISPLAY_PRICE_WITH_TAX != 'true') {
                ...
                if (isset($GLOBALS[$module]) && $GLOBALS[$module]->tax_class > 0) {
                    ...
                    $options_st['PAYMENTREQUEST_0_SHIPPINGAMT'] += $tax_adjustment_for_shipping;
                    $options_st['PAYMENTREQUEST_0_TAXAMT'] -= $tax_adjustment_for_shipping;
                }
            }
            $flag_subtotals_unknown_yet = $options_st['PAYMENTREQUEST_0_SHIPPINGAMT'] + $options_st['PAYMENTREQUEST_0_SHIPDISCAMT'] + $options_st['PAYMENTREQUEST_0_AMT'] + $options_st['PAYMENTREQUEST_0_TAXAMT'] + $options_st['PAYMENTREQUEST_0_ITEMAMT'] + $options_st['PAYMENTREQUEST_0_INSURANCEAMT'] == 0;
        } else {
        ...
        for ($i = 0, $n = count($order->products), $k = -1; $i < $n; $i++) {
            ...
            $k++;
            $options_li["L_PAYMENTREQUEST_0_NUMBER{$k}"] = $order->products[$i]['model'];
            $options_li["L_PAYMENTREQUEST_0_NAME{$k}"] = $order->products[$i]['name'] . ' [' . (int) $order->products[$i]['id'] . ']';
            // Append *** if out-of-stock.
            $options_li["L_PAYMENTREQUEST_0_NAME{$k}"] .= zen_get_products_stock($order->products[$i]['id']) - $order->products[$i]['qty'] < 0 ? STOCK_MARK_PRODUCT_OUT_OF_STOCK : '';
            // if there are attributes, loop thru them and add to description
            if (isset($order->products[$i]['attributes']) && sizeof($order->products[$i]['attributes']) > 0) {
                for ($j = 0, $n2 = sizeof($order->products[$i]['attributes']); $j < $n2; $j++) {
                    $options_li["L_PAYMENTREQUEST_0_NAME{$k}"] .= "\n " . $order->products[$i]['attributes'][$j]['option'] . ': ' . $order->products[$i]['attributes'][$j]['value'];
                }
                // end loop
            }
            ...
            if (is_float($order->products[$i]['qty']) && ($order->products[$i]['qty'] != (int) $order->products[$i]['qty'] || $flag_treat_as_partial)) {
                $options_li["L_PAYMENTREQUEST_0_NAME{$k}"] = '(' . $order->products[$i]['qty'] . ' x ) ' . $options_li["L_PAYMENTREQUEST_0_NAME{$k}"];
                // zen_add_tax already handles whether DISPLAY_PRICES_WITH_TAX is set
                $options_li["L_PAYMENTREQUEST_0_AMT{$k}"] = zen_round(zen_round(zen_add_tax($order->products[$i]['final_price'], $order->products[$i]['tax']), $decimals) * $order->products[$i]['qty'], $decimals);
                $options_li["L_PAYMENTREQUEST_0_QTY{$k}"] = 1;
                // no line-item tax component
            } else {
                $options_li["L_PAYMENTREQUEST_0_QTY{$k}"] = $order->products[$i]['qty'];
                $options_li["L_PAYMENTREQUEST_0_AMT{$k}"] = zen_round(zen_add_tax($order->products[$i]['final_price'], $order->products[$i]['tax']), $decimals);
            }
            $sub_total_li += $options_li["L_PAYMENTREQUEST_0_QTY{$k}"] * $options_li["L_PAYMENTREQUEST_0_AMT{$k}"];
            ...
            if ($order->products[$i]['onetime_charges'] != 0) {
                $k++;
                $options_li["L_PAYMENTREQUEST_0_NAME{$k}"] = MODULES_PAYMENT_PAYPALWPP_LINEITEM_TEXT_ONETIME_CHARGES_PREFIX . substr(htmlentities((string) $order->products[$i]['name'], ENT_QUOTES, 'UTF-8'), 0, 120);
                $options_li["L_PAYMENTREQUEST_0_AMT{$k}"] = zen_round(zen_add_tax($order->products[$i]['onetime_charges'], $order->products[$i]['tax']), $decimals);
                $options_li["L_PAYMENTREQUEST_0_QTY{$k}"] = 1;
                ...
                $sub_total_li += $options_li["L_PAYMENTREQUEST_0_AMT{$k}"];
                //        $subTotalTax += $optionsLI["L_PAYMENTREQUEST_0_TAXAMT$k"];
            }
            $number_of_line_items_processed = $k;
            $this->notify('NOTIFY_PAYPALWPP_GETLINEITEMDETAILS', $number_of_line_items_processed, $options_li);
        }
        ...
        if ($surcharges > 0) {
            ...
            $options_li["L_PAYMENTREQUEST_0_NAME{$k}"] = MODULES_PAYMENT_PAYPALWPP_LINEITEM_TEXT_SURCHARGES_LONG;
            $options_li["L_PAYMENTREQUEST_0_AMT{$k}"] = $surcharges;
            $options_li["L_PAYMENTREQUEST_0_QTY{$k}"] = 1;
        ...
        if ($credits_applied > 0) {
            $number_of_line_items_processed++;
            $k = $number_of_line_items_processed;
            $options_li["L_PAYMENTREQUEST_0_NAME{$k}"] = MODULES_PAYMENT_PAYPALWPP_LINEITEM_TEXT_DISCOUNTS_LONG;
            $options_li["L_PAYMENTREQUEST_0_AMT{$k}"] = -1 * $credits_applied;
            $options_li["L_PAYMENTREQUEST_0_QTY{$k}"] = 1;
        ...
        for ($k = 0, $n = $number_of_line_items_processed + 1; $k < $n; $k++) {
            $options_li["L_PAYMENTREQUEST_0_NAME{$k}"] = str_replace(['&', '=', '%'], '*', $options_li["L_PAYMENTREQUEST_0_NAME{$k}"]);
            $options_li["L_PAYMENTREQUEST_0_NAME{$k}"] = zen_clean_html($options_li["L_PAYMENTREQUEST_0_NAME{$k}"], 'strong');
            $options_li["L_PAYMENTREQUEST_0_NAME{$k}"] = substr($options_li["L_PAYMENTREQUEST_0_NAME{$k}"], 0, 127);
            $options_li["L_PAYMENTREQUEST_0_AMT{$k}"] = round($options_li["L_PAYMENTREQUEST_0_AMT{$k}"], 2);
            if (isset($options_li["L_PAYMENTREQUEST_0_NUMBER{$k}"])) {
                if ($options_li["L_PAYMENTREQUEST_0_NUMBER{$k}"] == '') {
                    unset($options_li["L_PAYMENTREQUEST_0_NUMBER{$k}"]);
                } else {
                    $options_li["L_PAYMENTREQUEST_0_NUMBER{$k}"] = str_replace(['&', '=', '%'], '*', $options_li["L_PAYMENTREQUEST_0_NUMBER{$k}"]);
                    $options_li["L_PAYMENTREQUEST_0_NUMBER{$k}"] = substr($options_li["L_PAYMENTREQUEST_0_NUMBER{$k}"], 0, 127);
                }
            }
        ...
        for ($j = 0; $j < $k; $j++) {
            $item_amt = $options_li["L_PAYMENTREQUEST_0_AMT{$j}"];
            $item_qty = $options_li["L_PAYMENTREQUEST_0_QTY{$j}"];
            $item_tax = $options_li["L_PAYMENTREQUEST_0_TAXAMT{$j}"] ?? 0;
        ...
        if ($sum_of_line_items == 0) {
            ...
            if ($options_st['PAYMENTREQUEST_0_SHIPPINGAMT'] == $options_st['PAYMENTREQUEST_0_AMT']) {
                $options_st['PAYMENTREQUEST_0_SHIPPINGAMT'] = 0;
            }
        }
        ...
        for ($k = 0, $n = $number_of_line_items_processed + 1; $k < $n; $k++) {
            if (isset($options_li["L_PAYMENTREQUEST_0_TAXAMT{$k}"])) {
                unset($options_li["L_PAYMENTREQUEST_0_TAXAMT{$k}"]);
            }
        }
        // if ITEMAMT >0 and subTotalLI > 0 and they're not equal ... OR subTotalLI minus sumOfLineItems isn't 0
        // check subtotals
        if (strval($options_st['PAYMENTREQUEST_0_ITEMAMT']) > 0 && strval($sub_total_li) > 0 && strval($sub_total_li) != strval($options_st['PAYMENTREQUEST_0_ITEMAMT']) || strval($sub_total_li) - strval($sum_of_line_items) != 0) {
            $this->zc_log('getLineItemDetails 5', 'Line-item subtotals do not add up properly. Line-item-details skipped.' . "\n" . strval($sum_of_line_items) . ' ' . strval($sub_total_li) . ' ' . print_r(array_merge($options_st, $options_li), true));
            $options_li = [];
            $options_li['L_PAYMENTREQUEST_0_NAME0'] = MODULES_PAYMENT_PAYPALWPP_AGGREGATE_CART_CONTENTS;
            $options_li['L_PAYMENTREQUEST_0_AMT0'] = $sum_of_line_items = $sub_total_li = $options_st['PAYMENTREQUEST_0_ITEMAMT'];
        }
        // check whether discounts are causing a problem
        if (strval($options_st['PAYMENTREQUEST_0_ITEMAMT']) <= 0) {
            $pre = ['ST' => $options_st, 'LI' => $options_li];
            $options_st['PAYMENTREQUEST_0_ITEMAMT'] = $options_st['PAYMENTREQUEST_0_AMT'];
            $options_li = [];
            $options_li['L_PAYMENTREQUEST_0_NAME0'] = MODULES_PAYMENT_PAYPALWPP_AGGREGATE_CART_CONTENTS;
            $options_li['L_PAYMENTREQUEST_0_AMT0'] = $sum_of_line_items = $sub_total_li = $options_st['PAYMENTREQUEST_0_ITEMAMT'];
            ...
            $options_st['PAYMENTREQUEST_0_SHIPPINGAMT'] = 0;
            $discount_problems_flag = true;
            $this->zc_log('getLineItemDetails 6', 'Discounts have caused the subtotal to calculate incorrectly. Line-item-details cannot be submitted.' . "\nBefore:" . print_r($pre, true) . "\nAfter:" . print_r(array_merge($options_st, $options_li), true));
        }
        ...
        if ((!isset($options_st['PAYMENTREQUEST_0_AMT']) || $options_st['PAYMENTREQUEST_0_AMT'] == 0 || $flag_subtotals_unknown_yet == true || $options_st['PAYMENTREQUEST_0_ITEMAMT'] == 0) && $discount_problems_flag != true) {
            $options_st['PAYMENTREQUEST_0_ITEMAMT'] = $sum_of_line_items;
            $options_st['PAYMENTREQUEST_0_TAXAMT'] = $sum_of_line_tax;
            if ($sub_total_shipping > 0) {
                $options_st['PAYMENTREQUEST_0_SHIPPINGAMT'] = $sub_total_shipping;
            }
            $options_st['PAYMENTREQUEST_0_AMT'] = $sum_of_line_items + $options_st['PAYMENTREQUEST_0_TAXAMT'] + $options_st['PAYMENTREQUEST_0_SHIPPINGAMT'];
        }
        $this->zc_log('getLineItemDetails 7 - subtotal comparisons', 'BEFORE line-item calcs: ' . print_r($subtotal_pre, true) . ($flag_subtotals_unknown_yet == true ? 'Subtotals Unknown Yet - ' : '') . 'AFTER doing line-item calcs: ' . print_r(array_merge($options_st, $options_li, $options_nb), true));
        // if subtotals are not adding up correctly, then skip sending any line-item or subtotal details to PayPal
        $st_all = round(strval($options_st['PAYMENTREQUEST_0_ITEMAMT'] + $options_st['PAYMENTREQUEST_0_TAXAMT'] + $options_st['PAYMENTREQUEST_0_SHIPPINGAMT'] + $options_st['PAYMENTREQUEST_0_SHIPDISCAMT'] + $options_st['PAYMENTREQUEST_0_HANDLINGAMT'] + $options_st['PAYMENTREQUEST_0_INSURANCEAMT']), 2);
        $st_diff = strval($options_st['PAYMENTREQUEST_0_AMT'] - $st_all);
        $st_diff_rounded = strval($st_all - round($options_st['PAYMENTREQUEST_0_AMT'], 2)) + 0;
        // unset any subtotal values that are zero
        if (isset($options_st['PAYMENTREQUEST_0_ITEMAMT']) && $options_st['PAYMENTREQUEST_0_ITEMAMT'] == 0) {
            unset($options_st['PAYMENTREQUEST_0_ITEMAMT']);
        }
        if (isset($options_st['PAYMENTREQUEST_0_TAXAMT']) && $options_st['PAYMENTREQUEST_0_TAXAMT'] == 0) {
            unset($options_st['PAYMENTREQUEST_0_TAXAMT']);
        }
        if (isset($options_st['PAYMENTREQUEST_0_SHIPPINGAMT']) && $options_st['PAYMENTREQUEST_0_SHIPPINGAMT'] == 0) {
            unset($options_st['PAYMENTREQUEST_0_SHIPPINGAMT']);
        }
        if (isset($options_st['PAYMENTREQUEST_0_SHIPDISCAMT']) && $options_st['PAYMENTREQUEST_0_SHIPDISCAMT'] == 0) {
            unset($options_st['PAYMENTREQUEST_0_SHIPDISCAMT']);
        }
        if (isset($options_st['PAYMENTREQUEST_0_HANDLINGAMT']) && $options_st['PAYMENTREQUEST_0_HANDLINGAMT'] == 0) {
            unset($options_st['PAYMENTREQUEST_0_HANDLINGAMT']);
        }
        if (isset($options_st['PAYMENTREQUEST_0_INSURANCEAMT']) && $options_st['PAYMENTREQUEST_0_INSURANCEAMT'] == 0) {
            unset($options_st['PAYMENTREQUEST_0_INSURANCEAMT']);
        }
    ...
    {
        global $order, $order_totals, $db, $do_pay_pal;
        ...
        if (!isset($_SESSION['cartID']) || $_SESSION['cartID'] === '') {
            $_SESSION['cartID'] = $_SESSION['cart']->cart_id;
        }
        ...
        $options = $this->get_line_item_details($this->select_currency());
        // Set currency and amount
        $options['PAYMENTREQUEST_0_CURRENCYCODE'] = $this->select_currency();
        ...
        $lc_code = $this->get_language_code();
        if ($lc_code != '') {
        ...
        $options['CUSTOMERSERVICENUMBER'] = defined('STORE_TELEPHONE_CUSTSERVICE') && STORE_TELEPHONE_CUSTSERVICE != '' ? substr((string) STORE_TELEPHONE_CUSTSERVICE, 0, 16) : '';
        // Store Name to appear on PayPal page
        $options['BRANDNAME'] = defined('MODULE_PAYMENT_PAYPALWPP_BRANDNAME') && strlen((string) MODULE_PAYMENT_PAYPALWPP_BRANDNAME) > 3 ? substr((string) MODULE_PAYMENT_PAYPALWPP_BRANDNAME, 0, 127) : substr(STORE_NAME, 0, 127);
        // Payment Transaction/Authorization Mode
        $options['PAYMENTREQUEST_0_PAYMENTACTION'] = MODULE_PAYMENT_PAYPALWPP_TRANSACTION_MODE == 'Auth Only' ? 'Authorization' : 'Sale';
        // for future:
        if (MODULE_PAYMENT_PAYPALWPP_TRANSACTION_MODE == 'Order') {
        ...
        // Use 'Mark' for normal Express Checkout (where customer has a PayPal account), 'Sole' for Account-Optional. But Account-Optional must be enabled in your PayPal account under Selling Preferences.
        ...
        // "Billing" or "Login" selects the style of landing page on PayPal site during checkout (ie: which "section" is expanded when arriving there)
        ...
        if ($_SESSION['cart']->get_content_type() == 'virtual' && zen_is_logged_in()) {
            $this->zc_log('ec-step1-addr_check', "cart contents is virtual and customer is logged in ... therefore options['NOSHIPPING']=1");
            $options['NOSHIPPING'] = 1;
        } else {
            $this->zc_log('ec-step1-addr_check', 'cart content is not all virtual (or customer is not logged in) ... therefore will be submitting address details');
            ...
            if (MODULE_PAYMENT_PAYPALWPP_CONFIRMED_ADDRESS != 'Yes' && ($address_arr = $this->get_override_address()) !== false) {
                $address_error = false;
                foreach (['entry_firstname', 'entry_lastname', 'entry_street_address', 'entry_city', 'entry_postcode', 'zone_code', 'countries_iso_code_2'] as $val) {
                    ...
                    if ($address_error == true) {
                        $this->zc_log('ec-step1-addr_check2', '$address_error = true because ' . $val . ' is blank.');
                    }
                }
                if ($address_error == false) {
                    ...
                    $options['PAYMENTREQUEST_0_SHIPTOSTREET'] = substr((string) $address_arr['entry_street_address'], 0, 100);
                    if ($address_arr['entry_suburb'] != '') {
                        $options['PAYMENTREQUEST_0_SHIPTOSTREET2'] = substr((string) $address_arr['entry_suburb'], 0, 100);
                    }
                    $options['PAYMENTREQUEST_0_SHIPTOCITY'] = substr((string) $address_arr['entry_city'], 0, 40);
                    $options['PAYMENTREQUEST_0_SHIPTOZIP'] = substr((string) $address_arr['entry_postcode'], 0, 20);
                    $options['PAYMENTREQUEST_0_SHIPTOSTATE'] = substr((string) $address_arr['zone_code'], 0, 40);
                    $options['PAYMENTREQUEST_0_SHIPTOCOUNTRYCODE'] = substr((string) $address_arr['countries_iso_code_2'], 0, 2);
                    if (!empty($address_arr['entry_telephone'])) {
                        $options['PAYMENTREQUEST_0_SHIPTOPHONENUM'] = substr((string) $address_arr['entry_telephone'], 0, 20);
                    }
                }
            }
            $this->zc_log('ec-step1-addr_check3', 'address details from override check:' . (empty($address_arr) ? ' <NONE FOUND>' : print_r($address_arr, true)));
        ...
        if (!empty($_SESSION['customer_first_name']) && !empty($_SESSION['customer_id'])) {
            ...
            $sql = $db->bind_vars($sql, ':custID', $_SESSION['customer_id'], 'integer');
            $zc_getemail = $db->Execute($sql);
            if ($zc_getemail->record_count() > 0 && $zc_getemail->fields['customers_email_address'] != '') {
                $options['EMAIL'] = $zc_getemail->fields['customers_email_address'];
            }
            if ($zc_getemail->record_count() > 0 && $zc_getemail->fields['customers_telephone'] != '' && (!isset($options['ADDROVERRIDE']) || isset($options['ADDROVERRIDE']) && $options['ADDROVERRIDE'] != 1)) {
        ...
        $this->zc_log('ec_step1 - 2 -submit', print_r(array_merge($options, ['RETURNURL' => $return_url, 'CANCELURL' => $cancel_url]), true));
         ...
         */
        $response = $do_pay_pal->set_express_checkout($return_url, $cancel_url, $options);
        $this->notify('NOTIFY_PAYMENT_PAYPALEC_TOKEN', $response, $options);
        $submission_check_one = true;
        $submission_check_two = true;
        ...
        if (isset($response['L_ERRORCODE0']) && $response['L_ERRORCODE0'] == '10413') {
            $this->zc_log('ec_step1 - 3 - removing tax portion', 'Tax Subtotal does not match sum of taxes for line-items. Tax details removed from line-item submission data.' . "\n" . print_r($options, true));
            //echo '1st submission REJECTED. {'.$response['L_ERRORCODE0'].'}<pre>'.print_r($options, true) . urldecode(print_r($response, true));
            $tsubtotal = 0;
            foreach ($options as $key => $value) {
                if (str_ends_with((string) $key, 'TAXAMT')) {
                    $tsubtotal += preg_replace('/[^0-9.\-]/', '', (string) $value);
                    unset($options[$key]);
                }
            }
            $options['PAYMENTREQUEST_0_TAXAMT'] = $tsubtotal;
            $amt = preg_replace('/[^0-9.%]/', '', (string) $options['PAYMENTREQUEST_0_AMT']);
            //      echo 'oldAMT:'.$amt;
            //      echo ' newTAXAMT:'.$tsubtotal;
            $taxamt = preg_replace('/[^0-9.%]/', '', $options['PAYMENTREQUEST_0_TAXAMT']);
            $shipamt = preg_replace('/[^0-9.%]/', '', (string) $options['PAYMENTREQUEST_0_SHIPPINGAMT']);
            $itemamt = preg_replace('/[^0-9.%]/', '', (string) $options['PAYMENTREQUEST_0_ITEMAMT']);
            $calculated_amount = $itemamt + $taxamt + $shipamt;
            if ($amt != $calculated_amount) {
        ...
        if (isset($response['L_ERRORCODE0']) && $response['L_ERRORCODE0'] == '10413') {
            $this->zc_log('ec_step1 - 4 - removing line-item details', 'PayPal designed their own mathematics rules. Dumbing it down for them.' . "\n" . print_r($options, true));
            //echo '2nd submission REJECTED. {'.$response['L_ERRORCODE0'].'}<pre>'.print_r($options, true) . urldecode(print_r($response, true));
            foreach ($options as $key => $value) {
                if (str_starts_with((string) $key, 'L_')) {
                    unset($options[$key]);
                }
            }
            $amt = preg_replace('/[^0-9.%]/', '', (string) $options['PAYMENTREQUEST_0_AMT']);
            $taxamt = preg_replace('/[^0-9.%]/', '', (string) $options['PAYMENTREQUEST_0_TAXAMT']);
            $shipamt = preg_replace('/[^0-9.%]/', '', (string) $options['PAYMENTREQUEST_0_SHIPPINGAMT']);
            $itemamt = preg_replace('/[^0-9.%]/', '', (string) $options['PAYMENTREQUEST_0_ITEMAMT']);
            $calculated_amount = $itemamt + $taxamt + $shipamt;
            if ($amt != $calculated_amount) {
         ...
         */
        $error = $this->_error_handler($response, 'SetExpressCheckout');
        // Success, so read the EC token
        $_SESSION['paypal_ec_token'] = preg_replace('/[^0-9.A-Z\-]/', '', urldecode((string) $response['TOKEN']));
        // prepare to redirect to PayPal so the customer can log in and make their selections
        $paypal_url = $this->get_pay_pal_login_server();
        ...
        $this->ec_redirect_url = $paypal_url . '?cmd=_express-checkout&token=' . $_SESSION['paypal_ec_token'] . $user_action_key;
    ...
    public function ec_step2(): void
    {
        ...
        if (empty($_SESSION['paypal_ec_token'])) {
            ...
            if (isset($_GET['token'])) {
                ...
                // sanitize this
                $_SESSION['paypal_ec_token'] = preg_replace('/[^0-9.A-Z\-]/', '', (string) $_GET['token']);
            } else {
                // no token -- not ready for this step -- send them back to checkout page with error
                $this->terminate_ec(MODULE_PAYMENT_PAYPALWPP_INVALID_RESPONSE, true);
            }
        }
        // debug
        ...
        $response = $do_pay_pal->get_express_checkout_details($_SESSION['paypal_ec_token']);
         ...
         * @since ZC v2.2.0
         */
        $error = $this->_error_handler($response, 'GetExpressCheckoutDetails');
        // Fill in possibly blank return values, prevents PHP notices in follow-on checking clause.
        $response_vars = [$this->request_prefix . 'SHIPTONAME', $this->request_prefix . 'SHIPTOSTREET', $this->request_prefix . 'SHIPTOSTREET2', $this->request_prefix . 'SHIPTOCITY', $this->request_prefix . 'SHIPTOSTATE', $this->request_prefix . 'SHIPTOZIP', $this->request_prefix . 'SHIPTOCOUNTRYCODE'];
        $address_received = '';
        ...
        if ($_SESSION['cart']->get_content_type() != 'virtual' && $address_received == '') {
            $this->terminate_ec(MODULES_PAYMENT_PAYPALWPP_TEXT_BLANK_ADDRESS, true, FILENAME_CREATE_ACCOUNT);
        }
        ...
        $response_optional = ['PAYMENTREQUEST_0_SHIPTOPHONENUM', 'PAYERID', 'EMAIL', 'PHONENUM', 'FIRSTNAME', 'LASTNAME', 'BUSINESS', 'PAYERSTATUS'];
        ...
        $step2_payerinfo = ['payer_id' => $response['PAYERID'], 'payer_email' => urldecode((string) $response['EMAIL']), 'payer_salutation' => '', 'payer_gender' => '', 'payer_firstname' => urldecode((string) $response['FIRSTNAME']), 'payer_lastname' => urldecode((string) $response['LASTNAME']), 'payer_business' => urldecode((string) $response['BUSINESS']), 'payer_status' => $response['PAYERSTATUS'], 'ship_country_code' => urldecode((string) $response[$this->request_prefix . 'SHIPTOCOUNTRYCODE']), 'ship_address_status' => empty($response[$this->request_prefix . 'ADDRESSSTATUS']) ? 'NONE' : urldecode((string) $response[$this->request_prefix . 'ADDRESSSTATUS']), 'ship_phone' => urldecode($response[$this->request_prefix . 'SHIPTOPHONENUM'] != '' ? $response[$this->request_prefix . 'SHIPTOPHONENUM'] : $response['PHONENUM']), 'order_comment' => isset($response['NOTE'], $response[$this->request_prefix . 'NOTETEXT']) ? urldecode((string) $response['NOTE']) . ' ' . urldecode((string) $response[$this->request_prefix . 'NOTETEXT']) : ''];
        ...
        if ($response[$this->request_prefix . 'SHIPTOSTREET2'] == $response[$this->request_prefix . 'SHIPTOSTREET']) {
            $response[$this->request_prefix . 'SHIPTOSTREET2'] = '';
        }
        // accomodate PayPal bug which incorrectly treats 'Yukon Territory' as YK instead of ISO standard of YT.
        if ($response[$this->request_prefix . 'SHIPTOSTATE'] === 'YK') {
            $response[$this->request_prefix . 'SHIPTOSTATE'] = 'YT';
        }
        // same with Newfoundland
        if ($response[$this->request_prefix . 'SHIPTOSTATE'] === 'NF') {
            $response[$this->request_prefix . 'SHIPTOSTATE'] = 'NL';
        }
        // process address details supplied
        $step2_shipto = ['ship_name' => urldecode((string) $response[$this->request_prefix . 'SHIPTONAME']), 'ship_street_1' => urldecode((string) $response[$this->request_prefix . 'SHIPTOSTREET']), 'ship_street_2' => urldecode((string) $response[$this->request_prefix . 'SHIPTOSTREET2']), 'ship_city' => urldecode((string) $response[$this->request_prefix . 'SHIPTOCITY']), 'ship_state' => ($response[$this->request_prefix . 'SHIPTOSTATE'] ?? '') !== '' ? urldecode((string) $response[$this->request_prefix . 'SHIPTOSTATE']) : urldecode((string) $response[$this->request_prefix . 'SHIPTOCITY']), 'ship_postal_code' => urldecode((string) $response[$this->request_prefix . 'SHIPTOZIP']), 'ship_country_code' => urldecode((string) $response[$this->request_prefix . 'SHIPTOCOUNTRYCODE']), 'ship_country_name' => urldecode((string) ($response['SHIPTOCOUNTRY'] ?? $response['SHIPTOCOUNTRYNAME'] ?? ''))];
        ...
        if (!isset($order) || !isset($order->info) || !is_array($order->info) || !zen_not_null($order)) {
            $this->zc_log('ec_step2 ', 'Re-instantiating $order object.');
            // -----
            ...
            $order_totals = $order_total_modules->pre_confirmation_check();
            $order_totals = $order_total_modules->process();
            $this->zc_log('ec_step2 ', 'Instantiated $order object contents: ' . print_r($order, true));
        }
        if ($order->info['total'] < 0.01 && urldecode((string) $response[$this->request_prefix . 'AMT']) > 0) {
            $order->info['total'] = urldecode((string) $response[$this->request_prefix . 'AMT']);
        }
    ...
    public function notify_order_cart_address_overrides(&$order, string $e, $unused, array &$customer_address_override, array &$delivery_address_override, array &$billing_address_override): void
    {
        ...
        $this->zc_log('ec_step2 - overrides - 1', 'START: paypal_ec_payer_info= ' . print_r($paypal_ec_payer_info, true));
        /**
        ...
        $sql = 'SELECT countries_id, address_format_id, countries_iso_code_2, countries_iso_code_3
              ...
              LIMIT 1';
        $sql1 = $db->bind_vars($sql, ':countryId', $paypal_ec_payer_info['ship_country_name'], 'string');
        $country1 = $db->Execute($sql1);
        $sql2 = $db->bind_vars($sql, ':countryId', $paypal_ec_payer_info['ship_country_code'], 'string');
        ...
        if (!$country1->EOF) {
            ...
            $sql = $db->bind_vars($sql, ':countryId:', $country_id, 'integer');
            $result = $db->Execute($sql);
            if ($result->EOF) {
                $this->notify('NOTIFY_PAYPAL_CUSTOMER_ATTEMPT_TO_USE_INVALID_COUNTRY_CODE');
                $this->zc_log('ec-step2 - override - 1b', 'Cannot use address due to country lookup/match failure.');
                $this->terminate_ec(MODULE_PAYMENT_PAYPALWPP_TEXT_INVALID_ZONE_ERROR, true, FILENAME_SHOPPING_CART);
            }
        }
        ...
        $sql = 'SELECT zone_id
              ...
              LIMIT 1';
        $sql = $db->bind_vars($sql, ':zCountry', $country_id, 'integer');
        $sql = $db->bind_vars($sql, ':zoneCode', $paypal_ec_payer_info['ship_state'], 'string');
        ...
        $customer_address_override['country'] = ['id' => $country_id, 'title' => $paypal_ec_payer_info['ship_country_name'], 'iso_code_2' => $paypal_ec_payer_info['ship_country_code'], 'iso_code_3' => $country_code3];
        ...
        $billing_address_override['country'] = ['id' => $country_id, 'title' => $paypal_ec_payer_info['ship_country_name'], 'iso_code_2' => $paypal_ec_payer_info['ship_country_code'], 'iso_code_3' => $country_code3];
        $billing_address_override['format_id'] = $address_format_id;
        $billing_address_override['zone_id'] = $state_id;
        // delivery
        if (strtoupper((string) $_SESSION['paypal_ec_payer_info']['ship_address_status']) !== 'NONE') {
            ...
            $delivery_address_override['country'] = ['id' => $country_id, 'title' => $paypal_ec_payer_info['ship_country_name'], 'iso_code_2' => $paypal_ec_payer_info['ship_country_code'], 'iso_code_3' => $country_code3];
    ...
    protected function ec_step2_finish(): void
    {
        ...
        if (!empty($_SESSION['customer_first_name']) && zen_is_logged_in()) {
            ...
            if ($bypass_address_creation) {
                $this->zc_log('ec_step2_finish - 2a', 'address-creation bypassed based on observer setting.');
            }
            ...
            if (!$bypass_address_creation && strtoupper((string) $_SESSION['paypal_ec_payer_info']['ship_address_status']) != 'NONE' && $_SESSION['paypal_ec_payer_info']['ship_street_1'] != '') {
                ...
                $address_book_id = $this->find_matching_address_book_entry($_SESSION['customer_id'], $order->delivery ?? $order->billing);
                // no match, so add the record
                if (!$address_book_id) {
                    $address_book_id = $this->add_address_book_entry($_SESSION['customer_id'], $order->delivery ?? $order->billing, false);
                }
            ...
            if (!empty($_SESSION['paypal_ec_markflow']) && $_SESSION['paypal_ec_markflow'] == 1) {
                $this->terminate_ec('', false, FILENAME_CHECKOUT_PROCESS);
            } else {
                $this->terminate_ec('', false, FILENAME_CHECKOUT_CONFIRMATION);
            }
        } else {
            ...
            if (trim((string) $paypal_ec_payer_info['payer_email']) == '') {
                $this->terminate_ec(MODULE_PAYMENT_PAYPALWPP_INVALID_RESPONSE, true);
            }
            ...
            $sql = $db->bind_vars($sql, ':emailAddress', $paypal_ec_payer_info['payer_email'], 'string');
            $check_customer = $db->Execute($sql);
            // debug
            $this->zc_log('ec_step2_finish - 4', 'Not logged in. Looking for account.' . "\n" . (int) $check_customer->record_count() . ' matching customer records found.');
            if (!$check_customer->EOF) {
                ...
                if ($check_customer->fields['customers_paypal_ec'] == '1') {
                    ...
                    $acct_exists = false;
                    // debug
                    $this->zc_log('ec_step2_finish - 5', 'Found temporary account - deleting it.');
                }
            }
            // Create an account, if the account does not exist
            if (!$acct_exists) {
                // debug
                $this->zc_log('ec_step2_finish - 6', 'No ZC account found for this customer. Creating new account.' . "\n" . '$this->new_acct_notify =' . $this->new_acct_notify);
                // Generate a random 8-char password
                $password = zen_create_random_value(8);
                // set the customer information in the array for the table insertion
                $sql_data_array = ['customers_firstname' => $paypal_ec_payer_info['payer_firstname'], 'customers_lastname' => $paypal_ec_payer_info['payer_lastname'], 'customers_email_address' => $paypal_ec_payer_info['payer_email'], 'customers_email_format' => ACCOUNT_EMAIL_PREFERENCE == '1' ? 'HTML' : 'TEXT', 'customers_telephone' => $paypal_ec_payer_info['ship_phone'], 'customers_fax' => '', 'customers_gender' => $paypal_ec_payer_info['payer_gender'], 'customers_newsletter' => '0', 'customers_password' => zen_encrypt_password($password), 'customers_paypal_payerid' => $_SESSION['paypal_ec_payer_id']];
                // insert the data
                ...
                $sql_data_array = ['customers_id' => $customer_id, 'entry_gender' => $paypal_ec_payer_info['payer_gender'], 'entry_firstname' => $paypal_ec_payer_info['payer_firstname'], 'entry_lastname' => $paypal_ec_payer_info['payer_lastname'], 'entry_street_address' => $paypal_ec_payer_info['ship_street_1'], 'entry_suburb' => $paypal_ec_payer_info['ship_street_2'], 'entry_city' => $paypal_ec_payer_info['ship_city'], 'entry_zone_id' => $state_id, 'entry_postcode' => $paypal_ec_payer_info['ship_postal_code'], 'entry_country_id' => $country_id];
                ...
                $sql = $db->bind_vars($sql, ':addrID', $address_id, 'integer');
                $sql = $db->bind_vars($sql, ':custID', $customer_id, 'integer');
                $db->Execute($sql);
            ...
            // debug
            $this->zc_log('ec_step2_finish - 7', 'Auto-Logged customer in. (' . $_SESSION['paypal_ec_payer_info']['payer_email'] . ') (' . $_SESSION['customer_id'] . ')' . "\n" . '$_SESSION[paypal_ec_temp]=' . $_SESSION['paypal_ec_temp']);
            // This is the address matching section
            // try to match it first
            // note: this is by no means 100%
            $address_book_id = $this->find_matching_address_book_entry($_SESSION['customer_id'], $order->delivery ?? $order->billing);
            ...
            // debug
            $this->zc_log('ec_step2_finish - 8', 'Exiting via terminateEC (from originally-not-logged-in mode).' . "\n" . 'Selected address: ' . $address_book_id . "\nOriginal was: " . (int) $original_default_address_id . "\nprepared data: " . print_r($order->customer, true));
     ...
     * @since ZC v1.3.7
     */
    public function set_shipping_method(): void
    {
    ...
    public function get_override_address()
    {
        ...
        if (!empty($_GET['markflow']) && zen_is_logged_in()) {
            ...
            $_SESSION['paypal_ec_markflow'] = 1;
            // debug
            $this->zc_log('getOverrideAddress - 1', 'Now in markflow mode.' . "\n" . 'SESSION[sendto] = ' . (int) $_SESSION['sendto']);
            ...
            if (!empty($_SESSION['sendto'])) {
                ...
                $sql = $db->bind_vars($sql, ':customerId', $_SESSION['customer_id'], 'integer');
            ...
            if ($disable_address_override === true) {
                $this->zc_log('getOverrideAddress - 1a', "Override disabled by observer request.\n");
                return false;
            }
            ...
            $sql = 'SELECT entry_firstname, entry_lastname, entry_company,
              ...
              LIMIT 1';
            $sql = $db->bind_vars($sql, ':addressId', $address_id, 'integer');
            $sql = $db->bind_vars($sql, ':customerId', $_SESSION['customer_id'], 'integer');
            ...
            if (!$address_arr->EOF) {
                ...
                $sql = $db->bind_vars($sql, ':zoneId', $address_arr->fields['entry_zone_id'], 'integer');
                ...
                $sql = $db->bind_vars($sql, ':countryId', $address_arr->fields['entry_country_id'], 'integer');
                ...
                // debug
                $this->zc_log('getOverrideAddress - 2', '$address_arr->fields = ' . print_r($address_arr->fields, true));
                // return address data.
                return $address_arr->fields;
            }
            // debug
            $this->zc_log('getOverrideAddress - 3', 'no override record found');
        }
        // debug
        $this->zc_log('getOverrideAddress - 4', 'not logged in and not in markflow mode - nothing to override');
        return false;
    }
    /**
     ...
     * @since ZC v1.3.7
     */
    public function find_matching_address_book_entry($customer_id, array $address_question_arr)
    {
        global $db;
        ...
        $sql = 'SELECT countries_id, address_format_id
                ...
                LIMIT 1';
        $sql = $db->bind_vars($sql, ':countryCode:', $address_question_arr['country']['iso_code_2'], 'string');
        $sql = $db->bind_vars($sql, ':countryName:', $address_question_arr['country']['title'], 'string');
        ...
        $sql = 'SELECT zone_id
            ...
            LIMIT 1';
        $sql = $db->bind_vars($sql, ':zoneId', $country_id, 'integer');
        $country_zone_check = $db->Execute($sql);
        $check_zone = $country_zone_check->record_count();
        $zone_id = 0;
        $log_msg = ['zone_id' => '-not found-'];
        ...
        if ($check_zone) {
            $sql = 'SELECT zone_id
              ...
              LIMIT 1';
            $sql = $db->bind_vars($sql, ':zoneId', $country_id, 'integer');
            $sql = $db->bind_vars($sql, ':zoneCode', $address_question_arr['state'], 'string');
            $zone = $db->Execute($sql);
            if (!$zone->EOF) {
                // grab the id
        ...
        // debug
        $this->zc_log('findMatchingAddressBookEntry - 1-stats', 'lookups:' . "\n" . print_r(array_merge($country->fields, ['zone_country_id' => $country_zone_check->fields], $log_msg), true) . "\n" . 'check_zone: ' . $check_zone . "\n" . 'zone:' . $zone_id . "\nSubmittedAddress:" . print_r($address_question_arr, true));
        ...
        $sql = $db->bind_vars($sql, ':zoneId', $zone_id, 'integer');
        $sql = $db->bind_vars($sql, ':countryId', $country_id, 'integer');
        $sql = $db->bind_vars($sql, ':customerId', $customer_id, 'integer');
        $answers_arr = $db->Execute($sql);
        // debug
        $this->zc_log('findMatchingAddressBookEntry - 2-read for match', "\nLookup RecordCount = " . $answers_arr->record_count());
        if (!$answers_arr->EOF) {
            ...
            $match_question = str_replace("\n", '', $address_question_arr['street_address']);
            $match_question = trim($match_question);
            $match_question = $match_question . str_replace("\n", '', $address_question_arr['suburb']);
            $match_question = $match_question . str_replace("\n", '', $address_question_arr['city']);
            $match_question = str_replace("\t", '', $match_question);
            $match_question = trim($match_question);
            $match_question = strtolower($match_question);
            $match_question = str_replace(' ', '', $match_question);
            // go through the data
            while (!$answers_arr->EOF) {
                ...
                $from_db = trim($from_db);
                if (!empty($answers_arr->fields['entry_suburb'])) {
                    $from_db = $from_db . str_replace("\n", '', $answers_arr->fields['entry_suburb']);
                }
                $from_db = $from_db . str_replace("\n", '', $answers_arr->fields['entry_city']);
                $from_db = str_replace("\t", '', $from_db);
                $from_db = trim($from_db);
                $from_db = strtolower($from_db);
                $from_db = str_replace(' ', '', $from_db);
                // debug
                $this->zc_log('findMatchingAddressBookEntry - 3a', "From PayPal:\r\n" . $match_question . "\r\n\r\nFrom DB:\r\n" . $from_db . "\r\n" . print_r($answers_arr->fields, true));
                // check the strings
                if (strlen($from_db) == strlen($match_question)) {
                    if ($from_db == $match_question) {
                        // exact match return the id
                        // debug
                        $this->zc_log('findMatchingAddressBookEntry - 3b', "Exact match:\n" . print_r($answers_arr->fields, true));
                        return $answers_arr->fields['address_book_id'];
                    }
                } elseif (strlen($from_db) > strlen($match_question)) {
                    if (str_starts_with($from_db, $match_question)) {
                        // we have a match return it (PP)
                        // debug
                        $this->zc_log('findMatchingAddressBookEntry - 3b', "partial match (PP):\n" . print_r($answers_arr->fields, true));
                        return $answers_arr->fields['address_book_id'];
                    }
                } else if (str_starts_with($match_question, $from_db)) {
                    // we have a match return it (DB)
                    // debug
                    $this->zc_log('findMatchingAddressBookEntry - 3b', "partial match (DB):\n" . print_r($answers_arr->fields, true));
                    return $answers_arr->fields['address_book_id'];
                }
                $answers_arr->move_next();
            }
        }
        // debug
        $this->zc_log('findMatchingAddressBookEntry - 4', 'no match');
        // no matches found
        return false;
    }
    /**
     ...
     * @since ZC v1.3.7
     */
    public function add_address_book_entry($customer_id, array $address_question_arr, $make_default = false)
    {
        global $db;
        // debug
        $this->zc_log('addAddressBookEntry - 1', 'address to add: ' . "\n" . print_r($address_question_arr, true));
        // if address is blank, don't do any matching
        if ($address_question_arr['street_address'] == '') {
            return false;
        }
        ...
        if (!$country->EOF) {
            ...
            if ($result->EOF) {
                ...
                $this->zc_log('addAddressBookEntry - 3', 'Failed to add address due to country restrictions');
                return false;
            }
        }
        // see if the country code has a state
        ...
        // truncate long data
        $address_question_arr['company'] = substr((string) $address_question_arr['company'], 0, zen_field_length(TABLE_ADDRESS_BOOK, 'entry_company'));
        $address_question_arr['street_address'] = substr((string) $address_question_arr['street_address'], 0, zen_field_length(TABLE_ADDRESS_BOOK, 'entry_street_address'));
        $address_question_arr['suburb'] = substr((string) $address_question_arr['suburb'], 0, zen_field_length(TABLE_ADDRESS_BOOK, 'entry_suburb'));
        $address_question_arr['city'] = substr((string) $address_question_arr['city'], 0, zen_field_length(TABLE_ADDRESS_BOOK, 'entry_city'));
        $address_question_arr['state'] = substr((string) $address_question_arr['state'], 0, zen_field_length(TABLE_ADDRESS_BOOK, 'entry_state'));
        $address_question_arr['postcode'] = substr((string) $address_question_arr['postcode'], 0, zen_field_length(TABLE_ADDRESS_BOOK, 'entry_postcode'));
        // now run the insert
        // this isn't the best way to get fname/lname but it will get the majority of cases
        [$fname, $lname] = explode(' ', (string) $address_question_arr['name']);
        $sql_data_array = [['fieldName' => 'entry_firstname', 'value' => $fname, 'type' => 'string'], ['fieldName' => 'entry_lastname', 'value' => $lname, 'type' => 'string'], ['fieldName' => 'entry_street_address', 'value' => $address_question_arr['street_address'], 'type' => 'string'], ['fieldName' => 'entry_postcode', 'value' => $address_question_arr['postcode'], 'type' => 'string'], ['fieldName' => 'entry_city', 'value' => $address_question_arr['city'], 'type' => 'string'], ['fieldName' => 'entry_country_id', 'value' => $country_id, 'type' => 'integer']];
        if ($address_question_arr['company'] != '' && $address_question_arr['company'] != $address_question_arr['name']) {
            ['fieldName' => 'entry_company', 'value' => $address_question_arr['company'], 'type' => 'string'];
        }
        ...
        if ($make_default) {
            ...
            $where_clause = $db->bind_vars($where_clause, ':customersID', $customer_id, 'integer');
        ...
        // debug
        $this->zc_log('addAddressBookEntry - 2', 'added address #' . $new_address_book_id . "\n" . 'SESSION[sendto] is now set to ' . $_SESSION['sendto']);
     ...
     * @since ZC v1.3.7
     */
    public function user_login($email_address, $redirect = true): bool
    {
        global $db, $order, $message_stack;
        ...
        $sql = $db->bind_vars($sql, ':custEmail', $email_address, 'string');
        $check_customer = $db->Execute($sql);
        if ($check_customer->EOF) {
            $this->terminate_ec(MODULE_PAYMENT_PAYPALWPP_TEXT_BAD_LOGIN, true);
        }
        ...
        $sql = $db->bind_vars($sql, ':custID', $check_customer->fields['customers_id'], 'integer');
        $sql = $db->bind_vars($sql, ':addrID', $check_customer->fields['customers_default_address_id'], 'integer');
            ...
            if (SHOW_SHOPPING_CART_COMBINED !== '0' && $zc_check_basket_before > 0) {
                if ($zc_check_basket_before != $zc_check_basket_after && $zc_check_basket_after > 0) {
                    if (SHOW_SHOPPING_CART_COMBINED === '2') {
                        ...
                        $message_stack->add_session('header', WARNING_SHOPPING_CART_COMBINED, 'caution');
                    } else {
                        // show warning and send to shopping cart for review ('1')
                        $message_stack->add_session('shopping_cart', WARNING_SHOPPING_CART_COMBINED, 'caution');
     ...
     * @since ZC v1.3.7
     */
    public function ec_delete_user($cid): void
    {
        global $db;
     ...
     * @since ZC v1.3.7
     */
    public function terminate_ec($error_msg = '', $kill_sess_vars = false, ?string $goto_page = ''): void
    {
        global $message_stack, $order, $order_total_modules;
        $error_msg = trim((string) $error_msg);
        if (str_ends_with($error_msg, '-')) {
            $error_msg = trim(substr($error_msg, 0, strlen($error_msg) - 1));
        }
        $stack_alert = 'header';
        // debug
        $this->_do_debug('PayPal test Log - terminateEC-A', 'goto page: ' . $goto_page . "\nerror_msg: " . $error_msg . "\n\nSession data: " . print_r($_SESSION, true));
        if ($kill_sess_vars) {
            ...
            unset($_SESSION['paypal_ec_markflow']);
            // debug
            $this->zc_log('termEC-1', 'Killed the session vars as requested');
        }
        $this->zc_log('termEC-2', 'BEFORE: $this->showPaymentPage = ' . (int) $this->show_payment_page . "\nToken Data:" . (!isset($_SESSION['paypal_ec_token']) ? 'not set' : $_SESSION['paypal_ec_token']));
        // force display of payment page if GV/DC active for this customer
        if (MODULE_ORDER_TOTAL_INSTALLED && $this->show_payment_page !== true && isset($_SESSION['paypal_ec_token'])) {
        ...
        // debug
        $this->zc_log('termEC-3', 'AFTER: $this->showPaymentPage = ' . (int) $this->show_payment_page);
        if (!empty($_SESSION['customer_first_name']) && !empty($_SESSION['customer_id'])) {
            if ($this->show_payment_page === true || $goto_page == FILENAME_CHECKOUT_PAYMENT) {
                // debug
                $this->zc_log('termEC-4', 'We ARE logged in, and $this->showPaymentPage === true');
                ...
                if (!isset($_SESSION['shipping']) || !isset($_SESSION['shipping']['cost']) || $_SESSION['shipping']['cost'] < 0) {
                    // debug
                    $this->zc_log('termEC-5', 'Have no shipping method selected, or shipping < 0 so set FILENAME_CHECKOUT_SHIPPING');
                    $redirect_path = FILENAME_CHECKOUT_SHIPPING;
                    $stack_alert = 'checkout_shipping';
                } else {
                    // debug
                    $this->zc_log('termEC-6', 'We DO have a shipping method selected, so goto PAYMENT');
                    $redirect_path = FILENAME_CHECKOUT_PAYMENT;
                    $stack_alert = 'checkout_payment';
                }
            } elseif ($goto_page) {
                // debug
                $this->zc_log('termEC-7', '$this->showPaymentPage NOT true, and have custom page parameter: ' . $goto_page);
                $redirect_path = $goto_page;
                $stack_alert = 'header';
                if ($goto_page == FILENAME_SHOPPING_CART) {
                    $stack_alert = 'shopping_cart';
                }
            } else {
                // debug
                $this->zc_log('termEC-8', '$this->showPaymentPage NOT true, and NO custom page selected ... using SHIPPING as default');
                $redirect_path = FILENAME_CHECKOUT_SHIPPING;
                $stack_alert = 'checkout_shipping';
            }
        } else {
            // debug
            $this->zc_log('termEC-9', 'We are NOT logged in, so set snapshot to Shipping page, and redirect to login');
            $_SESSION['navigation']->set_snapshot(FILENAME_CHECKOUT_SHIPPING);
            $redirect_path = FILENAME_LOGIN;
            $stack_alert = 'login';
        }
        if ($error_msg) {
            $message_stack->add_session($stack_alert, $error_msg, 'error');
        }
        // debug
        $this->zc_log('termEC-10', 'Redirecting to ' . $redirect_path . ' - Stack: ' . $stack_alert . "\n" . 'Message: ' . $error_msg . "\nSession Data: " . print_r($_SESSION, true));
     ...
     * @since ZC v1.3.7
     */
    public function _error_handler(array $response, string $operation = '', $ignore_codes = '')
    {
        global $message_stack, $do_pay_pal;
        $basic_error = !$response || isset($response['RESULT']) && $response['RESULT'] != 0 || isset($response['ACK']) && !strstr($response['ACK'], 'Success') || !isset($response['RESULT']) && !isset($response['ACK']);
        $ignore_list = explode(',', str_replace(' ', '', $ignore_codes));
        ...
        if (!empty($response['RESULT']) && $response['RESULT'] == 'Unauthorized: Unilateral') {
            $error_text = $response['RESULT'] . MODULE_PAYMENT_PAYPALWPP_TEXT_UNILATERAL;
            $message_stack->add_session($error_text, 'error');
        }
        /* Handle FMF Scenarios **/
        $response['L_ERRORCODE0'] = empty($response['L_ERRORCODE0']) ? 0 : $response['L_ERRORCODE0'];
        $response['L_LONGMESSAGE2'] = empty($response['L_LONGMESSAGE2']) ? '' : $response['L_LONGMESSAGE2'];
        if (in_array($operation, ['DoExpressCheckoutPayment', 'DoDirectPayment']) && (isset($response['PAYMENTINFO_0_PAYMENTSTATUS']) && $response['PAYMENTINFO_0_PAYMENTSTATUS'] == 'Pending' || isset($response['PAYMENTSTATUS']) && $response['PAYMENTSTATUS'] == 'Pending') && $response['L_ERRORCODE0'] == 11610) {
            $this->fmf_response = urldecode((string) $response['L_SHORTMESSAGE0']);
            $this->fmf_errors = [];
            if ($response['ACK'] == 'SuccessWithWarning' && isset($response['L_PAYMENTINFO_0_FMFPENDINGID0'])) {
                for ($i = 0; $i < 20; $i++) {
                    ...
                    $this->fmf_errors[] = ['key' => $pendingid, 'status' => $pendingid, 'desc' => $response["L_PAYMENTINFO_0_FMFPENDINGDESCRIPTION{$i}"]];
                }
            }
            return sizeof($this->fmf_errors) > 0 ? $this->fmf_errors : false;
        }
        ...
        $error_info = 'Problem occurred while customer ' . zen_output_string_protected($customer_info) . ' was attempting checkout with PayPal Express Checkout.' . "\n\n";
        $this->notify('NOTIFY_PAYPALWPP_ERROR_HANDLER', $response, $operation, $basic_error, $ignore_list, $error_info);
        switch ($operation) {
            case 'SetExpressCheckout':
                if ($basic_error) {
                    // if error, display error message. If debug options enabled, email dump to store owner
                    if ($this->enable_debugging) {
                        $this->_do_debug('PayPal Error Log - ec_step1()', "In function: ec_step1()\r\n\r\nValue List:\r\n" . str_replace('&', "\r\n", $do_pay_pal->_sanitize_log($do_pay_pal->_parse_name_value_list($do_pay_pal->last_param_list))) . "\r\n\r\nResponse:\r\n" . print_r($response, true));
                    }
                    $error_text = MODULE_PAYMENT_PAYPALWPP_TEXT_GEN_ERROR;
                    $error_num = urldecode($response['L_ERRORCODE0'] . ($response['RESULT'] ?? ''));
                    if (isset($response['RESULT']) && $response['RESULT'] == 25) {
                        $error_text = MODULE_PAYMENT_PAYPALWPP_TEXT_NOT_WPP_ACCOUNT_ERROR;
                    }
                    if ($response['L_ERRORCODE0'] == 10002) {
                        $error_text = MODULE_PAYMENT_PAYPALWPP_TEXT_SANDBOX_VS_LIVE_ERROR;
                    }
                    if ($response['L_ERRORCODE0'] == 10565) {
                        $error_text = MODULE_PAYMENT_PAYPALWPP_TEXT_WPP_BAD_COUNTRY_ERROR;
                        $_SESSION['payment'] = '';
                    }
                    if ($response['L_ERRORCODE0'] == 10736) {
                        $error_text = MODULE_PAYMENT_PAYPALWPP_TEXT_ADDR_ERROR;
                    }
                    if ($response['L_ERRORCODE0'] == 10752) {
                        $error_text = MODULE_PAYMENT_PAYPALWPP_TEXT_DECLINED;
                    }
                    // More optional response elements; initialize them to prevent follow-on PHP notices.
                    $response_optional = ['L_ERRORCODE1', 'L_SHORTMESSAGE1', 'L_LONGMESSAGE1', 'L_ERRORCODE2', 'L_SHORTMESSAGE2', 'L_LONGMESSAGE2'];
                    foreach ($response_optional as $optional) {
                        $response[$optional] ??= '';
                    }
                    $detailed_message = $error_text == MODULE_PAYMENT_PAYPALWPP_TEXT_GEN_ERROR || (int) trim($error_num) > 0 || $this->enable_debugging || $response['CURL_ERRORS'] != '' || $this->email_alerts ? $error_num . ' ' . urldecode(' ' . $response['L_SHORTMESSAGE0'] . ' - ' . $response['L_LONGMESSAGE0'] . (isset($response['RESPMSG']) ? ' ' . $response['RESPMSG'] : '') . ' ' . $response['CURL_ERRORS']) : '';
                    $detailed_email_message = $detailed_message == '' ? '' : $error_info . MODULE_PAYMENT_PAYPALWPP_TEXT_EMAIL_ERROR_MESSAGE . urldecode($response['L_ERRORCODE0'] . "\n" . $response['L_SHORTMESSAGE0'] . "\n" . $response['L_LONGMESSAGE0'] . $response['L_ERRORCODE1'] . "\n" . $response['L_SHORTMESSAGE1'] . "\n" . $response['L_LONGMESSAGE1'] . $response['L_ERRORCODE2'] . "\n" . $response['L_SHORTMESSAGE2'] . "\n" . $response['L_LONGMESSAGE2'] . ($response['CURL_ERRORS'] != '' ? "\n" . $response['CURL_ERRORS'] : '') . "\n\n" . 'Zen Cart message: ' . $error_text);
                    if (!isset($response['L_ERRORCODE0']) && isset($response['RESULT'])) {
                        $detailed_email_message .= "\n\n" . print_r($response, true);
                    }
                    if ($detailed_email_message != '') {
                        zen_mail(STORE_NAME, STORE_OWNER_EMAIL_ADDRESS, MODULE_PAYMENT_PAYPALWPP_TEXT_EMAIL_ERROR_SUBJECT . ' (' . $this->uncomment($error_num) . ')', $this->uncomment($detailed_email_message), STORE_OWNER, STORE_OWNER_EMAIL_ADDRESS, ['EMAIL_MESSAGE_HTML' => $this->uncomment($detailed_message)], 'paymentalert');
                    }
                    $this->terminate_ec($error_text . ' (' . $error_num . ') ' . $detailed_message, true);
                    return true;
                }
                break;
            case 'GetExpressCheckoutDetails':
                if ($basic_error || $_SESSION['paypal_ec_token'] != urldecode((string) $response['TOKEN'])) {
                    // if response indicates an error, send the customer back to checkout and display the error. Debug to store owner if active.
                    if ($this->enable_debugging) {
                        $this->_do_debug('PayPal Error Log - ec_step2()', "In function: ec_step2()\r\n\r\nValue List:\r\n" . str_replace('&', "\r\n", urldecode((string) $do_pay_pal->_sanitize_log($do_pay_pal->_parse_name_value_list($do_pay_pal->last_param_list)))) . "\r\n\r\nResponse:\r\n" . urldecode(print_r($response, true)));
                    }
                    $error_text = MODULE_PAYMENT_PAYPALWPP_TEXT_GEN_ERROR;
                    if ($response['L_ERRORCODE0'] == 13113) {
                        $error_text = MODULE_PAYMENT_PAYPALWPP_TEXT_PAYPAL_DECLINED;
                        $_SESSION['payment'] = '';
                    }
                    $this->terminate_ec($error_text . ' (' . $response['L_ERRORCODE0'] . ' ' . urldecode($response['L_SHORTMESSAGE0'] . $response['RESULT']) . ')', true);
                    return true;
                }
                break;
            case 'DoExpressCheckoutPayment':
                if ($basic_error || $_SESSION['paypal_ec_token'] != urldecode((string) $response['TOKEN'])) {
                    ...
                    if ($this->enable_debugging) {
                        $this->_do_debug('PayPal Error Log - before_process() - EC', "In function: before_process() - Express Checkout\r\n\r\nValue List:\r\n" . str_replace('&', "\r\n", $do_pay_pal->_sanitize_log($do_pay_pal->_parse_name_value_list($do_pay_pal->last_param_list))) . "\r\n\r\nResponse:\r\n" . print_r($response, true));
                    }
                    ...
                    $error_text = MODULE_PAYMENT_PAYPALWPP_INVALID_RESPONSE;
                    $error_num = urldecode($response['L_ERRORCODE0'] . ($response['RESULT'] ?? ''));
                    if ($response['L_ERRORCODE0'] == 10415) {
                        $error_text = MODULE_PAYMENT_PAYPALWPP_TEXT_ORDER_ALREADY_PLACED_ERROR;
                    }
                    if ($response['L_ERRORCODE0'] == 10417) {
                        $error_text = MODULE_PAYMENT_PAYPALWPP_TEXT_INSUFFICIENT_FUNDS_ERROR;
                    }
                    if ($response['L_ERRORCODE0'] == 10474) {
                        $error_text .= urldecode((string) $response['L_LONGMESSAGE0']);
                    }
                    $detailed_message = $error_text == MODULE_PAYMENT_PAYPALWPP_INVALID_RESPONSE || (int) trim($error_num) > 0 || $this->enable_debugging || $response['CURL_ERRORS'] != '' || $this->email_alerts ? $error_num . ' ' . urldecode(' ' . $response['L_SHORTMESSAGE0'] . ' - ' . $response['L_LONGMESSAGE0'] . ($response['RESULT'] ?? '') . ' ' . $response['CURL_ERRORS']) : '';
                    $detailed_email_message = $detailed_message == '' ? '' : $error_info . MODULE_PAYMENT_PAYPALWPP_TEXT_EMAIL_ERROR_MESSAGE . urldecode($response['L_ERRORCODE0'] . "\n" . $response['L_SHORTMESSAGE0'] . "\n" . $response['L_LONGMESSAGE0'] . ($response['L_ERRORCODE1'] ?? '') . "\n" . ($response['L_SHORTMESSAGE1'] ?? '') . "\n" . ($response['L_LONGMESSAGE1'] ?? '') . ($response['L_ERRORCODE2'] ?? '') . "\n" . ($response['L_SHORTMESSAGE2'] ?? '') . "\n" . ($response['L_LONGMESSAGE2'] ?? '') . ($response['CURL_ERRORS'] != '' ? "\n" . $response['CURL_ERRORS'] : '') . "\n\n" . 'Zen Cart message: ' . $error_text);
                    if (!isset($response['L_ERRORCODE0']) && isset($response['RESULT'])) {
                        $detailed_email_message .= "\n\n" . print_r($response, true);
                    }
                    ...
                    $this->terminate_ec($detailed_email_message == '' ? $error_text . ' (' . urldecode($response['L_SHORTMESSAGE0'] . $response['RESULT']) . ') ' : $detailed_message, true);
                    return true;
                }
                break;
            case 'DoRefund':
                if ($basic_error || !isset($response['RESPMSG']) && !isset($response['REFUNDTRANSACTIONID'])) {
                    // if error, display error message. If debug options enabled, email dump to store owner
                    if ($this->enable_debugging) {
                        $this->_do_debug('PayPal Error Log - ' . $operation, "Value List:\r\n" . str_replace('&', "\r\n", $do_pay_pal->_sanitize_log($do_pay_pal->_parse_name_value_list($do_pay_pal->last_param_list))) . "\r\n\r\nResponse:\r\n" . print_r($response, true));
                    }
                    $error_text = MODULE_PAYMENT_PAYPALWPP_TEXT_REFUND_ERROR;
                    if ($response['L_ERRORCODE0'] == 10009) {
                        $error_text = MODULE_PAYMENT_PAYPALWPP_TEXT_REFUNDFULL_ERROR;
                    }
                    if (!empty($response['RESULT']) && $response['RESULT'] == 105 || isset($response['RESPMSG'])) {
                        $response['L_SHORTMESSAGE0'] = ($response['RESULT'] ?? '') . ' ' . $response['RESPMSG'];
                    }
                    if (urldecode((string) $response['L_LONGMESSAGE0']) == 'This transaction has already been fully refunded') {
                        $response['L_SHORTMESSAGE0'] = urldecode((string) $response['L_LONGMESSAGE0']);
                    }
                    if (urldecode((string) $response['L_LONGMESSAGE0']) == 'Can not do a full refund after a partial refund') {
                        $response['L_SHORTMESSAGE0'] = urldecode((string) $response['L_LONGMESSAGE0']);
                    }
                    if (urldecode((string) $response['L_LONGMESSAGE0']) == 'The partial refund amount must be less than or equal to the remaining amount') {
                        $response['L_SHORTMESSAGE0'] = urldecode((string) $response['L_LONGMESSAGE0']);
                    }
                    if (urldecode((string) $response['L_LONGMESSAGE0']) == 'You can not refund this type of transaction') {
                        $response['L_SHORTMESSAGE0'] = urldecode((string) $response['L_LONGMESSAGE0']);
                    }
                    $error_text .= ' (' . urldecode((string) $response['L_SHORTMESSAGE0']) . ') ' . $response['L_ERRORCODE0'];
                    $message_stack->add_session($error_text, 'error');
                    return true;
                }
                break;
            ...
            case 'DoReauthorization':
                if ($basic_error) {
                    ...
                    $error_text = MODULE_PAYMENT_PAYPALWPP_TEXT_AUTH_ERROR;
                    $error_text .= ' (' . urldecode((string) $response['L_SHORTMESSAGE0']) . ') ' . $response['L_ERRORCODE0'];
                    $message_stack->add_session($error_text, 'error');
                    return true;
                }
            ...
            case 'DoCapture':
                if ($basic_error) {
                    ...
                    $error_text = MODULE_PAYMENT_PAYPALWPP_TEXT_CAPT_ERROR;
            ...
            case 'DoVoid':
                if ($basic_error) {
                    ...
                    $error_text = MODULE_PAYMENT_PAYPALWPP_TEXT_VOID_ERROR;
            ...
            case 'GetTransactionDetails':
                if ($basic_error) {
                    ...
                    $error_text = MODULE_PAYMENT_PAYPALWPP_TEXT_GETDETAILS_ERROR;
                    $error_text .= ' (' . urldecode((string) $response['L_SHORTMESSAGE0']) . ') ' . $response['L_ERRORCODE0'];
                    $message_stack->add_session($error_text, 'error');
                    return true;
                }
            ...
            case 'TransactionSearch':
                if ($basic_error) {
                    ...
                    $error_text = MODULE_PAYMENT_PAYPALWPP_TEXT_TRANSSEARCH_ERROR;
                    $error_text .= ' (' . urldecode((string) $response['L_SHORTMESSAGE0']) . ') ' . $response['L_ERRORCODE0'];
                    $message_stack->add_session($error_text, 'error');
                    return true;
                }
            ...
            default:
                if ($basic_error) {
                    ...
                    $error_text = MODULE_PAYMENT_PAYPALWPP_TEXT_GEN_API_ERROR;
                    $error_num .= ' (' . urldecode($response['L_SHORTMESSAGE0'] . ' <!-- ' . $response['RESPMSG']) . ' -->) ' . $response['L_ERRORCODE0'];
                    $detailed_message = $error_text == MODULE_PAYMENT_PAYPALWPP_TEXT_GEN_API_ERROR || $error_text == MODULE_PAYMENT_PAYPALWPP_TEXT_DECLINED || (int) trim($error_num) > 0 || $this->enable_debugging || $response['CURL_ERRORS'] != '' || $this->email_alerts ? urldecode(' ' . $response['L_SHORTMESSAGE0'] . ' - ' . $response['L_LONGMESSAGE0'] . ' ' . $response['CURL_ERRORS']) : '';
                    $detailed_email_message = $detailed_message == '' ? '' : MODULE_PAYMENT_PAYPALWPP_TEXT_EMAIL_ERROR_MESSAGE . ' ' . $response['RESPMSG'] . urldecode($response['L_ERRORCODE0'] . "\n" . $response['L_SHORTMESSAGE0'] . "\n" . $response['L_LONGMESSAGE0'] . $response['L_ERRORCODE1'] . "\n" . $response['L_SHORTMESSAGE1'] . "\n" . $response['L_LONGMESSAGE1'] . $response['L_ERRORCODE2'] . "\n" . $response['L_SHORTMESSAGE2'] . "\n" . $response['L_LONGMESSAGE2'] . ($response['CURL_ERRORS'] != '' ? "\n" . $response['CURL_ERRORS'] : '') . "\n\n" . 'Zen Cart message: ' . $detailed_message . "\n\n" . 'Transaction Response Details: ' . print_r($response, true) . "\n\n" . 'Transaction Submission: ' . urldecode((string) $do_pay_pal->_sanitize_log($do_pay_pal->_parse_name_value_list($do_pay_pal->last_param_list), true)));
                    if ($detailed_email_message != '') {
                        zen_mail(STORE_NAME, STORE_OWNER_EMAIL_ADDRESS, MODULE_PAYMENT_PAYPALWPP_TEXT_EMAIL_ERROR_SUBJECT . ' (' . $this->uncomment($error_num) . ')', $this->uncomment($detailed_message), STORE_OWNER, STORE_OWNER_EMAIL_ADDRESS, ['EMAIL_MESSAGE_HTML' => nl2br($this->uncomment($detailed_email_message))], 'paymentalert');
                    }
                    $message_stack->add_session($error_text . $error_num . $detailed_message, 'error');
                    return true;
                }
                break;
        }
     ...
     * @since v1.3.7.1
     */
    public function table_checkup(): void
    {
        global $db, $sniffer;
        $field_okay1 = method_exists($sniffer, 'field_type') ? $sniffer->field_type(TABLE_PAYPAL, 'txn_id', 'varchar(20)', true) : -1;
        $field_okay2 = $sniffer->field_exists(TABLE_PAYPAL, 'module_name') ? true : -1;
        $field_okay3 = $sniffer->field_exists(TABLE_PAYPAL, 'order_id') ? true : -1;
        if ($field_okay1 == -1) {
            ...
            while (!$result->EOF) {
                if ($result->fields['Field'] == 'txn_id') {
                    if ($result->fields['Type'] == 'varchar(20)') {
                        $field_okay1 = true;
                        // exists and matches required type, so skip to other checkup
                    } else {
                        $field_okay1 = $result->fields['Type'];
                        // doesn't match, so return what it "is"
                        break;
                    }
                }
                $result->move_next();
     ...
     * @since ZC v1.5.5
     */
    public function uncomment($string): string
    {
