...
class paypaldp extends base
{
    ...
    */
    ...
    public function selection(): array
    {
        ...
        $fieldsArray = [];
        $fieldsArray[] = ['title' => MODULE_PAYMENT_PAYPALDP_TEXT_CREDIT_CARD_FIRSTNAME,
        ...
        $fieldsArray[] = ['title' => MODULE_PAYMENT_PAYPALDP_TEXT_CREDIT_CARD_LASTNAME,
        ...
        if (sizeof($this->cards) > 0) {
            $fieldsArray[] = ['title' => MODULE_PAYMENT_PAYPALDP_TEXT_CREDIT_CARD_TYPE,
        ...
        $fieldsArray[] = ['title' => MODULE_PAYMENT_PAYPALDP_TEXT_CREDIT_CARD_NUMBER,
                               'field' => zen_draw_input_field('paypalwpp_cc_number', '', 'id="'.$this->code.'-cc-number"' . $onFocus . ' autocomplete="off"'),
                               'tag' => $this->code.'-cc-number'];
        $fieldsArray[] = ['title' => MODULE_PAYMENT_PAYPALDP_TEXT_CREDIT_CARD_EXPIRES,
        ...
        $fieldsArray[] = ['title' => MODULE_PAYMENT_PAYPALDP_TEXT_CREDIT_CARD_CHECKNUMBER,
        ...
        if (MODULE_PAYMENT_PAYPALDP_MERCHANT_COUNTRY == 'UK' && (CC_ENABLED_MAESTRO == '1' || CC_ENABLED_SOLO == '1')) {
            ...
            for ($i = $today['year'] - 10; $i <= $today['year']; $i++) {
                $issue_year[] = ['id' => $zcDate->output('%y', mktime(0, 0, 0, 1, 1, $i)), 'text' => $zcDate->output('%Y', mktime(0, 0, 0, 1, 1, $i))];
            }
            array_splice(
                $selection['fields'],
                4,
    ...
    public function pre_confirmation_check(): void
    {
        ...
        if (MODULE_PAYMENT_PAYPALDP_MERCHANT_COUNTRY == 'UK' && (!isset($_POST['MD']))) {
            ...
            if (isset($_SESSION['3Dsecure_requires_lookup']) && $_SESSION['3Dsecure_requires_lookup'] == true) {
                ...
                $lookup_data_array = ['currency' => $order->info['currency'],
                ...
                if ($errorNo === '0' && $_SESSION['3Dsecure_enrolled'] === 'Y') {
                    ...
                    if ($shouldContinue != 'Y') {
                        ...
                        if (in_array($errorNo, ['8000', '8010', '8020', '8030'])) {
                        ...
                        zen_mail(STORE_NAME, STORE_OWNER_EMAIL_ADDRESS, MODULE_PAYMENT_PAYPALDP_TEXT_EMAIL_ERROR_SUBJECT . ' ' . $reason, $errorText, STORE_OWNER, STORE_OWNER_EMAIL_ADDRESS, ['EMAIL_MESSAGE_HTML' => nl2br($errorText)], 'paymentalert');
                        zen_redirect(zen_href_link(FILENAME_CHECKOUT_PAYMENT, '', 'SSL', true, false));
    ...
    public function confirmation(): array
    {
        global $zcDate;
        $confirmation = ['title' => '',
                              'fields' => [['title' => MODULE_PAYMENT_PAYPALDP_TEXT_CREDIT_CARD_FIRSTNAME,
                                                ...
                                                ['title' => MODULE_PAYMENT_PAYPALDP_TEXT_CREDIT_CARD_NUMBER,
                                                      'field' => substr((string) $_POST['paypalwpp_cc_number'], 0, 4) . str_repeat('X', (strlen((string) $_POST['paypalwpp_cc_number']) - 8)) . substr((string) $_POST['paypalwpp_cc_number'], -4)],
                                                ...
                                                (isset($_POST['paypalwpp_cc_issuenumber']) ? ['title' => MODULE_PAYMENT_PAYPALDP_TEXT_ISSUE_NUMBER,
        ...
        if (MODULE_PAYMENT_PAYPALDP_MERCHANT_COUNTRY == 'UK' && $this->requiresLookup($_POST['paypalwpp_cc_number']) == true) {
            $confirmation['fields'][count($confirmation['fields'])] = [
                'title' => '',
                'field' => '<div id="' . $this->code.'-cc-securetext"><p>' .
                           '<a href="javascript:void window.open(\'vbv_learn_more.html\',\'vbv_service\',\'width=550,height=450,noreferrer\')">' .
                           zen_image(DIR_WS_IMAGES.'3ds/vbv_learn_more.gif') . '</a>' .
    ...
    {
        return ['ccFields' => ['wpp_cc_type' => 'paypalwpp_cc_type',
    ...
    public function before_process(): void
    {
        ...
        if (MODULE_PAYMENT_PAYPALDP_MERCHANT_COUNTRY == 'UK') {
            ...
            if (($_SESSION['3Dsecure_requires_lookup'] || $this->requiresLookup($_POST['wpp_cc_number']) == true)) {  // authentication attempt required?
                ...
                if (isset($_SESSION['3Dsecure_enroll_lookup_attempted']) == false || strcasecmp((string) $_SESSION['3Dsecure_enroll_lookup_attempted'], 'Y') != 0) {
                    ...
                    $errorText = $reason ."\n\nProblem occurred while customer " . $_SESSION['customer_id'] . ' ' . $_SESSION['customer_first_name'] . ' ' . $_SESSION['customer_last_name'] . ' was attempting checkout with 3D-Secure authentication.';
                    $errorText .= $this->code;
                    zen_mail(STORE_NAME, STORE_OWNER_EMAIL_ADDRESS, MODULE_PAYMENT_PAYPALDP_TEXT_EMAIL_ERROR_SUBJECT, $errorText, STORE_OWNER, STORE_OWNER_EMAIL_ADDRESS, ['EMAIL_MESSAGE_HTML' => nl2br($errorText)], 'paymentalert');
                    zen_redirect(zen_href_link(FILENAME_CHECKOUT_PAYMENT, '', 'SSL', true, false));
                }
        ...
        if (isset($order->delivery) && $order->delivery['street_address'] != '') {
            $optionsShip = ['SHIPTONAME'   => empty($order->delivery['name']) ? $order->delivery['firstname'] . ' ' . $order->delivery['lastname'] : $order->delivery['name'],
        ...
        if ($cc_issuedate_month && $cc_issuedate_year) {
            $optionsAll['CARDSTART'] = $cc_issuedate_month . substr((string) $cc_issuedate_year, -2);
        }
        ...
        if (!isset($optionsAll['AMT'])) {
            $optionsAll['AMT'] = round($order_amount, 2);
        }
        $response = $doPayPal->DoDirectPayment(
            $cc_number,
            $cc_checkcode,
            $cc_expdate_month . substr((string) $cc_expdate_year, -2),
            $cc_first_name,
            $cc_last_name,
            $cc_type,
            $optionsAll,
    ...
    public function after_process(): void
    {
        ...
        $paypal_order = ['order_id' => $insert_id,
                              ...
                              'payment_date' => convertToLocalTimeZone(trim((string) preg_replace('/[^0-9-:]/', ' ', $this->payment_time))),
                              'business' => '',
    ...
    public function _doRefund($oID, $amount = 'Full', $note = '')
    {
        ...
        if ($proceedToRefund) {
            ...
            if (!$error) {
                ...
                $messageStack->add_session(sprintf(MODULE_PAYMENT_PAYPALDP_TEXT_REFUND_INITIATED, urldecode((string) $response['GROSSREFUNDAMT']), urldecode((string) $response['REFUNDTRANSACTIONID']) . ($response['PNREF'] ?? '')), 'success');
                return true;
            }
        }
    }
    ...
    public function _doCapt($oID, $captureType = 'Complete', $amt = 0, string $currency = 'USD', $note = '')
    {
        ...
        if ($proceedToCapture) {
            ...
            if (!$error) {
                ...
                $messageStack->add_session(sprintf(MODULE_PAYMENT_PAYPALDP_TEXT_CAPT_INITIATED, urldecode((string) $response['AMT']), urldecode($response['AUTHCODE']). $response['PNREF']), 'success');
                return true;
            }
        }
    }
    ...
    public function _doVoid($oID, $note = '')
    {
        ...
        if (!$error) {
            ...
            $messageStack->add_session(sprintf(MODULE_PAYMENT_PAYPALDP_TEXT_VOID_INITIATED, urldecode((string) $response['AUTHORIZATIONID']) . $response['PNREF']), 'success');
            return true;
        }
    }
    ...
    public function getLineItemDetails($restrictedCurrency): array
    {
        ...
        for ($i = 0, $n = sizeof($order->products), $k = -1; $i < $n; $i++) {
            ...
            if ($order->products[$i]['onetime_charges'] != 0) {
                $k++;
                $optionsLI["L_NAME$k"]   = MODULES_PAYMENT_PAYPALWPP_LINEITEM_TEXT_ONETIME_CHARGES_PREFIX . substr(htmlentities((string) $order->products[$i]['name'], ENT_QUOTES, 'UTF-8'), 0, 120);
        ...
        for ($k = 0, $n = $numberOfLineItemsProcessed + 1; $k < $n; $k++) {
            $optionsLI["L_NAME$k"] = str_replace(['&','=','%'], '*', $optionsLI["L_NAME$k"]);
            ...
            if (isset($optionsLI["L_NUMBER$k"])) {
                if ($optionsLI["L_NUMBER$k"] == '') {
                    ...
                    $optionsLI["L_NUMBER$k"] = str_replace(['&','=','%'], '*', $optionsLI["L_NUMBER$k"]);
        ...
        for ($j = 0; $j < $k; $j++) {
            ...
            $itemTAX = ($optionsLI["L_TAXAMT$j"] ?? 0);
        ...
        if (!defined('PAYPALWPP_SKIP_LINE_ITEM_DETAIL_FORMATTING') || PAYPALWPP_SKIP_LINE_ITEM_DETAIL_FORMATTING != 'true' || in_array($order->info['currency'], ['JPY', 'NOK', 'HUF', 'TWD'])) {
            ...
            if (is_array($optionsLI)) {
                foreach ($optionsLI as $key => $value) {
                    if (str_starts_with($key, 'L_TAXAMT') && ($optionsLI[$key] == '' || $optionsLI[$key] == 0)) {
                        unset($optionsLI[$key]);
                    } else {
                        if (strstr($key, 'AMT')) {
    ...
    public function _errorHandler(array $response, string $operation = '', $ignore_codes = '')
    {
        ...
        if (in_array($operation, ['DoExpressCheckoutPayment', 'DoDirectPayment']) && !empty($response['PAYMENTSTATUS']) && $response['PAYMENTSTATUS'] === 'Pending' && isset($response['L_ERRORCODE0']) && $response['L_ERRORCODE0'] == 11610) {
            ...
            if ($response['ACK'] == 'SuccessWithWarning' && isset($response['L_FMFPENDINGID0'])) {
                for ($i = 0; $i < 20; $i++) {
                    $this->fmfErrors[] = ['key' => $response['L_FMFPENDINGID' . $i], 'status' => $response['L_FMFPENDINGID' . $i], 'desc' => $response['L_FMFPENDINGDESCRIPTION' . $i]];
                }
            }
        ...
        switch ($operation) {
            case 'DoDirectPayment':
                if ($basicError ||
                   ((isset($_SESSION['paypal_ec_token']) && isset($response['TOKEN'])) && $_SESSION['paypal_ec_token'] != urldecode($response['TOKEN']))) {
                    ...
                    zen_mail(STORE_NAME, STORE_OWNER_EMAIL_ADDRESS, MODULE_PAYMENT_PAYPALDP_TEXT_EMAIL_ERROR_SUBJECT . ' (' . zen_uncomment($errorNum) . ')', zen_uncomment($detailedEmailMessage), STORE_OWNER, STORE_OWNER_EMAIL_ADDRESS, ['EMAIL_MESSAGE_HTML' => nl2br(zen_uncomment($detailedEmailMessage))], 'paymentalert');
            ...
            default:
                if ($basicError) {
                    ...
                    if ($detailedEmailMessage != '') {
                        zen_mail(STORE_NAME, STORE_OWNER_EMAIL_ADDRESS, MODULE_PAYMENT_PAYPALDP_TEXT_EMAIL_ERROR_SUBJECT . ' (' . zen_uncomment($errorNum) . ')', zen_uncomment($detailedMessage . $explain), STORE_OWNER, STORE_OWNER_EMAIL_ADDRESS, ['EMAIL_MESSAGE_HTML' => nl2br(zen_uncomment($detailedEmailMessage))], 'paymentalert');
                    }
     ...
     * @since ZC v1.3.9a
     */
    public function clear_3DSecure_session_vars($thorough = false): void
    {
        if ($thorough) {
     ...
     * @since ZC v1.3.9a
     */
    public function determine3DSecureProtection($cardType, $ECI): string
    {
        $resultStatus = 'NOT PROTECTED';
        if (strcasecmp((string) $cardType, 'VISA') == 0) {
            if ((strcasecmp((string) $ECI, '05') == 0) || (strcasecmp((string) $ECI, '06') == 0)) {
                $resultStatus = 'PROTECTED';
            } else {
                $resultStatus = 'NOT PROTECTED';
            }
        } elseif (strcasecmp((string) $cardType, 'MASTERCARD') == 0) {
            if (strcasecmp((string) $ECI, '02') == 0) {
                $resultStatus = 'PROTECTED';
            } else {
                $resultStatus = 'NOT PROTECTED';
            }
        } elseif (strcasecmp((string) $cardType, 'JCB') == 0) {
            if ((strcasecmp((string) $ECI, '05') == 0) || (strcasecmp((string) $ECI, '06') == 0)) {
                $resultStatus = 'PROTECTED';
            } else {
                $resultStatus = 'NOT PROTECTED';
     ...
     * @since ZC v1.3.9a
     */
    public function get3DSecureLookupResponse(array $lookup_data_array): array
    {
        // Set some defaults
        ...
        $data .= '</CardinalMPI>';
        $debugData = str_replace(['[CDATA[' . $this->escapeXML(MODULE_PAYMENT_PAYPALDP_CARDINAL_MERCHANT) . ']]', '[CDATA[' . $this->escapeXML(MODULE_PAYMENT_PAYPALDP_CARDINAL_PASSWORD) . ']]', $this->escapeXML($lookup_data_array['cc3d_card_number']), $this->escapeXML($lookup_data_array['cc3d_checkcode'])], '********', $data);
        ...
        if ($errorNo != 0) {
            ...
            zen_mail(STORE_NAME, STORE_OWNER_EMAIL_ADDRESS, substr((string) $errorDesc, 0, 75) . ' (' . $errorNo . ')', $errorText, STORE_OWNER, STORE_OWNER_EMAIL_ADDRESS, ['EMAIL_MESSAGE_HTML' => nl2br($errorText)], 'paymentalert');
        }
        ...
        if (strcasecmp(MODULE_PAYMENT_PAYPALDP_CARDINAL_AUTHENTICATE_REQ, 'No') == 0) {
            $continue_flag = 'Y';
        } elseif (strcmp((string) $errorNo, '0') == 0) {
            ...
            $errorText .= "\n\nProblem occurred while customer " . $_SESSION['customer_id'] . ' ' . $_SESSION['customer_first_name'] . ' ' . $_SESSION['customer_last_name'] . ' was attempting checkout with 3D-Secure authentication.';
            zen_mail(STORE_NAME, STORE_OWNER_EMAIL_ADDRESS, CENTINEL_ERROR_CODE_1001_DESC . ' (' . CENTINEL_ERROR_CODE_1001 . ')', $errorText, STORE_OWNER, STORE_OWNER_EMAIL_ADDRESS, ['EMAIL_MESSAGE_HTML' => nl2br($errorText)], 'paymentalert');
            $continue_flag = 'Y';
        }
        ...
        return ['continue_flag' => $continue_flag,
                        'enrolled' => $enrolled,
     ...
     * @since ZC v1.3.9a
     */
    public function get3DSecureAuthenticateResponse(array $authenticate_data_array): array
    {
        ...
        $data .= '</CardinalMPI>';
        $debugData = str_replace(['[CDATA[' . $this->escapeXML(MODULE_PAYMENT_PAYPALDP_CARDINAL_MERCHANT) . ']]', '[CDATA[' . $this->escapeXML(MODULE_PAYMENT_PAYPALDP_CARDINAL_PASSWORD) . ']]'], '********', $data);
    ...
    public function send3DSecureHttp(string $url, $data, string $debugData): bool|string
    {
        ...
        if ((str_starts_with($url, 'http://')) || (str_starts_with($url, 'https://'))) {
            ...
            if (!$succeeded) {
                ...
                zen_mail(STORE_NAME, STORE_OWNER_EMAIL_ADDRESS, CENTINEL_ERROR_CODE_8030_DESC . ' (' . CENTINEL_ERROR_CODE_8030 . ')', $errorText, STORE_OWNER, STORE_OWNER_EMAIL_ADDRESS, ['EMAIL_MESSAGE_HTML' => nl2br($errorText)], 'paymentalert');
                ...
                $errorText .= "\n\nProblem occurred while customer " . $_SESSION['customer_id'] . ' ' . $_SESSION['customer_first_name'] . ' ' . $_SESSION['customer_last_name'] . ' was attempting checkout with 3D-Secure authentication.';
                zen_mail(STORE_NAME, STORE_OWNER_EMAIL_ADDRESS, CENTINEL_ERROR_CODE_8010_DESC . ' (' . CENTINEL_ERROR_CODE_8010 . ')', $errorText, STORE_OWNER, STORE_OWNER_EMAIL_ADDRESS, ['EMAIL_MESSAGE_HTML' => nl2br($errorText)], 'paymentalert');
            } else {
                ...
                if (strpos($result, '<ErrorNo>4243') > 0) {
                    ...
                    $errorText .= "\n\nProblem occurred while customer " . $_SESSION['customer_id'] . ' ' . $_SESSION['customer_first_name'] . ' ' . $_SESSION['customer_last_name'] . ' was attempting checkout with 3D-Secure authentication.';
                    zen_mail(STORE_NAME, STORE_OWNER_EMAIL_ADDRESS, CENTINEL_ERROR_CODE_4243_DESC . ' (' . CENTINEL_ERROR_CODE_4243 . ')', $errorText, STORE_OWNER, STORE_OWNER_EMAIL_ADDRESS, ['EMAIL_MESSAGE_HTML' => nl2br($errorText)], 'paymentalert');
                }
            }
        } else {
            ...
            $errorText .= "\n\nProblem occurred while customer " . $_SESSION['customer_id'] . ' ' . $_SESSION['customer_first_name'] . ' ' . $_SESSION['customer_last_name'] . ' was attempting checkout with 3D-Secure authentication.';
            zen_mail(STORE_NAME, STORE_OWNER_EMAIL_ADDRESS, CENTINEL_ERROR_CODE_8000_DESC . ' (' . CENTINEL_ERROR_CODE_8000 . ')', $errorText, STORE_OWNER, STORE_OWNER_EMAIL_ADDRESS, ['EMAIL_MESSAGE_HTML' => nl2br($errorText)], 'paymentalert');
        }
     ...
     * @since ZC v1.3.9a
     */
    public function escapeXML($elementValue): string
    {
        $escapedValue = str_replace('&', '&amp;', trim((string) $elementValue));
     ...
     * @since ZC v1.3.9a
     */
    public function setErrorResponse(string $errorNo, string $errorDesc): string
    {
     ...
     * @since ZC v1.3.9a
     */
    public function getISOCurrency($curr): string
    {
        $out = '';
     ...
     * @since ZC v1.3.9a
     */
    public function formatRawAmount($amount, string $curr): string
    {
        ...
        if ($curpos !== false) {
            return substr($strRetVal, 0, $curpos) . substr($strRetVal, $curpos + 1);
        }
        return $strRetVal;
    }
     ...
     * @since ZC v1.3.9a
     */
    public function requiresLookup($info): bool
    {
        if (is_numeric($info)) {
        ...
        if (in_array(strtoupper((string) $cardType), ['VISA', 'MASTERCARD', 'JCB', 'MAESTRO'])) {
            return true;
        }
        return false;
    }
     ...
     * @since ZC v1.3.9a
     */
    public function determineCardType($cardNumber): string
    {
...
class CardinalXMLParser
{
     ...
     * @since ZC v1.3.9a
     */
    public function elementData($parser, string $data): void
    {
    ...
    {
        if (str_starts_with((string) $this->elementValue, "\n")) {
            $this->elementValue = substr((string) $this->elementValue, 1);
        }
     ...
     * @since ZC v1.3.9a
     */
    public function deserializeXml($responseString): void
    {
