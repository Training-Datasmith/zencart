...
$sql = 'SELECT COUNT(*) as total
        ...
        LIMIT 1';
$sql = $db->bind_vars($sql, ':products_id', $_GET['products_id'], 'integer');
$sql = $db->bind_vars($sql, ':language_id', $_SESSION['languages_id'], 'integer');
...
$attribute_details_array_for_json = [];
...
$sql = 'SELECT DISTINCT popt.products_options_id, popt.products_options_name, popt.products_options_sort_order,
        ...
        WHERE patrib.products_id= :products_id
        AND popt.language_id = :language_id ' . $options_order_by;
$sql = $db->bind_vars($sql, ':products_id', $_GET['products_id'], 'integer');
$sql = $db->bind_vars($sql, ':language_id', $_SESSION['languages_id'], 'integer');
...
foreach ($products_options_names as $next_option_name) {
    ...
    $sql = 'SELECT pov.products_options_values_id, pov.products_options_values_name, pa.*
            ...
            AND   pa.options_id = :options_id ' . $order_by;
    $sql = $db->bind_vars($sql, ':products_id', $_GET['products_id'], 'integer');
    $sql = $db->bind_vars($sql, ':options_id', $products_options_id, 'integer');
    $sql = $db->bind_vars($sql, ':language_id', $_SESSION['languages_id'], 'integer');
    ...
    $selected_dropdown_attribute = false;
    // boolean, used for radio/checkbox/select
    ...
    foreach ($products_options as $next_option) {
        ...
        $input_field_id = 'attrib-' . $products_options_id . ($products_options_type === PRODUCTS_OPTIONS_TYPE_SELECT ? '' : '-' . $products_options_value_id);
        $selected_attribute = false;
        // boolean, used for radio/checkbox/select
        $data_properties = ' data-key="' . $input_field_id . '" ';
        // observers can insert data-x="y" values
        $field_disabled = '';
        // empty or disabled="disabled"
        $products_options_display_price = '';
        $new_attributes_price = '';
        $price_onetime = '';
        $products_options_array[] = ['id' => $products_options_value_id, 'text' => $next_option['products_options_values_name']];
        $attribute_details_array_for_json[$input_field_id] = array_merge(['field_id' => $input_field_id, 'name' => $next_option_name['products_options_name'], 'attr_id' => $next_option_name['products_options_id']], $next_option, $next_option_name);
        $zco_notifier->notify('NOTIFY_ATTRIBUTES_MODULE_START_OPTIONS_LOOP', $i++, $next_option, $next_option_name, $data_properties, $field_disabled, $attribute_details_array_for_json);
        // DEAL WITH PRICE AND WEIGHT DISPLAY
        if (CUSTOMERS_APPROVAL === '2' && !zen_is_logged_in() || STORE_STATUS === '1' || (CUSTOMERS_APPROVAL_AUTHORIZATION === '1' || CUSTOMERS_APPROVAL_AUTHORIZATION === '2') && $_SESSION['customers_authorization'] == '' || CUSTOMERS_APPROVAL === '2' && $_SESSION['customers_authorization'] == '2' || CUSTOMERS_APPROVAL_AUTHORIZATION === '2' && $_SESSION['customers_authorization'] != 0) {
            ...
            if ($next_option['options_values_price'] != 0 && $next_option['product_attribute_is_free'] !== '1' && $product_data['product_is_free'] !== '1') {
                ...
                $products_options_display_price = ATTRIBUTES_PRICE_DELIMITER_PREFIX . $next_option['price_prefix'] . $currencies->display_price($new_attributes_price, $products_tax_rate) . ATTRIBUTES_PRICE_DELIMITER_SUFFIX;
            ...
            } else if ($next_option['product_attribute_is_free'] === '1' && $product_data['product_is_free'] === '1') {
                $products_options_display_price = TEXT_ATTRIBUTES_PRICE_WAS . $next_option['price_prefix'] . $currencies->display_price($new_attributes_price, $products_tax_rate) . TEXT_ATTRIBUTE_IS_FREE;
            } else if (empty($new_attributes_price)) {
                $products_options_display_price = '';
            } else {
                $products_options_display_price = ATTRIBUTES_PRICE_DELIMITER_PREFIX . $next_option['price_prefix'] . $currencies->display_price($new_attributes_price, $products_tax_rate) . ATTRIBUTES_PRICE_DELIMITER_SUFFIX;
            }
        ...
        if ($flag_show_weight_attrib_for_this_prod_type == '1' && $next_option['products_attributes_weight'] != 0) {
            $products_options_display_weight = ATTRIBUTES_WEIGHT_DELIMITER_PREFIX . $next_option['products_attributes_weight_prefix'] . round($next_option['products_attributes_weight'], 2) . TEXT_PRODUCT_WEIGHT_UNIT . ATTRIBUTES_WEIGHT_DELIMITER_SUFFIX;
        ...
        if ($products_options->record_count() == 1 || in_array($products_options_type, [PRODUCTS_OPTIONS_TYPE_FILE, PRODUCTS_OPTIONS_TYPE_TEXT, PRODUCTS_OPTIONS_TYPE_CHECKBOX, PRODUCTS_OPTIONS_TYPE_RADIO, PRODUCTS_OPTIONS_TYPE_READONLY])) {
            $products_options_details = '';
        ...
        if ($products_options_type === PRODUCTS_OPTIONS_TYPE_RADIO) {
            if ($_SESSION['cart']->in_cart($prod_id)) {
            ...
            } else if (!empty($_POST['id']) && is_array($_POST['id'])) {
                ...
                $selected_attribute = $next_option['attributes_default'] === '1' ? true : $products_options->record_count() == 1;
            }
            ...
            switch ($next_option_name['products_options_images_style']) {
                case '0':
                    $tmp_radio .= zen_draw_radio_field($option_form_name, $products_options_value_id, $selected_attribute, 'id="' . $input_field_id . '" ' . $data_properties . $field_disabled) . '<label class="attribsRadioButton zero" for="' . $input_field_id . '">' . $products_options_details . '</label><br>' . "\n";
                    break;
                case '1':
                    $tmp_radio .= zen_draw_radio_field($option_form_name, $products_options_value_id, $selected_attribute, 'id="' . $input_field_id . '" ' . $data_properties . $field_disabled) . '<label class="attribsRadioButton one" for="' . $input_field_id . '">' . (!empty($next_option['attributes_image']) ? zen_image(DIR_WS_IMAGES . $next_option['attributes_image'], '', '', '') . '  ' : '') . $products_options_details . '</label><br>' . "\n";
                    break;
                case '2':
                    $tmp_radio .= zen_draw_radio_field($option_form_name, $products_options_value_id, $selected_attribute, 'id="' . $input_field_id . '" ' . $data_properties . $field_disabled) . '<label class="attribsRadioButton two" for="' . $input_field_id . '">' . $products_options_details . (!empty($next_option['attributes_image']) ? '<br>' . zen_image(DIR_WS_IMAGES . $next_option['attributes_image'], '', '', '') : '') . '</label><br>' . "\n";
                    break;
                case '3':
                    ...
                    if (!empty($next_option['attributes_image'])) {
                        $tmp_attributes_image .= '<div class="attribImg">' . zen_draw_radio_field($option_form_name, $products_options_value_id, $selected_attribute, 'id="' . $input_field_id . '" ' . $data_properties . $field_disabled) . '<label class="attribsRadioButton three" for="' . $input_field_id . '">' . zen_image(DIR_WS_IMAGES . $next_option['attributes_image']) . (PRODUCT_IMAGES_ATTRIBUTES_NAMES == '1' ? '<br>' . $next_option['products_options_values_name'] : '') . $products_options_details_noname . '</label>' . '</div>' . "\n";
                    } else {
                        $tmp_attributes_image .= '<div class="attribImg">' . zen_draw_radio_field($option_form_name, $products_options_value_id, $selected_attribute, 'id="' . $input_field_id . '" ' . $data_properties . $field_disabled) . '<br>' . '<label class="attribsRadioButton threeA" for="' . $input_field_id . '">' . $next_option['products_options_values_name'] . $products_options_details_noname . '</label>' . '</div>' . "\n";
                    }
                    break;
                case '4':
                    $tmp_attributes_image_row++;
                    ...
                    if (!empty($next_option['attributes_image'])) {
                        $tmp_attributes_image .= '<div class="attribImg">' . '<label class="attribsRadioButton four" for="' . $input_field_id . '">' . zen_image(DIR_WS_IMAGES . $next_option['attributes_image']) . (PRODUCT_IMAGES_ATTRIBUTES_NAMES == '1' ? '<br>' . $next_option['products_options_values_name'] : '') . (!empty($products_options_details_noname) ? '<br>' . $products_options_details_noname : '') . '</label><br>' . zen_draw_radio_field($option_form_name, $products_options_value_id, $selected_attribute, 'id="' . $input_field_id . '" ' . $data_properties . $field_disabled) . '</div>' . "\n";
                    } else {
                        $tmp_attributes_image .= '<div class="attribImg">' . '<label class="attribsRadioButton fourA" for="' . $input_field_id . '">' . $next_option['products_options_values_name'] . (!empty($products_options_details_noname) ? '<br>' . $products_options_details_noname : '') . '</label><br>' . zen_draw_radio_field($option_form_name, $products_options_value_id, $selected_attribute, 'id="' . $input_field_id . '" ' . $data_properties . $field_disabled) . '</div>' . "\n";
                    }
                    break;
                case '5':
                    $tmp_attributes_image_row++;
                    ...
                    if (!empty($next_option['attributes_image'])) {
                        $tmp_attributes_image .= '<div class="attribImg">' . zen_draw_radio_field($option_form_name, $products_options_value_id, $selected_attribute, 'id="' . $input_field_id . '" ' . $data_properties . $field_disabled) . '<br>' . '<label class="attribsRadioButton five" for="' . $input_field_id . '">' . zen_image(DIR_WS_IMAGES . $next_option['attributes_image']) . (PRODUCT_IMAGES_ATTRIBUTES_NAMES == '1' ? '<br>' . $next_option['products_options_values_name'] : '') . (!empty($products_options_details_noname) ? '<br>' . $products_options_details_noname : '') . '</label>' . '</div>' . "\n";
                    } else {
                        $tmp_attributes_image .= '<div class="attribImg">' . zen_draw_radio_field($option_form_name, $products_options_value_id, $selected_attribute, 'id="' . $input_field_id . '" ' . $data_properties . $field_disabled) . '<br>' . '<label class="attribsRadioButton fiveA" for="' . $input_field_id . '">' . $next_option['products_options_values_name'] . (!empty($products_options_details_noname) ? '<br>' . $products_options_details_noname : '') . '</label>' . '</div>' . "\n";
                    }
                    break;
            }
        }
        ...
        if ($products_options_type === PRODUCTS_OPTIONS_TYPE_CHECKBOX) {
            ...
            if ($_SESSION['cart']->in_cart($prod_id)) {
                ...
                foreach ($_POST['id'] as $key => $value) {
                    ...
                    } else if ($key == $products_options_id && $value == $products_options_value_id) {
                        $selected_attribute = true;
                        break;
                    }
                }
            ...
            switch ($next_option_name['products_options_images_style']) {
                case '0':
                    $tmp_checkbox .= zen_draw_checkbox_field($option_form_name . '[' . $products_options_value_id . ']', $products_options_value_id, $selected_attribute, 'id="' . $input_field_id . '" ' . $data_properties . $field_disabled) . '<label class="attribsCheckbox" for="' . $input_field_id . '">' . $products_options_details . '</label><br>' . "\n";
                    break;
                case '1':
                    $tmp_checkbox .= zen_draw_checkbox_field($option_form_name . '[' . $products_options_value_id . ']', $products_options_value_id, $selected_attribute, 'id="' . $input_field_id . '" ' . $data_properties . $field_disabled) . '<label class="attribsCheckbox" for="' . $input_field_id . '">' . (!empty($next_option['attributes_image']) ? zen_image(DIR_WS_IMAGES . $next_option['attributes_image'], '', '', '') . '  ' : '') . $products_options_details . '</label><br>' . "\n";
                    break;
                case '2':
                    $tmp_checkbox .= zen_draw_checkbox_field($option_form_name . '[' . $products_options_value_id . ']', $products_options_value_id, $selected_attribute, 'id="' . $input_field_id . '" ' . $data_properties . $field_disabled) . '<label class="attribsCheckbox" for="' . $input_field_id . '">' . $products_options_details . (!empty($next_option['attributes_image']) ? '<br>' . zen_image(DIR_WS_IMAGES . $next_option['attributes_image'], '', '', '') : '') . '</label><br>' . "\n";
                    break;
                case '3':
                    $tmp_attributes_image_row++;
                    if ($tmp_attributes_image_row > $next_option_name['products_options_images_per_row']) {
                    ...
                    if (!empty($next_option['attributes_image'])) {
                        $tmp_attributes_image .= '<div class="attribImg">' . zen_draw_checkbox_field($option_form_name . '[' . $products_options_value_id . ']', $products_options_value_id, $selected_attribute, 'id="' . $input_field_id . '" ' . $data_properties . $field_disabled) . '<label class="attribsCheckbox" for="' . $input_field_id . '">' . zen_image(DIR_WS_IMAGES . $next_option['attributes_image']) . (PRODUCT_IMAGES_ATTRIBUTES_NAMES === '1' ? '<br>' . $next_option['products_options_values_name'] : '') . $products_options_details_noname . '</label>' . '</div>' . "\n";
                    } else {
                        $tmp_attributes_image .= '<div class="attribImg">' . zen_draw_checkbox_field($option_form_name . '[' . $products_options_value_id . ']', $products_options_value_id, $selected_attribute, 'id="' . $input_field_id . '" ' . $data_properties . $field_disabled) . '<br>' . '<label class="attribsCheckbox" for="' . $input_field_id . '">' . $next_option['products_options_values_name'] . $products_options_details_noname . '</label>' . '</div>' . "\n";
                    }
                    break;
                case '4':
                    $tmp_attributes_image_row++;
                    ...
                    if (!empty($next_option['attributes_image'])) {
                        $tmp_attributes_image .= '<div class="attribImg">' . '<label class="attribsCheckbox" for="' . $input_field_id . '">' . zen_image(DIR_WS_IMAGES . $next_option['attributes_image']) . (PRODUCT_IMAGES_ATTRIBUTES_NAMES === '1' ? '<br>' . $next_option['products_options_values_name'] : '') . (!empty($products_options_details_noname) ? '<br>' . $products_options_details_noname : '') . '</label><br>' . zen_draw_checkbox_field($option_form_name . '[' . $products_options_value_id . ']', $products_options_value_id, $selected_attribute, 'id="' . $input_field_id . '" ' . $data_properties . $field_disabled) . '</div>' . "\n";
                    } else {
                        $tmp_attributes_image .= '<div class="attribImg">' . '<label class="attribsCheckbox" for="' . $input_field_id . '">' . $next_option['products_options_values_name'] . (!empty($products_options_details_noname) ? '<br>' . $products_options_details_noname : '') . '</label><br>' . zen_draw_checkbox_field($option_form_name . '[' . $products_options_value_id . ']', $products_options_value_id, $selected_attribute, 'id="' . $input_field_id . '" ' . $data_properties . $field_disabled) . '</div>' . "\n";
                    }
                    break;
                case '5':
                    $tmp_attributes_image_row++;
                    ...
                    if (!empty($next_option['attributes_image'])) {
                        $tmp_attributes_image .= '<div class="attribImg">' . zen_draw_checkbox_field($option_form_name . '[' . $products_options_value_id . ']', $products_options_value_id, $selected_attribute, 'id="' . $input_field_id . '" ' . $data_properties . $field_disabled) . '<br>' . zen_image(DIR_WS_IMAGES . $next_option['attributes_image']) . '<label class="attribsCheckbox" for="' . $input_field_id . '">' . (PRODUCT_IMAGES_ATTRIBUTES_NAMES === '1' ? '<br>' . $next_option['products_options_values_name'] : '') . (!empty($products_options_details_noname) ? '<br>' . $products_options_details_noname : '') . '</label>' . '</div>' . "\n";
                    } else {
                        $tmp_attributes_image .= '<div class="attribImg">' . zen_draw_checkbox_field($option_form_name . '[' . $products_options_value_id . ']', $products_options_value_id, $selected_attribute, 'id="' . $input_field_id . '" ' . $data_properties . $field_disabled) . '<br>' . '<label class="attribsCheckbox" for="' . $input_field_id . '">' . $next_option['products_options_values_name'] . (!empty($products_options_details_noname) ? '<br>' . $products_options_details_noname : '') . '</label>' . '</div>' . "\n";
                    }
                    break;
        ...
        if ($products_options_type === PRODUCTS_OPTIONS_TYPE_TEXT) {
            ...
            if (!empty($_POST['id']) && is_array($_POST['id'])) {
                ...
                if ($next_option_name['products_options_rows'] > 1) {
                    $tmp_html = '  <input disabled="disabled" type="text" name="remaining' . TEXT_PREFIX . $products_options_id . '" size="3" maxlength="3" value="' . $next_option_name['products_options_length'] . '"> ' . TEXT_MAXIMUM_CHARACTERS_ALLOWED . '<br>';
                    $tmp_html .= '<textarea class="attribsTextarea" name="' . $option_form_name . '" rows="' . $next_option_name['products_options_rows'] . '" cols="' . $next_option_name['products_options_size'] . '" onkeydown="characterCount(this.form[\'' . $option_form_name . '\'],this.form.remaining' . TEXT_PREFIX . $products_options_id . ',' . $next_option_name['products_options_length'] . ');" onkeyup="characterCount(this.form[\'' . $option_form_name . '\'],this.form.remaining' . TEXT_PREFIX . $products_options_id . ',' . $next_option_name['products_options_length'] . ');" id="' . $input_field_id . '">' . stripslashes($tmp_value) . '</textarea>' . "\n";
                    // $tmp_html .= '  <input type="reset">';
                } else {
                    $tmp_html = '<input type="text" name="' . $option_form_name . '" size="' . $next_option_name['products_options_size'] . '" maxlength="' . $next_option_name['products_options_length'] . '" value="' . htmlspecialchars($tmp_value, ENT_COMPAT, CHARSET, true) . '" id="' . $input_field_id . '"' . $data_properties . $field_disabled . '>  ';
                }
                $tmp_html .= $products_options_details;
                if (defined('ATTRIBUTES_ENABLED_TEXT_PRICES') && ATTRIBUTES_ENABLED_TEXT_PRICES === 'true') {
                    ...
                    if ($next_option['attributes_price_words'] != 0) {
                        $tmp_html .= TEXT_PER_WORD . $currencies->display_price($next_option['attributes_price_words'], $products_tax_rate) . ($next_option['attributes_price_words_free'] != 0 ? TEXT_WORDS_FREE . $next_option['attributes_price_words_free'] : '');
                    }
                    ...
                    if ($next_option['attributes_price_letters'] != 0) {
                        $tmp_html .= TEXT_PER_LETTER . $currencies->display_price($next_option['attributes_price_letters'], $products_tax_rate) . ($next_option['attributes_price_letters_free'] != 0 ? TEXT_LETTERS_FREE . $next_option['attributes_price_letters_free'] : '');
                    }
        ...
        if ($products_options_type === PRODUCTS_OPTIONS_TYPE_FILE) {
            ...
            if (zen_run_normal() && zen_check_show_prices()) {
                ...
                $tmp_html = '<input type="file" name="id[' . TEXT_PREFIX . $products_options_id . ']"  id="' . $input_field_id . '" ' . $data_properties . '><br>' . $file_attribute_value . "\n" . zen_draw_hidden_field(UPLOAD_PREFIX . $number_of_uploads, $products_options_id) . "\n" . zen_draw_hidden_field(TEXT_PREFIX . UPLOAD_PREFIX . $number_of_uploads, $file_attribute_value);
            }
            $tmp_html .= $products_options_details;
        }
        $zco_notifier->notify('NOTIFY_ATTRIBUTES_MODULE_FORMAT_VALUE', array_merge($next_option, $next_option_name), $data_properties, $field_disabled, $attribute_details_array_for_json);
        ...
        if ($next_option_name['products_options_images_style'] == '0' || ($option_is_text_or_file || $products_options_type === '0')) {
            if (!empty($next_option['attributes_image'])) {
                ...
                $tmp_attributes_image .= '<div class="attribImg">' . zen_image(DIR_WS_IMAGES . $next_option['attributes_image']) . (PRODUCT_IMAGES_ATTRIBUTES_NAMES === '1' ? $option_is_text_or_file === false ? '<br>' . $next_option['products_options_values_name'] : '' : '') . '</div>' . "\n";
            }
        }
    ...
    $zco_notifier->notify('NOTIFY_ATTRIBUTES_MODULE_BEFORE_ASSEMBLE_OUTPUTS', $next_option, $data_properties, $input_field_id, $field_disabled);
    $options_inputfield_id[] = $input_field_id;
    ...
    switch (true) {
        ...
        case $products_options_type == PRODUCTS_OPTIONS_TYPE_TEXT:
            $options_name[] = '<label class="attribsInput" for="' . $input_field_id . '">' . ($show_attributes_qty_prices_icon ? ATTRIBUTES_QTY_PRICE_SYMBOL : '') . $products_options_name . '</label>';
        ...
        case $products_options_type == PRODUCTS_OPTIONS_TYPE_FILE:
            $options_name[] = '<label class="attribsUploads" for="' . $input_field_id . '">' . ($show_attributes_qty_prices_icon ? ATTRIBUTES_QTY_PRICE_SYMBOL : '') . $products_options_name . '</label>';
        ...
        case $products_options->record_count() == 1:
            if ($show_attributes_qty_prices_icon) {
                $options_name[] = '<label class="switchedLabel ONE" for="' . $input_field_id . '">' . ATTRIBUTES_QTY_PRICE_SYMBOL . $products_options_name . '</label>';
            } else {
                $options_name[] = $products_options_name;
            }
            $options_html_id[] = 'drprad-attrib-' . $products_options_id;
            $options_menu[] = zen_draw_radio_field('id[' . $products_options_id . ']', $products_options_value_id, true, 'id="' . $input_field_id . '" ' . $data_properties . $field_disabled) . '<label class="attribsRadioButton" for="' . $input_field_id . '">' . $products_options_details . '</label>' . "\n";
            break;
        // SELECT dropdown
        case $products_options_type == PRODUCTS_OPTIONS_TYPE_SELECT:
            ...
            if ($show_attributes_qty_prices_icon) {
                ...
                $options_name[] = '<label class="attribsSelect" for="' . $input_field_id . '">' . $products_options_name . '</label>';
            }
            $options_html_id[] = 'drp-attrib-' . $products_options_id;
            $options_menu[] = zen_draw_pull_down_menu('id[' . $products_options_id . ']', $products_options_array, $selected_attribute, 'id="' . $input_field_id . '" ' . $data_properties . $field_disabled) . "\n";
            break;
        default:
...
$zco_notifier->notify('NOTIFY_ATTRIBUTES_MODULE_END', $prod_id, $options_name, $options_menu, $options_comment, $options_comment_position, $options_html_id, $options_attributes_image, $options_inputfield_id, $attribute_details_array_for_json);
