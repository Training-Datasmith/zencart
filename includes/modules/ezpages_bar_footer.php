        ...
        foreach ($pages_query as $page_query) {
            ...
            switch (true) {
                ...
                case $page_query['alt_url'] != '' && $page_query['page_open_new_window'] == '0':
                    $page_query_list_footer[$rows]['altURL'] = str_starts_with((string) $page_query['alt_url'], 'http') ? $page_query['alt_url'] : ($page_query['alt_url'] == '' ? '' : zen_href_link($page_query['alt_url'], '', 'SSL', true, true, true));
                    break;
            }
            // if altURL is specified, use it; otherwise, use EZPage ID to create link
            $page_query_list_footer[$rows]['link'] = $page_query_list_footer[$rows]['altURL'] == '' ? zen_href_link(FILENAME_EZPAGES, 'id=' . $page_query['pages_id'] . ($page_query['toc_chapter'] > 0 ? '&chapter=' . $page_query['toc_chapter'] : ''), 'SSL') : $page_query_list_footer[$rows]['altURL'];
            $page_query_list_footer[$rows]['link'] .= $page_query['page_open_new_window'] == '1' ? '" rel="noreferrer noopener" target="_blank' : '';
        }
        $var_links_list = $page_query_list_footer;
    }
}
// test for display
