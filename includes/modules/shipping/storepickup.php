 ...
 * @since ZC v1.1.0
 */
class storepickup extends Zen_Shipping
{
    /**
    ...
    public function __construct()
    {
        ...
        $this->icon = '';
        // add image filename here; must be uploaded to the /images/ subdirectory
    ...
    public function update_status(): void
    {
        ...
        $this->check_enabled_for_zone(MODULE_SHIPPING_STOREPICKUP_ZONE);
        // other status checks?
        if ($this->enabled) {
            // other checks here
            // -----
    ...
    public function quote($method = ''): array
    {
        ...
        $ways_translated = defined('MODULE_SHIPPING_STOREPICKUP_MULTIPLE_WAYS') ? trim((string) MODULE_SHIPPING_STOREPICKUP_MULTIPLE_WAYS) : '';
        $ways_default = trim(MODULE_SHIPPING_STOREPICKUP_LOCATIONS_LIST);
        $methods_to_parse = $ways_translated == '' ? $ways_default : $ways_translated;
        if ($methods_to_parse == '') {
            $this->methods_list[] = ['id' => $this->code, 'title' => trim((string) MODULE_SHIPPING_STOREPICKUP_TEXT_WAY), 'cost' => MODULE_SHIPPING_STOREPICKUP_COST];
        } else {
            $this->locations = explode(';', $methods_to_parse);
            $this->methods_list = [];
            foreach ($this->locations as $key => $val) {
                if ($method != '' && $method != $this->code . $key) {
                    continue;
                }
                ...
                $this->methods_list[] = ['id' => $this->code . $key, 'title' => trim($title), 'cost' => $cost];
            }
        }
        $this->quotes = ['id' => $this->code, 'module' => MODULE_SHIPPING_STOREPICKUP_TEXT_TITLE, 'methods' => $this->methods_list];
        if ($this->tax_class > 0) {
