...
if (PRODUCT_INFO_PREVIOUS_NEXT != 0) {
    ...
    */
    ...
    $sql = 'SELECT p.products_id, p.products_model, p.products_price_sorter, pd.products_name, p.products_sort_order
          FROM   ' . TABLE_PRODUCTS . ' p, ' . TABLE_PRODUCTS_DESCRIPTION . ' pd, ' . TABLE_PRODUCTS_TO_CATEGORIES . " ptc\n          WHERE  p.products_status = '1' AND p.products_id = pd.products_id AND pd.language_id= '" . (int) $_SESSION['languages_id'] . "' AND p.products_id = ptc.products_id AND ptc.categories_id = '" . (int) $current_category_id . "'" . $prev_next_order;
    $products_ids = $db->Execute($sql);
    $products_found_count = $products_ids->record_count();
    ...
    if ($products_found_count > 0) {
        ...
        $position = $counter = 0;
        $previous = -1;
        // identify as needing to go to the end of the list.
        $next_item = $id_array[0];
        // set the next as the first initially.
