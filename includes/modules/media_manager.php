 ...
 */
if ($zq_collections->record_count() > 0) {
    ...
    while (!$zq_collections->EOF) {
        ...
        if ($zq_media_manager->record_count() < 1) {
            $zv_product_has_media = false;
        } else {
            /**
             * build array of [collection_id][text] = collection-name
             */
            $za_media_manager[$zq_media_manager->fields['media_id']] = ['text' => $zq_media_manager->fields['media_name']];
            /**
            ...
            if ($zq_clips->record_count() < 1) {
                ...
                while (!$zq_clips->EOF) {
                    ...
                    $za_media_manager[$zq_media_manager->fields['media_id']]['clips'][$zq_clips->fields['clip_id']] = ['clip_filename' => $zq_clips->fields['clip_filename'], 'clip_type' => $zq_clip_type->fields['type_name']];
                    $zq_clips->move_next();
                }
            }
        }
        $zq_collections->move_next();
    }
}
