...
switch (true) {
    ...
    case CUSTOMERS_APPROVAL_AUTHORIZATION == '1' && !zen_is_logged_in():
         ...
         * customer must be logged in to browse
         */
        if (!in_array($_GET['main_page'], [FILENAME_LOGIN, FILENAME_LOGOFF, FILENAME_CREATE_ACCOUNT, FILENAME_PASSWORD_FORGOTTEN, FILENAME_CONTACT_US, FILENAME_PRIVACY, DOWN_FOR_MAINTENANCE_FILENAME])) {
            if (!isset($_GET['set_session_login'])) {
                $_GET['set_session_login'] = 'true';
                $_SESSION['navigation']->set_snapshot();
            }
