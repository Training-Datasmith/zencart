 ...
 * @since ZC v2.0.0
 */
function init_coupon_referrer_check(): ?string
{
    global $db;
    ...
    $sql = $db->bind_vars($sql, ':referrer', $domain, 'string');
 ...
 * @since ZC v2.0.0
 */
function init_coupon_request_check(): null|string|int|float|array
{
 ...
 * @since ZC v2.0.0
 */
function init_coupon_checks(): void
{
    global $language_loader, $message_stack;
    $coupon_code = init_coupon_request_check();
    if (empty($coupon_code)) {
        $coupon_code = init_coupon_referrer_check();
    }
    if (empty($coupon_code)) {
        return;
    }
    ...
    $language_loader->load_extra_language_files(DIR_FS_CATALOG . DIR_WS_LANGUAGES, $_SESSION['language'], 'ot_coupon.php', '/modules/order_total');
    $ot_coupon = new ot_coupon();
    if (!$ot_coupon->check()) {
        return;
    }
    $coupon_id = $ot_coupon->perform_validations($coupon_code);
    if (empty($coupon_id)) {
        // The coupon could not be applied for some reason
        $ot_coupon->set_message_stack_validation_alerts();
        return;
    }
    ...
    $message_stack->add('header', TEXT_VALID_COUPON, 'success');
}
init_coupon_checks();