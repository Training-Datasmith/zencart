...
if (isset($c_path_array, $c_path)) {
    for ($i = 0, $n = count($c_path_array); $i < $n; $i++) {
        $categories_query = 'SELECT categories_name
               FROM ' . TABLE_CATEGORIES_DESCRIPTION . '
              WHERE categories_id = ' . (int) $c_path_array[$i] . '
                AND language_id = ' . (int) $_SESSION['languages_id'];
        $categories = $db->Execute($categories_query, 1);
        if (!$categories->EOF) {
            $breadcrumb->add($categories->fields['categories_name'], zen_href_link(FILENAME_DEFAULT, 'cPath=' . implode('_', array_slice($c_path_array, 0, $i + 1))));
...
foreach ($get_terms as $next_get_term) {
    ...
    if (isset($_GET[$next_get_term_name])) {
        $sql = 'SELECT ' . $next_get_term['get_term_name_field'] . '
...
if (isset($_GET['products_id'])) {
    ...
    if (!empty($productname)) {
        $breadcrumb->add($productname, zen_href_link(zen_get_info_page($_GET['products_id']), 'cPath=' . $c_path . '&products_id=' . $_GET['products_id']));
    }
}