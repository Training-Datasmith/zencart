...
if (WARN_BEFORE_DOWN_FOR_MAINTENANCE === 'true' && DOWN_FOR_MAINTENANCE === 'false') {
    $message_stack->add('header', TEXT_BEFORE_DOWN_FOR_MAINTENANCE . PERIOD_BEFORE_DOWN_FOR_MAINTENANCE);
}
...
if (DOWN_FOR_MAINTENANCE === 'true' && zen_is_whitelisted_admin_ip()) {
    $message_stack->add('header', TEXT_ADMIN_DOWN_FOR_MAINTENANCE, 'warning');
}
...
if (defined('MODULE_PAYMENT_PAYPAL_IPN_DEBUG') && MODULE_PAYMENT_PAYPAL_IPN_DEBUG === 'true' || defined('MODULE_PAYMENT_PAYPAL_TESTING') && MODULE_PAYMENT_PAYPAL_TESTING === 'Test') {
    $message_stack->add('header', 'PAYPAL IS IN TESTING MODE', 'warning');
}
if (defined('MODULE_PAYMENT_AUTHORIZENET_AIM_STATUS') && MODULE_PAYMENT_AUTHORIZENET_AIM_STATUS === 'True' && defined('MODULE_PAYMENT_AUTHORIZENET_AIM_TESTMODE') && MODULE_PAYMENT_AUTHORIZENET_AIM_TESTMODE === 'Test' || defined('MODULE_PAYMENT_AUTHORIZENET_STATUS') && MODULE_PAYMENT_AUTHORIZENET_STATUS === 'True' && defined('MODULE_PAYMENT_AUTHORIZENET_TESTMODE') && MODULE_PAYMENT_AUTHORIZENET_TESTMODE === 'Test') {
    $message_stack->add('header', 'AUTHORIZENET IS IN TESTING MODE', 'warning');
}
if (defined('MODULE_SHIPPING_USPS_SERVER') && MODULE_SHIPPING_USPS_SERVER === 'test') {
    $message_stack->add('header', 'USPS IS IN TESTING MODE', 'warning');
}
// Alerts for EZ-Pages
if (EZPAGES_STATUS_HEADER === '2' && zen_is_whitelisted_admin_ip()) {
    $message_stack->add('header', TEXT_EZPAGES_STATUS_HEADER_ADMIN, 'caution');
}
if (EZPAGES_STATUS_FOOTER === '2' && zen_is_whitelisted_admin_ip()) {
    $message_stack->add('header', TEXT_EZPAGES_STATUS_FOOTER_ADMIN, 'caution');
}
if (EZPAGES_STATUS_SIDEBOX === '2' && zen_is_whitelisted_admin_ip()) {
    $message_stack->add('header', TEXT_EZPAGES_STATUS_SIDEBOX_ADMIN, 'caution');
}
if (defined('STRICT_ERROR_REPORTING') && STRICT_ERROR_REPORTING === true) {
    $message_stack->add('header', 'STRICT ERROR REPORTING IS ON', 'warning');
}
...
if (zen_is_logged_in() && !in_array($_GET['main_page'], [FILENAME_ADDRESS_BOOK_PROCESS, FILENAME_LOGOFF], true)) {
    $addresses_query = 'SELECT address_book_id, entry_country_id as country_id
           FROM   ' . TABLE_ADDRESS_BOOK . '
          WHERE  customers_id = :customersID';
    $addresses_query = $db->bind_vars($addresses_query, ':customersID', $_SESSION['customer_id'], 'integer');
    $addresses = $db->Execute($addresses_query);
    foreach ($addresses as $address) {
        if (zen_get_country_name($address['country_id'], true) === '') {
            $message_stack->add_session('addressbook', sprintf(ERROR_TEXT_COUNTRY_DISABLED_PLEASE_CHANGE, zen_get_country_name($address['country_id'], false)), 'error');
            zen_redirect(zen_href_link(FILENAME_ADDRESS_BOOK_PROCESS, 'edit=' . $address['address_book_id'], 'SSL'));
        }
    }
}
if (Customer::is_wholesale_customer()) {
    $message_stack->add('header', WHOLESALE_CUSTOMER_SHOPPING, 'success');
}