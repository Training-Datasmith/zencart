...
function zen_update_orders_history($orders_id, $message = '', $updated_by = null, $orders_new_status = -1, $notify_customer = -1, $email_include_message = true, $email_subject = '', $send_extra_emails_to = '', $filename = ''): int
{
    ...
    $osh_info = $GLOBALS['db']->execute_no_cache('SELECT customers_name, customers_email_address, orders_status, date_purchased
    ...
    if ($osh_info->EOF) {
        ...
        if ($orders_new_status != -1 && $orders_current_status != $orders_new_status || !empty($email_message)) {
            ...
            if (empty($updated_by)) {
                ...
                } elseif (isset($_SESSION['emp_admin_id'])) {
            ...
            $osh_sql = ['orders_id' => $orders_id, 'orders_status_id' => $orders_new_status, 'date_added' => 'now()', 'customer_notified' => $notify_customer, 'comments' => $message, 'updated_by' => $updated_by];
            ...
            $GLOBALS['zco_notifier']->notify('ZEN_UPDATE_ORDERS_HISTORY_AFTER_INSERT', $osh_id, $osh_sql, $paypal_lookup);
        }
    }
    return $osh_id;
}