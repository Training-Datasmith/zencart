...
{
    return (new Product((int) $product_id))->for_language((int) $language_id);
}
/**
 * @since ZC v1.5.7
 */
...
{
    global $zco_notifier, $breadcrumb, $robots_no_index;
    // make sure we got a Product-class instance and it's for the current product
    if ($product_info === null || $product_info::class !== 'Product' || $product_info->get_id() !== (int) $product_id) {
 ...
 * @since ZC v1.5.7
 */
function zen_enable_disabled_upcoming(?int $activation_date_time = null, bool $use_midnight = true, bool $output_messages_to_command_line = false): void
{
    global $db;
    if (empty($activation_date_time)) {
        $activation_date_time = time();
    }
    $format_specificity_string = $use_midnight ? 'Ymd' : 'YmdHis';
    $zc_disabled_upcoming_date = date($format_specificity_string, $activation_date_time);
    ...
    $count = $results->count();
    if ($output_messages_to_command_line) {
...
function zen_get_uprid(int|string $prid, array|string $params): string
{
    ...
    //
    $prid = (string) $prid;
    if (!is_array($params) || $params === [] || str_contains($prid, ':')) {
        return $prid;
    }
    // -----
...
{
    return (new Product((int) $product_id))->is_valid();
}
/**
...
{
    $product = (new Product((int) $product_id))->get_data_for_language((int) $language_id);
...
function zen_get_product_is_linked($product_id, $show_count = 'false')
{
    ...
    return (new Product((int) $product_id))->is_linked() ? 'true' : 'false';
}
/**
 ...
 * @since ZC v1.0.3
 */
function zen_get_products_stock(string|int $products_id): int|float
{
    global $zco_notifier;
    // Give an observer the chance to modify this function's return value.
    $products_quantity = 0;
    $quantity_handled = false;
    $zco_notifier->notify('ZEN_GET_PRODUCTS_STOCK', $products_id, $products_quantity, $quantity_handled);
    if ($quantity_handled) {
        return $products_quantity;
    }
    return (new Product(zen_get_prid($products_id)))->get_product_quantity();
}
/**
...
function zen_check_stock(string|int $products_id, $products_quantity): string
{
    ...
    if ($stock_left < 0) {
        ...
        $zco_notifier->notify('ZEN_CHECK_STOCK_MESSAGE', [$products_id, $products_quantity], $out_of_stock_message);
...
function zen_get_products_description($product_id, $language_id = null): string
{
    ...
    $data = $product->get_data_for_language((int) $language_id);
...
{
    return (new Product((int) $product_id))->get_info_page();
}
/**
...
{
    return (new Product((int) $product_id))->is_virtual();
}
/**
...
{
    return (new Product((int) $product_id))->allows_add_to_cart() ? 'Y' : 'N';
}
/**
...
{
    global $db;
    $key_name = zen_get_show_product_switch_name($lookup, $field, $prefix, $suffix, $field_prefix, $field_suffix);
    $sql = 'SELECT configuration_key, configuration_value FROM ' . TABLE_PRODUCT_TYPE_LAYOUT . " WHERE configuration_key = '" . zen_db_input($key_name) . "'";
    $zv_key_value = $db->Execute($sql, 1);
    if (!$zv_key_value->EOF) {
        return $zv_key_value->fields['configuration_value'];
    }
    $sql = 'SELECT configuration_key, configuration_value FROM ' . TABLE_CONFIGURATION . " WHERE configuration_key = '" . zen_db_input($key_name) . "'";
    $zv_key_value = $db->Execute($sql, 1);
    if (!$zv_key_value->EOF) {
        return $zv_key_value->fields['configuration_value'];
    }
 ...
 * @since ZC v1.3.0
 */
function zen_get_show_product_switch_name($lookup, string $field, string $prefix = 'SHOW_', string $suffix = '_INFO', string $field_prefix = '_', string $field_suffix = ''): string
{
    $type_handler = (new Product((int) $lookup))->get_type_handler();
...
{
    return (new Product((int) $product_id))->is_always_free_shipping();
}
/**
 ...
 * @since ZC v1.3.0
 */
function zen_products_lookup($product_id, $what_field = 'products_name', ?int $language = null): mixed
{
    $product = new Product((int) $product_id);
    $data = $product->get_data_for_language($language);
 ...
 * @since ZC v1.2.1d
 */
function zen_has_product_discounts($product_id): string
{
    global $db;
...
function zen_remove_product($product_id, $ptc = 'true'): void
{
    ...
    if (!$product_image->EOF) {
        ...
        if ($duplicate_image->fields['total'] < 2) {
            ...
            $image_parts = pathinfo((string) $products_image);
    ...
    if (ADDITIONAL_IMAGES_HANDLING === 'Database') {
        ...
        foreach ($images_query as $image) {
            ...
            $check_query = $db->Execute('SELECT COUNT(*) AS total FROM ' . TABLE_PRODUCTS_ADDITIONAL_IMAGES . " WHERE additional_image = '" . zen_db_input($image_name) . "' AND products_id != {$product_id}");
 ...
 * @since ZC v1.5.8a
 */
function zen_products_sort_order($include_order_by = true): string
{
    $product_sort = match (PRODUCT_INFO_PREVIOUS_NEXT_SORT) {
        0 => 'p.products_id',
        2 => 'p.products_model',
        3 => 'p.products_price_sorter, pd.products_name',
        4 => 'p.products_price_sorter, p.products_model',
        5 => 'pd.products_name, p.products_model',
        6 => 'products_sort_order, pd.products_name',
        default => 'pd.products_name',
    };
    if ($include_order_by) {
        return ' ORDER BY ' . $product_sort;
    }
    return $product_sort;
}