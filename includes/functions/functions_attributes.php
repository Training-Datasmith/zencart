...
function zen_requires_attribute_selection($products_id): bool
{
    ...
    $no_doubles = [PRODUCTS_OPTIONS_TYPE_RADIO, PRODUCTS_OPTIONS_TYPE_SELECT];
    $no_singles = [PRODUCTS_OPTIONS_TYPE_CHECKBOX, PRODUCTS_OPTIONS_TYPE_FILE, PRODUCTS_OPTIONS_TYPE_TEXT];
    if (PRODUCTS_OPTIONS_TYPE_READONLY_IGNORED === '0') {
        $no_singles[] = PRODUCTS_OPTIONS_TYPE_READONLY;
    }
    ...
    $zco_notifier->notify('NOTIFY_FUNCTIONS_LOOKUPS_REQUIRES_ATTRIBUTES_SELECTION', '', $query, $no_singles, $no_doubles);
    ...
    foreach ($result as $field) {
        ...
        if (in_array($field['options_type'], $no_doubles) && $field['number_of_choices'] > 1) {
            return true;
        }
        // if there's any type from $noSingles, can't add from listing
        if (in_array($field['options_type'], $no_singles)) {
            return true;
        }
    }
...
function zen_option_name_base_expects_no_values($option_name_id_array)
{
    ...
    if (count($option_name_id_array) > 1) {
        ...
        foreach ($option_name_id_array as $option_id) {
            $sql2 .= ':option_id:,';
            $sql2 = $db->bind_vars($sql2, ':option_id:', $option_id, 'integer');
        }
        $sql2 = rtrim((string) $sql2, ',');
        // Need to remove the final comma off of the above.
        $sql2 .= ')';
    } else {
        $sql2 = ' = :option_id:';
        $sql2 = $db->bind_vars($sql2, ':option_id:', $option_name_id_array[0], 'integer');
    }
    $sql = $db->bind_vars($sql, ':option_name_id:', $sql2, 'noquotestring');
    $sql_result = $db->Execute($sql);
    foreach ($sql_result as $opt_type) {
        $test_var = true;
        // Set to false in observer if the name is not supposed to have a value associated
...
function zen_has_product_attributes_downloads_status($product_id)
{
    ...
    $sql = 'SELECT pad.products_attributes_id
            ...
            WHERE pa.products_id = ' . (int) $product_id;
    return $db->Execute($sql, 1)->record_count() > 0;
}
/**
...
{
    global $db, $zco_notifier, $message_stack;
    ...
    if ($products_id_to === $products_id_from) {
        $message_stack->add_session(sprintf(WARNING_ATTRIBUTE_COPY_SAME_ID, $products_id_from, $products_id_to), 'caution');
        return false;
    }
    // no attributes found to copy
    if (!zen_has_product_attributes($products_id_from, false)) {
        $message_stack->add_session(sprintf(WARNING_ATTRIBUTE_COPY_NO_ATTRIBUTES, $products_id_from, zen_get_products_name($products_id_from)), 'caution');
        return false;
    }
    // invalid products_id
    if (!zen_products_id_valid($products_id_to)) {
        $message_stack->add_session(sprintf(WARNING_ATTRIBUTE_COPY_INVALID_ID, $products_id_to), 'caution');
        return false;
    }
    ...
    foreach ($products_copy_from as $copy_from) {
        ...
        if ($copy_attributes_duplicates_skipped == '1' && !$check_duplicate->EOF) {
            $message_stack->add_session(sprintf(TEXT_ATTRIBUTE_COPY_SKIPPING, (int) $copy_from['products_attributes_id'], $products_id_to), 'caution');
            // skip it
            continue;
        }
        ...
        if ($add_attribute === true) {
            ...
            $message_stack->add_session(sprintf(TEXT_ATTRIBUTE_COPY_INSERTING, (int) $copy_from['products_attributes_id'], $products_id_from, $products_id_to), 'success');
        ...
        if ($update_attribute === true) {
            ...
            $message_stack->add_session(sprintf(TEXT_ATTRIBUTE_COPY_UPDATING, (int) $copy_from['products_attributes_id'], $products_id_to), 'success');
...
function zen_has_product_attributes_downloads($product_id, $check_if_valid = false): string
{
    ...
    if (!$results->EOF) {
        return $results->record_count() . ' files';
    }
    return 'none';
}
/**
