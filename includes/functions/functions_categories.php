...
{
    global $c_path_array, $db;
    if ($current_category_id === null || empty($c_path_array) || $current_category_id === '') {
        // empty string check is deprecated in 1.5.8
        return 'cPath=' . (!empty($c_path_array) ? implode('_', $c_path_array) : $current_category_id);
    }
    // make copy so we can manipulate it later
    $c_path_categories = $c_path_array;
    $last_category_query = 'SELECT parent_id
                            FROM ' . TABLE_CATEGORIES . '
                            WHERE categories_id = ' . (int) $c_path_categories[count($c_path_categories) - 1];
    ...
    if (!isset($last_category->fields['parent_id'], $current_category->fields['parent_id'])) {
        array_pop($c_path_categories);
    } elseif ($last_category->fields['parent_id'] == $current_category->fields['parent_id']) {
        array_pop($c_path_categories);
    }
    $c_path_categories[] = $current_category_id;
    $c_path_new = implode('_', $c_path_categories);
    return 'cPath=' . trim($c_path_new, '_');
}
/**
 ...
 * @since ZC v1.0.3
 */
function zen_count_products_in_category($category_id, bool $include_inactive = false): int|float
{
...
function zen_count_distinct_products_in_category($category_id, bool $include_inactive = false): int
{
    ...
    $sql = 'SELECT count(DISTINCT p.products_id) as total ' . 'FROM ' . TABLE_PRODUCTS . ' p ' . 'LEFT JOIN ' . TABLE_PRODUCTS_TO_CATEGORIES . ' p2c USING (products_id) ' . 'WHERE p2c.categories_id in ' . $category_list;
    if (!$include_inactive) {
        $sql .= ' AND p.products_status = 1';
    }
    $products = $db->Execute($sql);
    return $products_count + (int) $products->fields['total'];
}
/**
...
function zen_has_category_subcategories($category_id): bool
{
    ...
    return $result->record_count() && $result->fields['count'] > 0;
}
/**
...
function zen_get_categories(array $categories_array = [], $parent_id = TOPMOST_CATEGORY_PARENT_ID, string $indent = '', $status_flag = null): array
{
    ...
    foreach ($results as $result) {
        ...
        $categories_array[] = ['id' => $result['categories_id'], 'text' => $indent . $result['categories_name']];
...
function zen_get_parent_categories(array &$categories, $category_id)
{
    ...
    foreach ($results as $result) {
        if ($result['parent_id'] == TOPMOST_CATEGORY_PARENT_ID) {
            return true;
        }
 ...
 * @since ZC v1.0.3
 */
function zen_parse_category_path(string $c_path): array
{
    // make sure the category IDs are integers
    $c_path_array = array_map(fn($value): int => (int) trim((string) $value), explode('_', $c_path));
    // make sure no duplicate category IDs exist which could lock us into a loop
    $tmp_array = [];
    foreach ($c_path_array as $value) {
 ...
 * @since ZC v1.5.8
 */
function zen_draw_pulldown_products(string $field_name, string $parameters = '', $exclude = [], bool $show_id = false, $set_selected = 0, bool $show_model = false, bool $show_current_category = false, string $order_by = '', $filter_by_option_name = null, bool $include_attributes = false): string
{
    ...
    $pulldown = new Product_Pulldown();
    if ($show_current_category) {
        $pulldown->set_category((int) $current_category_id);
    }
    $pulldown->include_attributes($include_attributes);
    if ((int) $filter_by_option_name > 0) {
        $pulldown->set_option_filter((int) $filter_by_option_name);
    }
    $pulldown->set_sort($sort_array)->exclude($exclude)->show_model($show_model)->set_default((int) $set_selected)->only_active($only_active)->show_id($show_id);
    return $pulldown->generate_pulldown_html($field_name, $parameters, false);
}
/**
 ...
 * @since ZC v1.5.8
 */
function zen_draw_pulldown_categories_having_products(string $field_name, string $parameters = '', array $exclude = [], bool $show_id = false, bool $show_parent = false, bool $show_full_path = false, $filter_by_option_name = null, bool $include_attributes = false): string
{
    if (!is_array($exclude)) {
        $exclude = [];
    }
    $pulldown = new Category_Pulldown();
    $pulldown->show_id($show_id)->show_parent($show_parent)->show_full_path($show_full_path)->exclude($exclude)->include_attributes($include_attributes || $show_full_path);
    if ((int) $filter_by_option_name > 0) {
        $pulldown->set_option_filter((int) $filter_by_option_name);
    }
    return $pulldown->generate_pulldown_html($field_name, $parameters, false);
...
function zen_get_categories_products_list($categories_id, bool $include_deactivated = false, bool $include_child = true, $parent_category = TOPMOST_CATEGORY_PARENT_ID, $display_limit = '')
{
    ...
    $is_c_path = strrpos($categories_id, '_');
    if ($is_c_path) {
        $child_cat_id = str_replace('_', '', substr($categories_id, strrpos($categories_id, '_')));
    } else {
        $child_cat_id = $categories_id;
    }
    $current_c_path = ($parent_category != TOPMOST_CATEGORY_PARENT_ID ? $parent_category . '_' : '') . $categories_id;
    $sql = 'SELECT p.products_id
            FROM ' . TABLE_PRODUCTS . ' p
            LEFT JOIN ' . TABLE_PRODUCTS_TO_CATEGORIES . ' p2c USING (products_id)
            WHERE p2c.categories_id = ' . (int) $child_cat_id . (!$include_deactivated ? ' AND p.products_status = 1' : '') . $display_limit;
    $results = $db->Execute($sql);
    foreach ($results as $result) {
        $categories_products_id_list[$result['products_id']] = $current_c_path;
    }
    if ($include_child) {
        $sql = 'SELECT categories_id
                FROM ' . TABLE_CATEGORIES . '
                WHERE parent_id = ' . (int) $child_cat_id;
        $results = $db->Execute($sql);
        foreach ($results as $result) {
            zen_get_categories_products_list($result['categories_id'], $include_deactivated, $include_child, $current_c_path, $display_limit);
        }
    }
...
function zen_generate_category_path($id, string $from = 'category', array $categories_array = [], $index = 0): mixed
{
    ...
    if ($from == 'product') {
        ...
        $categories = $db->Execute($sql);
        foreach ($categories as $p2c_result) {
            if ($p2c_result['categories_id'] == TOPMOST_CATEGORY_PARENT_ID) {
                $categories_array[$index][] = ['id' => TOPMOST_CATEGORY_PARENT_ID, 'text' => TEXT_TOP];
            } else {
                $sql = 'SELECT cd.categories_name, c.parent_id
                        FROM ' . TABLE_CATEGORIES . ' c
                        LEFT JOIN ' . TABLE_CATEGORIES_DESCRIPTION . ' cd ON (c.categories_id = cd.categories_id AND cd.language_id = ' . (int) $_SESSION['languages_id'] . ')
                        WHERE c.categories_id = ' . (int) $p2c_result['categories_id'];
                $category = $db->Execute($sql);
                $categories_array[$index][] = ['id' => $p2c_result['categories_id'], 'text' => $category->fields['categories_name']];
...
function zen_output_generated_category_path($category_id, string $from = 'category')
{
    ...
    foreach ($calculated_category_path as $outer_value) {
        foreach ($outer_value as $inner_value) {
            if ($from == 'category') {
                $calculated_category_path_string = $inner_value['text'] . '&nbsp;&gt;&nbsp;' . $calculated_category_path_string;
            } else {
                $calculated_category_path_string .= $inner_value['text'];
                $calculated_category_path_string .= ' [ ' . TEXT_INFO_ID . $inner_value['id'] . ' ] ';
    ...
    if (strlen((string) $calculated_category_path_string) < 1) {
        return TEXT_TOP;
    }
...
function zen_get_generated_category_path_ids($id, string $from = 'category')
{
    ...
    foreach ($calculated_category_path as $outer_value) {
        foreach ($outer_value as $inner_value) {
            $calculated_category_path_string .= $inner_value['id'] . '_';
        }
...
function zen_get_categories_status($categories_id): string
{
    ...
    $sql = 'SELECT categories_status
            FROM ' . TABLE_CATEGORIES . (!empty($categories_id) ? ' WHERE categories_id=' . (int) $categories_id : '');
    $check_status = $db->Execute($sql);
    if ($check_status->EOF) {
        return '';
    }
    // empty string means does not exist in zen_validate_categories()
...
function zen_validate_categories($ref_category_id, $target_category_id = 0, bool $reset_master_category = false): bool
{
    ...
    if ($ref_category_id === '' || zen_get_categories_status($ref_category_id) === '') {
        //REF does not exist
        $categories_valid = false;
        $message_stack->add_session(sprintf(WARNING_CATEGORY_SOURCE_NOT_EXIST, (int) $ref_category_id), 'warning');
    }
    if (!$reset_master_category && ($target_category_id === '' || zen_get_categories_status($target_category_id) === '')) {
        //TARGET does not exist
        $categories_valid = false;
        $message_stack->add_session(sprintf(WARNING_CATEGORY_TARGET_NOT_EXIST, (int) $target_category_id), 'warning');
    }
    if (!$reset_master_category && ($categories_valid && $ref_category_id === $target_category_id)) {
        //category IDs are the same
        $categories_valid = false;
        $message_stack->add_session(sprintf(WARNING_CATEGORY_IDS_DUPLICATED, $ref_category_id), 'warning');
    }
    if ($categories_valid) {
        $check_category_from = zen_get_linked_products_for_category((int) $ref_category_id);
        // check if REF has any products
        if (count($check_category_from) < 1) {
            //there are no products in the FROM category: invalid
            $categories_valid = false;
            $message_stack->add_session(sprintf(WARNING_CATEGORY_NO_PRODUCTS, (int) $ref_category_id), 'warning');
        }
        // check that TARGET has no subcategories
        if (!$reset_master_category && zen_childs_in_category_count($target_category_id) > 0) {
            //subcategories exist in the TO category: invalid
            $categories_valid = false;
            $message_stack->add_session(sprintf(WARNING_CATEGORY_SUBCATEGORIES, (int) $target_category_id), 'warning');
        }
    }
...
function zen_get_categories_info(int $parent_id = 0, string $category_path_string = ''): void
{
    ...
    foreach ($results as $result) {
        ...
        if ($subcategories->EOF) {
            // no subcategories
            $categories_info[] = ['categories_id' => $category_id, 'categories_name' => $category_name];
        } else {
    ...
    foreach ($categories as $category) {
        ...
        if (!$sub_categories_result->EOF) {
            if ($type === 'product') {
                ...
                //type is category
                $category_product_tree_array[] = ['id' => $category['categories_id'], 'text' => $spacing . $category['categories_name']];
        ...
        if ($type === 'product') {
            ...
            foreach ($products as $product) {
                if ((int) $product['products_id'] !== $products_filter) {
                    $category_product_tree_array[] = ['id' => $product['products_id'], 'text' => $spacing . htmlentities((string) $category['categories_name'], ENT_COMPAT) . ': ' . htmlentities((string) $product['products_model'], ENT_COMPAT) . ' - ' . htmlentities((string) $product['products_name'], ENT_COMPAT) . ' (#' . $product['products_id'] . ')'];
                }
            }
        }
    }
...
{
    global $db;
    $sql = 'SELECT categories_id FROM ' . TABLE_CATEGORIES . ' WHERE parent_id = ' . $category_id;
    $results = $db->Execute($sql);
    foreach ($results as $result) {
        ...
        if ($zq_type_to_cat->record_count() < 1) {
            $za_insert_sql_data = ['category_id' => (int) $result['categories_id'], 'product_type_id' => $product_type_id];
...
function zen_set_category_image($category_id, string $image_name = ''): void
{
    ...
    $sql = 'UPDATE ' . TABLE_CATEGORIES . '
            ...
            WHERE categories_id = ' . (int) $category_id;
    $sql = $db->bind_vars($sql, ':image_name', $image_name, 'stringIgnoreNull');
    $db->Execute($sql);
}
/**
 ...
 * @since ZC v1.3.8
 */
function zen_categories_lookup($categories_id, string $what_field = 'categories_name', $language = '')
{
    ...
    return $category_lookup->fields['lookup_field'];
}
/**
 * @param int $category_id
 * @since ZC v1.0.3
...
function zen_remove_category($category_id): void
{
    ...
    //
    $skip_sale_id = 0;
    //- Set for sale_categories_all check, in case the category's not present in sale_categories_selected
    foreach ($chk_sale_categories_selected as $sale_categories_selected) {
        ...
        if ($sale_categories_selected['sale_categories_selected'] == (int) $category_id && $sale_categories_selected['sale_categories_all'] == ',' . (int) $category_id . ',') {
            // delete record
        ...
        $categories_selected = explode(',', (string) $sale_categories_selected['sale_categories_selected']);
    ...
    foreach ($chk_sale_categories_all as $sale_categories_all) {
        if ($skip_sale_id == $sale_categories_all['sale_id']) {
            // was deleted
            continue;
        }
        $categories_all = explode(',', trim((string) $sale_categories_all['sale_categories_all'], ','));
 ...
 * @since ZC v1.0.3
 */
function zen_childs_in_category_count($category_id): int|float
{
    global $db;
    $categories_count = 0;
...
function zen_get_categories_name_from_product($product_id)
{
    ...
    $check_products_category = $db->Execute('SELECT products_id, master_categories_id
                                             FROM ' . TABLE_PRODUCTS . '
                                             WHERE products_id = ' . (int) $product_id);
    if ($check_products_category->EOF) {
        return '';
    }
 ...
 * @since ZC v1.2.0d
 */
function zen_get_products_to_categories($category_id, bool $include_inactive = false, string $counts_what = 'products'): string|array|float|int
{
    global $db;
    $products_linked = '';
    if ($include_inactive) {
    ...
    if ($cat_child_categories->record_count() > 0) {
        while (!$cat_child_categories->EOF) {
            ...
            $cat_child_categories->move_next();
        }
    }
