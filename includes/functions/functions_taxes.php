...
function zen_get_tax_rate($class_id, $country_id = -1, $zone_id = -1)
{
    ...
    $tax_rate = false;
    $zco_notifier->notify('NOTIFY_ZEN_GET_TAX_RATE_OVERRIDE', ['class_id' => $class_id, 'country_id' => $country_id, 'zone_id' => $zone_id], $tax_rate);
    if ($tax_rate !== false) {
        return $tax_rate;
    }
...
function zen_get_tax_description($class_id, $country_id = -1, $zone_id = -1)
{
    ...
    $tax_description = '';
    $zco_notifier->notify('NOTIFY_ZEN_GET_TAX_DESCRIPTION_OVERRIDE', ['class_id' => $class_id, 'country_id' => $country_id, 'zone_id' => $zone_id], $tax_description);
    if ($tax_description != '') {
...
function zen_get_multiple_tax_rates($class_id, $country_id = -1, $zone_id = -1, $tax_description = [])
{
    ...
    $rates_array = '';
    $zco_notifier->notify('NOTIFY_ZEN_GET_MULTIPLE_TAX_RATES_OVERRIDE', ['class_id' => $class_id, 'country_id' => $country_id, 'zone_id' => $zone_id, 'tax_description' => $tax_description], $rates_array);
    if (is_array($rates_array)) {
        return $rates_array;
    }
    if (Customer::is_tax_exempt() === true) {
        return [TAX_EXEMPT_DESCRIPTION => 0];
    }
    $rates_array = [];
    if ($country_id == -1 && $zone_id == -1) {
        if (zen_is_logged_in()) {
...
function zen_get_tax_rate_from_desc(string $tax_desc): float
{
    ...
    foreach ($tax_descriptions as $tax_description) {
        $sql = 'SELECT tax_rate
                ...
                AND trd.language_id = ' . (int) $_SESSION['languages_id'];
        $sql = $db->bind_vars($sql, ':taxDescLookup', $tax_description, 'string');
...
function zen_get_tax_locations($store_country = -1, $store_zone = -1)
{
    ...
    $tax_address = false;
    $zco_notifier->notify('ZEN_GET_TAX_LOCATIONS', ['country' => $store_country, 'zone' => $store_zone], $tax_address);
...
function zen_get_all_tax_descriptions($country_id = -1, $zone_id = -1)
{
    ...
    // Give an observer the chance to override this function's return.
    $tax_descriptions = '';
    $zco_notifier->notify('NOTIFY_ZEN_GET_ALL_TAX_DESCRIPTIONS_OVERRIDE', ['country_id' => $country_id, 'zone_id' => $zone_id], $tax_descriptions);
    ...
    foreach ($results as $result) {
        $tax_descriptions[] = $result['tax_description'];
    }
    return $tax_descriptions;
}
