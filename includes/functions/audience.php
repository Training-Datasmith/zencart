...
function get_audiences_list(string $query_category = 'email', $display_count = '', $lookup_email_address = ''): array
{
    ...
    if ($queries_list->RecordCount() > 1) {  // if more than one query record found
        $audience_list[] = ['id' => '', 'text' => TEXT_SELECT_AN_OPTION]; //provide a "not-selected" value
    }
    $show_count = false;
    ...
    if ($query_category == 'email') {
        ...
        foreach ($customers_values as $customers_value) {
            $audience_list[] = ['id' => $customers_value['customers_email_address'],
...
function parsed_query_string($read_string): string
{
    ...
    foreach ($allwords as $val) {
        ...
        if (str_starts_with($val, '{TABLE_')  && str_ends_with($val, '}')) { //check for leading and trailing {} braces
