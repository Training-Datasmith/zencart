 ...
 */
function get_audiences_list(string $query_category = 'email', $display_count = '', $lookup_email_address = ''): array
{
    ...
    $audience_list = [];
    if ($queries_list->record_count() > 1) {
        // if more than one query record found
        $audience_list[] = ['id' => '', 'text' => TEXT_SELECT_AN_OPTION];
    ...
    foreach ($queries_list as $query_list) {
        ...
        if ($show_count) {
            ...
            $count = $count_array->record_count();
        }
        ...
        $audience_list[] = ['id' => $query_list['query_name'], 'text' => $query_list['query_name'] . ($show_count ? ' (' . $count . ')' : '')];
    }
    ...
    if ($query_category == 'email') {
        ...
        $lookup_filter = $db->bind_vars($lookup_filter, ':lookupAddress:', $lookup_email_address, 'string');
        $customers_values = $db->Execute('select customers_email_address, customers_firstname, customers_lastname ' . 'from ' . TABLE_CUSTOMERS . " WHERE customers_email_format != 'NONE' " . $lookup_filter . 'order by customers_lastname, customers_firstname, customers_email_address');
        foreach ($customers_values as $customers_value) {
            $audience_list[] = ['id' => $customers_value['customers_email_address'], 'text' => $customers_value['customers_lastname'] . ', ' . $customers_value['customers_firstname'] . ' (' . $customers_value['customers_email_address'] . ')'];
        }
    }
 ...
 * @since ZC v1.2.0d
 */
function get_audience_sql_query($selected_entry, string $query_category = 'email'): array
{
    ...
    return ['query_name' => $query_name, 'query_string' => $query_string];
}
/**
 * @since ZC v1.2.0d
 */
function parsed_query_string($read_string): string
{
    $good_string = '';
