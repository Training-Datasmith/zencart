...
function zen_href_link($page = '', $parameters = '', $connection = 'NONSSL', $add_session_id = true, $search_engine_safe = true, $static = false, $use_dir_ws_catalog = true)
{
    ...
    $link = null;
    $zco_notifier->notify('NOTIFY_SEFU_INTERCEPT', [], $link, $page, $parameters, $connection, $add_session_id, $static, $use_dir_ws_catalog);
    if ($link !== null) {
        return $link;
    }
    if (empty($page)) {
    ...
    return preg_replace('~//$~', '/', (string) $link);
}
/**
 ...
 * @since ZC v1.2.4
 */
function zen_image_OLD($src, $title = '', $width = '', $height = '', ?string $parameters = ''): false|string
{
    global $template_dir;
...
function zen_image($src, $title = '', $width = '', $height = '', ?string $parameters = ''): string|false
{
    ...
    if (str_contains((string) $src, 'includes/templates') || str_contains((string) $src, 'includes/languages') || PROPORTIONAL_IMAGES_STATUS === '0') {
    ...
    } elseif (!str_starts_with((string) $src, 'http')) {
...
function zen_image_submit(string $image, $alt = '', ?string $parameters = '', $sec_class = ''): string
{
    ...
    if ((strtolower(IMAGE_USE_CSS_BUTTONS) === 'yes' || strtolower(IMAGE_USE_CSS_BUTTONS) === 'found' && !file_exists(DIR_FS_CATALOG . DIR_WS_TEMPLATE . 'buttons/' . $_SESSION['language'] . '/' . $image)) && mb_strlen((string) $alt) < 30) {
        return zen_css_button($image, $alt, 'submit', $sec_class, $parameters);
    }
 ...
 */
function zen_image_button(string $image, $alt = '', string $parameters = '', $sec_class = '')
{
    global $template, $current_page_base, $zco_notifier;
    // inject rollover class if one is defined. NOTE: This could end up with 2 "class" elements if $parameters contains "class" already.
    if (defined('IMAGE_ROLLOVER_CLASS') && IMAGE_ROLLOVER_CLASS != '') {
    ...
    if (strtolower(IMAGE_USE_CSS_BUTTONS) === 'yes' || strtolower(IMAGE_USE_CSS_BUTTONS) === 'found' && !file_exists(DIR_FS_CATALOG . DIR_WS_TEMPLATE . 'buttons/' . $_SESSION['language'] . '/' . $image)) {
        if (preg_match('/\.(png|gif|jpe?g|webp)/i', $image)) {
            return zen_css_button($image, $alt, 'button', $sec_class, $parameters);
        }
 ...
 * @since ZC v1.5.8
 **/
function zen_draw_button($text = '', ?string $added_classes = '', ?string $id = '', ?string $parameters = '', ?string $title = '', string $type = 'button'): string
{
    global $zco_notifier;
    // legacy support
    // remove .gif etc suffix if any
    $text = preg_replace('/\.(png|gif|jpe?g|webp)$/', '', (string) $text);
 ...
 * @since ZC v1.3.0
 */
function zen_css_button($image = '', string $text = '', $type = 'button', $sec_class = '', $parameters = ''): string
{
    global $css_button_text, $css_button_opts, $template, $current_page_base, $language;
    $button_name = basename((string) $image);
    ...
    if ($type == 'submit') {
        ...
        // of the button.
        $GLOBALS['zco_notifier']->notify('NOTIFY_ZEN_CSS_BUTTON_SUBMIT', ['button_name' => $button_name, 'text' => $text, 'sec_class' => $sec_class, 'parameters' => $parameters], $css_button);
    ...
    if ($type == 'button') {
        ...
        // of the button.
        $GLOBALS['zco_notifier']->notify('NOTIFY_ZEN_CSS_BUTTON_BUTTON', ['button_name' => $button_name, 'text' => $text, 'sec_class' => $sec_class, 'parameters' => $parameters], $css_button);
        if ($css_button == '') {
...
function zen_draw_separator($image = 'true', $width = '100%', $height = '1')
{
    ...
    } else if (!strstr((string) $image, DIR_WS_TEMPLATE_IMAGES)) {
 ...
 * @since ZC v1.0.3
 */
function zen_js_zone_list(string $country, string $form, string $field): string
{
    global $db;
 ...
 * @since ZC v1.0.3
 */
function zen_draw_form(?string $name, ?string $action, $method = 'post', ?string $parameters = ''): string
{
 ...
 * @since ZC v1.0.3
 */
function zen_draw_input_field($name, $value = '', ?string $parameters = '', ?string $type = 'text', $reinsert_value = true, $required = false)
{
    // -----
    // Give an observer the opportunity to **totally** override this function's operation.
    //
    $field = false;
    $GLOBALS['zco_notifier']->notify('NOTIFY_ZEN_DRAW_INPUT_FIELD_OVERRIDE', ['name' => $name, 'value' => $value, 'parameters' => $parameters, 'type' => $type, 'reinsert_value' => $reinsert_value, 'required' => $required], $field);
    if ($field !== false) {
        return $field;
    }
    ...
    //
    $GLOBALS['zco_notifier']->notify('NOTIFY_ZEN_DRAW_INPUT_FIELD', ['name' => $name, 'value' => $value, 'parameters' => $parameters, 'type' => $type, 'reinsert_value' => $reinsert_value, 'required' => $required], $field);
 ...
 * @since ZC v1.0.3
 */
function zen_draw_selection_field($name, ?string $type, $value = '', $checked = false, ?string $parameters = '')
{
    // -----
    // Give an observer the opportunity to **totally** override this function's operation.
    //
    $selection = false;
    $GLOBALS['zco_notifier']->notify('NOTIFY_ZEN_DRAW_SELECTION_FIELD_OVERRIDE', ['name' => $name, 'value' => $value, 'parameters' => $parameters, 'type' => $type, 'checked' => $checked], $selection);
    if ($selection !== false) {
        return $selection;
    }
    ...
    // Give an observer the opportunity to modify the just-rendered field.
    //
    $GLOBALS['zco_notifier']->notify('NOTIFY_ZEN_DRAW_SELECTION_FIELD', ['name' => $name, 'value' => $value, 'parameters' => $parameters, 'type' => $type, 'checked' => $checked], $selection);
    return $selection;
}
/**
 ...
 * @since ZC v1.0.3
 */
function zen_draw_textarea_field($name, ?string $width, ?string $height, string $text = '~*~*#', ?string $parameters = '', $reinsert_value = true)
{
    // -----
    // Give an observer the opportunity to **totally** override this function's operation.
    //
    $field = false;
    $GLOBALS['zco_notifier']->notify('NOTIFY_ZEN_DRAW_TEXTAREA_FIELD_OVERRIDE', ['name' => $name, 'width' => $width, 'height' => $height, 'text' => $text, 'parameters' => $parameters, 'reinsert_value' => $reinsert_value], $field);
    if ($field !== false) {
        return $field;
    }
    ...
    // Give an observer the opportunity to modify the just-rendered field.
    //
    $GLOBALS['zco_notifier']->notify('NOTIFY_ZEN_DRAW_TEXTAREA_FIELD', ['name' => $name, 'width' => $width, 'height' => $height, 'text' => $text, 'parameters' => $parameters, 'reinsert_value' => $reinsert_value], $field);
    return $field;
}
/**
...
{
    return zen_draw_input_field($name, '', ' size="50" ', 'file', false, $required);
}
/**
 ...
 * @since ZC v1.0.3
 */
function zen_draw_pull_down_menu($name, $values, $default = '', ?string $parameters = '', $required = false)
{
    // -----
    // Give an observer the opportunity to **totally** override this function's operation.
    //
    $field = false;
    $GLOBALS['zco_notifier']->notify('NOTIFY_ZEN_DRAW_PULL_DOWN_MENU_OVERRIDE', ['name' => $name, 'values' => $values, 'default' => $default, 'parameters' => $parameters, 'required' => $required], $field);
    if ($field !== false) {
        return $field;
    }
    $field = '<select ';
    ...
    //
    $GLOBALS['zco_notifier']->notify('NOTIFY_ZEN_DRAW_PULL_DOWN_MENU', ['name' => $name, 'values' => $values, 'default' => $default, 'parameters' => $parameters, 'required' => $required], $field);
    return $field;
}
/**
...
function zen_get_country_list($name, $selected = '', ?string $parameters = '')
{
    ...
    if (SHOW_CREATE_ACCOUNT_DEFAULT_COUNTRY !== '' && STORE_COUNTRY !== SHOW_CREATE_ACCOUNT_DEFAULT_COUNTRY) {
        $countries_at_top_of_list[] = SHOW_CREATE_ACCOUNT_DEFAULT_COUNTRY;
    }
    $countries_at_top_of_list[] = STORE_COUNTRY;
    ...
    foreach ($countries_at_top_of_list as $val) {
        // -----
    ...
    for ($i = 0, $n = count($countries); $i < $n; $i++) {
        ...
        foreach ($countries_at_top_of_list as $val) {
            if ($countries[$i]['countries_id'] == $val) {
                ...
                break;
                // found the duplicate, no further need to process this loop
            }
        }
        if (!$already_in_list) {
 ...
 * @since ZC v1.3.8
 */
function zen_href_params($page = '', ?string $parameters = ''): ?string
{
    global $current_page_base;
    $addparms = '';
    // if nofollow has already been set, ignore this function
    if (stristr((string) $parameters, 'nofollow')) {
        return $parameters;
    }
