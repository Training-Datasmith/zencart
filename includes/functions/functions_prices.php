...
function zen_get_products_special_price($product_id, $specials_price_only = false)
{
    ...
    if ($product->EOF || Customer::is_wholesale_customer() === true) {
        return false;
    }
    ...
    return match ($sale['sale_specials_condition']) {
        0 => number_format($sale_product_price, 4, '.', ''),
        1 => number_format($special_price, 4, '.', ''),
        2 => number_format($sale_special_price, 4, '.', ''),
        default => number_format($special_price, 4, '.', ''),
    };
}
/**
...
function zen_get_products_display_price($product_id)
{
    ...
    $has_wholesale_price = Customer::is_wholesale_customer() === true && $product_check->fields['products_price_w'] !== '0';
    ...
    if (SHOW_SALE_DISCOUNT_STATUS === '1' && ($display_special_price != 0 || $display_sale_price != 0)) {
        ...
        $zco_notifier->notify('NOTIFY_ZEN_GET_PRODUCTS_DISPLAY_PRICE_SALE', ['products_id' => $product_id, 'display_sale_price' => $display_sale_price, 'display_special_price' => $display_special_price, 'display_normal_price' => $display_normal_price, 'products_tax_class_id' => $product_check->fields['products_tax_class_id']], $pricing_handled, $show_sale_discount);
        if ($display_sale_price) {
            if (SHOW_SALE_DISCOUNT === '1') {
                ...
                $show_sale_discount = '<span class="productPriceDiscount">' . $show_sale_discount .= '<br>' . PRODUCT_PRICE_DISCOUNT_PREFIX . $currencies->display_price($display_normal_price - $display_sale_price, $products_tax_rate) . PRODUCT_PRICE_DISCOUNT_AMOUNT . '</span>';
            }
        } elseif (SHOW_SALE_DISCOUNT === '1') {
            $show_sale_discount = '<span class="productPriceDiscount">' . '<br>' . PRODUCT_PRICE_DISCOUNT_PREFIX . number_format(100 - $display_special_price / $display_normal_price * 100, (int) SHOW_SALE_DISCOUNT_DECIMALS) . PRODUCT_PRICE_DISCOUNT_PERCENTAGE . '</span>';
        } else {
            $show_sale_discount = '<span class="productPriceDiscount">' . '<br>' . PRODUCT_PRICE_DISCOUNT_PREFIX . $currencies->display_price($display_normal_price - $display_special_price, $products_tax_rate) . PRODUCT_PRICE_DISCOUNT_AMOUNT . '</span>';
        }
    }
    if ($display_special_price) {
        ...
        //
        $pricing_handled = false;
        $zco_notifier->notify('NOTIFY_ZEN_GET_PRODUCTS_DISPLAY_PRICE_SPECIAL', ['products_id' => $product_id, 'display_sale_price' => $display_sale_price, 'display_special_price' => $display_special_price, 'display_normal_price' => $display_normal_price, 'products_tax_class_id' => $product_check->fields['products_tax_class_id'], 'product_is_free' => $product_check->fields['product_is_free']], $pricing_handled, $show_normal_price, $show_special_price, $show_sale_price);
        $show_normal_price = '<span class="normalprice">' . $currencies->display_price($display_normal_price, $products_tax_rate) . ' </span>';
        if ($display_sale_price && $display_sale_price != $display_special_price) {
            $show_special_price = '&nbsp;' . '<span class="productSpecialPriceSale">' . $currencies->display_price($display_special_price, $products_tax_rate) . '</span>';
            if ($product_check->fields['product_is_free'] === '1') {
                $show_sale_price = '<br>' . '<span class="productSalePrice">' . PRODUCT_PRICE_SALE . '<s>' . $currencies->display_price($display_sale_price, $products_tax_rate) . '</s>' . '</span>';
            } else {
                $show_sale_price = '<br>' . '<span class="productSalePrice">' . PRODUCT_PRICE_SALE . $currencies->display_price($display_sale_price, $products_tax_rate) . '</span>';
            }
        } else {
            if ($product_check->fields['product_is_free'] === '1') {
                $show_special_price = '&nbsp;' . '<span class="productSpecialPrice">' . '<s>' . $currencies->display_price($display_special_price, $products_tax_rate) . '</s>' . '</span>';
            } else {
                $show_special_price = '&nbsp;' . '<span class="productSpecialPrice">' . $show_special_price .= $currencies->display_price($display_special_price, $products_tax_rate) . '</span>';
            }
            $show_sale_price = '';
        }
    } else {
        ...
        if ($display_sale_price) {
            ...
            if ($has_wholesale_price === true) {
                $show_normal_price = '<span class="normalprice">' . $currencies->display_price($display_normal_price, $products_tax_rate) . '</span>';
                $show_sale_price = '<span class="productSalePrice wholesale-price">' . PRODUCT_PRICE_WHOLESALE . $currencies->display_price($display_wholesale_price, $products_tax_rate) . '</span>';
            } elseif ($product_check->fields['product_is_free'] === '1') {
                $show_normal_price = '<span class="productFreePrice">' . '<s>' . $currencies->display_price($display_normal_price, $products_tax_rate) . '</s>' . '</span>';
            } else {
                $show_normal_price = '<span class="productBasePrice">' . $currencies->display_price($display_normal_price, $products_tax_rate) . '</span>';
            }
        }
    }
    if ($display_normal_price == 0) {
    ...
    //
    $tags_handled = false;
    $zco_notifier->notify('NOTIFY_ZEN_GET_PRODUCTS_DISPLAY_PRICE_FREE_OR_CALL', ['product_is_free' => $product_check->fields['product_is_free'], 'product_is_call' => $product_check->fields['product_is_call'], 'product_id' => $product_id], $tags_handled, $free_tag, $call_tag);
    // If Free, Show it
    if ($product_check->fields['product_is_free'] === '1') {
        $free_tag = '<br>';
 ...
 * @since ZC v1.1.0
 */
function zen_get_products_quantity_min_units_display($product_id, $include_break = true, $message_is_for_shopping_cart = false): string
{
    $result = zen_get_product_details($product_id);
    if ($result->EOF) {
        return '';
...
function zen_get_discount_calc($product_id, $attribute_id = 0, $attributes_amount = 0, $check_qty = 0)
{
    ...
    switch (true) {
        ...
        case $discount_type_id == 20:
            // This option should not do anything to basic attributes without further consideration of the overall effect on the price and the starting price.
        ...
        case $discount_type_id == 209:
            // This option for attributes should not do anything unless the price of the product is solely dependent on a single attribute, all attributes can be reduced a constant amount (non-zero).
...
function zen_get_products_sale_discount_type($product_id = false, $categories_id = false, $return_value = 'type')
{
    ...
    */
    ...
    foreach ($results as $result) {
        ...
        $categories = explode(',', (string) $result['sale_categories_all']);
    ...
    if ($check_special) {
        return $sale_maker_discount_type * 10 + 9;
    }
 ...
 * @since ZC v1.1.0
 */
function zen_get_products_sale_discount($product_id = false, $categories_id = false, $display_type = false): void
{
 ...
 * @since ZC v1.2.0d
 */
function zen_get_attributes_price_factor($price, $special, $factor, $offset): int|float
{
 ...
 * @since ZC v1.2.0d
 */
function zen_get_attributes_qty_prices_onetime($string, $qty): int|string|array
{
    if (empty($string)) {
        return 0;
    }
...
function zen_get_attributes_price_final($attribute_id, $qty = 1, $pre_selected = null, $include_onetime = false, $prod_priced_by_attr = false, $attributes_discounted = 0, $include_products_price_in = false)
{
    ...
    // onetime charges
    if ($include_onetime === true || $include_onetime === 'true') {
        // string for backward compat prior to 1.5.8
 ...
 * @since ZC v1.2.0d
 */
function zen_get_attributes_price_final_onetime($attribute_id, $qty = 1, $pre_selected_onetime = null): float|int|array
{
 ...
 * @since ZC v1.2.0d
 */
function zen_get_word_count($string, $free = 0): float|int
{
    $string = str_replace(["\r\n", "\n", "\r", "\t"], ' ', $string);
    if ($string !== '') {
 ...
 * @since ZC v1.2.0d
 */
function zen_get_word_count_price($string, $free = 0, $price = 0): int|float
{
 ...
 * @since ZC v1.2.0d
 */
function zen_get_letters_count($string, $free = 0): float|int
{
    $string = str_replace(["\r\n", "\n", "\r", "\t"], ' ', $string);
    $string = preg_replace('/[ ]+/', ' ', $string);
    $string = trim((string) $string);
 ...
 * @since ZC v1.2.0d
 */
function zen_get_letters_count_price($string, $free = 0, $price = 0): int|float
{
 ...
 * @since ZC v1.2.0d
 */
function zen_update_products_price_sorter($product_id): void
{
    global $db;
    if (empty($product_id)) {
        return;
 ...
 * @since ZC v1.2.1d
 */
function zen_parse_salemaker_categories($categories_csv): array
{
 ...
 * @since ZC v1.2.1d
 */
function zen_update_salemaker_product_prices($salemaker_id): bool
{
    global $db;
...
function zen_get_retail_or_wholesale_price($retail_price, string $wholesale_pricing_tier): string
{
    ...
    if (Customer::is_wholesale_customer() === false || empty($wholesale_pricing_tier)) {
    ...
    //
    $wholesale_index = Customer::get_customer_wholesale_tier() - 1;
