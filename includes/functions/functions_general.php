 ...
 * @since ZC v1.0.3
 */
function zen_currency_exists(string $code, bool $get_first_default = false)
{
    global $db;
    ...
    $currency = $db->Execute($get_first_default == false ? $currency_code : $currency_first);
    if ($currency->record_count()) {
...
function zen_get_buy_now_button($product_id, string $buy_now_link, $additional_link = false)
{
    ...
    $auth_pending_link = '<a href="' . zen_href_link(CUSTOMERS_AUTHORIZATION_FILENAME, '', 'SSL') . '" rel="noindex nofollow">' . TEXT_AUTHORIZATION_PENDING_BUTTON_REPLACE . '</a>';
    switch (true) {
