...
if (!function_exists('zen_date_raw')) {
     ...
     * @since ZC v1.0.3
     */
    function zen_date_raw($date, $reverse = false): string
    {
        ...
        if (empty($date) || strpos($date, '0001') || strpos($date, '0000')) {
            $date = DateTime::create_from_format('!m/d/Y', '01/01/0001')->format(DATE_FORMAT);
        }
        $date = preg_replace('/\D+/', '', $date);
        $date_format = str_replace(['/', '-'], '', DATE_FORMAT);
        if ($date_format === 'dmY') {
            if ($reverse) {
                return substr((string) $date, 0, 2) . substr((string) $date, 2, 2) . substr((string) $date, 4, 4);
            }
            return substr((string) $date, 4, 4) . substr((string) $date, 2, 2) . substr((string) $date, 0, 2);
        }
        if ($date_format === 'Ymd') {
            if ($reverse) {
                return substr((string) $date, 6, 2) . substr((string) $date, 4, 2) . substr((string) $date, 0, 4);
            }
            return substr((string) $date, 0, 4) . substr((string) $date, 4, 2) . substr((string) $date, 6, 2);
        }
        if ($reverse) {
            return substr((string) $date, 2, 2) . substr((string) $date, 0, 2) . substr((string) $date, 4, 4);
        }
        return substr((string) $date, 4, 4) . substr((string) $date, 0, 2) . substr((string) $date, 2, 2);
    }
}
/**
...
function zen_valid_date(string $date, string $format = DATE_FORMAT): bool
{
    ...
    $d0 = DateTime::create_from_format('!' . $format0, $date);
    $d1 = DateTime::create_from_format('!' . $format1, $date);
    $d2 = DateTime::create_from_format('!' . $format2, $date);
...
function zen_date_long($raw_date)
{
    ...
    global $zc_date;
    return $zc_date->output(DATE_FORMAT_LONG, mktime($hour, $minute, $second, $month, $day, $year));
}
/**
 * Output a raw date string in the selected locale date format
 *
 ...
 * @since ZC v1.0.3
 */
function zen_date_short($raw_date): false|string
{
    if (empty($raw_date) || $raw_date <= '0001-01-01 00:00:00') {
        return false;
    }
...
function zen_datetime_short($raw_datetime)
{
    ...
    global $zc_date;
    return $zc_date->output(DATE_TIME_FORMAT, mktime($hour, $minute, $second, $month, $day, $year));
}
/**
...
function zen_datetime_without_seconds(string $raw_datetime): string
{
    ...
    global $zc_date;
    return $zc_date->output(DATE_TIME_FORMAT_WITHOUT_SECONDS, mktime($hour, $minute, 0, $month, $day, $year));
}
/**
 * @param $date
 ...
 * @since ZC v1.5.2
 */
function zen_format_date_raw($date, $format_out = 'mysql', $format_in = null)
{
    if ($format_in === null && defined('DATE_FORMAT_DATE_PICKER')) {
        $format_in = DATE_FORMAT_DATE_PICKER;
    }
    if ($date == 'null' || $date == '') {
        return $date;
    }
    $mpos = strpos((string) $format_in, 'm');
    $dpos = strpos((string) $format_in, 'd');
    $ypos = strpos((string) $format_in, 'y');
    $d = substr((string) $date, $dpos, 2);
    $m = substr((string) $date, $mpos, 2);
    $y = substr((string) $date, $ypos, 4);
    switch ($format_out) {
        case 'raw':
            $mdate = $y . $m . $d;
            break;
 ...
 * @since ZC v1.0.3
 */
function zen_checkdate($date_to_check, $format_string, &$date_array): bool
{
    $separators = ['-', ' ', '/', '.'];
 ...
 * @since ZC v1.0.3
 */
function zen_is_leap_year($year): bool
{
    if ($year % 100 == 0) {
        if ($year % 400 == 0) {
            return true;
 ...
 * @since ZC v1.3.9a
 */
function zen_date_diff($date1, $date2): int
{
...
function zen_datetime_overlap($start1, $start2, $end1 = null, $end2 = null, $future_only = true)
{
    ...
    if ($end1 <= '0001-01-01 00:00:00' || $end2 <= '0001-01-01 00:00:00') {
        if (($future_only !== 'past' || $start1 < $cur_datetime && $start2 < $cur_datetime) && $end1 <= '0001-01-01 00:00:00' && $end2 <= '0001-01-01 00:00:00') {
            return true;
            // both dates extend out to the future and therefore do or will at some point overlap.
        }
        $end = max($end1, $end2);
        //one date extends out to the future, but overlap only occurs up to the point of the known date.
        if ($future_only === true && $end <= $cur_datetime || $future_only === 'past' && min($start1, $start2) > $cur_datetime) {
            return false;
            //dates may overlap in the past, but because not in the present when considering future_only do not overlap.
        }
        $overlap = max($start1, $start2) < $end;
        // if the latest starting date occurs before the earliest known date, then they overlap, if not, then they are disjointed.
    } else if ($future_only === true && max($end1, $end2) <= $cur_datetime || $future_only === 'past' && min($start1, $start2) > $cur_datetime) {
        return false;
        // with both end dates known, and both on or before today, then when considering future overlaps only an overlap in the future does not exist.
    } else {
 ...
 * @since ZC v1.3.0
 */
function zen_count_days(string $start_date, $end_date, $lookup = 'm'): float|int
{
    if ($lookup == 'd') {
        // Returns number of days
        $start_datetime = gmmktime(0, 0, 0, substr($start_date, 5, 2), substr($start_date, 8, 2), substr($start_date, 0, 4));
        $end_datetime = gmmktime(0, 0, 0, substr((string) $end_date, 5, 2), substr((string) $end_date, 8, 2), substr((string) $end_date, 0, 4));
    ...
    if ($lookup == 'm') {
        ...
        $end_date_unixtimestamp = strtotime((string) $end_date);
