...
{
    global $message_stack, $language_loader;
    if (IS_ADMIN_FLAG === true) {
    ...
    if ($language_loader->has_language_file(DIR_FS_CATALOG . DIR_WS_LANGUAGES, $_SESSION['language'], $file, $folder)) {
        $language_loader->load_extra_language_files(DIR_FS_CATALOG . DIR_WS_LANGUAGES, $_SESSION['language'], $file, $folder);
    } else {
        if ($page === 'inline') {
            ?>
          <div class="messageStackCaution">
             <?php 
            echo WARNING_COULD_NOT_LOCATE_LANG_FILE . $lang_file;
            ?>
          </div>
<?php 
        } else if (is_object($message_stack)) {
            if (IS_ADMIN_FLAG === false) {
                $message_stack->add($page, WARNING_COULD_NOT_LOCATE_LANG_FILE . $lang_file, 'caution');
            } else {
...
function zen_get_index_filters_directory($check_file, $dir_only = false): string
{
    ...
    $check_array = [];
    $check_array[] = DIR_WS_INCLUDES . 'index_filters/' . $template_dir . '/' . $zv_filename;
    $check_array[] = DIR_WS_INCLUDES . 'index_filters/' . $zv_filename;
    $check_array[] = DIR_WS_INCLUDES . 'index_filters/' . $template_dir . '/' . 'default_filter.php';
...
{
    global $message_stack, $zen_remove_error;
 ...
 * @since ZC v1.5.5
 */
function get_logs_data($max_to_list = 'count')
{
    global $zc_date;
    ...
    $logs = [];
    $i = 0;
    foreach ([DIR_FS_LOGS, DIR_FS_SQL_CACHE] as $purge_folder) {
        $purge_folder = rtrim((string) $purge_folder, '/');
        if (!file_exists($purge_folder)) {
            continue;
        }
        if (!is_dir($purge_folder)) {
            continue;
        }
        $dir = dir($purge_folder);
        while ($logfile = $dir->read()) {
            ...
            if ($max_to_list != 'count') {
                $filename = $purge_folder . '/' . $logfile;
                $logs[$i]['path'] = $purge_folder . '/';
                $logs[$i]['filename'] = $logfile;
                $logs[$i]['filesize'] = @filesize($filename);
                $logs[$i]['unixtime'] = @filemtime($filename);
                $logs[$i]['datetime'] = $zc_date->output(DATE_TIME_FORMAT, $logs[$i]['unixtime']);
            }
            $i++;
            if ($max_to_list != 'count' && $i >= $max_to_list) {
                break;
            }
        }
        $dir->close();
