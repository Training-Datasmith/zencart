 ...
 * @since ZC v1.2.0d
 */
function zen_set_featured_status(int $featured_id, int $status): void
{
    global $db;
 ...
 * @since ZC v1.2.0d
 */
function zen_expire_featured(): void
{
    global $db;
    $date_range = time();
 ...
 * @since ZC v1.2.0d
 */
function zen_start_featured(): void
{
    global $db;
    $date_range = time();
    $zc_featured_date = date('Ymd', $date_range);
    ...
    if ($featureds_on->record_count() > 0) {
    ...
    if ($featureds_off->record_count() > 0) {
...
function zen_set_featured_category_status(int $category_id, int $status): void
{
    ...
    $sql = 'UPDATE ' . TABLE_FEATURED_CATEGORIES . '
          SET status = ' . $status . ',
              date_status_change = now()
          WHERE featured_categories_id = ' . $category_id;
    $db->Execute($sql);
}