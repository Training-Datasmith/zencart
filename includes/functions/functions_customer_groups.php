...
function zen_customer_belongs_to_group($customer_id, $group_id, $ignore_cache = false): bool
{
    ...
    if (!$ignore_cache) {
        ...
        $results = $db->execute_no_cache($sql);
    }
    return !$results->EOF;
}
/**
...
function zen_assign_customer_to_group($customer_id, $group_id): bool
{
    ...
    $sql_data_array = ['customer_id' => (int) $customer_id, 'group_id' => (int) $group_id];
...
function zen_create_customer_group($group_name, $group_comment): string|int
{
    ...
    $sql_data_array = ['group_name' => zen_db_input($group_name), 'group_comment' => zen_db_input($group_comment)];
...
function zen_update_customer_group($group_id, $data): string|true
{
    ...
    foreach ($valid_fields as $field) {
        if (isset($data[$field])) {
            $sql_data_array[$field] = $db->prepare_input($data[$field]);
        }
    }
...
{
    global $db;
    $sql = 'SELECT group_comment FROM ' . TABLE_CUSTOMER_GROUPS . '
            WHERE group_name = :group_name:';
    $sql = $db->bind_vars($sql, ':group_name:', $group_name, 'stringIgnoreNull');
...
function zen_get_customer_group_id_from_name(string $group_name): int
{
    ...
    $sql = $db->bind_vars($sql, ':group_name:', $group_name, 'stringIgnoreNull');
    return (int) ($db->Execute($sql, 1)->fields['group_id'] ?? -1);
    // Don't assign 0, that's the "Everyone" default, -1 should NEVER match
}