...
class Mobile_Detect
{
     ...
     */
    public function __construct(protected ?Cache_Interface $cache = new Cache(), array $config = [])
    {
    ...
    public function get_http_header(string $header): ?string
    {
        ...
        return $this->http_headers[$header] ?? $this->http_headers[$alt_header] ?? null;
    }
            ...
            if (isset($this->http_headers[$mobile_header])) {
                if (isset($match_type['matches']) && is_array($match_type['matches'])) {
                    foreach ($match_type['matches'] as $_match) {
                        if (str_contains($this->http_headers[$mobile_header], (string) $_match)) {
                            return true;
                        }
                    }
                    return false;
    ...
    public function version(string $property_name, string $type = self::VERSION_TYPE_STRING): float|bool|string
    {
        ...
        if (true === isset($properties[$property_name])) {
            ...
            foreach ($properties[$property_name] as $property_match_string) {
                ...
                preg_match(sprintf('#%s#is', $property_pattern), (string) $this->user_agent, $match);
