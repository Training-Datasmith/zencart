...
use Bacon_Qr_Code\Common\Bit_Utils;
use Bacon_Qr_Code\Exception\InvalidArgumentException;
/**
 * Mask utility.
 */
final class Mask_Util
{
    /**#@+
     ...
     */
    public static function apply_mask_penalty_rule1(Byte_Matrix $matrix): int
    {
        return self::apply_mask_penalty_rule1internal($matrix, true) + self::apply_mask_penalty_rule1internal($matrix, false);
    }
    /**
     ...
     */
    public static function apply_mask_penalty_rule2(Byte_Matrix $matrix): int
    {
        $penalty = 0;
     ...
     * twice (i.e. 40 * 2).
     */
    public static function apply_mask_penalty_rule3(Byte_Matrix $matrix): int
    {
        $penalty = 0;
        $array = $matrix->get_array();
        $width = $matrix->get_width();
     ...
     */
    public static function apply_mask_penalty_rule4(Byte_Matrix $matrix): int
    {
        $num_dark_cells = 0;
        $array = $matrix->get_array();
        $width = $matrix->get_width();
        ...
        $num_total_cells = $height * $width;
        $dark_ratio = $num_dark_cells / $num_total_cells;
        $fixed_percent_variances = (int) floor(abs($dark_ratio - 0.5) * 20);
        return $fixed_percent_variances * self::N4;
    }
    /**
     ...
     */
    public static function get_data_mask_bit(int $mask_pattern, int $x, int $y): bool
    {
        switch ($mask_pattern) {
            case 0:
            ...
            case 3:
                ...
                break;
            case 4:
                $intermediate = Bit_Utils::unsigned_right_shift($y, 1) + (int) ($x / 3) & 0x1;
                break;
            case 5:
                $temp = $y * $x;
            ...
            case 7:
                ...
                break;
            default:
                throw new InvalidArgumentException('Invalid mask pattern: ' . $mask_pattern);
        }
     ...
     * orders respectively.
     */
    private static function apply_mask_penalty_rule1internal(Byte_Matrix $matrix, bool $is_horizontal): int
    {
        $penalty = 0;
        $i_limit = $is_horizontal ? $matrix->get_height() : $matrix->get_width();
        $j_limit = $is_horizontal ? $matrix->get_width() : $matrix->get_height();
        $array = $matrix->get_array();
        for ($i = 0; $i < $i_limit; ++$i) {
            $num_same_bit_cells = 0;
            $prev_bit = -1;
            for ($j = 0; $j < $j_limit; $j++) {
                $bit = $is_horizontal ? $array[$i][$j] : $array[$j][$i];
                if ($bit === $prev_bit) {
                    ++$num_same_bit_cells;
                } else {
                    if ($num_same_bit_cells >= 5) {
                        $penalty += self::N1 + ($num_same_bit_cells - 5);
                    }
