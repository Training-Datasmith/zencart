...
class category_tree extends base
{
     ...
     * @since ZC v1.2.0d
     */
    private ?string $categories_string = null;
    /*
     * Array of categories from database
     */
    private array $tree = [];
    public function zen_category_tree(string $product_type = 'all')
    {
        global $db, $c_path, $c_path_array;
        if ($product_type != 'all') {
        ...
        while (!$categories->EOF) {
            $this->tree[$categories->fields['categories_id']] = ['name' => $categories->fields['categories_name'], 'parent' => $categories->fields['parent_id'], 'level' => 0, 'path' => $categories->fields['categories_id'], 'image' => $categories->fields['categories_image'], 'next_id' => false];
            if (isset($parent_id)) {
            ...
            $categories->move_next();
        }
        if (zen_not_null($c_path)) {
            $new_path = '';
            foreach ($c_path_array as $key => $value) {
                unset($parent_id);
                unset($first_id);
                if ($product_type == 'all') {
                    $categories_query = 'select c.categories_id, cd.categories_name, c.parent_id, c.categories_image
                ...
                if ($rows->record_count() > 0) {
                    $new_path .= $value;
                    while (!$rows->EOF) {
                        $this->tree[$rows->fields['categories_id']] = ['name' => $rows->fields['categories_name'], 'parent' => $rows->fields['parent_id'], 'level' => $key + 1, 'path' => $new_path . '_' . $rows->fields['categories_id'], 'image' => $categories->fields['categories_image'], 'next_id' => false];
                        if (isset($parent_id)) {
                    ...
                    if (!empty($value) && !empty($this->tree[$value])) {
    ...
    public function zen_show_category(string $counter, $ii): array
    {
        ...
        if ($this->tree[$counter]['parent'] == TOPMOST_CATEGORY_PARENT_ID) {
            $c_path_new = 'cPath=' . $counter;
            $this->box_categories_array[$ii]['top'] = 'true';
        } else {
            $this->box_categories_array[$ii]['top'] = 'false';
            $c_path_new = 'cPath=' . $this->tree[$counter]['path'];
            $this->categories_string .= CATEGORIES_SEPARATOR_SUBS;
        }
        $this->box_categories_array[$ii]['path'] = $c_path_new;
        if (isset($c_path_array) && in_array($counter, $c_path_array)) {
