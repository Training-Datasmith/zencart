...
class order extends base
{
     ...
     */
    public bool $do_stock_decrement;
    /**
     ...
     */
    public ?bool $query_return_flag = null;
    /**
    ...
    {
        global $db;
        $this->query_return_flag = null;
        $this->notify('NOTIFY_ORDER_BEFORE_QUERY', [], $order_id);
        if ($this->query_return_flag === true) {
            return;
        }
        ...
        $this->order_id = $order_id = (int) $order_id;
        ...
        while (!$totals->EOF) {
            ...
            $this->totals[] = ['title' => $totals->fields['class'] === 'ot_coupon' ? $zc_coupon_link . $totals->fields['title'] . '</a>' : $totals->fields['title'], 'text' => $totals->fields['text'], 'class' => $totals->fields['class'], 'value' => $totals->fields['value']];
            $totals->move_next();
        }
        $this->info = ['order_id' => $this->order_id, 'customer_id' => $order->fields['customers_id'], 'currency' => $order->fields['currency'], 'currency_value' => $order->fields['currency_value'], 'payment_method' => $order->fields['payment_method'], 'payment_module_code' => $order->fields['payment_module_code'], 'shipping_method' => $order->fields['shipping_method'], 'shipping_module_code' => $order->fields['shipping_module_code'], 'coupon_code' => $order->fields['coupon_code'], 'cc_type' => $order->fields['cc_type'], 'cc_owner' => $order->fields['cc_owner'], 'cc_number' => $order->fields['cc_number'], 'cc_cvv' => $order->fields['cc_cvv'], 'cc_expires' => $order->fields['cc_expires'], 'date_purchased' => $order->fields['date_purchased'], 'orders_status' => $order->fields['orders_status'], 'total' => $order->fields['order_total'], 'tax' => $order->fields['order_tax'], 'shipping_tax_rate' => $order->fields['shipping_tax_rate'], 'last_modified' => $order->fields['last_modified'], 'ip_address' => $order->fields['ip_address'], 'language_code' => $order->fields['language_code'], 'order_weight' => $order->fields['order_weight'], 'is_wholesale' => $order->fields['is_wholesale']];
        /**
         * Dev Notes:
         ...
         */
        $this->customer = ['id' => $order->fields['customers_id'], 'name' => $order->fields['customers_name'], 'company' => $order->fields['customers_company'], 'street_address' => $order->fields['customers_street_address'], 'suburb' => $order->fields['customers_suburb'], 'city' => $order->fields['customers_city'], 'postcode' => $order->fields['customers_postcode'], 'state' => $order->fields['customers_state'], 'country' => $this->get_country_info($order->fields['customers_country']), 'format_id' => $order->fields['customers_address_format_id'], 'telephone' => $order->fields['customers_telephone'], 'email_address' => $order->fields['customers_email_address']];
        $this->customer['zone_id'] = $this->get_country_zone_id((int) $this->customer['country']['id'], $this->customer['state']);
        $this->delivery = ['name' => $order->fields['delivery_name'], 'company' => $order->fields['delivery_company'], 'street_address' => $order->fields['delivery_street_address'], 'suburb' => $order->fields['delivery_suburb'], 'city' => $order->fields['delivery_city'], 'postcode' => $order->fields['delivery_postcode'], 'state' => $order->fields['delivery_state'], 'country' => $this->get_country_info($order->fields['delivery_country']), 'format_id' => $order->fields['delivery_address_format_id']];
        $this->delivery['zone_id'] = $this->get_country_zone_id((int) $this->delivery['country']['id'], $this->delivery['state']);
        if ($order->fields['shipping_module_code'] === 'storepickup' || empty($this->delivery['name']) && empty($this->delivery['street_address'])) {
            $this->delivery = false;
        }
        $this->billing = ['name' => $order->fields['billing_name'], 'company' => $order->fields['billing_company'], 'street_address' => $order->fields['billing_street_address'], 'suburb' => $order->fields['billing_suburb'], 'city' => $order->fields['billing_city'], 'postcode' => $order->fields['billing_postcode'], 'state' => $order->fields['billing_state'], 'country' => $this->get_country_info($order->fields['billing_country']), 'format_id' => $order->fields['billing_address_format_id']];
        $this->billing['zone_id'] = $this->get_country_zone_id((int) $this->billing['country']['id'], $this->billing['state']);
        $index = 0;
        ...
        while (!$orders_products->EOF) {
            ...
            if ($precision !== 0 && str_contains((string) $new_qty, '.')) {
                $new_qty = rtrim((string) $new_qty, '0');
            }
            ...
            if ($attributes->record_count()) {
                $this->products[$index]['attributes'] = [];
                while (!$attributes->EOF) {
                    $this->products[$index]['attributes'][$subindex] = ['option' => $attributes->fields['products_options'], 'value' => $attributes->fields['products_options_values'], 'option_id' => $attributes->fields['products_options_id'], 'value_id' => $attributes->fields['products_options_values_id'], 'prefix' => $attributes->fields['price_prefix'], 'price' => $attributes->fields['options_values_price'], 'product_attribute_is_free' => (int) $attributes->fields['product_attribute_is_free'], 'weight' => $attributes->fields['products_attributes_weight'], 'weight_prefix' => $attributes->fields['products_attributes_weight_prefix']];
                    $subindex++;
                    $attributes->move_next();
                }
            }
            $this->info['tax_groups']["{$this->products[$index]['tax']}"] = '1';
            $this->notify('NOTIFY_ORDER_QUERY_ADD_PRODUCT', $this->products[$index], $index, $orders_products->fields);
            $index++;
            $orders_products->move_next();
        }
        $this->statuses = $this->get_status_history($this->order_id);
        $this->notify('NOTIFY_ORDER_AFTER_QUERY', IS_ADMIN_FLAG, $this->order_id, $order->fields);
        /**
         * @deprecated since v1.5.6; use NOTIFY_ORDER_AFTER_QUERY instead
         * @since ZC v1.5.8
         */
        if (IS_ADMIN_FLAG === true) {
            $this->notify('ORDER_QUERY_ADMIN_COMPLETE', ['orders_id' => $this->order_id]);
        }
    }
    protected function get_status_history($order_id, $language_id = null): array
    {
        global $db;
        ...
        $sql = $db->bind_vars($sql, ':ordersID', $order_id, 'integer');
        $sql = $db->bind_vars($sql, ':languageID', $language_id, 'integer');
     ...
     * @since ZC v1.5.8
     */
    protected function get_country_info(string $country): array
    {
        global $db;
        $sql = 'SELECT countries_id, countries_name, countries_iso_code_2, countries_iso_code_3, status
            FROM ' . TABLE_COUNTRIES . '
            WHERE countries_name = :country';
        $sql = $db->bind_vars($sql, ':country', $country, 'string');
        $results = $db->Execute($sql);
        if (!$results->EOF) {
            $result = $results->fields;
            return ['id' => $result['countries_id'], 'title' => $country, 'iso_code_2' => $result['countries_iso_code_2'], 'iso_code_3' => $result['countries_iso_code_3']];
        }
        return ['id' => 0, 'title' => $country, 'iso_code_2' => '', 'iso_code_3' => ''];
    }
    /**
     * @since ZC v1.5.8
     */
    protected function get_country_zone_id(int $countries_id, string $state)
    {
        global $db;
        $sql = 'SELECT zone_id
               FROM ' . TABLE_ZONES . "\n              WHERE zone_country_id = {$countries_id}\n                AND (zone_code = :state: OR zone_name = :state:)\n              LIMIT 1";
        $sql = $db->bind_vars($sql, ':state:', $state, 'string');
        $results = $db->Execute($sql);
    ...
    public function cart(): void
    {
        ...
        $payment_module = !empty($_SESSION['payment']) ? $_SESSION['payment'] : 'NOT SET YET';
        ...
        $this->info = [
            ...
            'payment_method' => isset($GLOBALS[$payment_module]) && is_object($GLOBALS[$payment_module]) ? $GLOBALS[$payment_module]->title : '',
            'payment_module_code' => isset($GLOBALS[$payment_module]) && is_object($GLOBALS[$payment_module]) ? $GLOBALS[$payment_module]->code : '',
            ...
            'is_wholesale' => (int) Customer::is_wholesale_customer(),
        ];
        /**
         * Dev Notes:
        ...
        $customer_addresses = $customer->get_data('addresses');
        $delivery_key = null;
        $bill_key = null;
        if (!empty($customer_addresses)) {
            $delivery_key = $this->get_address_key($customer_addresses, $sendto);
            $bill_key = $this->get_address_key($customer_addresses, $billto);
        }
        if (!empty($customer->get_data('customers_firstname'))) {
            $this->customer = $this->get_address($customer_addresses, $this->get_address_key($customer_addresses, $customer->get_data('customers_default_address_id')));
            $this->customer['telephone'] = $customer->get_data('customers_telephone');
            $this->customer['email_address'] = $customer->get_data('customers_email_address');
        }
        if ($this->content_type === 'virtual') {
            $this->delivery = ['firstname' => '', 'lastname' => '', 'company' => '', 'street_address' => '', 'suburb' => '', 'city' => '', 'postcode' => '', 'state' => '', 'zone_id' => 0, 'country' => ['id' => 0, 'title' => '', 'iso_code_2' => '', 'iso_code_3' => ''], 'country_id' => 0, 'format_id' => 0];
        } elseif (!is_null($delivery_key)) {
            $this->delivery = $this->get_address($customer_addresses, $delivery_key);
        }
        if (!is_null($bill_key)) {
            $this->billing = $this->get_address($customer_addresses, $bill_key);
        }
        // -----
        ...
        $current_addresses = ['customer' => $this->customer, 'delivery' => $this->delivery, 'billing' => $this->billing];
        ...
        [$tax_country_id, $tax_zone_id] = $this->determine_tax_address_zones($billto, $sendto);
        // -----
        ...
        $this->notify('NOTIFY_ORDER_CART_AFTER_ADDRESSES_SET', '', $tax_country_id, $tax_zone_id);
        $index = 0;
        ...
        for ($i = 0, $n = count($products); $i < $n; $i++) {
            ...
            if ($attributes_handled === false && !empty($products[$i]['attributes'])) {
                ...
                foreach ($products[$i]['attributes'] as $option => $value) {
                    ...
                    if ($value == PRODUCTS_OPTIONS_VALUES_TEXT_ID) {
                        ...
                        $attr_value = htmlspecialchars_decode((string) $attributes->fields['products_options_values_name'], ENT_COMPAT);
                    }
                    $this->products[$index]['attributes'][$subindex] = ['option' => $attributes->fields['products_options_name'], 'value' => $attr_value, 'option_id' => $option, 'value_id' => $value, 'prefix' => $attributes->fields['price_prefix'], 'price' => $attributes->fields['options_values_price'], 'discountable' => $attributes->fields['attributes_discounted']];
            ...
            $tax_rates = $this->set_tax_rates_for_product($products, $i, $index, $tax_country_id, $tax_zone_id);
            $this->products[$index]['tax_groups'] = $tax_rates;
            // -----
            // Update the order's subtotal and gather the tax-group-specific
            // product totals for the order's overall tax calculation.
            //
            $this->calculate_tax_for_product($index, $tax_rates);
            $index++;
        }
        // -----
        ...
        //
        $this->calculate_products_tax_for_order();
        ...
        if (isset($GLOBALS[$payment_module]) && is_object($GLOBALS[$payment_module])) {
            if (isset($GLOBALS[$payment_module]->order_status) && is_numeric($GLOBALS[$payment_module]->order_status) && $GLOBALS[$payment_module]->order_status > 0) {
                $this->info['order_status'] = $GLOBALS[$payment_module]->order_status;
            }
        }
     ...
     * @since ZC v1.5.7
     */
    protected function determine_tax_address_zones($bill_to_address_id, $ship_to_address_id): array
    {
        global $db;
        ...
        if (zen_is_logged_in()) {
            $tax_country_id = $tax_zone_id = -1;
            $tax_address_query = '';
            switch (STORE_PRODUCT_TAX_BASIS) {
                case 'Shipping':
                    $address_book_id = $this->content_type === 'virtual' ? $bill_to_address_id : $ship_to_address_id;
                    break;
                case 'Billing':
                    $address_book_id = $bill_to_address_id;
                    break;
                case 'Store':
            ...
            if ($tax_address->record_count() > 0) {
                $tax_country_id = $tax_address->fields['entry_country_id'];
                $tax_zone_id = $tax_address->fields['entry_zone_id'];
            }
        }
        return [$tax_country_id, $tax_zone_id];
    }
    /**
     * Determine tax RATES for product
     * @since ZC v1.5.7
     */
    protected function set_tax_rates_for_product(array $products, $loop, $index, $tax_country_id, $tax_zone_id)
    {
        $tax_rates = null;
        $this->notify('NOTIFY_ORDER_CART_EXTERNAL_TAX_RATE_LOOKUP', STORE_PRODUCT_TAX_BASIS, $products, $loop, $index, $tax_country_id, $tax_zone_id, $tax_rates);
        if ($tax_rates !== null) {
            return $tax_rates;
        }
        // Handle store-pickup scenario
        if (STORE_PRODUCT_TAX_BASIS === 'Shipping' && isset($_SESSION['shipping']['id']) && stristr((string) $_SESSION['shipping']['id'], 'storepickup')) {
            $tax_rates = zen_get_multiple_tax_rates($products[$loop]['tax_class_id'], STORE_COUNTRY, STORE_ZONE);
        ...
        $tax_rates = zen_get_multiple_tax_rates($products[$loop]['tax_class_id'], $tax_country_id, $tax_zone_id);
        $this->products[$index]['tax'] = zen_get_tax_rate($products[$loop]['tax_class_id'], $tax_country_id, $tax_zone_id);
        $this->products[$index]['tax_description'] = zen_get_tax_description($products[$loop]['tax_class_id'], $tax_country_id, $tax_zone_id);
        $this->products[$index]['tax_groups'] = $tax_rates;
        return $tax_rates;
    }
    /**
     ...
     * @since ZC v1.5.7
     */
    protected function calculate_tax_for_product($index, $tax_rates): void
    {
        ...
        //
        if (DISPLAY_PRICE_WITH_TAX === 'true') {
            $shown_price = zen_add_tax($product_final_price, $product_tax_rate) * $product_qty + zen_add_tax($product_onetime_charges, $product_tax_rate);
        } else {
            $shown_price = $product_final_price * $product_qty + $product_onetime_charges;
        }
        $this->info['subtotal'] += $shown_price;
        $this->notify('NOTIFY_ORDER_CART_SUBTOTAL_CALCULATE', ['shown_price' => $shown_price]);
        foreach ($tax_rates as $tax_description => $tax_rate) {
            if (!isset($this->info['tax_subtotals'][$tax_description])) {
                $this->info['tax_subtotals'][$tax_description] = ['tax_rate' => $tax_rate, 'subtotal' => 0];
            }
     ...
     * @since ZC v2.0.0
     */
    protected function calculate_products_tax_for_order(): void
    {
    ...
    public function create(array $zf_ot_modules): int|null
    {
        ...
        if (isset($this->info['cc_number']) && strlen($this->info['cc_number']) > 10) {
            $c_end = substr($this->info['cc_number'], -4);
            $c_offset = strlen($this->info['cc_number']) - 4;
            $c_start = substr($this->info['cc_number'], 0, $c_offset > 4 ? 4 : (int) $c_offset);
            $this->info['cc_number'] = str_pad($c_start, 6, 'X') . $c_end;
        }
        $sql_data_array = ['customers_id' => $_SESSION['customer_id'], 'customers_name' => $this->customer['firstname'] . ' ' . $this->customer['lastname'], 'customers_company' => $this->customer['company'], 'customers_street_address' => $this->customer['street_address'], 'customers_suburb' => $this->customer['suburb'], 'customers_city' => $this->customer['city'], 'customers_postcode' => $this->customer['postcode'], 'customers_state' => $this->customer['state'], 'customers_country' => $this->customer['country']['title'], 'customers_telephone' => $this->customer['telephone'], 'customers_email_address' => $this->customer['email_address'], 'customers_address_format_id' => $this->customer['format_id'], 'delivery_name' => $this->delivery['firstname'] . ' ' . $this->delivery['lastname'], 'delivery_company' => $this->delivery['company'], 'delivery_street_address' => $this->delivery['street_address'], 'delivery_suburb' => $this->delivery['suburb'], 'delivery_city' => $this->delivery['city'], 'delivery_postcode' => $this->delivery['postcode'], 'delivery_state' => $this->delivery['state'], 'delivery_country' => $this->delivery['country']['title'], 'delivery_address_format_id' => (int) $this->delivery['format_id'], 'billing_name' => $this->billing['firstname'] . ' ' . $this->billing['lastname'], 'billing_company' => $this->billing['company'], 'billing_street_address' => $this->billing['street_address'], 'billing_suburb' => $this->billing['suburb'], 'billing_city' => $this->billing['city'], 'billing_postcode' => $this->billing['postcode'], 'billing_state' => $this->billing['state'], 'billing_country' => $this->billing['country']['title'], 'billing_address_format_id' => $this->billing['format_id'], 'payment_method' => $this->info['payment_module_code'] === '' && $this->info['payment_method'] === '' ? PAYMENT_METHOD_GV : $this->info['payment_method'], 'payment_module_code' => $this->info['payment_module_code'] === '' && $this->info['payment_method'] === '' ? PAYMENT_MODULE_GV : $this->info['payment_module_code'], 'shipping_method' => $this->info['shipping_method'], 'shipping_module_code' => strpos((string) $this->info['shipping_module_code'], '_') > 0 ? substr((string) $this->info['shipping_module_code'], 0, strpos((string) $this->info['shipping_module_code'], '_')) : $this->info['shipping_module_code'], 'coupon_code' => $this->info['coupon_code'], 'cc_type' => $this->info['cc_type'] ?? '', 'cc_owner' => $this->info['cc_owner'] ?? '', 'cc_number' => $this->info['cc_number'] ?? '', 'cc_expires' => $this->info['cc_expires'] ?? '', 'date_purchased' => 'now()', 'orders_status' => $this->info['order_status'], 'order_total' => $this->info['total'], 'order_tax' => $this->info['tax'], 'shipping_tax_rate' => $this->info['shipping_tax_rate'] ?? 'null', 'currency' => $this->info['currency'], 'currency_value' => $this->info['currency_value'], 'ip_address' => $_SESSION['customers_ip_address'] . ' - ' . $_SERVER['REMOTE_ADDR'], 'language_code' => $_SESSION['languages_code'], 'order_weight' => $_SESSION['cart']->weight, 'is_wholesale' => $this->info['is_wholesale']];
        zen_db_perform(TABLE_ORDERS, $sql_data_array);
        $this->order_id = $this->info['order_id'] = $insert_id = $db->insert_ID();
        $this->notify('NOTIFY_ORDER_DURING_CREATE_ADDED_ORDER_HEADER', array_merge(['orders_id' => $this->order_id, 'shipping_weight' => $_SESSION['cart']->weight], $sql_data_array), $this->order_id);
        for ($i = 0, $n = count($zf_ot_modules); $i < $n; $i++) {
            $sql_data_array = ['orders_id' => $this->order_id, 'title' => $zf_ot_modules[$i]['title'], 'text' => $zf_ot_modules[$i]['text'], 'value' => is_numeric($zf_ot_modules[$i]['value']) ? $zf_ot_modules[$i]['value'] : '0', 'class' => $zf_ot_modules[$i]['code'], 'sort_order' => $zf_ot_modules[$i]['sort_order']];
        ...
        $sql_data_array = ['orders_id' => $this->order_id, 'orders_status_id' => $this->info['order_status'], 'date_added' => 'now()', 'customer_notified' => $customer_notification, 'comments' => $this->info['comments']];
        // -----
        ...
        if (isset($_SESSION['emp_admin_id'])) {
            ...
            $admin_id_sql = $db->bind_vars($admin_id_sql, ':adminid:', $_SESSION['emp_admin_id'], 'integer');
    ...
    {
        global $db;
        $this->notify('NOTIFIER_ADMIN_ZEN_REMOVE_ORDER', [], $this->order_id, $restock);
        if ($restock || $restock === 'on') {
            ...
            while (!$order_products->EOF) {
                ...
                $order_products->move_next();
            }
        }
        $db->Execute('delete from ' . TABLE_ORDERS . ' where orders_id = ' . (int) $this->order_id);
        ...
        $db->Execute('delete from ' . TABLE_COUPON_GV_QUEUE . '
                                    where order_id = ' . (int) $this->order_id . " and release_flag = 'N'");
        zen_record_admin_activity('Deleted order ' . (int) $this->order_id . ' from database via admin console.', 'warning');
    }
    /**
        ...
        for ($i = 0, $n = sizeof($this->products); $i < $n; $i++) {
            ...
            if ($this->do_stock_decrement) {
                if (DOWNLOAD_ENABLED === 'true') {
                    ...
                    $stock_values = $db->execute_no_cache($stock_query_raw . ' LIMIT 1');
                } else {
                    $stock_values = $db->execute_no_cache('SELECT * FROM ' . TABLE_PRODUCTS . ' WHERE products_id = ' . zen_get_prid($this->products[$i]['id']) . ' LIMIT 1');
                }
                $this->notify('NOTIFY_ORDER_PROCESSING_STOCK_DECREMENT_BEGIN', $i, $stock_values);
                if ($stock_values->record_count() > 0) {
                    ...
                    $db->Execute('UPDATE ' . TABLE_PRODUCTS . '
                        SET products_quantity = ' . $stock_left . $products_status_update . ' WHERE products_id = ' . zen_get_prid($this->products[$i]['id']) . ' LIMIT 1');
                    // for low stock email
            ...
            $sql_data_array = ['orders_id' => $zf_insert_id, 'products_id' => zen_get_prid($this->products[$i]['id']), 'products_model' => $this->products[$i]['model'], 'products_name' => $this->products[$i]['name'], 'products_price' => $this->products[$i]['price'], 'final_price' => $this->products[$i]['final_price'], 'onetime_charges' => $this->products[$i]['onetime_charges'], 'products_tax' => $this->products[$i]['tax'], 'products_quantity' => $this->products[$i]['qty'], 'products_priced_by_attribute' => $this->products[$i]['products_priced_by_attribute'], 'product_is_free' => $this->products[$i]['product_is_free'], 'products_discount_type' => $this->products[$i]['products_discount_type'], 'products_discount_type_from' => $this->products[$i]['products_discount_type_from'], 'products_prid' => $this->products[$i]['id'], 'products_weight' => (float) $this->products[$i]['weight'], 'products_virtual' => (int) $this->products[$i]['products_virtual'], 'product_is_always_free_shipping' => (int) $this->products[$i]['product_is_always_free_shipping'], 'products_quantity_order_min' => (float) $this->products[$i]['products_quantity_order_min'], 'products_quantity_order_units' => (float) $this->products[$i]['products_quantity_order_units'], 'products_quantity_order_max' => (float) $this->products[$i]['products_quantity_order_max'], 'products_quantity_mixed' => (int) $this->products[$i]['products_quantity_mixed'], 'products_mixed_discount_quantity' => (int) $this->products[$i]['products_mixed_discount_quantity']];
            ...
            $this->products_ordered_html .= '<tr>' . "\n" . '<td class="product-details" align="right" valign="top" width="30">' . $this->products[$i]['qty'] . '&nbsp;x</td>' . "\n" . '<td class="product-details" valign="top">' . nl2br((string) $this->products[$i]['name']) . ($this->products[$i]['model'] != '' ? ' (' . nl2br((string) $this->products[$i]['model']) . ') ' : '') . (!empty($this->products_ordered_attributes) ? "\n" . '<nobr>' . '<small><em>' . nl2br($this->products_ordered_attributes) . '</em></small>' . '</nobr>' : '') . '</td>' . "\n" . '<td class="product-details-num" valign="top" align="right">' . $currencies->display_price($this->products[$i]['final_price'], $this->products[$i]['tax'], $this->products[$i]['qty']) . '</td>' . "\n" . '</tr>' . "\n" . ($this->products[$i]['onetime_charges'] != 0 ? '<tr>' . "\n" . '<td class="product-details" colspan="2">' . nl2br(TEXT_ONETIME_CHARGES_EMAIL) . '</td>' . "\n" . '<td valign="top" align="right">' . $currencies->display_price($this->products[$i]['onetime_charges'], $this->products[$i]['tax'], 1) . '</td>' . "\n" . '</tr>' . "\n" : '');
        }
    ...
    public function send_order_email(?int $zf_insert_id = null): void
    {
        ...
        $html_msg['INTRO_DATE_ORDERED'] = $zc_date->output(DATE_FORMAT_LONG);
        ...
        $storepickup = str_contains((string) $this->info['shipping_module_code'], 'storepickup');
        ...
        if (!empty($_SESSION['payment']) && is_object($GLOBALS[$_SESSION['payment']])) {
            $cc_num_display = isset($this->info['cc_number']) && $this->info['cc_number'] !== '' ? str_repeat('X', strlen((string) $this->info['cc_number']) - 8) . substr((string) $this->info['cc_number'], -4) . "\n\n" : '';
        ...
        $html_msg['PAYMENT_METHOD_FOOTER'] = !empty($payment_class) && isset($GLOBALS[$payment_class]->email_footer) && is_object($GLOBALS[$_SESSION['payment']]) && $GLOBALS[$payment_class]->email_footer !== '' ? nl2br((string) $GLOBALS[$payment_class]->email_footer) : (isset($this->info['cc_type']) && $this->info['cc_type'] !== '' ? $this->info['cc_type'] . ' ' . $cc_num_display : '');
        ...
        if ($send_customer_email === true) {
            zen_mail($this->customer['firstname'] . ' ' . $this->customer['lastname'], $this->customer['email_address'], EMAIL_TEXT_SUBJECT . EMAIL_ORDER_NUMBER_SUBJECT . $zf_insert_id, $email_order, STORE_NAME, EMAIL_FROM, $html_msg, 'checkout', $this->attach_array, $customer_email_reply_to_name, $customer_email_reply_to_address);
        }
        ...
        if (SEND_EXTRA_ORDER_EMAILS_TO !== '') {
            ...
            $send_extra_order_email = true;
            $this->notify('NOTIFY_ORDER_INVOICE_CONTENT_FOR_ADDITIONAL_EMAILS', $zf_insert_id, $email_order, $html_msg, $send_extra_order_email);
            ...
            if ($send_extra_order_email) {
                zen_mail('', SEND_EXTRA_ORDER_EMAILS_TO, SEND_EXTRA_NEW_ORDERS_EMAILS_TO_SUBJECT . ' ' . EMAIL_TEXT_SUBJECT . EMAIL_ORDER_NUMBER_SUBJECT . $zf_insert_id, $email_order . $extra_info['TEXT'], STORE_NAME, EMAIL_FROM, $html_msg, 'checkout_extra', $this->attach_array, $this->customer['firstname'] . ' ' . $this->customer['lastname'], $this->customer['email_address']);
            }
        }
     ...
     * @since ZC v2.2.0
     */
    private function get_address_key(array $customer_addresses, int $book_id): null|int
    {
        foreach ($customer_addresses as $k => $address) {
            if (isset($address['address_book_id']) && $book_id === (int) $address['address_book_id']) {
                return $k;
            }
        }
        return null;
     ...
     * @since ZC v2.2.0
     */
    private function get_address(array $customer_addresses, int $array_key): array
    {
        $address = $customer_addresses[$array_key]['address'];
        return ['firstname' => $address['firstname'], 'lastname' => $address['lastname'], 'company' => $address['company'], 'street_address' => $address['street_address'], 'suburb' => $address['suburb'], 'city' => $address['city'], 'postcode' => $address['postcode'], 'state' => zen_not_null($address['state']) ? $address['state'] : $address['zone_name'], 'state_code' => zen_not_null($address['zone_iso']) ? $address['zone_iso'] : $address['zone_name'], 'zone_id' => $address['zone_id'], 'country' => ['id' => $address['country_id'], 'title' => $address['country_name'], 'iso_code_2' => $address['country_iso_2'], 'iso_code_3' => $address['country_iso']], 'country_id' => $address['country_id'], 'format_id' => $customer_addresses[$array_key]['format_id']];
    }
}