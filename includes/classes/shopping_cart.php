...
class Shopping_Cart extends base
{
    ...
    public function restore_contents()
    {
        ...
        foreach ($products as $next_product) {
            ...
            $attributes = $db->Execute('SELECT products_options_id, products_options_value_id, products_options_value_text
                  ...
                  WHERE customers_id = ' . (int) $_SESSION['customer_id'] . "\n                    AND products_id = '" . zen_db_input($uprid) . "' " . $order_by);
     ...
     * @since ZC v1.0.3
     */
    public function add_cart($product_id, $qty = 1, $attributes = [], $notify = true): void
    {
        global $db, $message_stack;
        if ($this->display_debug_messages) {
            $message_stack->add_session('header', 'FUNCTION ' . __FUNCTION__, 'caution');
        }
        ...
        if (!is_numeric($qty) || $qty < 0) {
            ...
            $message_stack->add_session('header', ERROR_CORRECTIONS_HEADING . ERROR_PRODUCT_QUANTITY_UNITS_SHOPPING_CART . $chk_link . ' ' . PRODUCTS_ORDER_QTY_TEXT . zen_output_string_protected($qty), 'caution');
            $qty = 0;
        }
        ...
        if ($this->in_cart($uprid)) {
            ...
            $this->contents[] = [$uprid];
            // @TODO - why is this line here? Appears to be removed in the call to cleanup(), so doesn't really serve any purpose here.
     ...
     * @since ZC v1.0.3
     */
    public function update_quantity(string $uprid, $quantity = 0, $attributes = [])
    {
        global $db, $message_stack;
        if ($this->display_debug_messages) {
            $message_stack->add_session('header', 'FUNCTION ' . __FUNCTION__ . ' $products_id: ' . $uprid . ' $quantity: ' . $quantity, 'caution');
        }
        if (!is_numeric($quantity) || $quantity < 0) {
            // adjust quantity when not a value
            $chk_link = '<a href="' . zen_href_link(zen_get_info_page($uprid), 'cPath=' . zen_get_generated_category_path_rev(zen_get_products_category_id($uprid)) . '&products_id=' . $uprid) . '">' . zen_get_products_name($uprid) . '</a>';
            $message_stack->add_session('header', ERROR_CORRECTIONS_HEADING . ERROR_PRODUCT_QUANTITY_UNITS_SHOPPING_CART . $chk_link . ' ' . PRODUCTS_ORDER_QTY_TEXT . zen_output_string_protected($quantity), 'caution');
            $quantity = 0;
        }
        ...
        if (STOCK_ALLOW_CHECKOUT === 'false' && $quantity > $chk_current_qty) {
            ...
            if (!$this->flag_duplicate_msgs_set) {
                $message_stack->add_session('shopping_cart', ($this->display_debug_messages ? '$_GET[main_page]: ' . $_GET['main_page'] . ' FUNCTION ' . __FUNCTION__ . ': ' : '') . WARNING_PRODUCT_QUANTITY_ADJUSTED . zen_get_products_name($uprid), 'caution');
            }
        }
                ...
                if ($blank_value === false) {
                    ...
                    if (zen_is_logged_in() && !zen_in_guest_checkout()) {
                        ...
                        if (is_array($value)) {
                            ...
                            // intentionally passing a string
                            $db->Execute($sql);
                        }
                    }
                }
     ...
     * @since ZC v2.0.0
     */
    protected function remove_uprid($uprid)
    {
        global $db;
     ...
     * @since ZC v1.0.3
     */
    public function in_cart($uprid): bool
    {
     ...
     * @since ZC v1.0.3
     */
    public function remove($uprid): void
    {
        $this->notify('NOTIFIER_CART_REMOVE_START', null, $uprid);
        $this->remove_uprid(zen_db_input($uprid));
        // assign a temporary unique ID to the order contents to prevent hack attempts during the checkout procedure
        $this->cart_id = $this->generate_cart_id();
    ...
    public function calculate()
    {
        ...
        foreach ($this->contents as $uprid => $data) {
            ...
            $free_shipping_total = 0;
            $product_total = 0;
            $total_onetime_charge = 0;
            $total_onetime_charge_no_discount = 0;
            $free_shipping_applied = false;
            $qty = $data['qty'];
            $prid = zen_get_prid($uprid);
            $product = (new Product($prid))->with_default_language()->get_data();
            if (empty($product)) {
                $this->remove_uprid($uprid);
                continue;
            }
            ...
            } elseif ($product['products_discount_type'] !== '0') {
            ...
            if ($is_free_shipping === true) {
                ...
                $free_shipping_total += $products_price;
                $this->free_shipping_weight += $qty * $product['products_weight'];
            }
            $product_total += $products_price;
            ...
            $adjust_downloads = 0;
            // attributes price
            $saved_product_total = $product_total;
            $attributes_total = 0;
            if (isset($this->contents[$uprid]['attributes'])) {
                foreach ($this->contents[$uprid]['attributes'] as $option => $value) {
                    ...
                    $options_values_price = zen_get_retail_or_wholesale_price($attribute_price->fields['options_values_price'], $attribute_price->fields['options_values_price_w']);
                    if ($attribute_price->fields['price_prefix'] === '-') {
                        ...
                        if ($product['product_is_always_free_shipping'] === '1' || $product['products_virtual'] === '1') {
                            ...
                            $free_shipping_total -= $shipping_attributes_price;
                        }
                        if ($attribute_price->fields['attributes_discounted'] === '1') {
                            // calculate proper discount for attributes
                            $new_attributes_price = zen_get_discount_calc($prid, $attributes_id, $options_values_price, $qty);
                            $product_total -= $new_attributes_price;
                        } else {
                            $product_total -= $options_values_price;
                        }
                        // calculate Product Price without Specials, Sales or Discounts
                        ...
                        if ($attribute_price->fields['attributes_discounted'] === '1') {
                            ...
                            $product_total += zen_str_to_numeric($options_values_price);
                        }
                        // calculate Product Price without Specials, Sales or Discounts
                    ...
                    if (!$check_download->EOF) {
                        ...
                        $this->download_count += $check_download->record_count() * $qty;
                        ...
                        $free_shipping_total += $new_attributes_price;
                    }
                    ////////////////////////////////////////////////
                    ...
                    if (ATTRIBUTES_ENABLED_TEXT_PRICES === 'true' && (string) zen_get_attributes_type($attributes_id) === (string) PRODUCTS_OPTIONS_TYPE_TEXT) {
                        $text_words = zen_get_word_count_price($this->contents[$uprid]['attributes_values'][$attribute_price->fields['options_id']], $attribute_price->fields['attributes_price_words_free'], $attribute_price->fields['attributes_price_words']);
                        $text_letters = zen_get_letters_count_price($this->contents[$uprid]['attributes_values'][$attribute_price->fields['options_id']], $attribute_price->fields['attributes_price_letters_free'], $attribute_price->fields['attributes_price_letters']);
                        $product_total += $text_letters;
                        $product_total += $text_words;
                        if ($is_free_shipping === true) {
                            $free_shipping_total += $text_letters;
                            $free_shipping_total += $text_words;
                        }
                        // calculate Product Price without Specials, Sales or Discounts
                    ...
                    if ($attribute_price->fields['attributes_price_factor'] > 0) {
                        ...
                        $product_total += $added_charge;
                        if ($is_free_shipping === true) {
                            $free_shipping_total += $added_charge;
                        }
                        // calculate Product Price without Specials, Sales or Discounts
                    ...
                    if ($attribute_price->fields['attributes_price_onetime'] > 0) {
                        $total_onetime_charge += $attribute_price->fields['attributes_price_onetime'];
                        // calculate Product Price without Specials, Sales or Discounts
                        $total_onetime_charge_no_discount += $attribute_price->fields['attributes_price_onetime'];
                    }
                    ...
                    if ($attribute_price->fields['attributes_price_factor_onetime'] > 0) {
                        ...
                        $total_onetime_charge += $added_charge;
                        // calculate Product Price without Specials, Sales or Discounts
                        ...
                        $total_onetime_charge_no_discount += $added_charge;
                    }
                    ...
                    ////////////////////////////////////////////////
                    $attributes_total += $product_total;
                }
                // eof foreach
            }
            // attributes price
            $product_total = $saved_product_total + $attributes_total;
            // attributes weight
            if (isset($this->contents[$uprid]['attributes'])) {
                foreach ($this->contents[$uprid]['attributes'] as $option => $value) {
            ...
            */
            $this->total += zen_add_tax($product_total, $products_tax) * $qty;
            $this->total += zen_add_tax($total_onetime_charge, $products_tax);
            $this->free_shipping_price += zen_add_tax($free_shipping_total, $products_tax) * $qty;
            if ($is_free_shipping === true) {
                $this->free_shipping_price += zen_add_tax($total_onetime_charge, $products_tax);
            }
            // ******* WARNING ADD ONE TIME ATTRIBUTES, PRICE FACTOR
            // calculate Product Price without Specials, Sales or Discounts
            $total_before_discounts = $total_before_discounts * $qty;
            $total_before_discounts += $total_onetime_charge_no_discount;
     ...
     * @since ZC v1.0.3
     */
    public function attributes_price($uprid): float|int
    {
        global $db, $currencies;
        ...
        foreach ($this->contents[$uprid]['attributes'] as $option => $value) {
            ...
            $discount_type_id = '';
            $sale_maker_discount = '';
            $options_values_price = zen_get_retail_or_wholesale_price($attribute_price['options_values_price'], $attribute_price['options_values_price_w']);
            if ($attribute_price['product_attribute_is_free'] === '1' && zen_get_products_price_is_free($prid)) {
                ...
                if (ATTRIBUTES_ENABLED_TEXT_PRICES === 'true' && (string) zen_get_attributes_type($attribute_price['products_attributes_id']) === (string) PRODUCTS_OPTIONS_TYPE_TEXT) {
                    $text_words = zen_get_word_count_price($this->contents[$uprid]['attributes_values'][$attribute_price['options_id']], $attribute_price['attributes_price_words_free'], $attribute_price['attributes_price_words']);
                    $text_letters = zen_get_letters_count_price($this->contents[$uprid]['attributes_values'][$attribute_price['options_id']], $attribute_price['attributes_price_letters_free'], $attribute_price['attributes_price_letters']);
                    $attributes_price += $text_letters + $text_words;
                }
                // attributes_price_factor
                if ($attribute_price['attributes_price_factor'] > 0) {
                    $added_charge = zen_get_attributes_price_factor(zen_get_products_base_price($prid), zen_get_products_special_price($prid, false), $attribute_price['attributes_price_factor'], $attribute_price['attributes_price_factor_offset']);
     ...
     * @since ZC v1.2.0d
     */
    public function attributes_price_onetime_charges($uprid, $qty): float|int
    {
        ...
        foreach ($this->contents[$uprid]['attributes'] as $option => $value) {
            ...
            if ($attribute_price['attributes_price_factor_onetime'] > 0) {
                $added_charge = zen_get_attributes_price_factor(zen_get_products_base_price($prid), zen_get_products_special_price($prid, false), $attribute_price['attributes_price_factor_onetime'], $attribute_price['attributes_price_factor_onetime_offset']);
     ...
     * @since ZC v1.0.3
     */
    public function attributes_weight($uprid): float|int
    {
        if (!isset($this->contents[$uprid]['attributes'])) {
            return 0;
        }
        ...
        foreach ($this->contents[$uprid]['attributes'] as $option => $value) {
            ...
            $new_attributes_weight = zen_get_product_is_always_free_shipping($prid) === false ? $attribute_weight_info->fields['products_attributes_weight'] : 0;
            // + or blank adds
    ...
    public function get_products(bool $check_for_valid_cart = false)
    {
        ...
        foreach ($this->contents as $uprid => $data) {
            ...
            if ($check_for_valid_cart === true) {
                ...
                if (isset($data['attributes'])) {
                    foreach ($data['attributes'] as $option_id => $value_id) {
                        ...
                        if ($chk_attributes_exist->EOF) {
                            $fix_once++;
                            $_SESSION['valid_to_checkout'] = false;
                            $chk_products_link = '<a href="' . zen_href_link(zen_get_info_page($prid), 'cPath=' . zen_get_generated_category_path_rev($product['master_categories_id']) . '&products_id=' . $prid) . '">' . $product['products_name'] . '</a>';
                ...
                if ($fix_once === 0) {
                    ...
                    if ($product['products_quantity_order_max'] != 0 && $check_quantity > $product['products_quantity_order_max'] && !isset($this->flag_duplicate_quantity_msgs_set[$prid]['max'])) {
                        $fix_once++;
                        $_SESSION['valid_to_checkout'] = false;
                        $_SESSION['cart_errors'] .= ERROR_PRODUCT . $product['products_name'] . ERROR_PRODUCT_QUANTITY_MAX_SHOPPING_CART . ERROR_PRODUCT_QUANTITY_ORDERED . $check_quantity . ' <span class="alertBlack">' . zen_get_products_quantity_min_units_display($prid, false, true) . '</span> ' . '<br>';
                ...
                if ($fix_once === 0) {
                    if ($check_quantity < $check_quantity_min && !isset($this->flag_duplicate_quantity_msgs_set[$prid]['min'])) {
                        $fix_once++;
                        $_SESSION['valid_to_checkout'] = false;
                        $_SESSION['cart_errors'] .= ERROR_PRODUCT . $product['products_name'] . ERROR_PRODUCT_QUANTITY_MIN_SHOPPING_CART . ERROR_PRODUCT_QUANTITY_ORDERED . $check_quantity . ' <span class="alertBlack">' . zen_get_products_quantity_min_units_display($prid, false, true) . '</span> ' . '<br>';
                ...
                if ($fix_once === 0) {
                    ...
                    if (fmod_round($check_quantity, $check_units) != 0 && !isset($this->flag_duplicate_quantity_msgs_set[$prid]['units'])) {
                        $_SESSION['valid_to_checkout'] = false;
                        $_SESSION['cart_errors'] .= ERROR_PRODUCT . $product['products_name'] . ERROR_PRODUCT_QUANTITY_UNITS_SHOPPING_CART . ERROR_PRODUCT_QUANTITY_ORDERED . $check_quantity . ' <span class="alertBlack">' . zen_get_products_quantity_min_units_display($prid, false, true) . '</span> ' . '<br>';
            ...
            $products_array[] = [
                ...
                // float
                'ships_in_own_box' => $product['product_ships_in_own_box'] ?? $product['products_ready_to_ship'] ?? null,
                // [0,1]
     ...
     * @since ZC v1.0.3
     */
    public function generate_cart_id(int $length = 5): string|false
    {
     ...
     * @since ZC v1.0.3
     */
    public function get_content_type($gv_only = false): float|int|string|false
    {
        global $db;
        ...
        if ($this->count_contents() === 0) {
            ...
            foreach ($this->contents as $uprid => $data) {
                $prid = (int) $uprid;
                $free_ship_check = (new Product($prid))->with_default_language()->get_data();
                if (str_starts_with($free_ship_check['products_model'] ?? '', 'GIFT')) {
                    ...
                    if ($gift_special !== false) {
                        if (!$gift_pba && !empty($gift_special) && (string) $gift_special !== $gift_price) {
    ...
    public function in_cart_mixed(int|string $uprid_to_check): float|int
    {
        ...
        $mixed_status = (new Product((int) $uprid_to_check))->with_default_language()->get('products_quantity_mixed');
    ...
    public function in_cart_mixed_discount_quantity($uprid_to_check)
    {
        ...
        // check if mixed is on
        $mixed_status = (new Product((int) $uprid_to_check))->with_default_language()->get('products_mixed_discount_quantity');
        // if mixed attributes is off return qty for current attribute selection
        if ($mixed_status === '0') {
            return $this->get_quantity($uprid_to_check);
        }
     ...
     * @since ZC v1.1.0
     */
    public function in_cart_check($check_what, $check_value = '1'): float|int
    {
        // if nothing is in cart return 0
        if (!is_array($this->contents)) {
            return 0;
        ...
        foreach ($this->contents as $uprid => $data) {
            ...
            $product_check = (new Product(zen_get_prid($uprid)))->with_default_language()->get_data();
     ...
     * @since ZC v1.3.0
     */
    public function action_update_product($goto, $parameters): void
    {
        ...
        for ($i = 0, $n = count($_POST['products_id']); $i < $n; $i++) {
            ...
            if (!is_numeric($_POST['cart_quantity'][$i]) || $_POST['cart_quantity'][$i] < 0) {
                ...
                $message_stack->add_session('header', ERROR_CORRECTIONS_HEADING . ERROR_PRODUCT_QUANTITY_UNITS_SHOPPING_CART . $chk_link . ' ' . PRODUCTS_ORDER_QTY_TEXT . zen_output_string_protected($_POST['cart_quantity'][$i]), 'caution');
            ...
            if (in_array($products_id, $cart_delete, false) || empty($_POST['cart_quantity'][$i])) {
                ...
                // maximum allowed
                $chk_mixed = zen_get_products_quantity_mixed($products_id);
                // use mixed
                ...
                if ($chk_mixed === true && !array_key_exists($prid, $change_state)) {
                    $change_check = $this->in_cart_product_mixed_changed($products_id, 'decrease');
                    // Returns full data on products.
                ...
                if ($this->display_debug_messages) {
                    $message_stack->add_session('header', 'FUNCTION ' . __FUNCTION__ . ' Products_id: ' . $products_id . ' cart_qty: ' . $cart_qty . ' <br>', 'caution');
                }
                $new_qty = $_POST['cart_quantity'][$i];
                // new quantity
                $current_qty = $this->get_quantity($products_id);
                ...
                if (STOCK_ALLOW_CHECKOUT === 'false' && $new_qty > $chk_current_qty) {
                    ...
                    $message_stack->add_session('shopping_cart', ($this->display_debug_messages ? 'FUNCTION ' . __FUNCTION__ . ': ' : '') . WARNING_PRODUCT_QUANTITY_ADJUSTED . zen_get_products_name($products_id), 'caution');
                }
                ...
                if ($adjust_max === 'true') {
                    if ($this->display_debug_messages) {
                        $message_stack->add_session('header', 'FUNCTION ' . __FUNCTION__ . '<br>' . ERROR_MAXIMUM_QTY . zen_get_products_name($products_id) . '<br>requested_qty: ' . $requested_qty . ' current_qty: ' . $current_qty, 'caution');
                    }
                    $message_stack->add_session('shopping_cart', ERROR_MAXIMUM_QTY . zen_get_products_name($products_id), 'caution');
                } else if ($_GET['main_page'] !== FILENAME_SHOPPING_CART) {
                    if (DISPLAY_CART === 'false' && $message_stack->size('shopping_cart') === 0) {
                        $message_stack->add_session('header', ($this->display_debug_messages ? 'FUNCTION ' . __FUNCTION__ . ': ' : '') . SUCCESS_ADDED_TO_CART_PRODUCTS, 'success');
     ...
     * @since ZC v1.3.0
     */
    public function action_add_product($goto, $parameters = []): void
    {
        global $db, $message_stack;
        if ($this->display_debug_messages) {
            $message_stack->add_session('header', 'A: FUNCTION ' . __FUNCTION__, 'caution');
        }
        $the_list = '';
        if (isset($_POST['products_id']) && is_numeric($_POST['products_id'])) {
            // verify attributes and quantity first
            if ($this->display_debug_messages) {
                $message_stack->add_session('header', 'A2: FUNCTION ' . __FUNCTION__, 'caution');
            }
            $adjust_max = 'false';
            if (isset($_POST['id']) && is_array($_POST['id'])) {
                ...
                foreach ($_POST['id'] as $key => $value) {
                    if (zen_get_attributes_valid($_POST['products_id'], $key, $value) === false) {
                        if (str_starts_with((string) $key, TEXT_PREFIX) === true && $value === '') {
                            $selection_text = '';
                        ...
                        $the_list .= TEXT_ERROR_OPTION_FOR . '<span class="alertBlack">' . zen_options_name($key) . '</span>' . $selection_text . '<span class="alertBlack">' . $value_text . '</span>' . '<br>';
                    }
                }
            }
            if (!is_numeric($_POST['cart_quantity']) || $_POST['cart_quantity'] <= 0) {
                ...
                if (!isset($_POST['shopping_cart_zero_or_less'])) {
                    ...
                    $message_stack->add_session('header', ERROR_CORRECTIONS_HEADING . ERROR_PRODUCT_QUANTITY_UNITS_SHOPPING_CART . $chk_link . ' ' . PRODUCTS_ORDER_QTY_TEXT . zen_output_string_protected($_POST['cart_quantity']), 'caution');
            ...
            if ($this->display_debug_messages) {
                $message_stack->add_session('header', 'B: FUNCTION ' . __FUNCTION__ . ' Products_id: ' . $_POST['products_id'] . ' cart_qty: ' . $cart_qty . ' $_POST[cart_quantity]: ' . $_POST['cart_quantity'] . ' <br>', 'caution');
            }
            ...
            if (STOCK_ALLOW_CHECKOUT === 'false' && $cart_qty + $new_qty > $chk_current_qty) {
                $new_qty = $chk_current_qty;
                $message_stack->add_session('shopping_cart', ($this->display_debug_messages ? 'C: FUNCTION ' . __FUNCTION__ . ': ' : '') . WARNING_PRODUCT_QUANTITY_ADJUSTED . zen_get_products_name($_POST['products_id']), 'caution');
            ...
            if ($add_max == 1 && $cart_qty == 1) {
                ...
                if (STOCK_ALLOW_CHECKOUT === 'false' && $new_qty + $cart_qty > $chk_current_qty) {
                    ...
                    if (!$this->flag_duplicate_msgs_set) {
                        $message_stack->add_session('shopping_cart', ($this->display_debug_messages ? 'D: FUNCTION ' . __FUNCTION__ . ': ' : '') . WARNING_PRODUCT_QUANTITY_ADJUSTED . zen_get_products_name($_POST['products_id']), 'caution');
                    }
                }
                // adjust quantity if needed
            ...
            if (zen_get_products_quantity_order_max($_POST['products_id']) == 1 && $this->in_cart_mixed($_POST['products_id']) == 1) {
                ...
                if ($the_list !== '') {
                    $message_stack->add('product_info', ERROR_CORRECTIONS_HEADING . $the_list, 'caution');
                } else {
                    // process normally
                        ...
                        for ($i = 1, $n = $_GET['number_of_uploads']; $i <= $n; $i++) {
                            ...
                            if (isset($_POST[$upload_prefix]) && !empty($_FILES['id']['tmp_name'][$text_prefix]) && (!isset($_POST[$upload_prefix], $_FILES['id']['tmp_name'][$text_prefix]) || $_FILES['id']['tmp_name'][$text_prefix] != 'none')) {
                                ...
                                if ($products_options_file->parse($text_prefix)) {
                                    $products_image_extension = substr((string) $products_options_file->filename, strrpos((string) $products_options_file->filename, '.'));
                                    if (zen_is_logged_in()) {
                                ...
                                if (!zen_get_attributes_valid($_POST['products_id'], $text_prefix, !empty($_POST[$text_prefix]) ? $_POST[$text_prefix] : '')) {
                                    $the_list .= TEXT_ERROR_OPTION_FOR . '<span class="alertBlack">' . zen_options_name($_POST[$upload_prefix]) . '</span>' . TEXT_INVALID_SELECTION . '<span class="alertBlack">' . ((int) $_POST[$text_prefix] === (int) PRODUCTS_OPTIONS_VALUES_TEXT_ID) ? TEXT_INVALID_USER_INPUT : zen_values_name($value) . '</span>' . '<br>';
                                    $new_qty = 0;
                                    // Don't increase the quantity of product in the cart.
                                }
                            }
                        }
                        if ($the_list !== '') {
            ...
            if ($adjust_max == 'true') {
                $message_stack->add_session('shopping_cart', ERROR_MAXIMUM_QTY . zen_get_products_name($_POST['products_id']), 'caution');
                if ($this->display_debug_messages) {
                    $message_stack->add_session('header', 'E: FUNCTION ' . __FUNCTION__ . '<br>' . ERROR_MAXIMUM_QTY . zen_get_products_name($_POST['products_id']), 'caution');
                }
            }
        }
        if (empty($the_list)) {
            ...
            if (DISPLAY_CART === 'false' && $_GET['main_page'] !== FILENAME_SHOPPING_CART && $message_stack->size('shopping_cart') === 0) {
                if (!isset($_POST['shopping_cart_zero_or_less']) || $_POST['shopping_cart_zero_or_less'] !== true) {
                    $message_stack->add_session('header', ($this->display_debug_messages ? 'FUNCTION ' . __FUNCTION__ . ': ' : '') . SUCCESS_ADDED_TO_CART_PRODUCT, 'success');
     ...
     * @since ZC v1.3.0
     */
    public function action_buy_now($goto, $parameters = []): void
    {
        global $message_stack;
        if ($this->display_debug_messages) {
            $message_stack->add_session('header', 'FUNCTION ' . __FUNCTION__ . ' $_GET[products_id]: ' . $_GET['products_id'], 'caution');
        }
        $this->flag_duplicate_msgs_set = false;
        $allow_into_cart = 'N';
        if (isset($_GET['products_id'])) {
            ...
            if ($allow_into_cart === 'Y') {
                ...
                if (!is_numeric($new_qty) || $new_qty < 0) {
                    ...
                    $message_stack->add_session('header', ERROR_CORRECTIONS_HEADING . ERROR_PRODUCT_QUANTITY_UNITS_SHOPPING_CART . $chk_link . ' ' . PRODUCTS_ORDER_QTY_TEXT . zen_output_string_protected($new_qty), 'caution');
                    $new_qty = 0;
                }
                if ($add_max == 1 && $cart_qty == 1) {
                    // do not add
                    $new_qty = 0;
                } elseif ($add_max != 0 && $new_qty + $cart_qty > $add_max) {
                    // adjust quantity if needed
                    $new_qty = $add_max - $cart_qty;
                }
        ...
        if (DISPLAY_CART === 'false') {
            if ($_GET['main_page'] !== FILENAME_SHOPPING_CART && $allow_into_cart === 'Y' && $message_stack->size('shopping_cart') === 0) {
                $message_stack->add_session('header', ($this->display_debug_messages ? 'FUNCTION ' . __FUNCTION__ . ': ' : '') . SUCCESS_ADDED_TO_CART_PRODUCTS, 'success');
                $this->notify('NOTIFIER_CART_OPTIONAL_SUCCESS_BUYNOW_ADDED_TO_CART', $_GET, $goto, $parameters);
            } elseif ($allow_into_cart !== 'Y') {
                //zen_redirect(zen_href_link(FILENAME_SHOPPING_CART));
                $message_stack->add_session('header', ($this->display_debug_messages ? 'FUNCTION ' . __FUNCTION__ . ': ' : '') . FAILED_TO_ADD_UNAVAILABLE_PRODUCTS, 'error');
            }
        }
        $exclude[] = 'action';
     ...
     * @since ZC v1.3.0
     */
    public function action_multiple_add_product($goto, $parameters = []): void
    {
        global $message_stack;
        if ($this->display_debug_messages) {
            $message_stack->add_session('header', 'FUNCTION ' . __FUNCTION__, 'caution');
        ...
        if (!empty($_POST['products_id']) && is_array($_POST['products_id'])) {
            ...
            foreach ($products_list as $key => $val) {
                ...
                $prod_id = preg_replace('/[^0-9a-f:.]/', '', (string) $key);
                if (is_numeric($val) && $val > 0) {
                    $adjust_max = false;
                    $qty = $val;
                    $add_max = zen_get_products_quantity_order_max($prod_id);
                    $cart_qty = $this->in_cart_mixed($prod_id);
                    $new_qty = $this->adjust_quantity($qty, $prod_id, 'shopping_cart');
                    // adjust new quantity to be no more than current in stock
                    $chk_current_qty = zen_get_products_stock($prod_id);
                    if (STOCK_ALLOW_CHECKOUT === 'false' && $new_qty > $chk_current_qty) {
                        $new_qty = $chk_current_qty;
                        $message_stack->add_session('shopping_cart', ($this->display_debug_messages ? 'FUNCTION ' . __FUNCTION__ . ': ' : '') . WARNING_PRODUCT_QUANTITY_ADJUSTED . zen_get_products_name($prod_id), 'caution');
                    }
                    if ($add_max == 1 && $cart_qty == 1) {
                        // do not add
                        $adjust_max = 'true';
                        ...
                        $this->add_cart($prod_id, $this->get_quantity($prod_id) + $new_qty);
                        $add_count++;
                    }
                    if ($adjust_max === 'true') {
                        if ($this->display_debug_messages) {
                            $message_stack->add_session('header', 'FUNCTION ' . __FUNCTION__ . '<br>' . ERROR_MAXIMUM_QTY . zen_get_products_name($prod_id), 'caution');
                        }
                        $message_stack->add_session('shopping_cart', ERROR_MAXIMUM_QTY . zen_get_products_name($prod_id), 'caution');
                    }
                }
                if (!is_numeric($val) || $val < 0) {
                    // adjust quantity when not a value
                    $chk_link = '<a href="' . zen_href_link(zen_get_info_page($prod_id), 'cPath=' . zen_get_generated_category_path_rev(zen_get_products_category_id($prod_id)) . '&products_id=' . $prod_id) . '">' . zen_get_products_name($prod_id) . '</a>';
                    $message_stack->add_session('header', ERROR_CORRECTIONS_HEADING . ERROR_PRODUCT_QUANTITY_UNITS_SHOPPING_CART . $chk_link . ' ' . PRODUCTS_ORDER_QTY_TEXT . zen_output_string_protected($val), 'caution');
                    $val = 0;
                }
            }
            // display message if all is good and not on shopping_cart page
            if (DISPLAY_CART === 'false') {
                if ($add_count && $_GET['main_page'] !== FILENAME_SHOPPING_CART && $message_stack->size('shopping_cart') === 0) {
                    $message_stack->add_session('header', ($this->display_debug_messages ? 'FUNCTION ' . __FUNCTION__ . ': ' : '') . SUCCESS_ADDED_TO_CART_PRODUCTS, 'success');
     ...
     * @since ZC v1.3.0
     */
    public function action_notify($goto, $parameters = ['ignored']): void
    {
        global $db;
        if (zen_is_logged_in() && !zen_in_guest_checkout()) {
            ...
            foreach ($notify as $product_id) {
                $sql = 'INSERT IGNORE INTO ' . TABLE_PRODUCTS_NOTIFICATIONS . '
     ...
     * @since ZC v1.3.0
     */
    public function action_notify_remove($goto, $parameters = ['ignored']): void
    {
        global $db;
     ...
     * @since ZC v1.3.0
     */
    public function action_customer_order($goto, array|string $parameters): void
    {
        global $message_stack;
        if ($this->display_debug_messages) {
            $message_stack->add_session('header', 'FUNCTION ' . __FUNCTION__, 'caution');
        ...
        // display message if all is good and not on shopping_cart page
        if (DISPLAY_CART === 'false') {
            if ($_GET['main_page'] !== FILENAME_SHOPPING_CART && $message_stack->size('shopping_cart') === 0) {
                $message_stack->add_session('header', ($this->display_debug_messages ? 'FUNCTION ' . __FUNCTION__ . ': ' : '') . SUCCESS_ADDED_TO_CART_PRODUCTS, 'success');
            } else {
                zen_redirect(zen_href_link(FILENAME_SHOPPING_CART));
            }
     ...
     * @since ZC v1.3.0
     */
    public function action_remove_product($goto, array|string $parameters): void
    {
     ...
     * @since ZC v1.3.0
     */
    public function action_cart_user_action($goto, $parameters): void
    {
     ...
     * @since ZC v1.3.6
     */
    public function adjust_quantity($check_qty, $product_id, $message_stack_position = 'shopping_cart')
    {
        global $message_stack;
        if (empty($message_stack_position)) {
            $message_stack_position = 'shopping_cart';
        }
        $precision = QUANTITY_DECIMALS > 0 ? (int) QUANTITY_DECIMALS : 0;
        if ($precision !== 0) {
        ...
        if ($check_qty != round(zen_str_to_numeric($check_qty), $precision)) {
            ...
            $message_stack->add_session($message_stack_position, ERROR_QUANTITY_ADJUSTED . zen_get_products_name($product_id) . ERROR_QUANTITY_CHANGED_FROM . $check_qty . ERROR_QUANTITY_CHANGED_TO . $new_qty, 'caution');
            return $new_qty;
        }
        return $check_qty;
    }
     ...
     * @since ZC v1.5.5b
     */
    public function in_cart_check_attrib_quantity($check_option_id, $check_option_values_id): float|int
    {
        // if nothing is in cart return 0
        if (!is_array($this->contents)) {
            return 0;
     ...
     * @since ZC v1.5.5b
     */
    public function in_cart_product_total_price($product_id): int|float
    {
     ...
     * @since ZC v1.5.5b
     */
    public function in_cart_product_total_quantity($product_id): float|int
    {
        $products = $this->get_products();
     ...
     * @since ZC v1.5.5b
     */
    public function in_cart_product_total_weight($product_id): int|float
    {
        $products = $this->get_products();
     ...
     * @since ZC v1.5.5b
     */
    public function in_cart_product_total_weight_category($category_id): int|float
    {
        $products = $this->get_products();
        $in_cart_product_weight = 0;
        foreach ($products as $product) {
     ...
     * @since ZC v1.5.5b
     */
    public function in_cart_product_total_price_category($category_id): int|float
    {
        $products = $this->get_products();
        $in_cart_product_price = 0;
        foreach ($products as $val) {
     ...
     * @since ZC v1.5.5b
     */
    public function in_cart_product_total_quantity_category($category_id): float|int
    {
        $products = $this->get_products();
        $in_cart_product_quantity = 0;
        foreach ($products as $val) {
     ...
     * @since ZC v1.5.6
     */
    public function in_cart_product_mixed_changed($product_id, $chk = false): true|array
    {
        global $db;
        ...
        // check if mixed is on
        $mixed_status = (new Product((int) $pr_id))->with_default_language()->get('products_quantity_mixed');
        ...
        for ($i = 0, $n = count($_POST['products_id']); $i < $n; $i++) {
            ...
            // $products[$i]['quantity']
            if (!is_numeric($_POST['cart_quantity'][$i]) || $_POST['cart_quantity'][$i] < 0) {
                $_POST['cart_quantity'][$i] = $current_qty;
                // Default response behavior in cart.
            }
            ...
            if ($_POST['cart_quantity'][$i] != $current_qty) {
                // identify that quantity changed
                $product_changed[$products_id] = $_POST['cart_quantity'][$i] - $current_qty;
                // Identify that the specific product changed and by how much the customer increased it.
                if (array_key_exists($prs_id, $product_total_change)) {
                ...
                switch (true) {
                    case $chk === 'increase':
                        // track only increases
                        if ($_POST['cart_quantity'][$i] > $current_qty) {
                            $product_tracked_changed[$products_id] = true;
                            // Identify that the specific product changed
                            $product_last_changed[$prs_id] = $products_id;
                            // Identify what the last changed product was.
                            $product_increase[] = $products_id;
                        }
                        break;
                    case $chk === 'decrease':
                        // track only decreases
                        if ($_POST['cart_quantity'][$i] < $current_qty) {
                            $product_tracked_changed[$products_id] = true;
                            // Identify that the specific product changed
                            $product_last_changed[$prs_id] = $products_id;
                        ...
                        break;
                    default:
                        // track the last that had a difference in quantity.
                        $product_tracked_changed[$products_id] = true;
                        // Identify that the specific product changed
                        $product_last_changed[$prs_id] = $products_id;
                        // Identify what the last changed product was.
        ...
        $changed_array = ['state' => false, 'changed' => $product_changed, 'total_change' => $product_total_change[$pr_id], 'last_changed' => $product_last_changed[$pr_id], 'increase' => $product_increase, 'decrease' => $product_decrease];
