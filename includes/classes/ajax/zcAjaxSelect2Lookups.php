 ...
 * @since ZC v2.2.0
 */
class Zc_Ajax_Select2lookups extends base
{
    protected bool $strip_tags = false;
    /**
     * @since ZC v2.2.0
     */
    public function get_products_for_specials(): bool|array
    {
        if (!isset($_POST['q'])) {
            return false;
        }
    ...
    protected function lookup_products_select2(string $lookup = '', array $exclusion_formulas = [], array $excluded_products = [], bool $show_id = true, bool $show_model = true): array
    {
        ...
        if ($lookup !== '') {
            ...
            $keyword_search_fields = ['pd.products_name', 'p.products_model', 'p.products_mpn', 'pd.products_description', 'p.products_id', 'p.products_price'];
            $search_query = zen_build_keyword_where_clause($keyword_search_fields, $q);
        }
        $query = $db->bind_vars($query, ':languageID', (int) $_SESSION['languages_id'], 'integer');
        ...
        foreach ($results as $result) {
            ...
            $records[] = ['id' => (string) $result['products_id'], 'text' => ($this->strip_tags ? strip_tags((string) $result['products_name']) : $result['products_name']) . ' (' . $currencies->format($display_price) . ')' . ($show_model ? ' [' . $result['products_model'] . '] ' : '') . ($show_id ? ' - ID# ' . $result['products_id'] : '')];
        }
        return $records;
    }
}