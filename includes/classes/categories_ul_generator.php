...
',
     ...
     * @since ZC v1.5.5
     */
    public function build_branch($parent_id, $level = 0, $submenu = true, string $parent_link = ''): string
    {
        ...
        if ($this->data[$parent_id]) {
            foreach ($this->data[$parent_id] as $category_id => $category) {
                ...
                if (isset($this->data[$category_id]) && (empty($this->max_level) || $this->max_level > $level + 1)) {
                    $result .= $this->build_branch($category_id, $level + 1, $submenu, $category_link . '_');
                }
     ...
     * @since ZC v1.5.5
     */
    public function build_tree($submenu = false, ?int $max_levels = null): string
    {
        if (empty($this->data)) {
            return '';
        }
        ...
        return $this->build_branch($this->root_category_id, 0, $submenu);
    }
}