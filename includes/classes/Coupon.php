...
class Coupon extends base
{
    ...
    public static function code_exists(string $code): bool
    {
        ...
        $sql = $db->bind_vars($sql, ':couponcode', $code, 'string');
        return $db->Execute($sql)->record_count() > 0;
    }
    /**
     * @since ZC v2.0.0
     */
     ...
     * @since ZC v2.0.0
     */
    public static function delete_duplicates(string $origin_prefix): array
    {
        global $db;
        $results = [];
        ...
        $sql = $db->bind_vars($sql, ':original_code:', $origin_prefix, 'noquotestring');
        $delete_duplicate_coupons_check = $db->Execute($sql);
        if ($delete_duplicate_coupons_check->record_count() > 0) {
    ...
    public static function make_duplicates(int|string $original_id, int|string $new_code, int $quantity): bool
    {
        for ($i = 1; $i <= $quantity; $i++) {
            ...
            $generated_code = static::generate_random_coupon_code((string) $original_id, $new_code_length, $new_code);
            if ($generated_code === '') {
                // cannot create code
                return false;
            }
    ...
    public static function clone(int|string $original_id, string $new_code): int|false
    {
        ...
        if (static::code_exists($new_code)) {
            return false;
        }
        $sql = 'SELECT *
                FROM ' . TABLE_COUPONS . '
        ...
        $sql_data_array = ['coupon_code' => zen_db_prepare_input($new_code), 'coupon_amount' => zen_db_prepare_input($copied_coupon->fields['coupon_amount']), 'coupon_product_count' => (int) $copied_coupon->fields['coupon_product_count'], 'coupon_type' => zen_db_prepare_input($copied_coupon->fields['coupon_type']), 'uses_per_coupon' => (int) $copied_coupon->fields['uses_per_coupon'], 'uses_per_user' => (int) $copied_coupon->fields['uses_per_user'], 'coupon_minimum_order' => (float) $copied_coupon->fields['coupon_minimum_order'], 'restrict_to_products' => zen_db_prepare_input($copied_coupon->fields['restrict_to_products']), 'restrict_to_categories' => zen_db_prepare_input($copied_coupon->fields['restrict_to_categories']), 'coupon_start_date' => $copied_coupon->fields['coupon_start_date'], 'coupon_expire_date' => $copied_coupon->fields['coupon_expire_date'], 'date_created' => 'now()', 'date_modified' => 'now()', 'coupon_zone_restriction' => (int) $copied_coupon->fields['coupon_zone_restriction'], 'coupon_calc_base' => (int) $copied_coupon->fields['coupon_calc_base'], 'coupon_order_limit' => (int) $copied_coupon->fields['coupon_order_limit'], 'coupon_is_valid_for_sales' => (int) $copied_coupon->fields['coupon_is_valid_for_sales'], 'coupon_active' => 'Y'];
        ...
        foreach ($new_coupon_descriptions as $new_coupon_description) {
            $sql_mdata_array = ['coupon_id' => (int) $cid, 'language_id' => (int) $new_coupon_description['language_id'], 'coupon_name' => zen_db_prepare_input('COPY: ' . $new_coupon_description['coupon_name']), 'coupon_description' => zen_db_prepare_input($new_coupon_description['coupon_description'])];
        ...
        foreach ($copy_coupon_restrictions as $copy_coupon_restriction) {
            $sql_rdata_array = ['coupon_id' => (int) $cid, 'product_id' => (int) $copy_coupon_restriction['product_id'], 'category_id' => (int) $copy_coupon_restriction['category_id'], 'coupon_restrict' => zen_db_prepare_input($copy_coupon_restriction['coupon_restrict'])];
     ...
     * @since ZC v2.0.0
     */
    public static function generate_random_coupon_code(string $salt = 'secret', $length = SECURITY_CODE_LENGTH, string $prefix = ''): string
    {
        $length = (int) $length;
        ...
        for ($i = 0; $i < $random_string_length; $i++) {
            ...
            if (!static::code_exists($prefix . $new_code)) {
     ...
     * @since ZC v2.0.0
     */
    public static function get_all_coupons_by_name(): array
    {
        global $db;
        ...
        foreach ($results as $coupon) {
            $coupons[] = ['coupon_id' => $coupon['coupon_id'], 'coupon_name' => $coupon['coupon_name'], 'coupon_code' => $coupon['coupon_code']];
        }
        return $coupons;
    }
}